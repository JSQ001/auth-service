...
public class PatternMatcherUtil {
    ...
    public static void commonCodeCheck(String code) {
        if (code.length() > 36) {
            throw new BizException(RespCode.ENCODING_LENGTH_SHOULD_NOT_EXCEED_36_BITS);
        } else if (PatternMatcherUtil.isChineseCharacterRegex(code)) {
            throw new BizException(RespCode.CHINESE_CHARACTERS_ARE_NOT_ALLOWED);
        } else if (!PatternMatcherUtil.validateCodeRegex(code)) {
            throw new BizException(RespCode.ENCODING_CANNOT_CONTAIN_ILLEGAL_CHARACTERS);
        }
    }
