...
import com.hand.hcf.app.workflow.workflow.persistence.ApprovalFormMapper;
...
public class ApprovalFormService extends BaseService<ApprovalFormMapper, ApprovalForm> {
     ...
     * @return
     */
    public List<ApprovalForm> listDTOByTenantAndCondition(Long tenantId, Long booksId, Long formTypeId, String formName, String remark, Boolean valid, Page page) {
        return selectPage(page, new EntityWrapper<ApprovalForm>()
                .eq("tenant_id", tenantId)
                .eq("set_of_books_id", booksId)
     ...
     * @return
     */
    public List<ApprovalForm> listBySetOfBooksAndCondition(Long booksId, List<Long> formTypeIds, String formName, String remark, Page page) {
        return selectPage(page, new EntityWrapper<ApprovalForm>()
                .eq("set_of_books_id", booksId)
                .eq("from_type", ApprovalFormEnum.CUSTOMER_FROM_TENANT.getId())
                ...
                .like(StringUtils.isNotEmpty(formName), "form_name", formName)
                .like(StringUtils.isNotEmpty(remark), "remark", remark)
                .eq("valied", RuleApprovalEnum.VALID.getId())
                .orderBy("id")).getRecords();
     ...
     * @param dto
     */
    public void associateCustomFormUserScope(ApprovalFormUserScopeDTO dto) {

        ApprovalForm form = getByOid(dto.getFormOid());
        ApprovalForm oldForm = new ApprovalForm();
        mapper.map(form, oldForm);
        if (!form.getVisibleUserScope().equals(dto.getVisibleScope())) {
            form.setVisibleUserScope(dto.getVisibleScope());
            updateById(form);
        }
     ...
     * @return
     */
    public ApprovalFormUserScopeDTO getAssociatedUserScope(UUID formOID, UUID userOID) {
        //查询表单
        ApprovalFormDTO form = this.getCustomForm(formOID);
        ApprovalFormUserScopeDTO dto = new ApprovalFormUserScopeDTO();
        dto.setVisibleScope(form.getVisibleUserScope());
        dto.setFormOid(formOID);
        //查询表单关联的人员组
        if (ApprovalFormUserScopeEnum.USER_GROUP.getId().equals(form.getVisibleUserScope())) {//自定义的人员组可见范围
            List<ApprovalFormUserGroupVO> list = baseMapper.listUserGroupByFormOid(formOID);
            if (!CollectionUtils.isEmpty(list)) {
                dto.setUserGroupOids(list.stream().map(u -> u.getUserGroupOID()).collect(Collectors.toList()));
                dto.setUserGroups(list);
            }

        } else if (ApprovalFormUserScopeEnum.DEPARTMENT.getId().equals(form.getVisibleUserScope())) {
            //查询表单关联的部门
            List<ApprovalFormDepartmentVO> departments = baseMapper.listDepartmentsByFormOid(formOID);
            dto.setDepartmentOids(departments.stream().map(u -> u.getDepartmentOID()).collect(Collectors.toList()));
            dto.setDepartments(departments);
        }

        //判空
        if (CollectionUtils.isEmpty(dto.getUserGroupOids())) {
            dto.setUserGroupOids(new ArrayList<>());
        }
        if (CollectionUtils.isEmpty(dto.getDepartmentOids())) {
            dto.setDepartmentOids(new ArrayList<>());
        }
        return dto;
    }
    ...
    public ApprovalFormDTO createCustomForm(ApprovalFormDTO approvalFormDTO, UUID userOid, Long tenantId) {
        ...
        CompanyCO company = baseClient.getCompanyByOid(OrgInformationUtil.getCurrentCompanyOid());
        ;
        if (!String.valueOf(ApprovalFormEnum.CUSTOMER_FROM_TENANT.getId()).equals(String.valueOf(approvalFormDTO.getFromType()))) {
            if (company != null) {
                approvalForm.setCompanyId(company.getId());
            }
        }
        approvalForm.setFormOid(UUID.randomUUID());
        approvalForm.setAsSystem(Boolean.FALSE);
        approvalForm.setVisibleUserScope(FormConstants.VISIBLE_USER_SCOPE_ALL);
        approvalForm.setTenantId(tenantId);
        approvalForm.setSetOfBooksId(approvalFormDTO.getSetOfBooksId() == null ? company.getSetOfBooksId() : approvalFormDTO.getSetOfBooksId());
        ...
        approvalForm.setVisibleCompanyScope(approvalFormDTO.getVisibleCompanyScope() == null ? ApprovalFormEnum.VISIBLE_COMPANY_SCOPE_ALL.getId() : approvalFormDTO.getVisibleCompanyScope());
    ...
    public List<ApprovalFormDTO> listDTOByUserOid(UUID userOid) {
        ...
        return listDTOByCompanyOid(companyOid);
    }

    public List<ApprovalFormDTO> listDTOByCompanyOid(UUID companyOid) {
        if (companyOid == null) {
            return new ArrayList<>();
        }

        return baseMapper.listDTOByQO(ApprovalFormQO.builder().companyOid(companyOid).build());
    }


    public List<ApprovalFormDTO> listTenantCustomFormsByCompanyOid(UUID companyOid, String booksID,Long formTypeId,String formName) {
        CompanyCO company = baseClient.getCompanyByOid(companyOid);
        List<ApprovalForm> list = null;
        if (company != null) {
            list = selectList(new EntityWrapper<ApprovalForm>()
                    .eq("tenant_id", company.getTenantId())
                    .eq("set_of_books_id", Long.parseLong(booksID))
        ...
        return new ArrayList<ApprovalFormDTO>();
    }
     ...
     * @return
     */
    public List<ApprovalForm> listTenantCustomByType(Long setOfBooksId) {
        ...
        if (CollectionUtils.isNotEmpty(allList)) {
            ...
            allList.stream().forEach(c -> {
                //账套表单适用所有公司
                if (String.valueOf(ApprovalFormEnum.VISIBLE_COMPANY_SCOPE_ALL.getId()).equals(String.valueOf(c.getVisibleCompanyScope()))) {
                    list.add(c);
                    //账套表单适用部分公司
                } else if (String.valueOf(ApprovalFormEnum.VISIBLE_COMPANY_SCOPE_NOT_ALL.getId()).equals(String.valueOf(c.getVisibleCompanyScope()))) {
                   /* if(CollectionUtils.isNotEmpty(customFormCompanyList)){
                        customFormCompanyList.stream().forEach(cf -> {
                            if(String.valueOf(c.getFormOid()).equals(String.valueOf(cf))){
                                list.add(c);
                            }
                        });
                    }*/
    ...
    //申请表单列表
    public List<ApprovalFormDTO> listCompanyCustomForms(UUID userOid) {
        //只取帐套有的，支掉公司级本身的
        List<ApprovalForm> list = listTenantCustomByType(baseClient.getCompanyByUserOid(userOid).getSetOfBooksId());
     ...
     * @return
     */
    public List<ApprovalFormSummaryDTO> listSummaryFormByUserOid(UUID userOid) {
        Long companyId = OrgInformationUtil.getCurrentCompanyId();
        if (companyId == null) {
            return null;
        }
        List<ApprovalFormDTO> dtos = this.listCompanyCustomForms(userOid);
     ...
     * @return
     */
    public ApprovalFormDTO getDTOByOidAndUserOid(UUID formOid, UUID userOid) {
    ...
    public ApprovalFormAllDTO updateApprovalForm(ApprovalFormAllDTO customFormMergeDTO, UUID userOID) {
        ...
        //2.更新适用人员,即可见域
        if (customFormMergeDTO.getApprovalFormUserScopeDTO() != null) {
            associateCustomFormUserScope(customFormMergeDTO.getApprovalFormUserScopeDTO());
        }
    ...
    public List<ApprovalFormSummaryDTO> selectFormBySetOfBooksIdAndFormType(Long setOfBooksId,Integer formTypeId){
    ...
    public static ApprovalFormDTO approvalFormToFormDTO(ApprovalForm approvalForm) {
        ...
        result.setVisibleUserScope(approvalForm.getVisibleUserScope());
        ...
        result.setVisibleCompanyScope(approvalForm.getVisibleCompanyScope());
        result.setSetOfBooksId(approvalForm.getSetOfBooksId());
        result.setCompanyId(approvalForm.getCompanyId());
    ...
    public static ApprovalForm approvalFormDTOToForm(ApprovalFormDTO approvalFormDTO) {
        ...
        approvalForm.setVisibleUserScope(approvalFormDTO.getVisibleUserScope());
        approvalForm.setFormCode(approvalFormDTO.getFormCode());
        approvalForm.setFromType(approvalFormDTO.getFromType());
        approvalForm.setVisibleCompanyScope(approvalFormDTO.getVisibleCompanyScope());
    ...
    public static ApprovalFormSummaryDTO approvalFormDTOToSummaryDTO(ApprovalFormDTO dto) {
        ...
        result.setVisibleUserScope(dto.getVisibleUserScope());
     ...
     * @return
     */
    public List<ApprovalForm> pageApprovalFormByCond(Long setOfBooksId, Long tenantId, String code, String name,Page page){
        List<ApprovalForm> customForms = baseMapper.selectPage(
                page,
                    new EntityWrapper<ApprovalForm>()
                            .eq("set_of_books_id", setOfBooksId)
                            .eq("tenant_id",tenantId)
                            .like(StringUtils.isNotEmpty(code), "form_code", code)
                            .like(StringUtils.isNotEmpty(name), "form_name", name)
                            .eq("form_type_id",801001)
                            .eq("from_type",2)
                            .orderBy("id")
            );
        return customForms;
