...
public class InterfaceController {
     ...
     */
    @GetMapping("/queryAll")
    public ResponseEntity<List<Interface>> getInterfacesAllByAppId(@RequestParam(required = true) Long appId,
                                                                      @RequestParam(required = false) Boolean enabled)  {
        List<Interface> list = interfaceService.listInterfacesByAppId(appId, enabled);
     ...
     * @apiGroup SysFrameWork
     * @apiParam (请求参数) {Long} [appId] 模块ID 不传则不控，传了则按其控制
    ...
    public ResponseEntity<List<InterfaceTreeDTO>> getInterfacesByKeyword(@RequestParam(required = false) String appId,
                                                                         @RequestParam(required = true) String keyword)  {
        if (StringUtils.isEmpty(keyword)) {
            // 当keyword为空时，不执行查询,直接返回
            return new ResponseEntity(null, HttpStatus.OK);
        }
        List<InterfaceTreeDTO> list = interfaceService.getInterfacesByKeyword(appId, keyword);
        return new ResponseEntity(list, HttpStatus.OK);
    }
     ...
     * {
     * "appName": "框架接口",
     * "appId": "1039069151636291585",
     * "listInterface": []
     * },
     * {
     * "appName": "测试",
     * "appId": "1039099364357242881",
     * "listInterface": [
     * {
     ...
     * {
     * "appName": "支付",
     * "appId": "1039112514121039874",
     * "listInterface": []
     * },
     * {
     * "appName": "预付款",
     * "appId": "1039114320263839745",
     * "listInterface": []
     * },
     * {
     * "appName": "菜单",
     * "appId": "1039342660359593986",
     * "listInterface": []
     * }
     * ]
     */
