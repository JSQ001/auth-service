...
import com.hand.hcf.app.base.dataAuthority.persistence.DataAuthorityRuleDetailMapper;
...
public class DataAuthorityRuleDetailService extends BaseService<DataAuthorityRuleDetailMapper,DataAuthorityRuleDetail>{
    ...
    private final ThreadLocal<Map<String,Map<String, SysCodeValue>>> systemCodeCache = new ThreadLocal<>();
    /**
     * 新建数据权限规则明细
     * @param entity
     * @return
        ...
        if(CollectionUtils.isNotEmpty(ruleDetails)){
            ruleDetails.stream().forEach(ruleDetail -> {
                ...
                if(! systemCodeCache.get().containsKey("3102")){
                    systemCodeCache.get().put("3102", sysCodeService.listEnabledSysCodeValueBySysCodeAnd("3102").stream().collect(Collectors.toMap(e -> e.getValue(), e -> e)));
                }
                SysCodeValue sysCodeValue = systemCodeCache.get().get("3102").get(ruleDetail.getDataScope());
                ...
                if(StringUtils.isNotEmpty(ruleDetail.getFiltrateMethod())){
                    ...
                    if(! systemCodeCache.get().containsKey("3103")){
                        systemCodeCache.get().put("3103", sysCodeService.listEnabledSysCodeValueBySysCodeAnd("3103").stream().collect(Collectors.toMap(e -> e.getValue(), e -> e)));
                    }
