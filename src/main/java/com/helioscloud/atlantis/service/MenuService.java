...
public class MenuService extends BaseService<MenuMapper,Menu> {
    ...
    public Menu updateMenu(Menu Menu){
        ...
        //校验ID是否在数据库中存在
        Menu rr = menuMapper.selectById(Menu.getId());
        if(rr == null){
     ...
     * @return
     */
    public List<Menu> getMenus(Boolean isDeleted,Boolean isEnabled,Page page){
     ...
     * @return
     */
    public List<Menu> getMenusByParentMenuId(Long parentMenuId,Boolean isDeleted,Boolean isEnabled,Page page){
        if(isDeleted == null || "".equals(isDeleted)){
            return menuMapper.selectPage(page,new EntityWrapper<Menu>()
                    .eq("is_deleted",false)
                    .eq(isEnabled != null && !"".equals(isEnabled),"is_enabled",isEnabled)
