...
import com.helioscloud.atlantis.domain.ArtemisAuthorizationCode;
...
public class ArtemisAuthorizationCodeServices implements AuthorizationCodeServices {
    private final Logger LOG = LoggerFactory.getLogger(ArtemisAuthorizationCodeServices.class);
    ...
    public ArtemisAuthorizationCodeServices(DataSource dataSource) {
    ...
    public String createAuthorizationCode(OAuth2Authentication authentication) {
        ArtemisAuthorizationCode hlyCode = getHLYCode(authentication);
        if (hlyCode != null) {
        ...
        hlyCode = new ArtemisAuthorizationCode(code, new Date(System.currentTimeMillis() + (codeValiditySeconds * 1000L)));
        store(hlyCode, authentication);
        return code;
    }
    ...
    private void store(ArtemisAuthorizationCode code, OAuth2Authentication authentication) {
    ...
    private OAuth2Authentication remove(String code) {
        ArtemisAuthorizationCode hlyCode;

        try {
            hlyCode = jdbcTemplate.queryForObject(selectAuthenticationSql,
                new RowMapper<ArtemisAuthorizationCode>() {
                    public ArtemisAuthorizationCode mapRow(ResultSet rs, int rowNum)
                        throws SQLException {
    ...
    public ArtemisAuthorizationCode getHLYCode(OAuth2Authentication authentication) {
        ArtemisAuthorizationCode code = null;

        String key = authenticationKeyGenerator.extractKey(authentication);
        try {
            code = jdbcTemplate.queryForObject(selectCodeFromAuthenticationSql,
                new RowMapper<ArtemisAuthorizationCode>() {
                    public ArtemisAuthorizationCode mapRow(ResultSet rs, int rowNum) throws SQLException {
    ...
    protected ArtemisAuthorizationCode deserializeHLYCode(byte[] code) {
