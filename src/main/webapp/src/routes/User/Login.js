...
import zh_CN from '../../i18n/zh_CN/login.json';
...
export default class Login extends React.Component {
  ...
  login = () => {
    ...
    axios({
      /** jiu.zhao 环境IP */
      url: 'http://127.0.0.1:9082/oauth/token',
      method: 'POST',
      headers: {
        'x-helios-client': 'web',
  ...
  getMenus = async () => {
    ...
    this.pageMap = functionPageDTOList.reduce((temp, item) => {
      temp[item.pageId] = { ...item, parent: item.contentRouter + item.functionRouter };
      return temp;
    }, {});
    ...
    const functions = contentFunctionDTOList
      .reduce((temp, item) => {
        if (functionIdList.indexOf(item.functionId) >= 0) {
          ...
          const fullUrl =
            item.contentRouter + item.functionRouter + '/' + (page ? page.pageUrl : '');
          const fullRouter =
            item.contentRouter + item.functionRouter + (page ? page.pageRouter : '');
          temp.push({ ...item, pageUrl: page ? page.pageUrl : '' });
          functionMap[item.functionId] = { ...item, fullUrl, fullRouter };
        }
        return temp;
      }, [])
      .filter(item => item);
    ...
    const contents = functions
      .reduce((temp, item) => {
        if (temp.findIndex(o => o.contentId === item.contentId) >= 0) return temp;
        temp.push(
          contentFunctionDTOList.find(o => o.contentId === item.contentId && !o.functionId)
        );
        return temp;
      }, [])
      .filter(item => item);
    ...
    const routerData = functionPageDTOList.reduce((temp, item) => {
      const router = item.contentRouter + item.functionRouter + item.pageRouter;
      temp[router] = {
        ...item,
        name: item.pageName,
        path: router,
  ...
  getQcode = () => {
    ...
    httpFetch
      .getQcode()
      .then(res => {
        const Request = new UrlSearch(res.data.link); // 实例化
        this.setState(
          {
    ...
    return (
      <div>
        <Input
          ...
          size="large"
          placeholder={'code/account或者邮箱'} // 格式：code/account或者邮箱
          onChange={this.inputUsernameHandler}
        />
        <div>
    ...
    if (phoneEmail) {
      return (
        <div>
          ...
          <Input
            size="large"
            placeholder={'请输入手机号/邮箱'} // /*'请输入手机号/邮箱'*/
  ...
  inputPhoneEmailNext = () => {
    ...
    if (phoneReg.test(phoneNum)) {
      ...
      message.error('请输入正确的手机号或邮箱地址'); /* '请输入正确的手机号或邮箱地址'*/
    }
  };
        ...
        <div>
          ...
          <div
            dangerouslySetInnerHTML={{
              __html: messages('login.sent.confirm.code', { phoneNum, type }),
            }}
  ...
  confirmNewPassword = () => {
    ...
    } else if (ResetPasswordService.regPasswordByRule(secondPassword, this.state.rule)) {
