...
class AdvancedSearchForm extends React.Component {
  ...
  componentDidMount() {
    ...
    formItems.map((item, i) => {
      ...
      if (item.dataSource) {
        this.setState({
          options: { ...this.state.options, [item.id]: JSON.parse(item.dataSource) },
        });
      } else {
          ...
          <Select disabled={!disabled} allowClear={item.allowClear} placeholder={item.placeholder}>
            {options[item.id] &&
              options[item.id].map(option => {
                return (
                  <Select.Option key={option[item.valueKey]}>{option[item.labelKey]}</Select.Option>
                );
              })}
          </Select>
        );
