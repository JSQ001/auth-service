...
import { Modal, Button, Input, Row, Col, Card, Checkbox } from 'antd';
...
class SelectEmployeeGroup extends React.Component {
  ...
  componentDidMount() {
    this.setState(
      { useData: this.props.selectedData || [], selectedData: this.props.selectedData || [] },
      () => {
        this.getList();
      }
    );
  ...
  getList = () => {
    ...
    let url = `${config.mdataUrl}/api/user/groups/company?page=0&size=10000`;
  ...
  render() {
    ...
    const { userGroup, toSelectList, useUserGroup, useSelectList } = this.state;
    return (
      <Modal
