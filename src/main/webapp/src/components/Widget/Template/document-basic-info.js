...
class DocumentBasicInfo extends Component {
  ...
  renderList = (title, value, linkId, onClick) => {
    return (
      <Row
                ...
                this.state.data.infoList.map((item, index) => {
                  return (
                    <Col key={index} span={6}>
                      {item && this.renderList(item.label, item.value, item.linkId, item.onClick)}
                    </Col>
                  );
                })}
            </Row>
                            ...
                            {!this.isImage(item) ? (
                              <a
                                href={
                                  this.isPDF(item)
                                    ? item.thumbnailUrl
                                    : `${config.baseUrl}/api/attachments/download/${
                                        item.attachmentOid
                              ...
                              <a onClick={e => this.onPreviewClick(e, item)}>{item.fileName}</a>
                            )}
                          </Col>
                        );
                      })}
          ...
          <Col span={8}>
            ...
            <div style={{ float: 'right' }}>
              ...
              <div style={{ fontSize: '18px' }} className="amount-content">
                ...
                {this.props.isDiy && data.diyLabel ? (
                  data.diyLabel.value
                ) : (
                  <span>
                    {data.currencyCode} {this.filterMoney(data.totalAmount)}
