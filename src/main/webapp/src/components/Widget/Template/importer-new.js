...
class ImporterNew extends React.Component {
    ...
    if (tabKey === 'UPDATE') {
      ...
      httpFetch
        ...
        .catch(e => {
          ...
          message.error(
            this.$t('importer.import.error.info') /*导入失败，请重试*/ + e.response.data.message
          );
        });
    } else {
  ...
  getErrorList = () => {
    httpFetch
      .get(`${this.props.errorDataQueryUrl}/${this.state.transactionID}`)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            fileList: [],
        ...
        message.success(
          this.$t('importer.import.success', { total: res.data.successEntities }) /*导入成功*/
        );
      })
      .catch(() => {
        this.setState({ uploading: false });
