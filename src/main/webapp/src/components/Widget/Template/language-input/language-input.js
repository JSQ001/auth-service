...
class LanguageInput extends React.Component {
  ...
  componentDidMount() {
    let { languageList, language } = this.props;
    ...
    let sysLanguageDefault = language.local ? language.local : 'zh_cn';
    let currentUseLanguage = language.local ? language.local.toLowerCase() : 'zh_cn';
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.language.local !== this.props.language.local) {
      currentUseLanguage = nextProps.language.local;
      this.setState({ currentUseLanguage });
    }
    // 先把逻辑转为Boolean值
    if (nextProps.isEdit) {
      const i18nName = this.initLanguageListForEdit(
  ...
  renderInp() {
    ...
    this.props.value === '' &&
      this.state.nameForShow === '' &&
      (classNameInput = 'language-input-no');
    if (this.props.textArea) {
      return (
        <textarea
  ...
  render() {
    ...
    if (
      (!this.state.first || this.props.validateStatus) &&
      (this.props.value === '' && this.state.name === '')
    ) {
...
function mapStateToProps(state) {
  return {
    languageList: state.languages.languageType,
