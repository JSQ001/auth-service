...
import OrgSearchList from 'widget/Template/select-depment-or-person/org-search-list-id';

import 'styles/components/template/select-depment-or-person/select-depment-or-person.scss';
import PropTypes from 'prop-types';
import SelectPersonService from './select-depment-or-person.service-id';
...
class SelectDepOrPerson extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!this.state.visible && nextProps.visible) this.showModal(nextProps);
  }
  ...
  getTenantAllDep(nextProps) {
    SelectPersonService.getTenantAllDep().then(response => {
      const selectedKeys = nextProps.selectedData.map(o => o.key);
      const selectedKeysDepData = nextProps.selectedData.map(o => ({
        type: 'DEP',
        title: o.label,
        key: o.key,
        originData: {
  ...
  removeSelectedKeysDepDataFromSearch(node) {
    ...
    for (let i = 0; i < list.length; i++) {
      let oid = list[i].userOid ? list[i].userOid : list[i].id;
      if (oid != key) {
        newList.push(list[i]);
      }
    }
  ...
  onSelectAfter = (selectedList, info) => {
    ...
    if (this.props.onlyPerson) {
      ...
      if (info.selected) {
        selectedKeysDepData.push(data);
        selectedKeys.push(data.key);
      } else {
        let index = selectedKeys.findIndex(o => o === data.key);
        selectedKeys.splice(index, 1);

        index = selectedKeysDepData.findIndex(o => o.key === data.key);
        selectedKeysDepData.splice(index, 1);
      }
  ...
  searchListOnClicked = item => {
    let oid = item.userOid ? item.userOid : item.id;
    //如果只能单选，直接把最后点击的加入，其他就不要
    if (!this.props.multiple) {
      let key = item.userOid ? item.userOid : item.id;
      this.setState({
    ...
    if (list.length < 1) {
      ...
      return list.map(item => {
        if (item.userOid) {
          ...
          if (_isInSelected(this.state.selectedKeysDepData, item.id)) {
          } else {
            return (
              <div className="selected-person-item" key={item.id}>
                <div className="type-icon">
                  <Icon type="folder" />
                </div>
                ...
                <div
                  className="remove-icon"
                  onClick={() => {
                    this.unSelectFromSearch(item.id);
                  }}
                >
                  <Icon type="close" />
                </div>
  ...
  renderSelected(list) {
    ...
    return list.map(item => {
      if (item.props && item.props.dataRef.originData.userOid) {
        ...
        return (
          <div
            className="selected-person-item"
            key={item.props ? item.props.dataRef.originData.id : item.originData.id}
          >
            <div className="type-icon">
              <Icon type="folder" />
            </div>
            <div className="name">
              {item.props ? item.props.dataRef.originData.name : item.title}
            </div>
            <div
              className="remove-icon"
              onClick={() => {
                this.unSelect(item.props ? item.props.dataRef.originData.id : item.originData.id);
              }}
            >
              <Icon type="close" />
            </div>
...
SelectDepOrPerson.propTypes = {
  ...
  selected: PropTypes.array, //已选择的，用oid的
  selectedData: PropTypes.array, //已选择的,用id的
