...
class SearchArea extends React.Component {
        ...
        searchForm.map(item => {
          ...
          if (item.type === 'lov' && values[item.id]) {
            if (item.single) {
              ...
              let result = values[item.id].map(value => {
    ...
    if (item.options.length === 0 || (item.options.length === 1 && item.options[0].temp)) {
      ...
      httpFetch[item.method](url, item.getParams).then(res => {
        ...
        if (item.dataKey && typeof data === 'object') {
          let result = JSON.stringify(data);
          data = new Function(`try {return ${result}.${item.dataKey} } catch(e) {}`)();
        }
      ...
      searchForm = searchForm.map(searchItem => {
        if (searchItem.id === item.id) {
          ...
          if (
            ...
            let dataOption = {};
            searchItem.options = [];
            dataOption[item.type === 'value_list' ? 'code' : this.getLastKey(item.valueKey)] =
              value.value;
            dataOption[item.type === 'value_list' ? 'messageKey' : this.getLastKey(item.labelKey)] =
              value.label;
      ...
      searchForm[index].items = searchForm[index].items.map((searchItem, index) => {
        if (searchItem.id === item.id) {
          ...
          if (
            ...
            let dataOption = {};
            searchItem.options = [];
            dataOption[item.type === 'value_list' ? 'code' : this.getLastKey(searchItem.valueKey)] =
              value[index].value;
            dataOption[
              item.type === 'value_list' ? 'messageKey' : this.getLastKey(searchItem.labelKey)
            ] =
              value[index].label;
            searchItem.options.push({
              label: value[index].label,
          ...
          return (
            <LanguageInput
              ...
              nameChange={(name, i18nName) =>
                this.props.eventHandle(item.event, { name, i18nName })
              }
      ...
      case 'select': {
        return (
          <Select
            ...
            notFoundContent={
              item.fetching ? <Spin size="small" /> : messages('agency.setting.no.result')
            }
            ...
            {item.items.map(searchItem => {
              return (
                <Col span={colSpan} key={searchItem.id}>
                  <FormItem label={this.$t(searchItem.label)} colon={false}>
  ...
  formItemChange(value) {
    ...
    timeObj.map(item => {
      if (
        value[item.from] &&
        getFieldValue(item.to) &&
        getFieldValue(item.to).isBefore(value[item.from], 'day')
      ) {
      ...
      if (
        value[item.to] &&
        getFieldValue(item.from) &&
        value[item.to].isBefore(getFieldValue(item.from), 'day')
      ) {
      ...
      children.push(
        ...
        >
          {item.type === 'items' ? (
            ...
            <FormItem {...formItemLayout} label={this.$t(item.label)}>
                      ...
                      return (
                        <Row className="list-row" key={item.key}>
                          ...
                          >
                            ...
                            <a>
                              {item.expand
                                ? messages('components.search.upload.fold')
                                : messages('components.search.upload.more')}
                              <Icon
