...
class Location extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.value) {
      try {
        ...
        if (
          (value.data &&
            value.key !== valueWillSet.address &&
            value.data.location &&
            value.data.location.lng !== valueWillSet.longitude) ||
          !value.data
        ) {
          this.setState({
            value: {
  ...
  search = (keyword, option) => {
    ...
    keyword &&
      baseService.searchLocation(keyword).then(res => {
          ...
          {options.map(option => {
            return (
              <Option key={option.id}>
                ...
                <div style={{ fontSize: 10, color: '' }}>{`${option.province}${option.city}${
                  option.district
                }${option.address}`}</div>
              </Option>
            );
