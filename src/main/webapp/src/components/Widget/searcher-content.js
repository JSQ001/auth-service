...
class SearcherContent extends React.Component {
  ...
  componentDidMount() {
    if (this.refs.scrollPage) {
      this.refs.scrollPage.addEventListener('scroll', () => {
        if (
          this.refs.scrollPage.scrollTop + this.refs.scrollPage.clientHeight >=
          this.refs.scrollPage.scrollHeight
        ) {
    ...
    if (name) {
      ...
      sections.map((item, index) => {
        ...
        if (index !== sections.length - 1) {
          result.push(
            <span className="target-key" key={index}>
              {key}
            </span>
          );
        }
  ...
  render() {
    ...
    return (
      <div
        className={`searcher-content-list ${showDetail ? '' : 'no-show-detail'}`}
        ref={isNeedToPage ? 'scrollPage' : ''}
      >
        {loading ? (
          <Spin />
        ) : filterData.length > 0 ? (
          filterData.map(item => (
            <div
              className={`searcher-list-item ${item._selected ? 'selected' : ''}`}
              key={item[key]}
              onClick={() => this.handleSelect(item)}
            >
              <div className="searcher-list-item-name">
                {this.renderNameByKey(item[labelKey], item)}
                {externalLabel && externalLabel(item)}
                <br />
                {descriptionKey && (
                  <span className="searcher-list-item-description">
                    {this.renderNameByKey(this.getDescription(item))}
                  </span>
                )}
              </div>
