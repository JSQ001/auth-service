...
class ListSelector extends React.Component {
  ...
  getList() {
    ...
    if (this.props.method === 'post') {
      for (let name in this.props.extraParams) {
        url += `&${name}=${this.props.extraParams[name]}`;
      }
    }
  ...
  onRow = (record, index) => {
    const { showRowClick, onRowMouseEnter, onRowMouseLeave } = this.props;
    return {
      onClick: () => {
        !showRowClick && this.handleRowClick(record);
      },
      onMouseEnter: e => {
...
ListSelector.propTypes = {
  ...
  onRowMouseEnter: PropTypes.func, // 鼠标移入行
