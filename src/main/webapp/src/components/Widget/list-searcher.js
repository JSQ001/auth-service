...
class ListSearcher extends React.Component {
  ...
  getData = (searchParams, isSearch = false, isPage = false, searching = false) => {
    ...
    httpFetch[this.props.method](url, searchParams)
      .then(res => {
        if (listKey) {
        ...
        filterData = isSearch
          ? isPage
            ? this.unique([...filterData, ...res.data])
            : res.data
  ...
  componentWillReceiveProps = nextProps => {
    ...
    if (nextProps.selectedData && nextProps.selectedData.length > 0 && data.length > 0) {
      let key = nextProps.searcherItem
        ? nextProps.searcherItem.key
        : searcherData[nextProps.type].key;
      nextProps.selectedData.map(selectedItem => {
        data.map(item => {
          if (item[key] === selectedItem[key]) item._selected = true;
        });
      });
      this.setState({ data });
    }
    ...
    if (!isNeedToPage) {
      if (key) {
        data.map(item => {
          ...
          if (
            name.indexOf(key) > -1 ||
            (this.getDescription(item) && this.getDescription(item).indexOf(key) > -1)
          )
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Search onChange={this.handleChangeKey} placeholder={searchPlaceHolder} />
        <div className="table-header">
          <div className="table-header-title">
            {this.$t('common.total', { total: totalNumber })}
            {/* 共 total 条数据 */}
          </div>
        </div>
        <SearcherContent
