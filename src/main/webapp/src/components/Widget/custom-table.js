...
class CustomTable extends Component {
  ...
  componentDidMount() {
    ...
    this.props.columns &&
      this.props.columns.map(item => {
        if (item.tooltips) {
          item.render = value => <Popover content={value}>{value}</Popover>;
        }
      });
  ...
  //搜索 ,flag是否需要留在当前页
  search = (params, flag = false) => {
    let pagination = this.state.pagination;
    pagination.current = flag ? pagination.current : 1;
    this.setState({ page: flag ? this.state.page : 0, pagination, params }, this.getList);
  };
  ...
  getList = url => {
    ...
    httpFetch[methodType || 'get'](url, searchParams)
      .then(res => {
        let pagination = {
