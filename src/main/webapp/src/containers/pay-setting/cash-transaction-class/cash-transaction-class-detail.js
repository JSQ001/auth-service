...
class CashTransactionClassDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      newData: [],
      cashTransactionClass: '/pay-setting/cash-transaction-class/cash-transaction-class',
    };
  }
  ...
  backFunc = () => {
    const { setOfBookId } = this.state;
    this.props.dispatch(
      routerRedux.push({
        pathname: this.state.cashTransactionClass + `${setOfBookId ? '/' + setOfBookId : ''}`,
      })
    );
  };
