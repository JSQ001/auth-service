...
class NewPaymentCompanySetting extends React.Component {
  ...
  getPaymentMethodCategory() {
    ...
    this.getSystemValueList(2106).then(res => {
      res.data.values.map(data => {
        ducumentCategoryOptions.push({ label: data.name, value: data.value });
      });
      this.setState({
        ducumentCategoryOptions,
      });
      ...
      if (!err) {
        if (JSON.stringify(this.props.params) === '{}') {
          ...
          paymentCompanySettingService
            .addOrUpdataPaymentCompanySetting(toValue)
            .then(res => {
  ...
  getDocumentType = companyId => {
    ...
    paymentCompanySettingService.getDocumentType(companyId).then(res => {
      res.data.map(item => {
        ducumentTypeOptions.push({ label: `${item.reportTypeName}`, value: item.id });
      });
      this.setState({
    ...
    return (
      <div className="new-payment-company-setting">
        <Form onSubmit={this.handleSave}>
          <FormItem {...formItemLayout} label={messages('paymentCompanySetting.setOfBooks')}>
            {getFieldDecorator('setOfBooksId', {
              ...
              <Select placeholder={messages('common.please.select')} disabled={true}>
                {this.state.setOfBooksOption.map(option => {
                  return (
                    <Option value={option.value} lable={option.label} key={option.value}>
                      {option.label}
                    </Option>
                  );
                })}
          ...
          <FormItem {...formItemLayout} label={messages('paymentCompanySetting.priorty')}>
          ...
          <FormItem {...formItemLayout} label={messages('paymentCompanySetting.ducumentCategory')}>
            {getFieldDecorator('ducumentCategory', {
              ...
              <Select
                onSelect={this.handleDucumentCategory}
          ...
          <FormItem {...formItemLayout} label={messages('paymentCompanySetting.ducumentType')}>
            {getFieldDecorator('ducumentTypeId', {
              ...
              <Select
                disabled={
                  !(
                    JSON.stringify(this.props.form.getFieldValue('companyId')) !== '[]' &&
                    this.props.form.getFieldValue('ducumentCategory')
                  )
                }
                placeholder={messages('common.please.select')}
                allowClear={true}
...
export default connect(
  ...
  null,
  { withRef: true }
)(WrappedNewPaymentCompanySetting);
