...
export default {
  ...
  getCompanyByCode(companyCode) {
    return httpFetch.get(`${config.mdataUrl}/api/company/by/term?companyCode=${companyCode}`);
  },
  /**
  ...
  getList(page, pageSize, temp, params) {
    let url = `${
      config.payUrl
    }/api/CompanyBank/selectByCompanyId?page=${page}&size=${pageSize}${temp}`;
  ...
  getCompanyBankByCompanyBankId(companyBankId) {
    return httpFetch.get(
      `${config.payUrl}/api/CompanyBank/selectById?companyBankId=${companyBankId}`
    );
  },
  /**
  ...
  getPayWayList(companyBankId, type, temp1, temp2) {
    let url = `${
      config.payUrl
    }/api/cash/payment/method/get/payment/by/bankId/and/code?companyBankId=${companyBankId}&type=${type}${temp1}${temp2}`;
    return httpFetch.get(`${url}`);
  },
  /**
   * add-pay-way 编辑保存
   */
  savePayWay(temp) {
    let url = `${config.payUrl}/api/comapnyBankPayment/insertOrUpdate`;
  ...
  saveAuthorization(toValue) {
    let url = `${config.payUrl}/api/companyBankAuth/insertOrUpdate`;
  ...
  deletePayWay(recordId) {
    return httpFetch.delete(`${config.payUrl}/api/comapnyBankPayment/deleteById?id=${recordId}`);
  },
  /**
   * 公司详情编辑
   */
  updateHandle(temp) {
    let url = `${config.payUrl}/api/CompanyBank/insertOrUpdate`;
    return httpFetch.post(`${url}`, temp);
  },
  /**
