...
class AddAuthorization extends React.Component {
  ...
  componentDidMount() {
    this.setState({
      ...
      queryFlag: false,
      extraParams: {
        companyId: this.props.params.record.companyId,
        departmentId: this.props.params.record.departmentId,
      },
    });
  }
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      if (!err) {
        ...
        values.authorizeDateTo &&
          (values.authorizeDateTo = new Date(values.authorizeDateTo.format('YYYY-MM-DD')));

        values.authorizeCompanyId = values.authorizeCompanyId[0].id;
        let userCode = '';
        if (values.authorizeEmployeeId[0]) {
          userCode = values.authorizeEmployeeId[0].userCode;
        ...
        toValue.versionNumber = 1;
        toValue.employeeCode = values.authorizeEmployeeId ? userCode : this.state.employeeCode;
        // httpFetch.post(`${config.payUrl}/api/companyBankAuth/insertOrUpdate`, toValue).then((res) => {
    ...
    if (info) {
      if (info.name) {
        this.props.form.setFieldsValue({
          authorizeCompanyName: info.name,
          authorizeEmployeeId: [],
          authorizeEmployeeName: '',
        });
  ...
  deptChange = value => {
    ...
    if (info) {
      if (info.name) {
        ...
        this.setState({
          extraParams: { ...this.state.extraParams, departmentId: info.departmentId },
        });
      ...
      this.setState({
        extraParams: { ...this.state.extraParams, departmentId: '' },
      });
    }
  };
  ...
  employeeChange = value => {
    ...
    if (info) {
      if (info.userName) {
        ...
        this.setState({ employeeCode: info.userCode });
      }
    }
  };
          ...
          <Form onSubmit={this.handleSave}>
            <FormItem {...formItemLayout} label={messages('company.startDateActive')}>
              {getFieldDecorator('authorizeDateFrom', {
                ...
                initialValue: params.authorizeDateFrom
                  ? moment(params.authorizeDateFrom, dateFormat)
                  : null,
              })(
            ...
            <FormItem {...formItemLayout} label={messages('company.endDateActive')}>
              {getFieldDecorator('authorizeDateTo', {
                ...
                initialValue: params.authorizeDateTo
                  ? moment(params.authorizeDateTo, dateFormat)
                  : null,
            ...
            <FormItem {...formItemLayout} label={messages('common.column.status')}>
              {getFieldDecorator('enabled', {
                ...
                <Switch
                  checked={this.props.form.getFieldValue('enabled') ? true : false}
            ...
            <FormItem {...formItemLayout} label={messages('authorization.employeeCode')}>
              {getFieldDecorator('authorizeEmployeeId', {
                ...
                //   message: messages('common.please.enter')
                // }],
                initialValue: params.authorizeEmployeeId
                  ? [{ userOid: params.authorizeEmployeeId, userCode: params.employeeCode }]
                  : [],
              })(
                <Chooser
                  placeholder={messages('common.please.select')}
                  type="select_authorization_user_new"
