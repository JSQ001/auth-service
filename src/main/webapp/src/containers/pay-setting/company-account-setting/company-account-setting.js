...
class WrappedCompanyAccountSetting extends React.Component {
    ...
    this.state = {
      ...
      searchForm: [
        ...
        } /*公司名称*/,
        {
          ...
          colSpan: 6,
          id: 'companyBankName',
          label: messages('bank.account.bankAccountName'),
        } /*银行账户名称*/,
        {
          type: 'input',
          colSpan: 6,
          id: 'companyBankCode',
          label: messages('bank.account.bankAccountNumber'),
        } /*银行账户账号*/,
      ],
      searchParams: {},
      columns: [
        {
          title: messages('company.setOfBooksName'),
        ...
        {
          title: messages('bank.account.companyCode'),
          dataIndex: 'companyCode',
          width: 100,
          align: 'left',
          render: (remark, record, index) => (
            <Popover content={record.companyCode}>{record.companyCode}</Popover>
          ),
        },
        {
          title: messages('bank.account.companyName'),
          dataIndex: 'companyName',
          width: 100,
          align: 'left',
          render: (remark, record, index) => (
            <Popover content={record.companyName}>{record.companyName}</Popover>
          ),
        },
        {
          title: messages('bank.account.bankName'),
          dataIndex: 'bankName',
          width: 110,
          align: 'left',
          render: (remark, record, index) => (
            <Popover content={record.bankName}>{record.bankName}</Popover>
          ),
        },
        {
          title: messages('bank.account.country'),
          dataIndex: 'countryCode',
          width: 60,
          align: 'left',
          render: (remark, record, index) => (
            <Popover content={record.country}>{record.country}</Popover>
          ),
        },
        {
          title: messages('bank.account.opening'),
          dataIndex: 'city',
          width: 120,
          align: 'left',
          render: (remark, record, index) => (
        ...
        {
          title: messages('bank.account.bankAddress'),
          dataIndex: 'bankAddress',
          width: 100,
          align: 'left',
          render: (remark, record, index) => (
            <Popover content={record.bankAddress}>{record.bankAddress}</Popover>
          ),
        },
        {
          title: messages('bank.account.bankAccountName'),
          dataIndex: 'bankAccountName',
          width: 100,
          align: 'left',
          render: (remark, record, index) => (
            <Popover content={record.bankAccountName}>{record.bankAccountName}</Popover>
          ),
        },
        {
          title: messages('bank.account.bankAccountNumber'),
          dataIndex: 'bankAccountNumber',
          width: 100,
          align: 'left',
          render: (remark, record, index) => (
            <Popover content={record.bankAccountNumber}>{record.bankAccountNumber}</Popover>
          ),
        },
        {
          title: messages('bank.account.currencyCode'),
        ...
        {
          title: messages('bank.account.remark'),
          dataIndex: 'remark',
          width: 50,
          align: 'left',
          tooltips: true,
        ...
        },
        {
          ...
          render: (text, record) => (
            <span>
              <a style={{ marginRight: 10 }} onClick={e => this.rowClick(e, record)}>
                {messages('company.maintain.detail')}
              </a>
            </span>
          ),
        },
      ...
      newData: [],
      bankAccountPageDetail:
        '/pay-setting/company-account-setting/bank-account-detail/:companyBankId', //银行账户详情
    };
  }
  ...
  getList = key => {
    ...
    // let url = `${config.payUrl}/api/CompanyBank/selectByCompanyId?page=${this.state.page}&size=${this.state.pageSize}${this.props.main.tenantMode ? "" : "&companyId=" + this.props.company.id}`;
    ...
    let temp = '';
    return companyAccountSettingService
      .getList(this.state.page, this.state.pageSize, temp, params)
      .then(response => {
