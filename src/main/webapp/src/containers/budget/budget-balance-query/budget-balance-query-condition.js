...
class BudgetBalanceCondition extends React.Component {
          ...
          render: (text, record) => (
            <span>
              <a onClick={e => this.useCondition(e, record)}>
                {this.$t('budget.balance.use.condition') /*使用方案*/}
              </a>
              <span className="ant-divider" />
              <Popconfirm
                onConfirm={e => this.deleteCondition(e, record)}
                title={
  ...
  getList = () => {
    ...
    return httpFetch
      .get(`${config.budgetUrl}/api/budget/balance/query/header/list/solution`, { page, size })
      .then(res => {
        this.setState({
          data: res.data,
          loading: false,
  ...
  deleteCondition = (e, record) => {
    this.setState({ loading: true });
    httpFetch
      .delete(`${config.budgetUrl}/api/budget/balance/query/header/${record.id}`)
      .then(res => {
        this.setState({ loading: false });
        this.getList();
      });
    ...
    return (
      <div>
        ...
        <div className="slide-footer">
          <Button type="primary" onClick={this.getList} loading={loading}>
            {this.$t('budget.balance.refresh.list')}
          </Button>
        </div>
      </div>
    );
