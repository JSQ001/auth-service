      ...
      columns: [
        ...
        },
        {
          ...
          render: (recode, text) => {
            switch (recode) {
              case 1001: {
                return <Badge status="processing" text={this.$t('budgetJournal.editor')} />; //编辑中
              }
              case 1003: {
                return <Badge status="default" text={this.$t('budgetJournal.returnCommit')} />; //撤回
              }
              case 1005: {
                return (
                  <Badge
                    status="error"
                    text={this.$t('budgetJournal.rejection.of.examination.and.approval')}
                  />
                ); //审批驳回
              }
              case 1004: {
                return (
                  <Badge status="success" text={this.$t('budgetJournal.approval.and.approval')} />
                ); //审批通过
              }
              case 1002: {
                return <Badge status="warning" text={this.$t('budgetJournal.approval')} />; //审批中
              }
              case 5001: {
                return <Badge status="default" text={this.$t('budgetJournal.to.review')} />; //复核
              }
              case 5002: {
                return (
                  <Badge status="default" text={this.$t('budgetJournal.backswing.submission')} />
                ); //反冲提交
              }
              case 5003: {
                return <Badge status="default" text={this.$t('budgetJournal.recoil.check')} />; //反冲复核
              }
            }
          },
        },
