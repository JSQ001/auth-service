...
class BudgetBalanceResult extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      menuText: {
        ...
        expReserveAmount: `${this.$t('budget.balance.total')}${this.$t(
          'budget.balance.budget.rsv'
        )}`,
        expUsedAmount: `${this.$t('budget.balance.total')}${this.$t('budget.balance.budget.usd')}`,
        expAvailableAmount: `${this.$t('budget.balance.total')}${this.$t(
          'budget.balance.budget.avb'
        )}`,
      },
      page: 0,
      pageSize: 10,
  ...
  componentWillMount() {
    httpFetch
      .get(`${config.budgetUrl}/api/budget/balance/query/header/${this.props.match.params.id}`)
      .then(res => {
        let companyNumber = 0;
        res.data.queryLineList.map(item => {
          if (item.parameterCode === 'COMPANY')
            companyNumber = item.allFlag
              ? this.$t('common.all')
              : this.$t('common.total1', { total: item.queryParameterList.length });
        });
        this.setState({
          condition: {
  ...
  getList = () => {
    ...
    return httpFetch
      .get(
        `${config.budgetUrl}/api/budget/balance/query/results/${this.props.match.params.id}?page=${
          this.state.page
        ...
        if (res.data) {
          data = res.data.queryResultList.map((item, index) => {
            ...
            item.schedule =
              item.bgtAmount === 0
                ? 0
                : (item.expReserveAmount + item.expUsedAmount) / item.bgtAmount;
            return item;
          });
  ...
  render() {
    ...
    return (
      <div className="budget-balance-result">
        ...
        <SlideFrame
          title={slideFrameParam.title}
          width="70%"
          show={showSlideFrameFlag}
          onClose={() => this.setState({ showSlideFrameFlag: false })}
        >
          <BudgetBalanceAmountDetail params={{ ...slideFrameParam, visible: showSlideFrameFlag }} />
        </SlideFrame>
        ...
        <div className="footer-operate">
          <Button
            ...
            onClick={() => {
              this.props.dispatch(
                routerRedux.replace({
                  pathname: '/budget/budget-balance/budget-balance',
                })
              );
            }}
          >
            {this.$t('budget.balance.modify.params')}
          </Button>
          <Button onClick={this.getList} style={{ marginLeft: 10 }}>
            {this.$t('budget.balance.search.again')}
          </Button>
          <Button
            style={{ marginLeft: 10 }}
            onClick={() => this.setState({ showExporterFlag: true })}
          >
