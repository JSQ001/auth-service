...
class BudgetBalance extends React.Component {
    ...
    this.getSystemValueList(2012).then(res => {
      ...
      res.data.values &&
        res.data.values.map(data => {
          queryLineListTypeOptions.push({ label: data.name, value: data.value });
        });
  ...
  renderColumns = (index, dataIndex) => {
    ...
    console.log(params, paramValueMap);
    switch (dataIndex) {
      case 'type': {
        return (
          <Select
  ...
  getValueListOptions = item => {
    if (item.options.length === 0 || (item.options.length === 1 && item.options[0].temp)) {
      this.getSystemValueList(item.valueListCode).then(res => {
        let options = [];
        res.data.values.map(data => {
          options.push({ label: data.name, key: data.value, value: data });
        });
        let searchForm = this.state.searchForm;
  ...
  setFieldsByStructureId = () => {
    ...
    structureId &&
      httpFetch.get(`${config.budgetUrl}/api/budget/structures/${structureId}`).then(res => {
        ...
        let searchForm = this.state.searchForm;
        searchForm[4].items[0].isRequired = searchForm[4].items[1].isRequired =
