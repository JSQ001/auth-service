...
class ApplicationType extends Component {
      ...
      columns: [
        {
          ...
          render: (setOfBooksName, record) => {
            return (
              <Popover content={record.setOfBooksName}>
  ...
  dimensionSetting = id => {
    this.props.dispatch(
      routerRedux.push({
        pathname: '/document-type-manage/travel-application-type/dimension-setting/' + id,
      })
    );
  };
  ...
  distributionCompany = record => {
    this.props.dispatch(
      routerRedux.push({
        pathname:
          '/document-type-manage/travel-application-type/distribution-company/' +
          record.id +
          '/' +
          record.setOfBooksId,
      })
    ...
    return (
      <div>
        ...
        <CustomTable
          ...
          columns={columns}
          url={`${config.expenseUrl}/api/travel/application/type/pageByCondition`}
