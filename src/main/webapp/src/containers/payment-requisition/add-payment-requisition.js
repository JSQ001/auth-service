...
class AddPaymentRequsition extends React.Component {
    ...
    this.setState(
      ...
      () => {
        ...
        */
        let url = `${
          config.expenseUrl
        }/api/expense/report/type/queryByformTypes?roleType=TENANT&setOfBooksId=${
          this.props.company.setOfBooksId
        }`;
        let param = {
          formTypes: this.props.params.typeDeatilParams.formTypes,
        };
        httpFetch.get(url, param).then(res => {
          let list = [];
          res.data.map(item => {
  ...
  getBankList = (parterId, partnerCategory) => {
    if (partnerCategory === 'EMPLOYEE') {
      httpFetch
        .get(`${config.mdataUrl}/api/contact/bank/account/user/id?userID=${parterId}`)
        .then(res => {
          this.setState({
      ...
      httpFetch.get(`${config.mdataUrl}/api/ven/bank?vendorInfoId=${parterId}`).then(res => {
        this.setState({
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (lineData.availableAmount < values.amount) {
          message.error(this.$t('acp.payment.amountError' /* 本次申请金额大于可申请金额：*/));
          return;
        }
              ...
              <Col span={12} className="payment-form">
                <div className="new-payment-requisition-line">
                  <Form>
                        ...
                        <FormItem>
                          {getFieldDecorator('refDocumentNumber', {
                            rules: [
                              {
                                required: true,
                                message: this.$t('acp.select.publicReport' /* 请先选择报账单*/),
                              },
                            ],
                            ...
                            <Select
                              onDropdownVisibleChange={() => {
                                this.setState({ isReload: false, nowPage: 'type' });
                              }}
                    ...
                    <div className="common-item-title">{this.$t('acp.detail' /* 详情*/)}</div>
                    ...
                    <FormItem {...formItemLayout} label={this.$t('acp.bank.number' /*银行账号*/)}>
