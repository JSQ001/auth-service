...
class NewPaymentRequisitionDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      editPaymentRequisition: '/my-payment-requisition/new-payment-requisition/:id/:typeId', //新建付款申请单
        ...
        if (res.status == 200) {
          ...
          if (!headerInfo.businessCode) {
            headerInfo = {
              ...
              currencyCode: 'CNY',
              infoList: [
                { label: '申请人', value: res.data.employeeName },
  ...
  wrapClosee = content => {
    ...
    return React.createElement(content, Object.assign({}, newProps.match, newProps));
  };
    ...
    return (
      <div style={{ paddingBottom: 100 }} className="pre-payment-common">
        <Spin spinning={false}>
          ...
          >
            <div>{this.wrapClosee(ExpreportDetail)}</div>
          </Modal>
