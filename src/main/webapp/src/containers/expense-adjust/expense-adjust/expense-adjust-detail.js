...
class ExpenseAdjustDetail extends React.Component {
  ...
  //返回
  onCancel = () => {
    this.props.dispatch(routerRedux.push({ pathname: '/my-expense-adjust/my-expense-adjust' }));
  };
  ...
  handleHeadEdit = () => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/my-expense-adjust/edit-expense-adjust/${this.state.headerData.id}/${
          this.state.headerData.expAdjustTypeId
        }`,
      })
    );
  };
    ...
    return (
      <div className="adjust-content" style={{ marginBottom: 50, paddingBottom: 20 }}>
        ...
        <div style={{ marginTop: 20, marginBottom: 0, boxShadow: '0 2px 8px rgba(0, 0, 0, 0.15)' }}>
          ...
          <ApproveHistory type="801006" oid={headerData.documentOid} infoData={approveHistory} />
        </div>
        <ApprotionInfo
          close={() => {
        ...
        <ImporterNew
          ...
          errorUrl={`${config.expenseUrl}/api/expense/adjust/lines/import/error/export/${
            this.props.match.params.id
          }/true`}
