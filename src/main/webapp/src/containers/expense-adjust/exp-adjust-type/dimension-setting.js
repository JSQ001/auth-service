...
import service from './exp-adjust-type.service';
...
class DimensionSetting extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      companyTypeList: [
        ...
        { label: '调整单类型代码', id: 'expAdjustTypeCode' },
        { label: '调整单类型名称', id: 'expAdjustTypeName' },
        { label: '状态', id: 'enabled' },
      ],
      companyTypeInfo: {},
      columns: [
  ...
  getBasicInfo = () => {
    const { match } = this.props;
    service.getExpenseAdjustTypeById(match.params.id).then(res => {
      this.setState(
        {
  ...
  handleBack = () => {
    this.props.dispatch(
      routerRedux.push({
        pathname: `/document-type-manage/exp-adjust-type/exp-adjust-type/${
