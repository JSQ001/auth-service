...
class DistributionCompanyExpAdjustType extends Component {
    ...
    this.state = {
      ...
      columns: [
        ...
        { title: this.$t('supplier.company.type'), dataIndex: 'companyType' },
        {
          ...
          dataIndex: 'enabled',
          render: (value, record) => {
            return <Checkbox onClick={e => this.handleEnabledClick(e, record)} checked={value} />;
          },
        },
      ],
      data: [],
  ...
  getBasicInfo = () => {
    expAdjustService.getExpenseAdjustTypeById(this.props.match.params.id).then(res => {
      ...
      this.setState({
        companyTypeInfo: res.data.expenseAdjustType,
        selectorItem,
      });
    });
  };
  ...
  handleBack = () => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: '/document-type-manage/exp-adjust-type/exp-adjust-type/:setOfBooksId'.replace(
          ':setOfBooksId',
  ...
  render() {
    ...
    return (
      <div className="company-distribution" style={{ paddingBottom: 20 }}>
        <div
          className="jsq"
          style={{ background: '#f5f5f5', width: '100%', height: '90px', borderRadius: '4' }}
        >
          {periodRow}
        </div>
        ...
        <div className="table-header">
          <div className="table-header-buttons">
            <Button type="primary" onClick={() => this.handleListShow(true)}>
              {this.$t('budget.item.batchCompany')}
            </Button>
          </div>
        </div>
        <CustomTable
...
export default connect(
  ...
  null,
  { withRef: true }
)(DistributionCompanyExpAdjustType);
