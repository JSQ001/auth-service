...
class ExpAdjustType extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      pageSize: 10,
      setOfBookList: [],
      setOfBooksId: Number(this.props.match.params.setOfBooksId)
      ...
      searchForm: [
        {
          ...
          id: 'setOfBooksId',
          colSpan: 6,
          label: this.$t({ id: 'adjust.setOfBooks' } /*账套*/),
          options: [],
          labelKey: 'name',
          valueKey: 'id',
          isRequired: 'true',
        ...
        },
        {
          ...
          label: this.$t('adjust.adjustTypeCategory'),
          colSpan: 6,
          event: 'CATE',
          valueKey: 'value',
          options: [
        ...
        },
        {
          type: 'input',
          colSpan: 6,
          id: 'expAdjustTypeCode',
          label: this.$t({ id: 'adjust.expAdjustTypeCode' } /**单据类型代码*/),
        },
        {
          type: 'input',
          colSpan: 6,
          id: 'expAdjustTypeName',
          label: this.$t({ id: 'adjust.expAdjustTypeName' } /**单据类型名称*/),
        },
      ],
      columns: [
        {
          title: this.$t({ id: 'adjust.setOfBooks' } /*账套*/),
        ...
        {
          title: this.$t({ id: 'adjust.expAdjustTypeCode' } /**单据类型代码*/),
          dataIndex: 'expAdjustTypeCode',
          width: '10%',
        ...
        {
          title: this.$t({ id: 'adjust.expAdjustTypeName' } /**单据类型名称*/),
          dataIndex: 'expAdjustTypeName',
          width: '13%',
          render: expAdjustTypeName => (
            <Popover content={expAdjustTypeName}>{expAdjustTypeName}</Popover>
          ),
        },
        {
          title: this.$t({ id: 'adjust.adjustTypeCategory' } /**调整类型*/),
          dataIndex: 'adjustTypeCategory',
          width: '9%',
          render: adjustTypeCategory => {
            return <span>{adjustType[adjustTypeCategory]}</span>;
          },
        },
        {
          title: this.$t({ id: 'adjust.budgetFlag' } /**预算管控*/),
        ...
        {
          title: this.$t({ id: 'adjust.accountFlag' } /**核算*/),
          dataIndex: 'accountFlag',
          width: '5%',
          render: (accountFlag, record) =>
            record.adjustTypeCategory === '1001' ? (
        ...
        {
          title: this.$t({ id: 'adjust.formName' } /**关联表单类型*/),
          dataIndex: 'formName',
          width: '13%',
          render: formName => <Popover content={formName}>{formName}</Popover>,
        },
        {
          title: this.$t({ id: 'adjust.enabled' } /**状态*/),
          dataIndex: 'enabled',
          width: '7%',
          render: enabled => (
            <Badge
          ...
          render: (text, record) => (
            <span>
              ...
              <a onClick={() => this.handleDistribute(record)}>
                {this.$t({ id: 'adjust.company.distribution' }) /**公司分配*/}
              </a>
              {/* <Divider type="vertical" />
  ...
  onAccountFlagClick = (e, record) => {
    ...
    expAdjustService
      .updateBudgetorAccountFlag(record.id, !record.accountFlag, record.budgetFlag)
      .then(res => {
        if (res.status === 200) {
  ...
  deleteItem = (e, record) => {
    ...
    e.stopPropagation();
    expAdjustService
      .deleteExpAdjustTypeById(record.id)
      .then(res => {
        if (res.status === 200) {
      ...
      .catch(e => {
        if (e.response) {
          message.error(
            this.$t({ id: 'adjust.delete.fail' } /**删除失败*/) + `${e.response.data.message}`
          );
        }
      });
  };
  ...
  handleNew = () => {
    ...
    this.setState({
      showSlideFrame: true,
      nowType: { setOfBooksId: this.state.setOfBooksId },
      slideFrameTitle: this.$t({ id: 'adjust.new.expAdjustType' } /**新建费用调整单类型*/),
    });
  };
  //关掉侧滑页面之后
    ...
    return (
      <div className="pre-payment-container">
        <h3 className="header-title">
          {this.$t({ id: 'adjust.expense.adjust.type.define' }) /**费用调整单类型定义*/}
        </h3>
        <SearchArea
        ...
        />
        <SlideFrame title={slideFrameTitle} show={showSlideFrame} onClose={this.onSlideFrameClose}>
          <NewExpAdjustType
