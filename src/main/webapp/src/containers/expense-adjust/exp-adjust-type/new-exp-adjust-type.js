  ...
  constructor(props) {
    ...
    this.state = {
      ...
      adjustTypeCategoryList: [
        { value: '1001', label: this.$t('exp.adjust.exp.detail') },
        { value: '1002', label: this.$t('exp.adjust.exp.add') },
      ],
      //预算置灰勾选
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (this.state.applyEmployee == '1002' && !this.state.departmentOrUserGroupIdList.length) {
          message.warning(
            this.$t({ id: 'adjust.departmentGroupIdList.warn' } /*请至少选择一个部门*/)
          );
          return;
        }
        ...
        let params = {
          ...
          departmentOrUserGroupIdList:
            this.state.applyEmployee == '1001' ? [] : this.state.departmentOrUserGroupIdList,
        };
        this.setState({ loading: true });
        // httpFetch[this.state.nowType.id ? 'put' : 'post'](`${config.expAdjustUrl}/api/expense/adjust/types`, params).then(res => {
        expAdjustService
          .saveExpAdjustTypeLineData(this.state.nowType.id ? 'put' : 'post', params)
          .then(res => {
            this.setState({ loading: false });
            message.success(
              this.$t({ id: 'common.save.success' }, { name: values.expAdjustTypeName })
            );
          ...
          .catch(e => {
            if (e.response) {
              message.error(
                this.$t({ id: 'adjust.save.fail' } /*保存失败*/) + `${e.response.data.message}`
              );
            }
            this.setState({ loading: false });
          });
      ...
      <div>
        <Form onSubmit={this.handleSave}>
          <div className="common-item-title">
            {this.$t({ id: 'adjust.essential.information' }) /*基本信息*/}
          </div>
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'adjust.formName' } /**关联表单类型*/)}
          >
            {getFieldDecorator('formOid', {
              ...
              <Select
                allowClear
                onFocus={requisitionList.length === 0 ? this.onFormFocus : () => {}}
                placeholder={this.$t({ id: 'common.please.select' })}
              >
          ...
          <div className="common-item-title">
            {this.$t({ id: 'adjust.expense.adjust.set' }) /*费用调整设置*/}
          </div>
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'adjust.adjustTypeCategory' } /**调整类型*/)}
          >
            {getFieldDecorator('adjustTypeCategory', {
              ...
              <Select
                placeholder={this.$t({ id: 'common.please.select' })}
                onChange={this.onAdjustTypeChange}
              >
          ...
          <FormItem {...formItemLayout} label={this.$t({ id: 'adjust.accountFlag' } /**核算*/)}>
            ...
            })(<Checkbox disabled={accountFlagDisabled} />)}
          </FormItem>
          <div className="common-item-title">
            {this.$t({ id: 'adjust.expense.type.set' }) /*费用类型设置*/}
          </div>
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'adjust.optional.expense.type' }) /*可选费用类型*/}
          >
            <div>
              ...
              <Select
                ref="SelectExpenseType"
                onDropdownVisibleChange={this.showSelectExpenseType}
                placeholder={this.$t({ id: 'common.please.select' })}
                disabled={allExpense}
                value={
                  allExpense
                    ? this.$t({ id: 'adjust.all.type' } /*全部类型*/)
                    : this.$t(
                        { id: 'adjust.expense.type.selected' },
                        { total: `${expenseIdList.length}` } /*已选择了多少个类型*/
                      )
                }
              />
            </div>
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'adjust.applicable.personnel' }) /*适用人员*/}
          >
