...
class SelectExpenseType extends Component {
  constructor(props) {
    ...
    this.state = {
      searchForm: [
        {
          type: 'input',
          id: 'code',
          label: this.$t({ id: 'adjust.expense.type.code' } /*费用类型代码*/),
        },
        {
          type: 'input',
          id: 'name',
          label: this.$t({ id: 'adjust.expense.type.name' } /*费用类型名称*/),
        },
        {
          type: 'select',
          id: 'range',
      ...
      columns: [
        {
          title: this.$t({ id: 'adjust.expense.type.icon' } /*图标*/),
          dataIndex: 'iconUrl',
          render: (value, record) => {
  ...
  getList = () => {
    ...
    return expAdjustService
      .getExpenseType(model)
      .then(res => {
        this.setState(
          {
            data: res.data,
      ...
      .catch(e => {
        message.error(
          this.$t(
            { id: 'adjust.getExpenseTypeData.error' } /*获取数据失败，请稍后重试或联系管理员*/
          )
        );
  ...
  render() {
    ...
    return (
      <Modal
        title={this.$t({ id: 'adjust.choose.expense.type' }) /*选择费用类型*/}
        visible={visible}
        width={800}
        onOk={this.handleOk}
        onCancel={onCancel}
