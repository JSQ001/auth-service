...
import Importer from 'widget/Template/importer.js';
...
class DimensionDeValue extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      selectorItem: {
        title: '批量分配公司',
        url: `${config.mdataUrl}/api/dimension/item/assign/company/filter/by/setOfBooksId`,
        searchForm: [
  ...
  onCompanyClick = (e, record) => {
    ...
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/admin-setting/dimension-definition/batch-company/${setOfBooksId}/${dimensionId}/${
          record.id
        }`,
      })
    );
      ...
      <div>
        <div className="ant-row" style={{ padding: '20px 0' }}>
          <div className="ant-col-18">
            <Button type="primary" style={{ marginRight: '20px' }} onClick={this.addNewDimValue}>
              新建维值
            </Button>
            <Button style={{ marginRight: '20px' }} onClick={this.handelImport}>
              导入维值
            </Button>
        ...
        >
          <ValueForm params={modelData} close={this.closeFormModal} dimensionId={dimensionId} />
        </SlideFrame>
        <ListSelector
        ...
        <ImporterNew
          ...
          templateUrl={`${config.mdataUrl}/api/dimension/item/template`}
          deleteDataUrl={`${config.mdataUrl}/api/dimension/item/import/delete`}
          errorUrl={`${config.mdataUrl}/api/dimension/item/import/error/export`}
          errorDataQueryUrl={`${config.mdataUrl}/api/dimension/item/import/query/result`}
          uploadUrl={`${config.mdataUrl}/api/dimension/item/import?dimensionId=${dimensionId}`}
          fileName="维值导入模板"
          afterClose={() => {
            this.setState({ importerVisible: false });
          }}
