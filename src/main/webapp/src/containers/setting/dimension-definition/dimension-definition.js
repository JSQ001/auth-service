...
class Dfinition extends Component {
    ...
    this.state = {
      searchForm: [
        ...
        },
        {
          ...
          label: '状态',
          colSpan: 6,
          options: [{ value: false, label: '禁用' }, { value: true, label: '启用' }],
          valueKey: 'value',
          labelkey: 'label',
        },
      ],
  ...
  getSetOfBookList = () => {
    baseService
      ...
      .then(res => {
        ...
        const { searchForm: form, setOfBooksId } = this.state;
        form[0].options = list;
  ...
  clear = () => {
    ...
    this.setState({ searchForm: form, setOfBooksId });
  ...
  render() {
    ...
    return (
      <div>
        ...
        <CustomTable
          columns={columns}
          url={`${config.mdataUrl}/api/dimension/page/by/cond?setOfBooksId=${setOfBooksId}`}
