...
class NewDataAuthority extends React.Component {
  ...
  canceEditHandle = targetKey => {
    DataAuthorityService.deletRuleItem(targetKey).then(res => {
      if (res.status === 200) {
        message.success(this.$t('base.rules.deleted.successfully')); /*规则删除成功！*/
      }
    });
    let { renderNewChangeRules } = this.state;
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        let params = {
          ...
          dataAuthorityRules: rules.length
            ? dataAuthorityRules.length
              ? dataAuthorityRules
              : undefined
            : undefined,
          deleted: false,
        ...
        DataAuthorityService.saveDataAuthority(params)
          .then(res => {
            if (res.status === 200) {
              this.props.close();
              message.success(
                this.$t('base.data.access.successfully.saved')
              ); /*数据权限保存成功！*/
            }
          })
          .catch(e => {
  ...
  returnDataAuthorityRules = (values, getRulesArr, rules) => {
        ...
        dataAuthorityRuleDetails: [
          {
            ...
            dataScopeDesc: values[`dataScope1-${rules[i].key}`]
              ? dataScopeDesc[values[`dataScope1-${rules[i].key}`]].label
              : undefined,
            filtrateMethod: values[`filtrateMethod1-${rules[i].key}`]
              ? values[`filtrateMethod1-${rules[i].key}`]
              : undefined,
            filtrateMethodDesc: values[`filtrateMethod1-${rules[i].key}`]
              ? filtrateMethodDesc[values[`filtrateMethod1-${rules[i].key}`]].label
              : null,
          ...
          },
          {
            ...
            dataScopeDesc: values[`dataScope2-${rules[i].key}`]
              ? dataScopeDesc[values[`dataScope2-${rules[i].key}`]].label
              : undefined,
            filtrateMethod: values[`filtrateMethod2-${rules[i].key}`]
              ? values[`filtrateMethod2-${rules[i].key}`]
              : undefined,
            filtrateMethodDesc: values[`filtrateMethod2-${rules[i].key}`]
              ? filtrateMethodDesc[values[`filtrateMethod2-${rules[i].key}`]].label
              : null,
          ...
          },
          {
            ...
            dataScopeDesc: values[`dataScope3-${rules[i].key}`]
              ? dataScopeDesc[values[`dataScope3-${rules[i].key}`]].label
              : undefined,
            filtrateMethod: values[`filtrateMethod3-${rules[i].key}`]
              ? values[`filtrateMethod3-$${rules[i].key}`]
              : undefined,
            filtrateMethodDesc: values[`filtrateMethod3-${rules[i].key}`]
              ? filtrateMethodDesc[values[`filtrateMethod3-${rules[i].key}`]].label
              : null,
          ...
          },
          {
            ...
            dataScopeDesc: values[`dataScope4-${rules[i].key}`]
              ? dataScopeDesc[values[`dataScope4-${rules[i].key}`]].label
              : undefined,
            filtrateMethod: values[`filtrateMethod4-${rules[i].key}`]
              ? values[`filtrateMethod4-${rules[i].key}`]
              : undefined,
            filtrateMethodDesc: values[`filtrateMethod4-${rules[i].key}`]
              ? filtrateMethodDesc[values[`filtrateMethod4-${rules[i].key}`]].label
              : null,
    ...
    if (dataAuthorityRules.length) {
      for (let i = 0; i < dataAuthorityRules.length; i++) {
        dataAuthorityRules = dataAuthorityRules.filter(
          item => item.dataAuthorityRuleName !== undefined
        );
    ...
    return (
      <div className="new-payment-method">
        ...
        <Form onSubmit={this.handleSave}>
          ...
          <FormItem {...formItemLayout} label={this.$t('base.data.access.name')} /*数据权限名称*/>
            {getFieldDecorator('dataAuthorityName', {
              ...
              <div>
                <LanguageInput
                  ...
                  i18nName={
                    newDataPrams.i18n && newDataPrams.i18n.dataAuthorityName
                      ? newDataPrams.i18n.dataAuthorityName
                      : null
                  }
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t('base.data.access.specification')} /*数据权限说明*/
          >
            {getFieldDecorator('description', {
              ...
              <div>
                <LanguageInput
                  ...
                  i18nName={
                    newDataPrams.i18n && newDataPrams.i18n.description
                      ? newDataPrams.i18n.description
                      : null
                  }
                  placeholder={this.$t('common.please.enter') /* 请输入 */}
                  isEdit={newDataPrams.id ? true : false}
          ...
          <div>
            {this.$t('base.data.permissions')}
            {/*数据权限设置*/}
          </div>
          <Divider />
          <Alert message={this.$t('base.desc.code13')} type="info" showIcon />
          <Spin spinning={false}>
            <div style={{ marginTop: 24 }}>{renderNewChangeRules}</div>
            <div style={{ marginTop: 24 }}>
              <Row>
                <Col offset={3} span={18}>
                  <Button type="dashed" style={{ high: 40, width: '100%' }} onClick={this.addApply}>
                    <Icon type="plus" />
                    {this.$t('base.add.rules')}
                  </Button>
                  {/*添加规则*/}
                </Col>
              </Row>
