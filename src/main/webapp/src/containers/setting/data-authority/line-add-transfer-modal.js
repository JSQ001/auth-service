...
class LineAddTransferModal extends React.Component {
      ...
      if (item.details.length) {
        return (
          <TreeNode
            ...
            title={
              <span>
                <span className="left-title-w1">
                  {item.code}-{item.name}
                </span>
                {item.isSelectAll ? (
                  <a
                    className="right-tree-title"
                    onClick={e => this.cancelSelectAll(e, selectTreeNode, item)}
                  >
                    {this.$t('base.to.cancel.all')}
                  </a>
                ) : (
                  <a
                    className="right-tree-title"
                    onClick={e => this.handleSelectAll(e, selectTreeNode, item)}
                  >
                    {this.$t('base.future.generations')}
                  </a>
                )}
              </span>
            }
  ...
  treeNodeSelect = (selectedKeys, info) => {
    ...
    if (info.selected) {
      ...
      selectedTreeInfo.splice(
        selectedTreeInfo.findIndex(o => o.id == info.node.props.dataRef.id),
        1
      );
    }
  ...
  cancelSelectAll = (e, selectTreeNode, item) => {
    ...
    this.setState({
      treeData,
      selectTreeNodes: [...selectedTreeInfo.map(o => o.id)],
      selectedTreeInfo,
    });
  };
  /**删除右边数据 */
  ...
  renderItems = selectedTreeInfo => {
    return selectedTreeInfo.map(item => {
      return (
        <Row key={item.valueKey ? item.valueKey : item.id} style={{ marginTop: 5 }}>
          <Col span={22} style={{ fontSize: 13 }}>
            {item.valueKey
              ? `${item.valueKeyCode}-${item.valueKeyDesc}`
              : `${item.code}-${item.name}`}
          </Col>
          <Col
            span={2}
  ...
  render() {
    ...
    return (
      ...
      >
        <Row>
          <Col span={12}>
            <Card title={this.$t('base.stay.district')}>
              {/*待选区*/}
              <Search
                style={{ marginBottom: 8 }}
                enterButton
                placeholder={isAddCompany ? this.$t('base.desc.code1') : this.$t('base.desc.code2')}
                onSearch={this.onTreeSelecSearch}
              />
              <div
                className="treeStyle"
                style={{ height: 290, overflowX: 'hidden', overflowY: 'scroll' }}
              >
                {isShowTreeNode ? (
                  ...
                  searchListInfo.map(list => (
                    <Row key={list.id} className="listStyle1">
                      <Col onClick={e => this.clickList(list)}>
                        {list.code}-{list.name}
                      </Col>
                    </Row>
                  ))
          ...
          <Col span={10} offset={2}>
            <Card title={this.$t('base.has.chosen')}>
              {/*已选择*/}
              <Search
                style={{ marginBottom: 8 }}
                placeholder={
                  isAddCompany ? this.$t('base.desc.code1') : this.$t('base.desc.code2')
                } /*请输入部门代码 名称*/ /*请输入公司代码 名称*/
                enterButton
