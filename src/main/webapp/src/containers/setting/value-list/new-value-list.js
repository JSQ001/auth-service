...
class ValueList extends React.Component {
  ...
  componentWillMount() {
    let systemFlag = this.props.match.params.tab === 'SYSTEM' ? true : false;
    if (this.props.match.params.id) {
      ...
      //新增
      this.setState({ typeFlag: systemFlag ? 'SYSTEM' : 'CUSTOM', systemFlag });
    }
  }
  ...
  getInfo = () => {
    valueListService.getValueListInfo(this.state.id).then(res => {
      ...
      this.setState({
        ...
        _form: JSON.parse(JSON.stringify(form)),
      });
    });
  };
  ...
  handleSave = () => {
    ...
    (params.typeFlag = this.state.typeFlag), this.setState({ loading: true });
  ...
  handleEdit = () => {
    this.setState({ edit: true, editBase: false });
  };
  ...
  handleCancel = () => {
    if (this.state.id) {
      ...
      this.setState({
        ...
        form: JSON.parse(JSON.stringify(this.state._form)),
      });
    } else {
      //新增
      let pathname = '/admin-setting/value-list/value-list/:tab'.replace(
        ':tab',
        this.props.match.params.tab
      );
      if (this.state.systemFlag) {
        pathname = '/system/value-list-system/value-list-system/:tab'.replace(
          ':tab',
          this.props.match.params.tab
        );
      }
  ...
  renderForm() {
    let form = this.state.form;
    let systemFlag = this.state.systemFlag;
    let spanLength = systemFlag ? 6 : 8;
    let options = [
        ...
        <Row gutter={12}>
          ...
          <Col span={spanLength}>
            ...
            <Input
              ...
              disabled={
                this.state.id !== null && this.state.id !== undefined && this.state.id !== ''
              }
            ...
            <Col span={spanLength}>
              ...
              <div>
                <Select
                  ...
                  defaultValue={this.state.typeFlag === 'CUSTOM' ? 'SYSTEM' : this.state.typeFlag}
                >
                  {options.map(option => {
        ...
        <Row>
          ...
          {systemFlag ? (
            <Col span={spanLength}>
              ...
              <div>{this.state.typeFlag === 'SYSTEM' ? '系统级值列表' : '初始化值列表'}</div>
            </Col>
          ) : null}
          <Col span={spanLength}>
            <div className="form-title">
            ...
            <div>
              <Badge
                status={this.state.form.enabled ? 'success' : 'error'}
                text={
          ...
          <div>
            <div className="common-top-area">
              <div className="common-top-area-title">
                ...
                {(!edit && typeFlag === 'CUSTOM') || systemFlag ? (
                  <span className="title-edit" onClick={this.handleEdit}>
                    {this.state.editBase ? messages('common.edit') : ''}
                  </span>
                ) : null}
              </div>
            ...
            {id && (
              <div>
                ...
                <Table
                  ...
                  columns={typeFlag !== 'SYSTEM' || systemFlag ? columnsCustom : columnsSystem}
                  dataSource={data}
                  pagination={pagination}
                  loading={tableLoading}
                  size="middle"
                  rowSelection={typeFlag !== 'SYSTEM' || systemFlag ? rowSelection : null}
                  bordered
                />
