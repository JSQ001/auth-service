...
class ValueList extends React.Component {
  ...
  componentDidMount() {
    ...
    let status = this.props.match.params.tab;
    this.setState(
      {
        pagination,
        status,
  ...
  getList() {
    ...
    valueListService
      .getValueListList(pagination.page, pagination.pageSize, searchParams)
      .then(res => {
  ...
  handleRowClick = record => {
    ...
    let pathname = `/admin-setting/value-list/value-list-detail/${record.codeOid}/${
      record.id
    }/:tab`.replace(':tab', this.state.status);
    if (this.state.status === 'SYSTEM') {
      pathname = `/system/value-list-system/value-list-detail-system/${record.codeOid}/${
        record.id
      }/:tab`.replace(':tab', this.state.status);
    }
    this.props.dispatch(
  ...
  goValueListPage = () => {
    let pathname = '/admin-setting/value-list/new-value-list/:tab'.replace(
      ':tab',
      this.state.status
    );
    if (this.state.status === 'SYSTEM') {
      pathname = '/system/value-list-system/new-value-list-system/:tab'.replace(
        ':tab',
        this.state.status
      );
    }
