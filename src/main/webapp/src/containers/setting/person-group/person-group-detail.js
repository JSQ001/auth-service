...
class PersonGroupDetail extends React.Component {
   ...
   * @param i    条件中项目对应的序号
   */
  showConditionSelectorHandle = (index, name, itemIndex, selectedData) => {
    ...
    selectedData.map(item => {
      item[valueKey] = item.conditionValue;
      item[labelKey] = item.description;
    });
    this.setState({ showListSelector: true, selectedData, labelKey, valueKey });
  };
  //处理条件添加弹框点击ok,添加值
  handleListOk = result => {
  ...
  _afterSelectDepartment(arr) {
    ...
    conditionViewDTOS[this.editingRule.index].conditionDetails[
      ...
      .concat(arr)
      .map(item => {
        item.conditionValue = item.departmentId;
        item.description = item.name;
        !obj[item.departmentId] && allCondition.push(item);
      });
  ...
  handleNew = () => {
    ...
    let selectorItem = chooserData['filter_user'];
    this.setState({
      selectorItem: selectorItem,
      extraParams: {
        //弹窗额外的参数
