...
class RuleInfo extends React.Component {
   ...
   * @param i    条件中项目对应的序号
   */
  showAddByName = (name, i, index) => {
    this.props.showConditionSelectorHandle(this.props.index, name, i, index);
  };
  /**
   * 移除一个条件项目
   * @param name     标题：部门、级别、类型、职务
  ...
  renderConditionDetailByName = (condition, name) => {
      ...
      domRender = (
        <div className="rule-info-row">
          <div className="rule-condition-checkbox">
            <Checkbox checked={condition.conditionDetails[i].enabled} disabled={true} />
          </div>
    ...
    function _renderConditionTitle(name) {
      if (name === 'Department') {
        return (
          <div className="rule-condition-title">
            {/*部门:*/}
    ...
    function _renderConditionItem(values) {
      ...
      if (values.length > 0) {
        ...
        //最后一个不要添加逗号
        domConditon.push(
          <div className="item" key={values.length - 1}>
      ...
      domRender = (
        <div className="rule-info-row">
          ...
          <div className="rule-condition-sub-title edit-rule-condition-sub-title">
            {this._renderSelectByConditionLogic(
              condition.conditionDetails[i].conditionLogic,
              name,
              i
            )}
          </div>
  ...
  _renderConditionTitle(name) {
    if (name === 'Department') {
      return (
        <div className="rule-condition-title edit-rule-condition-title">
          {/*部门:*/}
          {messages('person.group.rule.dep')}
        </div>
      );
  ...
  _renderConditionItem(values, name, n) {
    ...
    for (let i = 0; i < values.length; i++) {
      domConditon.push(
        ...
        >
          {values[i].description || values[i].name}
        </Tag>
      );
    }
    if (name === 'Department') {
      domConditon.push(
        <Tag
          ...
          key={name}
          onClick={() => {
            this.showAddByName(name, n, values);
          }}
        >
          <Icon type="plus" />
          {/*添加部门*/}
  ...
  render() {
    ...
    return <div className="rule-info">{domRender}</div>;
  }
}
