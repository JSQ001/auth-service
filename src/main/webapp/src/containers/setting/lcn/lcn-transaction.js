...
class LcnTransaction extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      showDetail: false,
      modelTimes: null,
      managerUrl: '/admin-setting/lcn-manager/lcn-manager',
      columns: [
        {
          title: '模块名称',
          dataIndex: 'name',
      ...
      columns2: [
        ...
        },
        {
          ...
          render: (text, record) => (
            <span>
              <a onClick={e => this.detail(e, record)}>详情</a>
              <span className="ant-divider" />
              <Popconfirm title={'确定要补偿吗？'} onConfirm={e => this.compensate(e, record)}>
                <a>补偿</a>
              </Popconfirm>
              <span className="ant-divider" />
              <Popconfirm title={'确定要删除吗？'} onConfirm={e => this.delete(e, record)}>
                <a>删除</a>
              </Popconfirm>
            </span>
          ),
  ...
  rowClick1 = record => {
    httpFetch
      .get(`${config.txManagerUrl}/admin/modelInfos?path==${record.modelTimes}`)
      .then(res => {
        this.setState({
          data2: res.data,
        });
