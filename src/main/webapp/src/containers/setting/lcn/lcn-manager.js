...
class LcnManager extends React.Component {
  ...
  getJson = () => {
    httpFetch
      .get(`${config.txManagerUrl}/admin/json`)
      .then(res => {})
      .catch(() => {
  ...
  getOther = () => {
    let hasCompensate = '无';
    httpFetch
      .get(`${config.txManagerUrl}/admin/hasCompensate`)
      .then(res => {
        if (res.data) {
          hasCompensate = '有';
        } else {
    ...
    return (
      <div className="header-title">
        ...
        <div className="table-header">
          <Button type="primary" style={{ marginRight: 10 }} onClick={this.getList}>
            刷新
          </Button>
          <Button type="primary" style={{ marginRight: 10 }} onClick={this.goModel}>
            在线模块
          </Button>
