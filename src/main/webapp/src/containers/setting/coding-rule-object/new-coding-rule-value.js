  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        let method = this.props.params.nowCodingRuleValue
          ? 'updateCodingRuleValue'
          : 'addCodingRuleValue';
        codingRuleService[method](values)
          .then(res => {
          ...
          .catch(e => {
            if (e.response) {
              message.error(
                `${messages('common.create.filed') /*新建失败*/}, ${e.response.data.message}`
              );
            }
            this.setState({ loading: false });
          });
    ...
    switch (segmentType) {
      case '10':
        result = (
          <FormItem
            {...formItemLayout}
            label={messages('code.rule.value.fixed.character') /*固定字符*/}
          >
            {getFieldDecorator('segmentValue', {
              ...
              initialValue: this.props.params.nowCodingRuleValue
                ? this.props.params.nowCodingRuleValue.segmentValue
                : '',
            })(<Input />)}
          </FormItem>
        );
      ...
      case '20':
        result = (
          <FormItem
            {...formItemLayout}
            label={messages('code.rule.value.date.format') /*日期格式*/}
          >
            {getFieldDecorator('dateFormat', {
              ...
              initialValue: this.props.params.nowCodingRuleValue
                ? this.props.params.nowCodingRuleValue.dateFormat
                : null,
            })(
              <Select
          ...
          <div>
            <FormItem {...formItemLayout} label={messages('code.rule.value.digit') /*位数*/}>
              {getFieldDecorator('length', {
                ...
                initialValue: this.props.params.nowCodingRuleValue
                  ? this.props.params.nowCodingRuleValue.length
                  : 4,
              })(<InputNumber min={1} />)}
            </FormItem>
            <FormItem {...formItemLayout} label={messages('code.rule.value.step.length') /*步长*/}>
              {getFieldDecorator('incremental', {
                ...
                initialValue: this.props.params.nowCodingRuleValue
                  ? this.props.params.nowCodingRuleValue.incremental
                  : 1,
              })(<InputNumber min={1} />)}
            </FormItem>
            <FormItem
              {...formItemLayout}
              label={messages('code.rule.value.start.value') /*开始值*/}
            >
              {getFieldDecorator('startValue', {
                ...
                initialValue: this.props.params.nowCodingRuleValue
                  ? this.props.params.nowCodingRuleValue.startValue
                  : 1,
              })(<InputNumber min={1} />)}
            </FormItem>
          </div>
    ...
    return (
      <div>
        <Form onSubmit={this.handleSave}>
          <FormItem
            {...formItemLayout}
            label={messages('code.rule.value.sequence.number') /*顺序号*/}
          >
            {getFieldDecorator('sequence', {
              ...
              initialValue: this.props.params.nowCodingRuleValue
                ? this.props.params.nowCodingRuleValue.sequence
                : this.props.params.nowSequence,
            })(<InputNumber min={1} />)}
          </FormItem>
          <FormItem
            {...formItemLayout}
            label={messages('code.rule.value.parameter.name') /*参数名称*/}
          >
            {getFieldDecorator('segmentType', {
              ...
              initialValue: this.props.params.nowCodingRuleValue
                ? this.props.params.nowCodingRuleValue.segmentType
                : '',
            })(
              <Select
                placeholder={messages('common.please.select') /* 请选择 */}
          ...
          <FormItem {...formItemLayout} label={messages('common.column.status') /* 状态 */}>
            {getFieldDecorator('enabled', {
              initialValue: this.props.params.nowCodingRuleValue
                ? !!this.props.params.nowCodingRuleValue.enabled
                : true,
