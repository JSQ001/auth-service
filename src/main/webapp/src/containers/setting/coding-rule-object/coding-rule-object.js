...
class CodingRule extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          title: messages('code.rule.document.type') /*单据类型'*/,
          dataIndex: 'documentTypeName',
          width: '40%',
          align: 'center',
        },
        {
          title: messages('code.rule.apply.company') /*应用公司*/,
  ...
  handleNew = () => {
    this.props.dispatch(
      routerRedux.push({
        pathname: '/admin-setting/coding-rule-object/new-coding-rule-object',
      })
    );
  ...
  handleRowClick = record => {
    this.props.dispatch(
      routerRedux.push({
        pathname: '/admin-setting/coding-rule-object/coding-rule/' + record.id,
      })
    );
