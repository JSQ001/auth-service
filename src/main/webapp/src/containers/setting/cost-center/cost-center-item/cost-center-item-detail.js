...
class CostCenterItemDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      tabs: [
        ...
        {
          key: 'DEPARTMENT',
          name: this.$t('cost.center.item.detail.batchadd.dep'),
        } /*按部门添加(含子部门)*/,
      ],
      columns: [
        {
  ...
  getCostCenterItemDetail() {
    CCService.getCostCenterItemDetail(this.props.match.params.itemId).then(res => {
      let data = res.data;
  ...
  getDepartmentList = () => {
    ...
    CCService.getCostcenteritemDepartmentList(this.state.infoData.id, params).then(response => {
  ...
  getUserList() {
    ...
    CCService.getCostCenterItemUsers(params, this.props.match.params.itemId).then(response => {
      response.data.map(item => {
        item.key = item.id;
      });
      pagination.total = Number(response.headers['x-total-count']);
  ...
  costCenterItemAssociateDepsDTO = arr => {
    ...
    CCService.asDepartmentToCostcenteritem(params).then(res => {
      //操作成功
      message.success(this.$t('cost.center.item.detail.success'));
      this.getDepartmentList();
    });
  ...
  removeUserFromCostCenterItemAssociation = (e, record) => {
    CCService.removeUserFromCostCenterItemAssociation(
      record.userOid,
  ...
  removeDepFromCostCenterItemAssociation = (e, record) => {
    CCService.delDepartmentToCostcenteritem(record.id).then(res => {
      //操作成功
      this.getDepartmentList();
      message.success(this.$t('cost.center.item.detail.success'));
    });
  ...
  renderTabs = () => {
    ...
    const suffix = this.state.costCenterItemUserName ? (
      <Icon type="close-circle" onClick={this.emitEmptyForUser} />
    ) : null;
    const suffixDep = this.state.costCenterItemDepName ? (
  ...
  render() {
    ...
    return (
      <div className="cost-center-item-detail">
        ...
        <p />
        {infoData.public ? <div /> : this.renderDepComponents(nowStatus)}
        {infoData.public ? (
          <div />
        ) : (
