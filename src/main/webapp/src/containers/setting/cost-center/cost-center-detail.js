...
import { Button, Badge, message, Icon, Input, Menu, Dropdown, Modal, Upload, Popover } from 'antd';
...
class CostCenterDetail extends React.Component {
  ...
  getCostCenterDetail() {
    CCService.getCostCenterDetail(this.props.match.params.id).then(response => {
      this.setState({
  ...
  getList(isClearRowSelection) {
    ...
    CCService.getCostCenterItemsAll(this.props.match.params.id, params).then(response => {
  ...
  handleNew = () => {
    ...
    this.props.dispatch(
      routerRedux.push({
        pathname: `/admin-setting/cost-center/cost-center-detail/cost-center-item/new-cost-center-item/${
          this.props.match.params.id
        }/NEW/${this.props.match.params.setOfBooksId}`,
      })
    );
  };
  //成本中心项编辑
  ...
  batchAddPerson = arr => {
    ...
    CCService.costCenterItemAssociateUsersDTO(params).then(res => {
      //操作成功
  ...
  refreshRowSelection() {
    ...
    this.state.depNeedMoveOut.map(costCenterItemOid => {
      this.state.data.map((item, index) => {
        if (item.costCenterItemOid === costCenterItemOid) selectedRowKeys.push(index);
      });
    });
  ...
  showTransactionLogDialog = transactionOid => {
    CCService.getCostCenterBatchTransactionLog(transactionOid).then(res => {
      let data = res.data;
  ...
  render() {
    ...
    //   rowSelection = null;
    // }
    const suffix = this.state.costCenterItemName ? (
        ...
        <div className="table-header">
          ...
          <div className="table-header-buttons">
            ...
            <Button
              ...
              disabled={this.state.depNeedMoveOut.length < 1}
            >
              {this.$t('common.enabled' /*启用*/)}
            </Button>
            <Button
        ...
        >
          <div className="import-person-modal-wrap">
            <div className="f-left import-person-modal-left">
              ...
              <div className="download-list-item" onClick={this.downloadTemplatePerson}>
                {/*点击下载模板成本中心项-员工*/}
