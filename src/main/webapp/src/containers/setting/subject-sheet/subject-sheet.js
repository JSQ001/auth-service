...
class SubjectSheet extends React.Component {
    ...
    this.state = {
      ...
      columns: [
        ...
        },
        {
          ...
          width: '30%',
          render: accountSetDesc => <Popover content={accountSetDesc}>{accountSetDesc}</Popover>,
        },
        {
  ...
  getList() {
    ...
    let url = `${config.mdataUrl}/api/account/set/query?&page=${pagination.page}&size=${
      pagination.pageSize
    }`;
  ...
  handleRowClick = (e, value) => {
    ...
    this.props.dispatch(
      routerRedux.push({
        pathname: `/admin-setting/subject-sheet/subject-sheet/subject-sheet-detail/${value.id}`,
      })
    );
  };
      ...
      <div className="budget-organization">
        ...
        <div className="table-header">
          ...
          <div className="table-header-buttons">
            <Button type="primary" onClick={this.newItemShowSlide}>
              {/*新建*/}
        ...
        <SlideFrame
          title={
            JSON.stringify(updateParams) === '{}'
              ? this.$t({ id: 'subject.sheet.new' })
              : this.$t({ id: 'subject.sheet.edit' })
          }
          show={showSlideFrameNew}
          onClose={() => this.setState({ showSlideFrameNew: false })}
        >
