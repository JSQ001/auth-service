...
class SubSubjectMaintain extends React.Component {
    ...
    this.state = {
      ...
      columns: [
        ...
        },
        {
          ...
          width: '40%',
          align: 'center',
          render: accountName => <Popover content={accountName}>{accountName}</Popover>,
        },
        {
        ...
        },
        {
          ...
          render: (text, record) => {
            return (
              <div>
                <Popconfirm
                  onConfirm={e => this.deleteItem(e, record.id)}
                  title={this.$t({ id: 'common.confirm.delete' })}
                >
                  {/* 你确定要删除吗 */}
                  <a
                    href="#"
  ...
  deleteItem = (e, id) => {
    httpFetch
      .delete(`${config.mdataUrl}/api/accounts/hierarchy/${id}`)
      .then(res => {
        if (res.status === 200) {
  ...
  deleteItemBatch = (e, record) => {
    ...
    httpFetch
      .delete(`${config.mdataUrl}/api/accounts/hierarchy/batch/delete`, this.state.selectedRowKeys)
      .then(res => {
        if (res.status === 200) {
          message.success(this.$t({ id: 'common.operate.success' }));
          this.setState(
  ...
  getAddedSubList() {
    ...
    let url = `${config.mdataUrl}/api/accounts/hierarchy/parent/query?parentAccountId=${
      this.state.searchParams.parentAccountId
    }&info=${this.state.searchParams.info}&size=${this.state.pageSize}&page=${this.state.page}`;
    return httpFetch
      .get(url)
      .then(response => {
  ...
  handleListOk = result => {
    ...
    result.result.map(item => {
      param.push({
        parentAccountId: this.state.updateParams.accountId,
        subAccountId: item.subAccountId,
      });
    });
    httpFetch
      .post(`${config.mdataUrl}/api/accounts/hierarchy/batch/insert`, param)
      .then(response => {
    ...
    return (
      <div className="subject-sheet">
        <div className="table-header">
          ...
          <div className="table-header-buttons">
            <Button type="primary" onClick={() => this.showListSelector(true)}>
              {this.$t({ id: 'subject.add.sub.subject' })}
            </Button>{' '}
            {/* 新建 */}
            <Popconfirm
              onConfirm={e => this.deleteItemBatch(e)}
              title={this.$t({ id: 'common.confirm.delete' })}
            >
              <Button
        ...
        <ListSelector
          ...
          extraParams={{
            parentAccountId: this.state.updateParams.accountId,
            accountSetId: this.state.updateParams.accountSetId,
          }}
