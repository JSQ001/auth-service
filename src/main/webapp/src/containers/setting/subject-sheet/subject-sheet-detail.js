...
class SubjectSheetDetail extends React.Component {
    ...
    this.state = {
        ...
        },
        {
          ...
          render: enabled => (
            <Badge
              ...
              text={
                enabled ? this.$t({ id: 'common.enabled' }) : this.$t({ id: 'common.disabled' })
              }
            />
          ),
        },
      ...
      columns: [
        ...
        },
        {
          ...
          render: summaryFlag => (
            <Badge
              status={summaryFlag ? 'success' : 'error'}
                ...
                <div>
                  <span>
                    ...
                    <a
                      ...
                      onClick={e =>
                        this.subSubjectItem(record.id, this.props.match.params.accountSetId)
                      }
                    >
                      {this.$t({ id: 'subject.sub.subject' }) /*子科目*/}
                    </a>
                  </span>
                </div>
              );
  ...
  componentWillMount() {
    //根据科目表ID，取科目表信息
    httpFetch
      .get(`${config.mdataUrl}/api/account/set/${this.props.match.params.accountSetId}`)
      .then(res => {
  ...
  getList() {
    ...
    let url = `${config.mdataUrl}/api/accounts/query?accountSetId=${
      this.props.match.params.accountSetId
    }&info=${this.state.searchParams.info}&accountType=${
      this.state.searchParams.accountType
    }&size=${this.state.pagination.pageSize}&page=${this.state.pagination.page}`;
    return httpFetch
      .get(url)
      .then(response => {
  ...
  showSlideNew = (flag, record, accountTypeOptions, isNew) => {
    this.setState({
      ...
      showSlideFrameTitle: isNew
        ? this.$t({ id: 'subject.sheet.new.subject' }) /*新建科目*/
        : this.$t({ id: 'subject.sheet.edit.subject' }) /*编辑科目*/,
    });
  };
  ...
  headerUpdateHandle = values => {
    ...
    httpFetch
      .put(`${config.mdataUrl}/api/account/set`, toValue)
      .then(res => {
        this.setState(
          {
            infoData: res.data,
        ...
        <div className="table-header">
          <div className="table-header-title">
            <h2 style={{ marginBottom: -10 }}>
              {this.$t({ id: 'subject.sheet.detail' }) /*科目明细*/}
            </h2>
          </div>
          <div className="table-header-title">
            ...
            <Row>
              ...
              <Col span="3">
                <Select
                  ...
                  onFocus={this.clickAccountTypeSelect}
                >
          ...
          <div style={{ fontSize: 14, margin: '20px 0' }}>
            <a
              onClick={() => {
                ...
                this.props.dispatch(
                  routerRedux.push({
                    pathname: `/admin-setting/subject-sheet/subject-sheet`,
                  })
                );
              }}
            >
