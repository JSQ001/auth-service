  ...
  handleClose = params => {
    this.setState(
      ...
      () => {
        ...
        params && this.table.search({ ...this.state.searchParams, parameterLevel: 'TENANT' }, true);
      }
    );
  };

  render() {
    const { visible, record, sob, searchForm, columns } = this.state;
    return (
      <div className="parameter-definition">
        <div className="content-sob" style={{ marginTop: 15 }}>
          ...
          <CustomTable
            ...
            url={`${
              config.mdataUrl
            }/api/parameter/setting/page/by/level/cond?parameterLevel=TENANT`}
