...
class ExpenseTypeCustom extends React.Component {
  ...
  handleDrop = (widget, index) => {
    ...
    nowWidgets.map(nowWidget => {
      if (targetRule.type.indexOf(nowWidget.fieldType) > -1) limit++;
    });
  ...
  getWidgetType = () => {
    ...
    customWidget.map(item => {
      if (widgetType === item.fieldType) result = item.name;
    });
    return result;
  };
  ...
  handleSave = () => {
    ...
    nowWidgets.map((widget, index) => {
      ...
      if (widget.fieldType === 'LOCATION') widget.messageKey = 'location';
      //同行人控件增加participant messageKey，app用到
      if (widget.fieldType === 'PARTICIPANT') widget.messageKey = 'participant';
      //参与人控件增加participants messageKey，app用到
      if (widget.fieldType === 'PARTICIPANTS') widget.messageKey = 'participants';
      //开始结束日期增加dateCombined messageKey, app用到
      if (widget.fieldType === 'START_DATE_AND_END_DATE') widget.messageKey = 'dateCombined';
    });
    if (!hasError) {
      this.setState({ saving: true });
      expenseTypeService
        .saveExpenseTypeFields(expenseType.id, nowWidgets)
        .then(res => {
            ...
            {loading ? (
              ...
              <div className="widget-area">
                <div className="widget-list">
                  ...
                  <div className="widget-list">
                    {customWidget
                      .filter(widget => widget.type === 'SYSTEM' && widget.id !== '9')
                      .map(widget => <DragWidgetItem widget={widget} key={widget.id} />)}
                  </div>
          ...
          <Col span={8} className="widget-setting">
            {nowWidget ? (
              <div>
                ...
                {(nowWidget.fieldType === 'CUSTOM_ENUMERATION' ||
                  ...
                  <div>
                    <div className="widget-setting-title">
                      {messages('expense.type.custom.value.list')}
                    </div>
                    {loading ? (
                      <Spin />
                    ) : (
                ...
                <div className="widget-setting-title">
                  <Checkbox
                    onChange={e => this.handleChangeCheckBox(e, 'required')}
                    checked={nowWidget.required}
                    disabled={!tenantMode}
                  >
                    {messages('expense.type.required')}
                  </Checkbox>
                </div>
                <div className="widget-setting-title">
                  <Checkbox
                    onChange={e => this.handleChangeCheckBox(e, 'showOnList')}
                    checked={nowWidget.showOnList}
                    disabled={!tenantMode}
                  >
                    {messages('expense.type.show')}
                  </Checkbox>
                </div>
                <div className="widget-setting-title">
                  <Checkbox
                    onChange={e => this.handleChangeCheckBox(e, 'printHide')}
                    checked={nowWidget.printHide}
                    disabled={!tenantMode}
                  >
                    {messages('expense.type.hide.when.print')}
                  </Checkbox>
                </div>
                <div className="widget-setting-title">
                  <Checkbox
                    onChange={e => this.handleChangeCheckBox(e, 'editable')}
                    checked={nowWidget.editable}
                    disabled={!tenantMode}
                  >
                    {messages('expense.type.edit')}
                  </Checkbox>
                </div>
                <div className="widget-setting-title">
                  <Checkbox
                    onChange={this.handleChangeReportKeyEnabled}
                    checked={enabledReportKey}
                    disabled={!tenantMode}
                  >
                    {messages('expense.type.report')}
                    <span className="widget-setting-checkbox-info">
                      {messages('expense.type.report.content')}
                    </span>
                  </Checkbox>
                  {!loading &&
                    enabledReportKey && (
                      ...
                      >
                        {reportValueList
                          .filter(enumeration => enumeration.enabled)
                          .map(enumeration => (
                            <Option value={enumeration.value} key={enumeration.value}>
...
export default connect(
  ...
  null,
  { withRef: true }
)(DragDropContext(HTML5Backend)(ExpenseTypeCustom));
