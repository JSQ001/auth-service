...
class ExpenseTypeScope extends React.Component {
  ...
  handleSave = () => {
    ...
    expenseTypeService
      .saveExpenseTypeScope(target, expenseType.id)
      .then(res => {
  ...
  render() {
    const { userValue, saving, loading, companys } = this.state;
    ...
    return (
      <div>
        <Row gutter={20}>
          <Col span={4}>
            <div style={{ textAlign: 'right', fontWeight: 'bold' }}>适用公司:</div>
          </Col>
          <Col span={8}>
            ...
            {this.state.value == 2 && (
              <div style={{ width: 200 }}>
                <Chooser
                  ...
                  valueKey="id"
                  showNumber
                  listExtraParams={{ setOfBooksId: this.props.expenseTypeSetOfBooks.id }}
