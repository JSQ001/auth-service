...
class CustomExpenseType extends React.Component {
  ...
  handleNewExpenseType = () => {
    ...
    this.props.dispatch(
      routerRedux.push({
        pathname: '/admin-setting/expense-type/new-expense-type/0',
      })
    );
  };
  ...
  handleEditExpenseType = id => {
    let path =
      this.state.activeKey == '1'
        ? '/admin-setting/expense-type/application-type-detail/'
        : '/admin-setting/expense-type/expense-type-detail/';
    this.props.dispatch(
      routerRedux.push({
        pathname: path + id,
      })
  ...
  handleClickEditMenu = (e, expenseTypeCategory) => {
    this.setState({
      travelTypeFlag: expenseTypeCategory.travelTypeFlag,
    });
    switch (e.key) {
      case 'rename':
        this.setState({
          nowEditCategory: JSON.parse(JSON.stringify(expenseTypeCategory)),
        ...
        if (expenseTypeCategory.expenseTypes.length > 0)
          Modal.warning({
            title: messages('expense.type.move.before.remove', {
              name: this.state.activeKey === '1' ? '申请' : '费用',
            }), //删除前请先移动该大类下的费用类型
          });
        else
          expenseTypeService
            .deleteCategory(expenseTypeCategory.id)
            .then(res => {
  ...
  handleEditCategory = () => {
    const { nowEditCategory, travelTypeFlag } = this.state;
  ...
  handleNewApplicationType = () => {
    this.props.dispatch(
      routerRedux.push({
        pathname: '/admin-setting/expense-type/new-application-type/0',
      })
    );
  };
                  ...
                  {typeSorting && typeSortingIndex === index ? (
                    ...
                    tenantMode && (
                      <div className="expense-type-category-operate">
                        <Dropdown
                          trigger={['click']}
                          overlay={this.renderButtonMenu(expenseTypeCategory)}
                        >
  ...
  onTravelTypeChange = e => {
    this.setState({
      travelTypeFlag: e.target.checked,
    });
  };
    ...
    return (
      <div className="custom-expense-type">
        ...
        >
          ...
          />
          <Checkbox
            checked={this.state.travelTypeFlag}
            onChange={this.onTravelTypeChange}
            style={{ marginTop: 20 }}
            disabled={!!nowEditCategory.id}
          >
            差旅类
          </Checkbox>
        </Modal>
