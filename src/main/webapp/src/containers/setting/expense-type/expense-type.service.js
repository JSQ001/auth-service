...
export default {
  ...
  deleteCategory(expenseTypeCategoryOid) {
    return httpFetch.delete(
      `${config.expenseUrl}/api/expense/types/category?id=${expenseTypeCategoryOid}`
    );
  },
  ...
  getExpenseTypeCode(setOfBooksId) {
    return httpFetch.get(`${config.baseUrl}/invoice/api/expense/type/code/produce`, {
      setOfBooksId,
    });
  },
  ...
  getTypes(typeCategoryId) {
    return httpFetch.get(`${config.expenseUrl}/api/expense/types/query/by/category`, {
      typeCategoryId,
    });
  },
  //获取差旅类型
  getTravelTypes() {
    return httpFetch.get(`/api/custom/enumerations/template/by/type?type=TRAVEL_TYPE`);
  },
};
