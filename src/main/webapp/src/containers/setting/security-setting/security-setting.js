...
class SecuritySetting extends React.Component {
  ...
  getCompanySecuritySetting() {
    SSService.getCompanySecuritySetting(this.props.company.companyOid).then(response => {
  ...
  getClientInfo() {
    SSService.getClientInfo().then(res => {
      this.setState({
        ClientInfo: res.data,
      });
    });
  ...
  handleInteger = (e, key) => {
    ...
    value[key] =
      e.target.value === ''
        ? this.state.securitySetting[key]
        : parseInt(e.target.value.toString().split('.')[0]);
    this.props.form.setFieldsValue(value);
  };

  renderEnterpriseKey() {
    return this.state.enterpriseKey ? (
      <span className="security-setting-display">
        {this.state.ClientInfo.clientSecret}
        <Button style={{ marginLeft: 10 }} onClick={() => this.handleEnterpriseKey(false)}>
          {messages('security.hides')}
        </Button>
      </span>
    ) : (
      <span className="security-setting-hide">
        <Button onClick={() => this.handleEnterpriseKey(true)}>{messages('security.view')}</Button>
      </span>
    );
  }
  ...
  handleSubmit = e => {
    ...
    let value = {
      passwordLengthMin:
        typeof this.props.form.getFieldValue('passwordLengthMin') === 'undefined'
          ? securitySetting.passwordLengthMin
          : parseInt(this.props.form.getFieldValue('passwordLengthMin')),
      passwordLengthMax:
        typeof this.props.form.getFieldValue('passwordLengthMax') === 'undefined'
          ? securitySetting.passwordLengthMax
          : parseInt(this.props.form.getFieldValue('passwordLengthMax')),
      passwordRule: this.state.securitySetting.passwordRule,
      passwordExpireDays:
        typeof this.props.form.getFieldValue('passwordExpireDays') === 'undefined'
          ? this.state.securitySetting.passwordExpireDays
          : this.props.form.getFieldValue('passwordExpireDays'),
      passwordAttemptTimes:
        typeof this.props.form.getFieldValue('passwordAttemptTimes') === 'undefined'
          ? this.state.securitySetting.passwordAttemptTimes
          : this.props.form.getFieldValue('passwordAttemptTimes'),
      autoUnlockDuration:
        typeof this.props.form.getFieldValue('autoUnlockDuration') === 'undefined'
          ? this.state.securitySetting.autoUnlockDuration
          : this.props.form.getFieldValue('autoUnlockDuration'),
      passwordRepeatTimes:
        typeof this.props.form.getFieldValue('passwordRepeatTimes') === 'undefined'
          ? this.state.securitySetting.passwordRepeatTimes
          : this.props.form.getFieldValue('passwordRepeatTimes'),
      dimissionDelayDays:
        typeof this.props.form.getFieldValue('dimissionDelayDays') === 'undefined'
          ? this.state.securitySetting.dimissionDelayDays
          : this.props.form.getFieldValue('dimissionDelayDays'),
    ...
    SSService.updateCompanySecuritySetting(value).then(response => {
            ...
            <Form onSubmit={this.handleSubmit}>
              ...
              <FormItem {...formItemLayout} className="security-setting-formItem">
                ...
                {getFieldDecorator('passwordLengthMin', {
                  ...
                  <InputNumber
                    onBlur={value => this.handleInteger(value, 'passwordLengthMin')}
                    min={6}
                ...
                {getFieldDecorator('passwordLengthMax', {
                  ...
                  <InputNumber
                    onBlur={value => this.handleInteger(value, 'passwordLengthMax')}
                    style={{ marginLeft: 5 }}
              ...
              <FormItem {...formItemLayout} className="security-setting-formItem">
                ...
                {getFieldDecorator('passwordExpireDays', {
                  ...
                  <InputNumber
                    onBlur={value => this.handleInteger(value, 'passwordExpireDays')}
              ...
              <FormItem {...formItemLayout} className="security-setting-formItem">
                ...
                <span className="formItem-value-history">
                  {messages('security.forbidden')}&nbsp;&nbsp;
                </span>
                {getFieldDecorator('passwordRepeatTimes', {
                  initialValue: securitySetting.passwordRepeatTimes,
                })(
                  <InputNumber
                    onBlur={value => this.handleInteger(value, 'passwordRepeatTimes')}
                    min={0}
                    max={24}
                    disabled={!this.props.tenantMode}
                    placeholder={messages('common.please.enter')}
              ...
              <FormItem {...formItemLayout} className="security-setting-formItem">
                ...
                {getFieldDecorator('passwordAttemptTimes', {
                  ...
                  <InputNumber
                    onBlur={value => this.handleInteger(value, 'passwordAttemptTimes')}
                    className="formItem-value-passwordExpireDays"
                    min={0}
                    max={10}
                    disabled={!this.props.tenantMode}
              ...
              <FormItem {...formItemLayout} className="security-setting-formItem">
                ...
                {getFieldDecorator('autoUnlockDuration', {
                  ...
                  <InputNumber
                    onBlur={value => this.handleInteger(value, 'autoUnlockDuration')}
                    className="formItem-value-passwordExpireDays"
                    min={0}
                    max={10080}
                    disabled={!this.props.tenantMode}
              ...
              <hr className="security-setting-slid" />
              {/*其它规则*/}
              <div className="security-setting-Rule">{messages('security.other.rule')}</div>

              <FormItem {...formItemLayout} className="security-setting-formItem">
                {/*账号失效时间*/}
                <span className="item-5">{messages('security.account.time')}</span>
                <span className="formItem-value-max">
                  {messages('security.departing.employees')}&nbsp;
                </span>
                {getFieldDecorator('dimissionDelayDays', {
                  ...
                  <InputNumber
                    onBlur={value => this.handleInteger(value, 'dimissionDelayDays')}
                    min={0}
                    max={180}
                    disabled={!this.props.tenantMode}
                    placeholder={messages('common.please.enter')}
                ...
                <span>
                  {' '}
                  {messages('security.departing.tips')}&nbsp;&nbsp;
                  <span>
                    <span className="inner-tips">{messages('security.departing.tips.inner')}</span>
                  </span>
                </span>
              </FormItem>
              ...
              <FormItem {...formItemLayout}>
                {getFieldDecorator('createDataType')(
                  <div className="security-setting-formItem">
                    ...
                    >
                      <Radio style={radioStyle} disabled={!this.props.tenantMode} value={1001}>
                        {messages('security.create.manually')}
                        <span className="inner-tips-manually">
                          {messages('security.create.manually.inner')}
                        </span>
                      </Radio>
                      <Radio style={radioStyle} disabled={!this.props.tenantMode} value={1002}>
                        {messages('security.interface')}
                      </Radio>
                      <span
                        className="create-data-type"
                        style={{ marginLeft: this.props.language.local === 'zh_cn' ? 0 : 86 }}
                      />
                    </RadioGroup>
                    <div className="formItem-value-group-tips">
                      {messages('security.info.type')}
                    </div>
                    <br />
                  </div>
                )}
