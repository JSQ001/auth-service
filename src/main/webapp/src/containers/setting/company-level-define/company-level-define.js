...
class CompanyLevelDefine extends React.Component {
      ...
      searchForm: [
        {
          type: 'input',
          id: 'companyLevelCode',
          label: this.$t('company.level.define.company.code') /*公司级别代码*/,
        },
        {
          type: 'input',
          id: 'companyLevelDescription',
          label: this.$t('person.group.desc') /*描述*/,
        },
      ],
      columns: [
        {
          title: this.$t('company.level.define.company.code') /*公司级别代码*/,
          key: 'companyLevelCode',
          dataIndex: 'companyLevelCode',
          align: 'center',
        },
        {
          title: this.$t('person.group.desc') /*描述*/,
          key: 'description',
  ...
  getList = () => {
    ...
    let url = `${config.mdataUrl}/api/companyLevel/selectByInput`;
  ...
  onSearch = result => {
    ...
    this.setState(
      ...
      () => {
        let code = this.state.searchParams.companyLevelCode
          ? this.state.searchParams.companyLevelCode
          : '';
        let description = this.state.searchParams.companyLevelDescription
          ? this.state.searchParams.companyLevelDescription
          : '';
        httpFetch
          .get(
            `${
              config.mdataUrl
            }/api/companyLevel/selectByInput?companyLevelCode=${code}&description=${description}&page=0&size=${pageSize}`
          )
          .then(res => {
            if (res.status === 200) {
              this.setState({
  ...
  handleModalOk = (e, key) => {
    ...
    this.props.form.validateFieldsAndScroll((errs, values) => {
      ...
      httpFetch
        .post(`${config.mdataUrl}/api/companyLevel/insertOrUpdate`, params)
        .then(res => {
          if (res.status === 200) {
            key === 'edit'
              ? message.success(this.$t('wait.for.save.modifySuccess') /*修改成功*/)
              : message.success(this.$t('company.level.define.add.success') /*新建成功*/);
          } else {
        ...
        .catch(res => {
          switch (res.response.data.message) {
            ...
            //       break;
            case '公司级别描述为空':
              message.error(this.$t('company.level.define.empty.description'));
              break;
            case '公司级别已被引用不可修改':
              message.error(this.$t('company.level.define.can.not.edit'));
              break;
            case '公司级别code重复':
              message.error(this.$t('company.level.define.code.repeat'));
              break;
          ...
          <div style={{ margin: '10px 0' }}>
            ...
            >
              <Alert
                ...
                description={
                  this.$t(
                    'company.level.define.save.not.edit'
                  ) /*【公司级别代码】 保存后将不可修改*/
                }
                showIcon
              />
                      ...
                      rules: [
                        {
                          required: true,
                          message: this.$t(
                            'company.level.define.code.not.empty'
                          ) /*公司级别代码不能为空*/,
                        },
                      ],
                    })(<Input />)}
                ...
                <Form hideRequiredMark={false} style={{ marginTop: '-20px' }}>
                  <FormItem label="描述">
                    {getFieldDecorator('des', {
                      ...
                      <LanguageInput
                        name={record.description ? record.description : ''}
                        isEdit={false}
            ...
            >
              ...
              <div style={{ marginTop: 20 }}>
                <div className="company-level-modal">
                  <div className="company-level-text">
                    {this.$t('company.level.define.company.code') /*公司级别代码：*/}
                  </div>
                ...
                <div className="company-level-modal">
                  <div className="company-level-text-switch">
                    {this.$t('common.column.status') /*状态*/}
                  </div>
                  <div className="company-level-switch" style={{ marginLeft: '10px' }}>
                    <Switch checked={record.enabled} onChange={this.onSwitchChange} />
                  </div>
                </div>
              </div>
            </Modal>
