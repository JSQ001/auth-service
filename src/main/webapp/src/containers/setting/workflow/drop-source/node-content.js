...
class NodeContent extends React.Component {
  ...
  handleHover = hoverIndex => {
    ...
    //如果跟上次一样，则不刷新视图
    if (hoverIndex !== lastHoverIndex) this.setState({ hoverIndex, lastHoverIndex: hoverIndex });
  };
  ...
  renderList = () => {
    ...
    return widgetList.map((widget, index) => (
      <div
        key={widget.counterFlag}
        className={`list-sort-widget ${isOver && hoverIndex - 1 === index && 'hover'}`}
      >
        <DropWidgetItem
          widget={widget}
          index={index}
    ...
    return connectDropTarget(
      <div className="phone-content">
        ...
        <div
          ...
          className={`widget-list-item end-node ${endNodeSelected ? 'selected' : ''}`}
          onClick={() => {
            this.props.onSelect(-1, endNodeWidget);
          }}
        >
          <img src={endImg} />
