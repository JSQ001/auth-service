...
class AddPersonModal extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.visible && !this.props.visible) {
      ...
      const depByApplicantKeys = [];
      const depByDeptKeys = [];
      const approveUser = [];
      const approveUserGroup = [];
      let directManager = false;
      const { users } = nextProps;
      if (users) {
        users.forEach(o => {
          if (o.departmentType === 1) {
            if (o.approverType === 6100) {
              directManager = true;
            } else {
              depByApplicantKeys.push(o.approverType);
            }
          } else if (o.departmentType === 2) {
            depByDeptKeys.push(o.approverType);
          } else if (o.approverType === 6001) {
            approveUser.push({
              userOid: o.approverEntityOid,
              fullName: o.name,
            });
          } else if (o.approverType === 6003) {
            approveUserGroup.push({
              userGroupOid: o.approverEntityOid,
              name: o.name,
            });
          }
        });
    ...
    workflowService
      ...
      .then(res => {
        this.setState({ allApproverType: res.data }, () => {
          let departmentByLeader =
            this.state.allApproverType[
              'RuleEnumDTO(key=0, value=按申请人所在的组织架构审批, remark=)'
            ] || [];
          let departmentByBill =
            this.state.allApproverType[
              'RuleEnumDTO(key=0, value=按单据上的组织架构审批, remark=)'
            ] || [];
        ...
        res.data.map(item => {
          departmentByLeader.map(departmentItem => {
            if (String(departmentItem.key) === String(item.positionCode)) {
              item.i18n &&
                item.i18n.positionName &&
                item.i18n.positionName.map(nameItem => {
                  nameItem.language === this.props.language.code &&
                    (departmentItem.value = nameItem.value);
                });
            }
          });
  ...
  getCostCenterApprover = () => {
    ...
    (this.props.formInfo.customFormFields || []).map(item => {
      if (item.messageKey === 'select_cost_center') costCenterFields.push(item);
    });
    Promise.all(
        ...
        if (item.dataSource && JSON.parse(item.dataSource)) {
          ...
          if (costCenterItem.approverEntityOid) {
            return new Promise((resolve, reject) => {
              CCService.getCostCenterDetail(costCenterItem.approverEntityOid)
                .then(res => {
      ...
      costCenterDeptApprover.map((item, index) => {
        ...
        costCenterDeptApprover[index].remark = this.$t(
          'setting.key1265' /*按单据上的成本中心的主要部门经理审批*/
        );
      });
  ...
  setDefaultUserAndGroup = ruleApprovers => {
    ...
    this.setState({
      approveUser,
      approveUserGroup,
      defaultApproveUserOid,
      defaultApproveUserGroupOid,
    });
  };
  ...
  handleDeleteUserGroup = oid => {
    ...
    approveUserGroup &&
      approveUserGroup.map((item, index) => {
  ...
  handleAddApprover = params => {
    ...
    if (params.length) {
      workflowService
        .addNoticeUsers({ ruleNoticeOid, users: params })
        .then(res => {
          res.data &&
            res.data.map(item => {
              if (item.approverType === 6001) {
                approveUser.map((userItem, index) => {
                  userItem.userOid === item.approverEntityOid &&
                    (approveUser[index].ruleApproverOid = item.ruleApproverOid);
                });
                defaultApproveUserOid.push(item.approverEntityOid);
              }
              if (item.approverType === 6003) {
                approveUserGroup.map((groupItem, index) => {
                  groupItem.userGroupOid === item.approverEntityOid &&
                    (approveUserGroup[index].ruleApproverOid = item.ruleApproverOid);
                });
                defaultApproveUserGroupOid.push(item.approverEntityOid);
              }
            });
    ...
    return (
      <div className="add-person-modal">
        ...
        <Modal
          ...
          footer={
            <Button type="primary" loading={loadingAddPerson} onClick={this.handleOK}>
              {this.$t('common.ok')}
            </Button>
          }
        >
          <Spin spinning={loading}>
            <List itemLayout="horizontal">
              ...
              <ListItem className="dep-by-bill">
                <div>
                  ...
                  <Checkbox
                    ...
                    onChange={e =>
                      this.setState({ containsApportionmentDepartmentManager: e.target.checked })
                    }
                  >
                    通知分摊组织领导
                  </Checkbox>
                ...
                <TagSelect
                  hideCheckAll
                  value={depByDeptKeys}
                  onChange={value => this.setState({ depByDeptKeys: value })}
                >
              ...
              {costCenterApprover &&
                !!costCenterApprover.length && (
                  <ListItem className="cost-center">
                    <div>
                      <h4>
                        {personType === 1
                          ? this.$t('setting.key1261' /*按单据上的成本中心经理审批*/)
                      ...
                      <Checkbox
                        ...
                        onChange={e =>
                          this.setState({
                            containsApportionmentCostCenterManager: e.target.checked,
                          })
                        }
                      >
                        {personType === 1
                          ? this.$t('setting.key1263' /*分摊成本中心经理会签*/)
                          : this.$t('setting.key1264' /*知会分摊成本中心经理*/)}
                      </Checkbox>
                    </div>
                    <TagSelect
                      hideCheckAll
                      value={costCenterKeys}
                      onChange={values => this.setState({ costCenterKeys: values })}
                    >
                      {costCenterApprover.map(item => (
                        <TagSelect.Option
              ...
              {costCenterDeptApprover &&
                !!costCenterDeptApprover.length && (
                  <ListItem className="cost-center-dept">
                    <div>
                      <h4>
                        {personType === 1
                          ? this.$t('setting.key1265' /*按单据上的成本中心的主要部门经理审批*/)
                      ...
                      >
                        {personType === 1
                          ? this.$t('setting.key1267' /*分摊后，对应的部门经理会签*/)
              ...
              <ListItem className="user-group-approver">
                ...
                <div style={{ marginBottom: 5 }}>
                  {this.$t('setting.key1279' /*只能引用到人员组中*/)}
                </div>
                {approveUserGroup &&
                  approveUserGroup.map(item => (
                    <Tag
                      key={item.userGroupOid}
                      closable
                      onClose={() => this.handleDeleteUserGroup(item.userGroupOid)}
                    >
                      {item.name}
                    </Tag>
                  ))}
        ...
        <ListSelector
          ...
          onCancel={() => {
            this.setState({ userGroupVisible: false });
          }}
        />
      </div>
    );
