...
class NodeConditionList extends React.Component {
  ...
  handleAdditionModalShow = (condition, ruleApproverOid) => {
    ...
    condition.map(rule => {
      if (rule.remark === 'cust_list') {
        //成本中心属性条件的field可能会重复，因此加上refCostCenterOid区分
        defaultAdditionOid.push(
          rule.refCostCenterOid ? `${rule.field}_${rule.refCostCenterOid}` : rule.field
        );
      } else if (rule.remark === 'judge_cost_center') {
        //【申请人=成本中心经理】与【成本中心】的fieldOid一样，因此加上remark区分
  ...
  handleEditCondition = (condition, approverIndex, conditionIndex) => {
    ...
    Object.keys(ruleApprovers[approverIndex].ruleConditions).map((key, index) => {
      if (index === conditionIndex) {
        ruleApprovers[approverIndex].ruleConditions[key].map(
      ...
      <div className="node-condition-list">
        <Spin spinning={loading}>
          {ruleApprovers.map((item, approverIndex) => {
            ...
            Object.keys(item.ruleConditions || {}).map(key => {
              ruleConditions.push(item.ruleConditions[key]);
            });
            let title = (
              <div className="collapse-header">
                ...
                {(item.containsApportionmentDepartmentManager ||
                  ...
                  item.containsApportionmentCostCenterPrimaryDepartmentManager) &&
                  `【${this.$t('setting.key1328' /*含分摊*/)}】`}
              </div>
            );
            let extra = (
              <div className="header-right">
                ...
                >
                  <a
                    onClick={() => {
                      this.handleAdditionModalShow([], item.ruleApproverOid);
                    }}
                  >
                        ...
                        <ListItem key={conditionIndex}>
                              ...
                              <div className="edit-and-delete">
                                ...
                                <Popconfirm
                                  ...
                                  onConfirm={() =>
                                    this.handleDeleteCondition(
                                      condition,
                                      condition[0].batchCode,
                                      item.ruleApproverOid
                                    )
                                  }
                                >
                                  <a>{this.$t('common.delete')}</a>
                          ...
                          {!!isEdit ? (
                            <ConditionForm
                              ...
                              addCondition={() =>
                                this.handleAdditionModalShow(condition, item.ruleApproverOid)
                              }
                              saveNewHandle={condition =>
                                this.handleSaveCondition(condition, item.ruleApproverOid, 'new')
                              }
                              saveUpdateHandle={condition =>
                                this.handleSaveCondition(condition, item.ruleApproverOid, 'update')
                              }
