...
class ConditionForm extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      this.handleDeleteValueDetail(
        nextProps.deleteTagValue.remark,
        nextProps.deleteTagValue.value,
        nextProps.deleteTagValue.fieldOid,
        nextProps.deleteTagValue.index
      );
    }
  }
  ...
  checkConditionType = remark => {
    ...
    if (
      ...
      remark === 'default_function_amount' ||
      remark === 'number'
    ) {
      return 'double'; //浮点类型
    }
    ...
    if (
      ...
      remark === 'text_area' ||
      remark === 'select_box' ||
      remark === 'default_user_work_number' ||
      remark === 'default_user_position'
    ) {
      return 'text'; //文本类型
    }
    if (
      ...
      remark === 'default_travel_application_version'
    ) {
      return 'boolean'; //布尔类型
    }
    if (
      ...
      remark === 'default_user_applicant' ||
      remark === 'default_user_department' ||
      remark === 'default_user_direct_leadership'
    ) {
      return 'custList'; //值列表类型
    }
    if (
      remark === 'default_user_sex' ||
      remark === 'default_user_level' ||
      remark === 'default_user_post' ||
      remark === 'default_user_category' ||
      remark === 'default_user_department_extend' ||
      remark === 'custom_form_department_extend'
    ) {
  ...
  getTypeValues = item => {
    ...
    workflowService.getBatchTypeList(params).then(res => {
      this.setState({
        typeValueVisible: true,
        valueList: res.data.values,
  ...
  getDepartmentExtend = item => {
    this.setState(
      ...
      () => {
        workflowService.getValueListInfo(item.customEnumerationOid).then(res => {
          this.setState({
            departmentExtendLoading: false,
  ...
  handleAddExtendField = () => {
    ...
    condition.map((item, index) => {
      if (
        ...
        item.remark === this.state.remarkCurrent &&
        item.field === this.state.currentFieldOid
      ) {
        let valueDetail = JSON.parse(condition[index].valueDetail || '{}');
        ...
        if (valueDetail.value && valueDetail.value.length) {
          valueOids.map((oid, oidIndex) => {
            if (remark === 'default_department_path') {
              ...
              (remark === 'select_department' || remark === 'default_user_department') &&
              valueDetail.value.indexOf(oid) === -1
            ) {
      ...
      if (i === index) {
        ...
        if (item.remark === remark && remark === 'select_cost_center') {
          ...
          costCenterValue.map((costCenterItem, costCenterIndex) => {
            costCenterItem.costCenterItemOid === value &&
              costCenterValue.splice(costCenterIndex, 1);
          });
  ...
  handleSave = () => {
    ...
    condition.map(item => {
      ...
      if (
        saveAble &&
        !!Number(item.fieldContent) &&
        (!item.valueDetail ||
          (!JSON.parse(item.valueDetail).value || !JSON.parse(item.valueDetail).value.length)) &&
        (!item.valueDetail ||
          (!JSON.parse(item.valueDetail).valueOids ||
            !JSON.parse(item.valueDetail).valueOids.length))
      ) {
        message.error(this.$t('setting.key1473', { arg1: item.name } /*请输入{arg1}的条件值*/));
        saveAble = false;
      }
      if (saveAble && (type === 'long' || type === 'double' || type === 'date')) {
        if (!item.valueDetail) {
          message.error(
            this.$t('setting.key1474', { arg1: item.name } /*请选择{arg1}的条件符，条件值*/)
          );
          saveAble = false;
        } else {
          ...
          if (type === 'date') {
            if (new Date(leftValue).getTime() > new Date(rightValue).getTime()) {
              message.error(
                this.$t(
                  'setting.key1477',
                  { arg1: item.name } /*请输入合法的{arg1}条件值，第一个时间不能比第二个时间晚*/
                )
              );
              saveAble = false;
            }
            ...
            if (
              ...
              message.error(
                this.$t(
                  'setting.key1478',
                  { arg1: item.name } /*请输入合法的{arg1}条件值，表达式不合法*/
                )
              );
              saveAble = false;
            }
  ...
  handelSaveNewParams = params => {
    const { ruleNoticeOid } = this.props;
    ...
    params.map((item, index) => {
      ...
      params[index].entityType = 7004; //场景关联类型, 固定值
      ...
      params[index].entityOid = ruleNoticeOid;
      if (
      ...
      .then(res => {
        ...
        condition.map((item, index) => {
          this.state.condition.map(conditionItem => {
            ...
            if (
              conditionItem.remark === 'default_user_department_extend' ||
              conditionItem.remark === 'custom_form_department_extend'
            ) {
  ...
  render() {
    ...
    const {
      ...
      valueList,
      currentValue,
      departmentExtendVisible,
      allCustomFormFields,
      currentExtendValue,
      departmentExtendLoading,
    } = this.state;
    return (
      <div className="condition-form">
          ...
          {containRuleList.length &&
            ...
            condition.map((item, index) => {
                ...
                return (
                  <Row className="type-text" key={index}>
                    ...
                    <span className="condition-list-item">
                      ...
                      >
                        {(item.remark === 'default_user_work_number' ||
                        item.remark === 'default_user_position'
                          ? haveRuleList
                          : containRuleList
                        ).map(symbolsItem => {
                          return <Option key={symbolsItem.key}>{symbolsItem.name}</Option>;
              ...
              if (type === 'custList') {
                ...
                if (item.remark === 'default_department_path') optionList = containRuleList;
                if (
                  item.remark === 'default_applicant_company' ||
                  item.remark === 'default_user_applicant' ||
                  item.remark === 'default_user_department' ||
                  item.remark === 'default_user_direct_leadership'
                ) {
                  //申请人公司不可能为空，去掉【为空／不为空】选项
                  optionList = [];
                  inRuleList.map(item => {
                    if (item.key === '9009' || item.key === '9010') optionList.push(item);
                  });
                }
                return (
                      ...
                      item.remark === 'default_user_department' ? (
                        ...
                        >
                          ...
                          <SelectDepOrPerson
                            ...
                            onlyDep={true}
                            onCancel={() => {
                              this.setState({ [`dept${index}`]: false });
                            }}
                        ...
                        <div className="selector-container">
                          <Selector
                            ...
                            value={
                              item.value ||
                              item.showValue ||
                              (this.props.itemValueRender(item, true, index) &&
                                this.props.itemValueRender(item, true, index)[0])
                            }
              ...
              if (type === 'selector') {
                let custListSelectorItem = {
                  url: `${
                    config.baseUrl
                  }/api/custom/enumeration/system/by/type?systemCustomEnumerationType=${
                    item.fieldContent
                  }`,
                  label: 'messageKey',
                  key: 'value',
                ...
                let departmentExtend = {
                  url: `${config.baseUrl}/api/custom/forms/${
                    this.props.profile['department.custom.form']
                  }/simple`,
                  label: 'fieldName',
                  key: 'fieldOid',
        ...
        <Modal
          ...
          onCancel={() => {
            this.setState({ typeValueVisible: false });
          }}
        >
          {valueList.length === 0 ? (
            <div style={{ minHeight: '80px', textAlign: 'center', color: 'gray' }}>
              {this.$t('setting.key1307' /*暂无数据*/)}
            </div>
          ) : (
            <div style={{ minHeight: '80px' }}>
              ...
              >
                {valueList.map(item => {
                  return (
                    <TagSelect.Option
                      value={item.customEnumerationItemOid}
                      key={item.customEnumerationItemOid}
                    >
                      {item.messageKey}
                    </TagSelect.Option>
                  );
        ...
        <Modal
          ...
          onCancel={() => {
            this.setState({ departmentExtendVisible: false });
          }}
        >
          <Spin spinning={departmentExtendLoading}>
            {allCustomFormFields.length === 0 ? (
              ...
              <div style={{ minHeight: '80px' }}>
                <TagSelect
                  value={currentExtendValue}
                  onChange={value => {
                    this.setState({ currentExtendValue: value });
                  }}
                >
                  {allCustomFormFields.map(item => {
                    return (
                      <TagSelect.Option value={item.customEnumerationItemOid} key={item.id}>
                        {item.messageKey}
                      </TagSelect.Option>
                    );
                  })}
        ...
        />
        <Modal
          title={this.$t('setting.key1308' /*选择费用类型*/)}
