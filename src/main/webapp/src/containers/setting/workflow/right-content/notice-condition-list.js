...
import { Form, Select, message, List, Popconfirm, Tag, Spin, ConfigProvider } from 'antd';
const ListItem = List.Item;
import PropTypes from 'prop-types';

import ConditionForm from 'containers/setting/workflow/right-content/notice-condition-form';
...
class NodeConditionList extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      approvalAndDepLevel: [
        ...
        {
          id: 1,
          name: this.$t('setting.key1309' /*本级*/),
          depLevel: this.$t('setting.key1291' /*一级*/),
        },
        {
          id: 2,
          name: this.$t('setting.key1310' /*第二级*/),
          depLevel: this.$t('setting.key1292' /*二级*/),
        },
        {
          id: 3,
          name: this.$t('setting.key1311' /*第三级*/),
          depLevel: this.$t('setting.key1293' /*三级*/),
        },
        {
          id: 4,
          name: this.$t('setting.key1312' /*第四级*/),
          depLevel: this.$t('setting.key1294' /*四级*/),
        },
        {
          id: 5,
          name: this.$t('setting.key1313' /*第五级*/),
          depLevel: this.$t('setting.key1295' /*五级*/),
        },
        {
          id: 6,
          name: this.$t('setting.key1314' /*第六级*/),
          depLevel: this.$t('setting.key1296' /*六级*/),
        },
        {
          id: 7,
          name: this.$t('setting.key1315' /*第七级*/),
          depLevel: this.$t('setting.key1297' /*七级*/),
        },
        {
          id: 8,
          name: this.$t('setting.key1316' /*第八级*/),
          depLevel: this.$t('setting.key1298' /*八级*/),
        },
        {
          id: 9,
          name: this.$t('setting.key1317' /*第九级*/),
          depLevel: this.$t('setting.key1299' /*九级*/),
        },
        {
          id: 10,
          name: this.$t('setting.key1318' /*第十级*/),
          depLevel: this.$t('setting.key1300' /*十级*/),
        },
      ],
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.ruleApprovalNodeOid !== nextProps.basicInfo.ruleApprovalNodeOid) {
      ...
      //节点不变，新增删除或修改审批人时需要重新获取ruleApprovers
      if (nextProps.basicInfo.ruleApprovers) {
        if (this.state.ruleApprovers.length !== nextProps.basicInfo.ruleApprovers.length) {
          //新增删除
          ...
          this.state.ruleApprovers.map((item, index) => {
            if (
              item.ruleApproverOid !== nextProps.basicInfo.ruleApprovers[index].ruleApproverOid ||
              item.level !== nextProps.basicInfo.ruleApprovers[index].level
            )
              //申请人等级修改
              ruleApproverIsNotChange = false;
          });
          !ruleApproverIsNotChange &&
            this.setState({ ruleApprovers: deepCopy(nextProps.basicInfo.ruleApprovers) || [] });
        }
      }
    }
  ...
  getExpenseTypeList = () => {
    ...
    if (expenseTypeOid.length) {
      return new Promise((resolve, reject) => {
        baseService
          .getExpenseTypesByFormOidV2(params)
          .then(res => {
  ...
  //获取部门列表
  getDepartmentList = () => {
    const { ruleConditions } = this.props;
    const departmentOid = [];
    Object.values(ruleConditions).map(item => {
      item.map(m => {
        if (m.remark === 'select_department' || m.remark === 'default_user_department') {
          //部门
          m.valueDetail &&
            JSON.parse(m.valueDetail).value.map(oid => {
              departmentOid.push(oid);
            });
        }
        if (m.remark === 'default_department_path') {
          if (m.valuesOids) {
            JSON.parse(m.valuesOids).value.map(oid => {
              departmentOid.push(oid);
            });
          } else {
            m.valueDetail &&
              JSON.parse(m.valueDetail).value.map(oid => {
                departmentOid.push(oid.replace('|', ''));
              });
          }
        }
      });
    ...
    if (departmentOid.length) {
      return new Promise((resolve, reject) => {
        workflowService
          .getDepartmentSimpleList(departmentOid)
          .then(res => {
  ...
  getCostCenterList = () => {
    ...
    ruleApprovers.map(approver => {
      (approver.ruleConditionList || []).map(item => {
        if (item.remark === 'select_cost_center') {
          //成本中心
          item.valueDetail &&
            JSON.parse(item.valueDetail).value.map(oid => {
    ...
    if (costCenterOid.length) {
      return new Promise((resolve, reject) => {
        workflowService
          .getBatchCostCenterList(costCenterOid)
          .then(res => {
  ...
  getCompanyList = () => {
    ...
    ruleApprovers.map(approver => {
      Object.values(approver.ruleConditions || []).map(item => {
        item.map(m => {
          if (m.remark === 'select_company' || m.remark === 'default_applicant_company') {
            //公司控件
            m.valueDetail &&
              JSON.parse(m.valueDetail).value.map(oid => {
                companyOid.push(oid);
    ...
    if (companyOid.length) {
      return new Promise((resolve, reject) => {
        workflowService
          .getBatchCompanyItemList(companyOid)
          .then(res => {
    ...
    ruleApprovers.map(approver => {
      (approver.ruleConditionList || []).map(item => {
        if (
          ...
          item.remark === 'default_user_direct_leadership'
        ) {
          //公司控件
          item.valueDetail &&
            JSON.parse(item.valueDetail).value.map(oid => {
  ...
  getEntityList = () => {
    ...
    if (entityOid.length) {
      return new Promise((resolve, reject) => {
        workflowService
          .getBatchCorporationEntityList(entityOid)
          .then(res => {
  ...
  getValueList = () => {
    ...
    if (valueListOid.length) {
      return new Promise((resolve, reject) => {
        workflowService
          .getCustomEnumerationList(valueListOid)
          .then(res => {
  ...
  getTypeValueList = () => {
    ...
    ruleApprovers.map(approver => {
      (approver.ruleConditionList || []).map(item => {
        if (
          item.fieldContent === '1001' ||
          item.fieldContent === '1007' ||
          item.fieldContent === '1008' ||
          item.fieldContent === '1002'
        ) {
          //值列表
          item.valueDetail &&
            JSON.parse(item.valueDetail).valueOids.map(oid => {
    ...
    if (typeValueListOid.length) {
      return new Promise((resolve, reject) => {
        workflowService
          .getCustomEnumerationList(typeValueListOid)
          .then(res => {
  ...
  handleAdditionModalShow = (condition, ruleApproverOid) => {
    this.props.handleAdditionModalShow(condition, ruleApproverOid);
  ...
  handleAddCondition = (rules, batchCode) => {
    if (rules.length) {
      ...
      if (batchCode) {
        ruleApprovers.map(approver => {
          if (approver.ruleApproverOid === this.state.approverOidForAddRule) {
            this.handleDeleteCondition(
              approver.ruleConditions[batchCode],
              batchCode,
              this.state.approverOidForAddRule
            );
          }
        });
      }
  ...
  handleEditCondition = (condition, approverIndex, conditionIndex) => {
    this.props.handleEditCondition(condition, approverIndex, conditionIndex);
  };
        ...
        if (type === 'update') {
          ...
          conditions.map(conditionItem => {
            ...
            ruleConditions[batchCode].map((ruleItem, ruleIndex) => {
              ...
              if (
                ruleItem.remark === 'default_user_department_extend' ||
                ruleItem.remark === 'custom_form_department_extend'
              ) {
                ruleItem.field = ruleItem.field.split(',')[0];
              }
              if (
                ruleItem.remark === conditionItem.remark &&
                ruleItem.field === conditionItem.field &&
                (conditionItem.remark !== 'cust_list' ||
                  ruleItem.refCostCenterOid === conditionItem.refCostCenterOid)
              ) {
  ...
  handleCancelEditCondition = () => {
    this.setState(
      {
        ruleConditions: deepCopy(this.props.ruleConditions),
        isRuleInEdit: false,
      },
      () => {
        this.props.judgeRuleInEdit(this.state.isRuleInEdit);
  ...
  renderConditionItem = (item, isEdit, i) => {
    switch (item.remark) {
      case 'default_department_level': //部门层级
        return (
          item.valueDetail &&
          (JSON.parse(item.valueDetail).value || []).map((code, index) =>
            this.state.approvalAndDepLevel.map(level => {
              if (String(level.id) === String(code))
                return isEdit
                  ? this.renderConditionCustListTag(
                      index,
      ...
      case 'default_department_path': //部门路径
        return (
          item.valueDetail &&
          (JSON.parse(item.valueDetail).value || []).map((depName, index) => {
            ...
            return isEdit
              ? this.renderConditionCustListTag(
                  index,
                  'default_department_path',
                  item.showValue && item.showValue[depName.replace('|', '')],
                  departmentOid,
                  null,
                  i
                )
      ...
      case 'default_department_role': //部门角色
        return (
          item.valueDetail &&
          (JSON.parse(item.valueDetail).value || []).map((id, index) =>
            JSON.parse(item.fieldContent || '[]').map(field => {
              if (String(field.id) === String(id))
                return isEdit
                  ? this.renderConditionCustListTag(
                      index,
                      'default_department_role',
                      field.name,
                      id,
                      null,
                      i
      ...
      case 'default_expense_type': //费用类型
        return (
          item.valueDetail &&
          (JSON.parse(item.valueDetail).value || []).map((oid, index) => {
            item.showValue = item.showValue || [];
            return (this.state.expenseTypeList.length
              ? this.state.expenseTypeList
              : item.showValue
            ).map(expense => {
                ...
                return isEdit ? (
                  ...
                  <span>
                    ...
                    <span style={{ color: '#aaa' }}>
                      {expense.enable || expense.enabled
                        ? ''
                        : `（${this.$t('setting.key1326' /*禁*/)}）`}
                    </span>
                    {index < JSON.parse(item.valueDetail).value.length - 1 ? '、' : ''}
                  </span>
                );
      ...
      case 'currency_code': //币种
        return (
          item.valueDetail &&
          (JSON.parse(item.valueDetail).value || []).map(
            (code, index) =>
              isEdit
                ? this.renderConditionCustListTag(
                    index,
                    'currency_code',
                    constants.getTextByValue(code, 'cashName') || code,
                    code,
                    null,
                    i
                  )
      ...
      case 'default_corporation_entity':
        return (
          ...
          (JSON.parse(item.valueDetail).value || []).map((oid, index) => {
            ...
            return isEdit
              ? this.renderConditionCustListTag(index, item.remark, entityName, oid, item.field, i)
      ...
      case 'default_user_level':
        return (
          item.valueDetail &&
          (JSON.parse(item.valueDetail).valueOids || []).map((valueOids, index) => {
            item.showValue = item.showValue || {};
            this.state.typeValueList.map(value => {
              value.customEnumerationItemOid === valueOids &&
                (item.showValue[valueOids] = value.messageKey);
            });
            return isEdit
              ? this.renderConditionCustListTag(
                  index,
                  item.remark,
                  item.showValue[valueOids],
                  valueOids,
                  item.field,
                  i
                )
  ...
  renderConditionCustListTag = (index, type, name, value, fieldOid = null, i) => {
    return (
      <Tag
        key={index}
        closable
        onClose={e => this.handleDeleteValueItem(e, type, value, fieldOid, i)}
      >
  ...
  render() {
    ...
    const list = Object.keys(ruleConditions || {}).map(key => ruleConditions[key]);
    return (
      <ConfigProvider renderEmpty={() => null}>
        <div style={{ padding: '0 20px' }} className="node-condition-list">
                  ...
                  <ListItem key={conditionIndex}>
                      ...
                      {!isEdit && (
                        <div className="edit-and-delete">
                          <a
                            className="edit"
                            onClick={() =>
                              this.handleEditCondition(condition, approverIndex, conditionIndex)
                            }
                          >
                            {this.$t('common.edit')}
                          </a>
                          <Popconfirm
                            ...
                            onConfirm={() =>
                              this.handleDeleteCondition(condition, condition[0].batchCode, '')
                            }
                          >
                    ...
                    {!!isEdit ? (
                      <ConditionForm
                        ...
                        afterDeleteTagValue={() => {
                          this.setState({ deleteTagValue: {} });
                        }}
                        addCondition={() =>
                          this.handleAdditionModalShow(condition, notice.ruleApproverOid)
                        }
                        saveNewHandle={onApproverChange}
                        saveUpdateHandle={onApproverChange}
                        cancelHandle={handleCancelEditCondition}
                        itemValueRender={this.renderConditionItem}
                        ruleNoticeOid={this.props.ruleNoticeOid}
                      />
                    ) : (
                          ...
                          return (
                            <div key={i} className="condition-container">
                              ...
                              &nbsp;&nbsp;【<span className="symbol">
                                {constants.getTextByValue(item.symbol, 'symbolFilter')}
                              </span>】&nbsp;&nbsp;
