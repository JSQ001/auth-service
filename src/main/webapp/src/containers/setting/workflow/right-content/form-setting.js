...
class FormSetting extends React.Component {
  ...
  getProperty = () => {
    ...
    workflowService.getCustomFormProperty(this.props.formOid).then(res => {
      ...
      this.setState({
        ...
        counterSignRuleForSubmitter: res.data.enableCounterSignForSubmitter
          ? res.data.counterSignRuleForSubmitter
          : 10,
  ...
  getSelectedCompany = () => {
    ...
    workflowService.getBatchCompanyItemList(this.state.companyOid).then(res => {
      this.setState({
    ...
    return (
      <div className="form-setting">
        <Spin spinning={loading}>
          <Card
            ...
            title={
              <div className="card-title">
                ...
                <span>
                  {this.$t(
                    'setting.key1333' /*审批单据时，同意单据，并额外指定一名或几名员工进行审批*/
                  )}
                </span>
              </div>
            }
          >
            <RadioGroup
              onChange={e => this.handleSignChange(e.target.value, 2)}
              value={enableCounterSign}
            >
              <Radio value={false}>{this.$t('setting.key1334' /*不允许加签*/)}</Radio>
              <Radio value={true}>{this.$t('setting.key1335' /*允许加签*/)}</Radio>
            </RadioGroup>
            <RadioGroup
              onChange={e => this.handleSignChange(true, e.target.value)}
              value={counterSignRule}
            >
              <Row>
                <Col offset={1}>
                  <Radio value={2} disabled={!enableCounterSign}>
              ...
              <Row>
                <Col offset={1}>
                  <Radio value={1} disabled={!enableCounterSign}>
          ...
          <Card
            ...
            title={
              <div className="card-title">
                ...
                <span>
                  {this.$t(
                    'setting.key1339' /*提交人（含代理人）提交单据时，先选择一名或几名指定员工进行审批，再按配置的流程进行审批*/
                  )}
                </span>
              </div>
            }
          >
            <RadioGroup
              onChange={e => this.handleSubmitterChange(e.target.value, 2)}
              value={enableCounterSignForSubmitter}
            >
              <Radio value={false}>{this.$t('setting.key1340' /*不允许自选审批人*/)}</Radio>
              <Radio value={true}>{this.$t('setting.key1341' /*允许自选审批人*/)}</Radio>
            </RadioGroup>
            <RadioGroup
              onChange={e => this.handleSubmitterChange(true, e.target.value)}
              value={counterSignRuleForSubmitter}
            >
              <Row>
                <Col offset={1}>
                  <Radio value={2} disabled={!enableCounterSignForSubmitter}>
                    {this.$t('setting.key1336' /*按顺序全部通过后，进入下一个节点*/)}
                  </Radio>
                </Col>
              </Row>
              <Row>
                <Col offset={1}>
                  <Radio value={1} disabled={!enableCounterSignForSubmitter}>
                    {this.$t('setting.key1337' /*任意一人通过，进入下一个节点*/)}
                  </Radio>
                </Col>
              </Row>
          ...
          <Card
            ...
            title={
              <div className="card-title">
                ...
                <span>
                  {this.$t('setting.key1343' /*当配置的审批流程中，出现一个员工重复审批的情况时*/)}
                </span>
              </div>
            }
          >
            <RadioGroup
              onChange={e => this.handleFilterChange(e.target.value, 3, 2)}
              value={filterTypeRuleMode}
            >
              <Radio value="">{this.$t('setting.key1344' /*每次都进行审批*/)}</Radio>
              <Radio value="5">
                {this.$t('setting.key1345' /*依据下列设定，不需要每次都进行审批*/)}
              </Radio>
            </RadioGroup>
            <RadioGroup
              onChange={e => this.handleFilterChange('5', e.target.value, filterTypeRule)}
              value={filterRule}
            >
              <Row>
                <Col offset={1} className={filterTypeRuleMode === '' ? 'remark-disabled' : ''}>
                  {this.$t(
                    'setting.key1346' /*当前审批人与下列所选角色作比较，若重复则无需再审批*/
                  )}
                  <span className="remark">
                    （{this.$t('setting.key1347' /*加签人重复时，仍需审批*/)}）
                  </span>
                </Col>
              </Row>
              <Row>
                <Col offset={1}>
                  <Radio value={3} disabled={filterTypeRuleMode === ''}>
            ...
            <RadioGroup
              onChange={e => this.handleFilterChange('5', filterRule, e.target.value)}
              value={filterTypeRule}
            >
              <Row>
                <Col offset={1} className={filterTypeRuleMode === '' ? 'remark-disabled' : ''}>
              ...
              <Row>
                <Col offset={1}>
                  <Radio value={2} disabled={filterTypeRuleMode === ''}>
                    {this.$t(
                      'setting.key1352' /*校验当前工作流（即校验最新一次提交中已审批的环节）*/
                    )}
                  </Radio>
                </Col>
              </Row>
              <Row>
                <Col offset={1}>
                  <Radio value={1} disabled={filterTypeRuleMode === ''}>
                    {this.$t(
                      'setting.key1353' /*校验历史工作流（即校验历史审批（包含最新一次提交中已审批的）环节）*/
                    )}
                  </Radio>
                </Col>
              </Row>
            ...
            <div style={{ display: filterTypeRule === 1 ? 'block' : 'none' }}>
              <Row>
                <Col offset={2}>{this.$t('setting.key1354' /*特殊情况*/)}</Col>
              </Row>
              <Row>
                <Col offset={2}>
                  <Checkbox
                    onChange={e => {
                      this.setState({ enableAmountFilter: e.target.checked });
                    }}
              ...
              <Row>
                <Col offset={2}>
                  <Checkbox
                    onChange={e => {
                      this.setState({ enableExpenseTypeFilter: e.target.checked });
                    }}
          ...
          <Card
            ...
            title={
              <div className="card-title">
                ...
                <span>
                  {this.$t('setting.key1358' /*当员工A授权给员工B，允许代替自己填写单据*/)}
                </span>
              </div>
            }
          >
            <Checkbox
              onChange={e => {
                this.handleProxyChange('strategy1', e.target.checked);
              }}
            ...
            <Checkbox
              onChange={e => {
                this.handleProxyChange('strategy2', e.target.checked);
              }}
          ...
          <Card
            type="inner"
            className="card-container select-company-card"
            title={
              <div className="card-title">
                {this.$t('setting.key1361' /*选人范围*/)}
                <span>
                  {this.$t('setting.key1362' /*对加签、自选审批人等需要选人的功能，规定选人范围*/)}
                </span>
              </div>
            }
          >
            <Row>
              <div className="remark">
                {this.$t('setting.key1363' /*注: 若未选择公司，则默认允许选择所有公司的人员*/)}
              </div>
              <Col span={5}>
          ...
          <Card
            ...
            title={
              <div className="card-title">
                {' '}
                {this.$t('setting.key1365' /*邮件通知*/)}
                <span>
                  {this.$t('setting.key1366' /*审批流中，需要通过邮件通知申请人的环节、动作*/)}
                </span>
              </div>
            }
          >
            <Row>
              ...
              <Col span={6}>
                ...
                <Checkbox
                  onChange={e => {
                    this.setState({ printSendEmail: e.target.checked });
                  }}
                  checked={printSendEmail}
                >
                  &nbsp;&nbsp;{this.$t('setting.key1368' /*打印环节*/)}&nbsp;&nbsp;
                </Checkbox>
              </Col>
              <Col span={6}>
                {' '}
                <Checkbox
                  onChange={e => {
                    this.setState({ endSendEmail: e.target.checked });
                  }}
                  checked={endSendEmail}
                >
                  &nbsp;&nbsp;{this.$t('setting.key1369' /*结束环节*/)}
                </Checkbox>
              </Col>
            </Row>
            <Row>
              ...
              <Col span={6}>
                ...
                <Checkbox
                  onChange={e => {
                    this.setState({ rejectSendEmail: e.target.checked });
                  }}
        ...
        <ListSelector
          ...
          onOk={value =>
            this.setState({
              ...
              rightSelectedCompany: value.result,
              companySelectorShow: false,
            })
          }
