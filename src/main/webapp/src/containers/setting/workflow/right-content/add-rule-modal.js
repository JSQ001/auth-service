...
class AddApproveRuleModal extends React.Component {
  ...
  getList = () => {
    if (!this.state.formFieldList) {
      ...
      workflowService
        .getFormFields(this.props.formOid)
        .then(res => {
  ...
  checkConditionType = remark => {
    ...
    if (
      ...
      remark === 'select_document_department'
    ) {
  ...
  handleOK = () => {
    ...
    chosenRuleOids.map((oid, index) => {
      list.map(item => {
        if (
          (item.messageKey !== 'cust_list' &&
            item.messageKey !== 'judge_cost_center' &&
            item.fieldOid === oid) ||
          (item.messageKey === 'cust_list' &&
            item.refCostCenterOid &&
            `${item.fieldOid}_${item.refCostCenterOid}` === oid) ||
          ...
          item.messageKey === 'select_cost_center' &&
            (param.customEnumerationOid = JSON.parse(item.dataSource || '{}').costCenterOid);
  ...
  render() {
    ...
    return (
      <div className="add-approve-rule-modal">
        ...
        <Modal
          title={this.$t('setting.key1284' /*请选择要添加的审批条件*/)}
          visible={visible}
          getContainer={() => {
