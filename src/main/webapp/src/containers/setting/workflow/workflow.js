          ...
          <div className="setOfBooks-container">
            <Row className="setOfBooks-select">
              ...
              <Col span={3}>
                ...
                >
                  {constants.documentType.map(item => (
        ...
        {loading ? (
          ...
          <Spin spinning={pasteLoading}>
            <div>
              <RadioGroup
                defaultValue={true}
                onChange={e => this.setState({ showEnableList: e.target.value })}
              >
                      ...
                      <div>
                        ...
                        {showEnableList && (
                          <div className="card-title-extra">
                            ...
                            >
                              {this.$t('setting.key1429' /*查看编辑*/)}
                            </a>
                            {/*审批流复制粘贴功能只适用于自定义审批模式 bug 21262*/}
                            {item.ruleApprovalChain &&
                              item.ruleApprovalChain.approvalMode === 1005 && (
                                <div style={{ display: 'inline-block' }}>
                                  <span className="ant-divider" />
                                  <a
                                    onClick={() => {
                                      this.setState({ sourceFormOid: item.formOid });
                                    }}
                                  >
                                    {this.$t('setting.key1430' /*复制*/)}
                                  </a>
                                  {sourceFormOid && <span className="ant-divider" />}
                                  {sourceFormOid && (
                                    <a
                                      onClick={() => {
                                        this.showConfirmModal(item.formOid);
                                      }}
                                    >
                                      {this.$t('setting.key1431' /*粘贴*/)}
                                    </a>
                                  )}
                                </div>
                              )}
                          ...
                          return (
                            <div key={node.ruleApprovalNodeOid} className="node-container">
                              <div>
                                ...
                                {index < item.ruleApprovalChain.ruleApprovalNodes.length - 1 && (
                                  <Icon type="arrow-right" className="right-arrow" />
                                )}
                              </div>
                              <p className="node-remark">
                                {node.type === 1005
                                  ? this.$t('setting.key1252' /*结束*/)
                                  : node.remark}
                              </p>
                            </div>
                          );
                        ...
                        item.ruleApprovalChain.approvalMode !== 1005 && (
                          <div className="node-container">
                            <div className="approval-block">
                              {this.$t('setting.key1419' /*审*/)}
                            </div>
                            <p className="node-remark">
                              {item.ruleApprovalChain.approvalMode === 1002 &&
                                this.$t('setting.key1413' /*部门经理*/)}
                              {item.ruleApprovalChain.approvalMode === 1003 &&
                                this.$t('setting.key1420' /*选人审批*/)}
                              {item.ruleApprovalChain.approvalMode === 1006 &&
                                this.$t('setting.key1421' /*英孚审批*/)}
                            </p>
                          </div>
                        )}
                    </Row>
              ...
              {showEnableList &&
                !enabledData.length && (
                  <div className="no-form-container">
                    ...
                    <p>
                      {this.$t(
                        'setting.key1432' /*无【已启用】单据，请先前往 表单设计器 启用表单*/
                      )}
                    </p>
                  </div>
                )}
