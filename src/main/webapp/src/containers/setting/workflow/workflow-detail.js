...
class WorkflowDetail extends React.Component {
  ...
  componentDidMount() {
    ...
    workflowService
      .getFormFields(this.props.match.params.formOid)
      .then(res => {
        this.setState({ formFieldList: res.data }, () => {
          Promise.all([this.getForm(), this.getApprovalChain()]).then(() => {
  ...
  getForm = () => {
    return new Promise((resolve, reject) => {
      workflowService
        .getCustomForm(this.props.match.params.formOid)
        .then(res => {
  ...
  getApprovalChain = () => {
    return new Promise((resolve, reject) => {
      workflowService
        .getApprovalChainDetail(this.props.match.params.formOid)
        .then(res => {
        ...
        if (node.ruleApprovers && node.ruleApprovers.length) {
          node.ruleApprovers.map(approver => {
            if (approver.ruleConditionList) {
              ...
              ruleConditionList.length &&
                ruleConditionList.map(condition => {
                  condition.remark === 'judge_cost_center' &&
                    judgeCostCenterList.map(judgeCostCenter => {
                      if (
                        condition.field === judgeCostCenter.fieldOid &&
                        condition.remark === judgeCostCenter.messageKey
                      ) {
                  ...
                  condition.remark === 'select_cost_center' &&
                    selectCostCenterList.map(selectCostCenter => {
                      if (
                        condition.field === selectCostCenter.fieldOid &&
                        condition.remark === selectCostCenter.messageKey
                      ) {
              ...
              if (
                ruleConditions[judgeCostCenterBatchCode] &&
                ruleConditions[judgeCostCenterBatchCode].length
              ) {
              ...
              if (
                ruleConditions[selectCostCenterBatchCode] &&
                ruleConditions[selectCostCenterBatchCode].length
              ) {
  ...
  handleNodeSelect = (nodeType, widget, index) => {
    if (this.state.chosenNodeWidget.ruleApprovalNodeOid === widget.ruleApprovalNodeOid) {
      ...
      this.setState(
        {
          //先将选中的节点类型置空，否则如果选中前后的节点类型一样的话，部分数据可能不会刷新
          chosenNodeType: null,
        },
        () => {
          this.setState({
      ...
      if (!approverNotChange) {
        ...
        workflowService.getApprovalChainDetail(this.props.match.params.formOid).then(res => {
          ...
          this.setState({
            ...
            chainInfo: res.data,
            chosenNodeType:
              res.data.ruleApprovalNodes &&
              res.data.ruleApprovalNodes[this.state.choseNodeIndex].type,
        ...
        <Spin spinning={loading}>
          <Row>
            <Col span={4} className="node-container">
              ...
              <div className="approve-type" style={{ paddingBottom: 20 }}>
                <div>
                ...
                <p>
                  ...
                  {(approvalMode === 1003 || approvalMode === 1006) &&
                    this.$t('setting.key1418' /*由提交人选择人员依次进行审批*/)}
                </p>
              </div>
              {(approvalMode === 1002 || approvalMode === 1003 || approvalMode === 1006) && (
            ...
            {showFormSetting && (
              <Col span={18} className="right-content">
                <FormSetting formOid={this.props.match.params.formOid} />
              </Col>
            )}
            {!showFormSetting && (
                      ...
                      {formInfo.formType &&
                        (String(formInfo.formType).charAt(0) === '3' ||
                          ...
                          <Row style={{ marginTop: 15 }}>
                            ...
                            <Col span={3}>
                              <Checkbox
                                defaultChecked={chosenNodeWidget.printFlag}
