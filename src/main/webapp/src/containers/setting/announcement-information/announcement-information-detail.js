...
import { Tabs, Input, Button, Form, Checkbox, message, Switch, Icon, Spin, Modal } from 'antd';
...
class AnnouncementInformationDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      tempImgList: [],
      contentId: 1,
      curId: this.props.match.params.id, //当前信息Id
    };
  }
  ...
  handleSave = values => {
    ...
    values.enabled = this.state.isEnabled;
    ...
    this.props.match.params.Oid && (values.id = this.state.curId); //编辑下传入id
  ...
  handleCancel = () => {
    this.props.dispatch(
      routerRedux.push({
        pathname: `/admin-setting/announcement-information/announcement-information`,
      })
    );
  };
