  ...
  getCurrencyName = () => {
    ...
    httpFetch
      .get(
        `${
          config.mdataUrl
        }/api/currencyI18n/select/not/created/currency?baseCurrencyCode=${baseCurrency}&setOfBooksId=${setOfBooksId}&tenantId=${tenantId}`
      )
      .then(res => {
        if (res.status === 200) {
          this.setState({
  ...
  onSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      if (!err) {
        ...
        httpFetch
          .post(`${config.mdataUrl}/api/currency/rate`, values)
          .then(res => {
            ...
            if (res.status === 200) {
              ...
              this.props.dispatch(
                routerRedux.push({
                  pathname: `/admin-setting/currency-setting/currency-setting/${setOfBooksId}/${baseCurrency}/${baseCurrencyName}`,
                })
              );
          ...
          .catch(() => {
            ...
            message.error(
              this.$t(
                'currency.setting.add.enable.auto.rate'
              ) /*'当前账套尚未启用汇率自动更新服务,请先开启!'*/,
              3
            );
            this.props.dispatch(
              routerRedux.push({
                pathname: `/admin-setting/currency-setting/currency-setting`,
              })
            );
            //   this.context.router.replace({
            //     pathname: backUrl,
  ...
  render() {
    ...
    return (
      <Form className="currency-setting-add" onSubmit={this.onSave}>
        <FormItem
          {...formItemLayout}
          label={this.$t('currency.setting.add.new.enable') /*状态enable*/}
        >
        ...
        <FormItem {...formItemLayout} label={this.$t('currency.setting.currency.name') /*币种名*/}>
          {getFieldDecorator('currencyName', {
            ...
            <Select
              onSelect={this.onCurrencyNameChange}
            ...
            >
              {currencyOptions.map((item, index) => {
                return (
                  <Option key={`${item.currencyCode}+${item.currencyName}`}>
        ...
        <FormItem
          {...formItemLayout}
          label={this.$t('currency.setting.rate.apply.date') /*汇率生效日期*/}
        >
        ...
        <FormItem
          {...formItemLayout}
          label={this.$t('currency.setting.effective.rate') /*生效汇率*/}
        >
        ...
        <FormItem
          {...formItemLayout}
          label={this.$t('currency.setting.enable.auto.rate') /*启用自动汇率*/}
        >
