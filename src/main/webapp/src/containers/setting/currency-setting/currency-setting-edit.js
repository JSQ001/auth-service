...
class CurrencySettingEdit extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          title: this.$t('dimension.detail.no') /*编号*/,
          dataIndex: 'index',
          align: 'center',
          render: (text, record, index) => {
            return index + this.state.indexAdd + 1;
          },
        },
        {
        ...
        {
          title: this.$t('currency.setting.add.currency') /*本币*/,
  ...
  getCurrencyDetail = () => {
    ...
    httpFetch.get(`${config.mdataUrl}/api/currency/rate`, params).then(res => {
      this.setState(
        {
          record: res.data,
  ...
  getRateHistory = () => {
    ...
    httpFetch
      .get(
        `${config.mdataUrl}/api/currency/rate/history/list?currencyRateOid=${
    ...
    if (e) {
      Modal.confirm({
        ...
        onOk: () => {
          httpFetch
            .put(
              `${config.mdataUrl}/api/currency/status/enable?currencyRateOid=${
                record.currencyRateOid
              }&enable=${e}`
            )
            .then(res => {
        ...
        this.setState(
          ...
          () => {
            if (
              ...
              httpFetch.put(`${config.mdataUrl}/api/currency/rate`, record).then(res => {
                if (res.status === 200) {
                  this.setState(
                    {
                      outEditRate: false,
  ...
  onEnableAutoUpdate = e => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        httpFetch
          .put(
            `${config.mdataUrl}/api/currency/status/enable/auto/update?tenantId=${
              this.props.company.tenantId
            }&setOfBooksId=${this.props.match.params.setOfBooksId}&enableAutoUpdate=${
              e.target.checked
  ...
  handleModalOK = () => {
    ...
    if (typeof operatedRecord.rate !== 'number' || operatedRecord.rate <= 0) {
      ...
      httpFetch.put(`${config.mdataUrl}/api/currency/rate`, operatedRecord).then(res => {
        if (res.status === 200) {
  ...
  onSearchHistoryRate = () => {
    ...
    httpFetch.get(`${config.mdataUrl}/api/currency/rate/history/list`, params).then(res => {
      if (res.status === 200) {
        this.setState({
          page: 0,
          data: res.data.records,
  ...
  back = () => {
    ...
    this.props.dispatch(
      routerRedux.push({
        ...
        pathname: `/admin-setting/currency-setting/currency-setting/${setOfBooksId}/${functionalCurrencyCode}/${functionalCurrencyName}`,
      })
    );
  };
