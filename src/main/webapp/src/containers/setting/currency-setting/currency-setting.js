...
class CurrencySetting extends React.Component {
    ...
    this.state = {
      ...
      setOfBooksId:
        this.props.match.params.setOfBooksId === '0'
          ...
          : this.props.match.params.setOfBooksId, //初始化页面时从接口获取的2个字段，掉接口要用
      tenantId: this.props.company.tenantId, //初始化页面时从接口获取的2个字段，掉接口要用
      functionalCurrencyCode:
        this.props.match.params.functionalCurrencyCode === '0'
          ? ''
          : this.props.match.params.functionalCurrencyCode, //选择账套后，后面的本位币的code
      functionalCurrencyName:
        this.props.match.params.functionalCurrencyName === '0'
          ? ''
          : this.props.match.params.functionalCurrencyName, //选择账套后，后面的本位币的name
      ...
      columns: [
        ...
        },
        {
          ...
          dataIndex: 'enableAutoUpdate',
          align: 'center',
          render: enableAutoUpdate => (
            <Badge
              status={enableAutoUpdate ? 'success' : 'error'}
              text={
                enableAutoUpdate
  ...
  getSetOfBooks = () => {
    ...
    httpFetch.get(`${config.mdataUrl}/api/setOfBooks/by/tenant`).then(res => {
      if (res.status === 200) {
        this.setState(
          {
            loading: true,
  ...
  getCurrencyByLanguage = () => {
    ...
    httpFetch.get(`${config.mdataUrl}/api/Currency/Language/getCurrencyByLanguage`).then(res => {
      if (res.status === 200) {
        this.setState(
          {
  ...
  getRateDiffer = () => {
    ...
    httpFetch
      .get(
        `${
          config.baseUrl
        }/api/tenant/config/by/tenantId?setOfBooksId=${setOfBooksId}&tenantId=${tenantId}`
      )
      .then(res => {
        if (res.status === 200) {
          if (res.data) {
  ...
  getTableData = () => {
    ...
    httpFetch.get(`${config.mdataUrl}/api/currency/rate/list`, params).then(res => {
      if (res.status === 200) {
        ...
        this.setState({
          ...
          functionalCurrencyName:
            res.data.records.length > 0
              ? res.data.records[0].baseCurrencyName
              : this.state.functionalCurrencyName,
          loading: false,
          pagination,
        });
      }
  ...
  onSave = () => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      httpFetch.post(`${config.baseUrl}/api/tenant/config/input`, params).then(res => {
        if (res.status === 200) {
  ...
  onAutoRateChange = () => {
    ...
    httpFetch
      .put(
        `${
          config.mdataUrl
        }/api/currency/status/enable/auto/update?tenantId=${tenantId}&setOfBooksId=${setOfBooksId}&enableAutoUpdate=${enableAutoUpdate}`
      )
      .then(res => {
        if (res.status === 200) {
          this.getTableData();
  ...
  onAddNewRate = () => {
    ...
    this.props.dispatch(
      routerRedux.push({
        pathname: `/admin-setting/currency-setting/currency-setting/currency-setting-add/${functionalCurrencyCode}/${functionalCurrencyName}/${setOfBooksId}/${
  ...
  handleRowClick = record => {
    ...
    this.props.dispatch(
      routerRedux.push({
        pathname: `/admin-setting/currency-setting/currency-setting/currency-setting-edit/${enableAutoUpdate}/${
          record.currencyRateOid
        }/${functionalCurrencyName}/${functionalCurrencyCode}/${setOfBooksId}`,
      })
    );
  ...
  tableApplyDate = (text, record, index) => {
    const { isBatchEditing } = this.state;
    if (
      record.currencyCode === record.baseCurrencyCode &&
      record.currencyName === record.baseCurrencyName
    ) {
      return '-';
    }
    if (
      isBatchEditing &&
      !record.enableAutoUpdate &&
      record.currencyCode !== record.baseCurrencyCode
    ) {
      return (
        <span
        ...
        >
          <DatePicker
            onChange={date => this.tableApplyDateChange(date, record, index)}
  ...
  tableRate = (text, record, index) => {
    ...
    if (
      ...
      return (
        ...
        >
          <InputNumber
            onChange={value => this.tableRateChange(value, record, index)}
            value={record.rate}
            min={0.0000001}
            precision={7}
            step={0.0000001}
      ...
      data.map((item, index) => {
        if (!item.enableAutoUpdate) {
          ...
          if (changeData.indexOf(index) === -1) {
            item.applyDate = moment(new Date())
              .utc()
              .format();
          } else {
            item.applyDate = moment(item.applyDate)
              .utc()
              .format();
          }
        }
      ...
      this.setState(
        ...
        () => {
          if (applyDateChange || rateChange) {
            httpFetch.put(`${config.mdataUrl}/api/currency/rate/list`, data).then(res => {
              if (res.status === 200) {
      ...
      <div className="currency-setting-wrap">
        <Spin spinning={loading}>
            ...
            <span className="currency-setting-currency">
              ...
              {functionalCurrencyCode &&
                functionalCurrencyName && (
                  <Select
                    style={{ width: 200 }}
                    value={`${functionalCurrencyCode} ${functionalCurrencyName}`}
                    disabled
                  />
                )}
            </span>
              ...
              {isEditing ? (
                        ...
                        rules: [
                          {
                            required: true,
                            message: this.$t(
                              'currency.setting.deviate.not.empty'
                            ) /*偏离汇率不能为空*/,
                          },
                        ],
                        ...
                        rules: [
                          {
                            required: true,
                            message: this.$t(
                              'currency.setting.forbidden.not.empty'
                            ) /*禁止汇率不能为空*/,
                          },
                        ],
                ...
                <span>
                  ...
                  {this.props.tenantMode && (
                    <Icon type="edit" style={{ color: '#1890ff' }} onClick={this.onEdit} />
                  )}
                </span>
              )}
            </Form>
          ...
          <div className="currency-setting">
            ...
            <br />
            {enableAutoUpdate && (
              <span className="currency-setting-note">
          ...
          <div className="currency-setting-modal">
            ...
            >
              ...
              <div>
                ...
                <span
                  className="currency-setting-blue"
                ...
                {this.$t(
                  'currency.setting.agree.observe'
                ) /*按钮后，即视为用户接受并同意遵守本协议的约定。*/}
              </div>
