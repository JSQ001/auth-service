...
export default {
  ...
  getWidgetList(formType, type) {
    return httpFetch.get(`${config.workflowUrl}/api/form/gui/widgets/all/filter`, {
      formType,
      type,
    });
  },
  ...
  getExpenseTypeListByBooksID(booksID) {
    return httpFetch.get(
      `${config.workflowUrl}/api/expense/type/by/setOfBooks?setOfBooksId=${booksID}`,
      { page: 0, size: 1000 }
    );
  },
  ...
  getExpenseTypeScope(formOid, subsidyType) {
    return httpFetch.get(
      `${config.workflowUrl}/api/v2/custom/forms/${formOid}/selected/expense/types`,
      { isAll: true, subsidyType: subsidyType }
    );
  },
  ...
  removeFormProperty(formOid, propertyNames) {
    return httpFetch.delete(
      `${
        config.workflowUrl
      }/api/custom/forms/property?formOid=${formOid}&propertyNames=${propertyNames}`
    );
  },
  ...
  getCustomEnumeration(page, size, enabled, typeFlag) {
    return httpFetch.get(`${config.baseUrl}/api/custom/enumerations`, {
      page,
      size,
      enabled,
      typeFlag,
  ...
  getFormField(formOid) {
    return httpFetch.get(
      `${config.workflowUrl}/api/custom/forms/property/travel/configuration/${formOid}`
    );
  },
  /**
