...
class FormPermission extends React.Component {
  ...
  initEditExpense = () => {
    const { form } = this.context;
    if (
      (form.formType === 3001 || form.formType === 3002 || form.formType === 3003) &&
      form.referenceOid
    ) {
      this.setState({
  ...
  handleSave = type => {
    ...
    if (type === 'company') {
      ...
      formService
        .setCompanyPermission(permissionData)
        .then(res => {
    ...
    if (type === 'user') {
      ...
      formService
        .updateUserScope(permissionData)
        .then(res => {
          message.success(this.$t('common.save.success', { name: '' }));
    ...
    if (type === 'expense') {
      ...
      formService
        .updateExpenseTypeScope(
          form.formOid,
          this.state.expenseSelectedList,
          this.state.visibleExpenseScope
        )
        .then(res => {
          message.success(this.$t('common.save.success', { name: '' }));
      ...
      <Row>
        <Col span={16} offset={3}>
          <div className="user-list-wrap-title">
            ...
            <span>
              {this.$t('form.setting.form.tip02') /*如需编辑适用人员，请前往相关联的申请单设置*/}
            </span>
          </div>
          <div className="user-list-wrap">{this.renderUser(this.context.userScope)}</div>
        </Col>
      </Row>
    );
  };
    ...
    return (
      <div>
        ...
        <Row style={{ marginTop: 12, marginBottom: 20 }}>
          <Col span={8} offset={3}>
            {!isEdit2 && (
              <Button type="primary" onClick={() => this.handleChangeStatus('user')}>
            ...
            {isEdit2 && (
              <div>
                ...
                <Button onClick={() => this.handleChangeStatus('user', true)}>
      ...
      <Row>
        <Col span={16} offset={3}>
          <div className="expense-type-list-wrap-title">
            ...
            <span>
              {this.$t(
                'form.setting.form.tip03'
              ) /*如需编辑可见费用类型，请前往相关联的申请单设置*/}
            </span>
          </div>
    ...
    return (
      <div>
        <Row>
          <Col span={8} offset={3}>
            <RadioGroup onChange={this.handleChangeExpense} value={visibleExpenseScope}>
              <Radio disabled={!isEdit3} value={1001}>
                {this.$t('form.setting.form.allExpense') /*全部费用*/}
              </Radio>
              <Radio disabled={!isEdit3} value={1002}>
                {this.$t('form.setting.form.partExpense') /*部分费用*/}
              </Radio>
            </RadioGroup>
            {visibleExpenseScope === 1002 && (
              <Chooser
                ...
                listExtraParams={{
                  setOfBooksId: this.props.tenantMode
                    ? this.context.booksID
        ...
        <Row style={{ marginTop: 12, marginBottom: 20 }}>
          <Col span={8} offset={3}>
            {!isEdit3 && (
              <Button type="primary" onClick={() => this.handleChangeStatus('expense')}>
                {this.$t('common.edit') /*编辑*/}
              </Button>
            )}
            {isEdit3 && (
              <div>
                <Button
                  ...
                  type="primary"
                  loading={loading3}
                  onClick={() => this.handleSave('expense')}
                >
                  {this.$t('common.save') /*保存*/}
                </Button>
                <Button onClick={() => this.handleChangeStatus('expense', true)}>
                  {this.$t('common.cancel') /*取消*/}
                </Button>
              </div>
            )}
  ...
  renderExpenseTypeScope = expenseType => {
    if (expenseType.visibleScope === 1001) {
      return <div>{this.$t('form.setting.form.allExpense') /*全部费用*/}</div>;
    }
  ...
  renderUser = users => {
    if (users.visibleScope === 1001) {
      return <div>{this.$t('form.setting.form.allCompany') /*全公司人员*/}</div>;
    }
  ...
  goBack = () => {
    this.props.dispatch(
      routerRedux.push({
        pathname: `/admin-setting/form-list/form-list/0`,
      })
    );
  };
  ...
  render() {
    ...
    return (
      <div className="form-permission" style={{ paddingBottom: 50 }}>
        {this.props.tenantMode && (
          <div>
            <div className="info-title">
              {this.$t('form.setting.form.divide.company') /*分配公司*/}&nbsp;&nbsp;&nbsp;<span
                style={{ color: '#989898', fontSize: 14 }}
              >
                {this.$t('form.setting.form.tip04') /*当前账套下启用的公司*/}
              </span>
            </div>
            <Row>
              <Col span={8} offset={3}>
                <RadioGroup onChange={this.handleChangeCompany} value={visibleCompanyScope}>
                  <Radio disabled={!isEdit1} value={1}>
                    {this.$t('form.setting.form.allCompany2') /*全部公司*/}
                  </Radio>
                  <Radio disabled={!isEdit1} value={2}>
                    {this.$t('form.setting.form.partCompany') /*部分公司*/}
                  </Radio>
                </RadioGroup>
                {visibleCompanyScope === 2 && (
                  <Chooser
            ...
            <Row style={{ marginTop: 12, marginBottom: 20 }}>
              <Col span={8} offset={3}>
                {!isEdit1 && (
                  <Button type="primary" onClick={() => this.handleChangeStatus('company')}>
                    {this.$t('common.edit') /*编辑*/}
                  </Button>
                )}
                {isEdit1 && (
                  <div>
                    <Button
                      ...
                      type="primary"
                      loading={loading1}
                      onClick={() => this.handleSave('company')}
                    >
                      {this.$t('common.save') /*保存*/}
                    </Button>
                    <Button onClick={() => this.handleChangeStatus('company', true)}>
                      {this.$t('common.cancel') /*取消*/}
                    </Button>
                  </div>
                )}
        ...
        {canEditExpense && (
          <div>
            {this.props.tenantMode || this.context.form.fromType != 2
              ? this.renderTenantUser()
              : this.renderSobUser()}
          </div>
        )}
        {!canEditExpense && (
          <Row>
            <Col span={8} offset={3}>
              <div>
                {this.$t(
                  'form.setting.form.tip05'
                ) /*注：关联报销单的分配人员与申请单一致，如需更改请在申请单页面操作*/}
              </div>
            </Col>
          </Row>
        )}
