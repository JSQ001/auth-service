...
class HuilianyiManagement extends React.Component {
      ...
      if (item.propertyName === 'application.close.enabled') {
        this.setState(
          ...
          () => {
            this.setState({
              autoStopNumRequired:
                this.state.autoStop.propertyValue === '1' || this.state.autoStop.propertyValue === 1
                  ? true
                  : false,
            });
          }
      ...
      if (item.propertyName === 'application.close.restart.enabled') {
        this.setState(
          ...
          () => {
            this.setState({
              restartNumRequired:
                this.state.employeeRestartUse.propertyValue === '1' ||
                this.state.employeeRestartUse.propertyValue === 1
                  ? true
                  : false,
            });
  ...
  render() {
    ...
    const {
      ...
      employeeAutoStop,
      employeeRestartUse,
      autoStopDaysDisabled,
      employeeRestartDays,
      employeeRestartDaysDisabled,
      autoStopNumRequired,
      restartNumRequired,
      bookingPreference,
    } = this.state;
        ...
        <Form hideRequiredMark={true}>
          <div className="huilianyi-alert">
            <Alert
              ...
              description={
                this.$t('form.setting.huilianyi.enable.rules') /*设置该单据是否启用相应的管控规则*/
              }
              type="info"
              showIcon
            />
            ...
            <div className="huilianyi-module">
              <FormItem label={this.$t('form.setting.huilianyi.control.standard') /*差旅控制标准*/}>
                {getFieldDecorator('travelApplypolicy', {
                  ...
                  initialValue:
                    travelApplypolicy.propertyValue === 'true' ||
                    travelApplypolicy.propertyValue === true
                      ? true
                      : false,
              ...
              <span className="huilianyi-note">
                {this.$t(
                  'form.setting.huilianyi.auto.apply'
                ) /*启用后，该单据在添加行程时将自动应用公司设置的差旅标准，包括机票舱等、折扣、酒店单价上限、火车座席等*/}
              </span>
              <div>
                {this.$t('form.setting.huilianyi.go.to') /*前往*/}
                <a
                  onClick={() => {
                    this.context.router.push(travelPolicy.url);
                  }}
                >
                  {this.$t('travel.policy.travel.standard.setting') /*差旅标准设置*/}
                </a>
            ...
            <div className="huilianyi-module">
              <FormItem label={this.$t('form.setting.huilianyi.version.change') /*版本变更管理*/}>
                {getFieldDecorator('versionChange', {
                  ...
                  valuePropName: 'checked',
                  initialValue:
                    versionChange.propertyValue === 'true' || versionChange.propertyValue === true
                      ? true
                      : false,
                })(<Switch disabled={disabled} />)}
              </FormItem>
              ...
              <span className="huilianyi-note">
                {this.$t(
                  'form.setting.huilianyi.enable.employee.stop'
                ) /*启用后，差旅申请单审批通过后，员工可以修改行程。*/}
              </span>
            </div>
            <div className="huilianyi-module">
              <div className="module-form-switch">
                <FormItem label={this.$t('form.setting.huilianyi.auto.stop') /*自动停用*/}>
                  {getFieldDecorator('autoStop', {
                    ...
                    valuePropName: 'checked',
                    initialValue:
                      autoStop.propertyValue === '1' || autoStop.propertyValue === 1 ? true : false,
                  })(
                    <Switch
                      disabled={disabled}
                      onChange={e => this.onSwitchChange(e, 'autoStop')}
                    />
                  )}
                </FormItem>
              </div>
              <div className="module-form-item">
                <span className="module-form-message">
                  {this.$t('form.setting.huilianyi.after.travel') /*差旅结束后*/}
                </span>
                <FormItem>
                  {getFieldDecorator('autoStopDays', {
                    ...
                    <InputNumber
                      ...
                      min={0}
                      precision={0}
                      disabled={autoStopDaysDisabled}
                    />
                  )}
                </FormItem>
                <span className="module-form-message">
                  {this.$t('form.setting.huilianyi.after.days') /*天后系统自动停用*/}
                </span>
              </div>
              <Divider dashed />
              <span className="huilianyi-note">
                {this.$t(
                  'form.setting.huilianyi.after.enable.stop'
                ) /*启用后，差旅结束后，在设置天数内自动停用。*/}
              </span>
            </div>
            <div className="huilianyi-module">
              <FormItem
                label={this.$t('form.setting.huilianyi.allow.auto.stop') /*允许员工自行停用*/}
              >
                {getFieldDecorator('employeeAutoStop', {
                  ...
                  valuePropName: 'checked',
                  initialValue:
                    employeeAutoStop.propertyValue === '1' || employeeAutoStop.propertyValue === 1
                      ? true
                      : false,
                })(<Switch disabled={disabled} />)}
              </FormItem>
              ...
              <span className="huilianyi-note">
                {this.$t(
                  'form.setting.huilianyi.enable.employee.stop'
                ) /*启用后，差旅申请单审批通过后，申请单内出现停用按钮，员工可自行停用差旅申请单。*/}
              </span>
            </div>
            <div className="huilianyi-module">
              <div className="module-form-switch">
                <FormItem
                  label={
                    this.$t('form.setting.huilianyi.enable.employee.restart') /*允许员工重新启用*/
                  }
                >
                  {getFieldDecorator('employeeRestartUse', {
                    ...
                    valuePropName: 'checked',
                    initialValue:
                      employeeRestartUse.propertyValue === '1' ||
                      employeeRestartUse.propertyValue === 1
                        ? true
                        : false,
                  })(
                    <Switch
                      disabled={disabled}
                      onChange={(e, key) => this.onSwitchChange(e, 'employeeRestartUse')}
                    />
                  )}
                </FormItem>
              </div>
              <div className="module-form-item">
                <span className="module-form-message">
                  {this.$t('form.setting.huilianyi.after.restart') /*重新启用*/}
                </span>
                <FormItem>
                  {getFieldDecorator('employeeRestartDays', {
                    ...
                    <InputNumber
                      ...
                      min={0}
                      precision={0}
                      disabled={employeeRestartDaysDisabled}
                    />
                  )}
                </FormItem>
                <span className="module-form-message">
                  {this.$t('form.setting.huilianyi.after.days2') /*天后系统自动停用*/}
                </span>
              </div>
              <Divider dashed />
              <span className="huilianyi-note">
                {this.$t(
                  'form.setting.huilianyi.employee.restart.apply'
                ) /*启用后，当申请单停用后，申请单内启用按钮，支持员工重新启用申请单。*/}
              </span>
            </div>
            <div className="huilianyi-module">
                ...
                {getFieldDecorator('bookingPreference', {
                  ...
                  >
                    <Option value="Consolidated">
                      {this.$t('form.setting.huilianyi.privilege.book.together') /*优先统一订票*/}
                    </Option>
                    <Option value="Individual">
                      {this.$t('form.setting.huilianyi.privilege.book.alone') /*优先各自订票*/}
                    </Option>
                  </Select>
                )}
              </FormItem>
              <Divider dashed />
              <span className="huilianyi-note">
                {this.$t('form.setting.huilianyi.book.default') /*为差旅申请设置订票方式默认值*/}
              </span>
            </div>
          </div>
