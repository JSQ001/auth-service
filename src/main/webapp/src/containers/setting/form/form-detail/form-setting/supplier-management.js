...
class SupplierManagement extends React.Component {
      ...
      if (son !== 'costCenterCustom') {
        for (let grandson in data[son]) {
          ...
          if (data[son][grandson].hasOwnProperty('enable')) {
            ...
            if (
              data[son][grandson].hasOwnProperty('required') ||
              data[son][grandson].hasOwnProperty('show') ||
              data[son][grandson].hasOwnProperty('control')
            ) {
              data[son][grandson]['enable'] =
                data[son][grandson]['control'] &&
                data[son][grandson]['required'] &&
                data[son][grandson]['show'];
            }
          }
        }
      } else {
  ...
  initFloatDays = (controlFields, type) => {
    ...
    if (type === 'flight') {
      if (controlFields.hasOwnProperty('departBeginDate')) {
        controlFields['departBeginDate'].floatDays = this.initFloatDaysNum(
          'start',
          false,
          controlFields['departBeginDate'].floatDays,
          controlFields['departBeginDate'].internationalFloatDays
        );
        controlFields['departBeginDate'].internationalFloatDays = this.initFloatDaysNum(
          'start',
          true,
          controlFields['departBeginDate'].floatDays,
          controlFields['departBeginDate'].internationalFloatDays
      ...
      if (controlFields.hasOwnProperty('returnEndDate')) {
        controlFields['returnEndDate'].floatDays = this.initFloatDaysNum(
          'end',
          false,
          controlFields['returnEndDate'].floatDays,
          controlFields['returnEndDate'].internationalFloatDays
        );
        controlFields['returnEndDate'].internationalFloatDays = this.initFloatDaysNum(
          'end',
          true,
          controlFields['returnEndDate'].floatDays,
          controlFields['returnEndDate'].internationalFloatDays
  ...
  initSelect2Value = key => {
    ...
    for (let keyName in costCenterCustom) {
      if (keyName === key) {
        if (
          costCenterCustom[key]['index'] !== '' &&
          costCenterCustom[key]['index'] !== null &&
          options[costCenterCustom[key]['index']]
        ) {
  ...
  onSaveClick = () => {
    ...
    propertyList.map(item => {
      ...
      if (item.propertyName === 'hotel.itinerary.enable') {
        hotelItinerary =
          item.propertyValue === 'true' || item.propertyValue === true ? true : false;
      }
      ...
      if (item.propertyName === 'travel.allowance.disabled') {
        travelAllowances =
          item.propertyValue === 'true' || item.propertyValue === true ? false : true;
      }
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        this.setState({
          manageType:
            values.manageType === this.$t('form.setting.supplier.merge.travel') /*合并行程*/
              ? 1001
              : 1002,
        });
          ...
          if (typeof values[keyName1] === 'object') {
            for (let keyName2 in values[keyName1]) {
              if (keyName2 !== 'preBookingDays') {
                ...
                this.state.controlFieldsFlight.preBookingDays =
                  values.controlFieldsFlight.preBookingDays;
              }
            }
          }
        }
        for (let keyFlight in controlFieldsFlight) {
          if (keyFlight !== 'preBookingDays') {
            controlFieldsFlight[keyFlight]['enable'] =
              controlFieldsFlight['show'] ||
              controlFieldsFlight['required'] ||
              controlFieldsFlight['control'];
          }
        }
        for (let keyHotel in controlFieldsHotel) {
          controlFieldsHotel[keyHotel]['enable'] =
            controlFieldsHotel['show'] ||
            controlFieldsHotel['required'] ||
            controlFieldsHotel['control'];
        }
        for (let keyTrain in controlFieldsTrain) {
          controlFieldsTrain[keyTrain]['enable'] =
            controlFieldsTrain['show'] ||
            controlFieldsTrain['required'] ||
            controlFieldsTrain['control'];
        }
  ...
  render() {
    ...
    const {
      ...
      cost4FirSelDisabled,
      cost5FirSelDisabled,
      cost1ThirdSelDisabled,
      cost2ThirdSelDisabled,
      cost3ThirdSelDisabled,
      cost4ThirdSelDisabled,
      cost5ThirdSelDisabled,
    } = this.state;
        ...
        <div className="form-setting-supplier">
          <div className="supplier-alert">
            <Alert
              ...
              description={
                this.$t(
                  'form.setting.travel.info.to.supplier'
                ) /*下方的设置决定了供应商如何对差旅行程管控，若启用相应功能后，融智汇会将相应信息传递给供应商*/
              }
              type="info"
              showIcon
            />
          ...
          <Form hideRequiredMark={true}>
            <div className="supplier-global">
              <span className="supplier-module-title">
                {this.$t('form.setting.supplier.global') /*全局*/}
              </span>
              <div className="supplier-module">
                <FormItem
                  label={this.$t('form.setting.supplier.travel.control.mode') /*行程管控模式*/}
                >
                  <Divider dashed />
                  {getFieldDecorator('manageType', {
                    ...
                    initialValue:
                      manageType === 1001
                        ? this.$t('form.setting.supplier.merge.travel') /*合并行程*/
                        : this.$t('form.setting.supplier.independent.travel') /*独立行程*/,
                  })(
                    <RadioGroup disabled={disabled}>
                      <Radio
                        value={this.$t('form.setting.supplier.independent.travel') /*独立行程*/}
                      >
                        {this.$t('form.setting.supplier.independent.travel') /*独立行程*/}
                      </Radio>
                      <Radio
                        value={this.$t('form.setting.supplier.merge.travel') /*合并行程*/}
                        checked=""
                      >
                        {this.$t('form.setting.supplier.merge.travel') /*合并行程*/}
                      </Radio>
              ...
              <div className="supplier-module">
                <FormItem label={this.$t('form.setting.supplier.validity.control') /*有效期管控*/}>
                  ...
                  {getFieldDecorator('expiryDateControl', {
                    ...
                    initialValue:
                      expiryDateControl.propertyValue === 'true' ||
                      expiryDateControl.propertyValue === true
                        ? true
                        : false,
                  })(
              ...
              <div className="supplier-module-switch">
                <FormItem label={this.$t('form.setting.supplier.cost.center1') /*成本中心1*/}>
                  {getFieldDecorator('costCenterCustom.costCenter1.enabled', {
                    ...
                    <Switch
                      onChange={e => this.switchChange(e, 'costCenter1')}
                      disabled={disabled}
                    />
                  )}
                </FormItem>
                      ...
                      {getFieldDecorator('costCenter1Fields1', {
                        ...
                        <Select
                          ...
                          onSelect={(value, option, key) =>
                            this.handleCenter1Fields1Change(value, option, 'costCenter1')
                          }
                        >
                          <Option value={this.$t('accounting.source.stateValue') /*固定值*/}>
                            {this.$t('accounting.source.stateValue') /*固定值*/}
                          </Option>
                          <Option
                            value={this.$t('form.setting.supplier.system.filed') /*系统字段*/}
                          >
                            {this.$t('form.setting.supplier.system.filed') /*系统字段*/}
                          </Option>
                        </Select>
                      )}
                    </FormItem>
                        ...
                        <FormItem>
                          {getFieldDecorator('costCenter1Fields2', {
                            ...
                            <Select
                              ...
                              disabled={disabled || cost1FirSelDisabled}
                              onSelect={(value, option, key) =>
                                this.handleCenter1Fields2Change(value, option, 'costCenter1')
                              }
                            >
                              {options.map((item, index) => {
                                return (
                                  item.valueTypes && (
                                    <Option value={item.show} key={index}>
                                      {item.show}
                                    </Option>
                                  )
                                );
                      ...
                      <div className="costCenter-second-select">
                        <FormItem>
                          {getFieldDecorator('costCenter1Fields3', {
                            ...
                            >
                              {fields1ThrOptions &&
                                fields1ThrOptions.map((item, index) => {
                                  return (
                                    <Option value={item.viewName} key={index}>
                                      {item.viewName}
                                    </Option>
                                  );
                                })}
              ...
              <div className="supplier-module-switch">
                <FormItem label={this.$t('form.setting.supplier.cost.center2') /*成本中心2*/}>
                  {getFieldDecorator('costCenterCustom.costCenter2.enabled', {
                    ...
                    <Switch
                      onChange={e => this.switchChange(e, 'costCenter2')}
                      disabled={disabled}
                    />
                  )}
                </FormItem>
                      ...
                      {getFieldDecorator('costCenter2Fields1', {
                        ...
                        <Select
                          ...
                          onSelect={(value, option) =>
                            this.handleCenter1Fields1Change(value, option, 'costCenter2')
                          }
                        >
                          <Option value={this.$t('accounting.source.stateValue') /*固定值*/}>
                            {this.$t('accounting.source.stateValue') /*固定值*/}
                        ...
                        <FormItem>
                          {getFieldDecorator('costCenter2Fields2', {
                            ...
                            <Select
                              ...
                              disabled={disabled || cost2FirSelDisabled}
                              onSelect={(value, option) =>
                                this.handleCenter1Fields2Change(value, option, 'costCenter2')
                              }
                            >
                              {options.map((item, index) => {
                                return (
                      ...
                      <div className="costCenter-second-select">
                        <FormItem>
                          {getFieldDecorator('costCenter2Fields3', {
                            ...
                            >
                              {fields2ThrOptions &&
                                fields2ThrOptions.map((item, index) => {
                                  return (
                                    <Option value={item.viewName} key={index}>
                                      {item.viewName}
                                    </Option>
              ...
              <div className="supplier-module-switch">
                <FormItem label={this.$t('form.setting.supplier.cost.center3') /*成本中心3*/}>
                  {getFieldDecorator('costCenterCustom.costCenter3.enabled', {
                    ...
                    <Switch
                      onChange={e => this.switchChange(e, 'costCenter3')}
                      disabled={disabled}
                    />
                  )}
                </FormItem>
                    ...
                    <FormItem>
                      {getFieldDecorator('costCenter3Fields1', {
                        ...
                        <Select
                          ...
                          onSelect={(value, option) =>
                            this.handleCenter1Fields1Change(value, option, 'costCenter3')
                          }
                        >
                          <Option value={this.$t('accounting.source.stateValue') /*固定值*/}>
                            {this.$t('accounting.source.stateValue') /*固定值*/}
                        ...
                        <FormItem>
                          {getFieldDecorator('costCenter3Fields2', {
                            ...
                            <Select
                              ...
                              disabled={disabled || cost3FirSelDisabled}
                              onSelect={(value, option) =>
                                this.handleCenter1Fields2Change(value, option, 'costCenter3')
                              }
                            >
                              {options.map((item, index) => {
                                return (
                      ...
                      <div className="costCenter-second-select">
                        <FormItem>
                          {getFieldDecorator('costCenter3Fields3', {
                            ...
                            >
                              {fields3ThrOptions &&
                                fields3ThrOptions.map((item, index) => {
                                  return (
                                    <Option value={item.viewName} key={index}>
                                      {item.viewName}
                                    </Option>
              ...
              <div className="supplier-module-switch">
                <FormItem label={this.$t('form.setting.supplier.custom.field1') /*自定义字段1*/}>
                  {getFieldDecorator('costCenterCustom.costCenter4.enabled', {
                    ...
                    <Switch
                      onChange={e => this.switchChange(e, 'costCenter4')}
                      disabled={disabled}
                    />
                  )}
                </FormItem>
                    ...
                    <FormItem>
                      {getFieldDecorator('costCenter4Fields1', {
                        ...
                        <Select
                          ...
                          onSelect={(value, option) =>
                            this.handleCenter1Fields1Change(value, option, 'costCenter4')
                          }
                        >
                          <Option value={this.$t('accounting.source.stateValue') /*固定值*/}>
                            {this.$t('accounting.source.stateValue') /*固定值*/}
                        ...
                        <FormItem>
                          {getFieldDecorator('costCenter4Fields2', {
                            ...
                            <Select
                              ...
                              disabled={disabled || cost4FirSelDisabled}
                              onSelect={(value, option, key) =>
                                this.handleCenter1Fields2Change(value, option, 'costCenter4')
                              }
                            >
                              {options.map((item, index) => {
                                return (
                      ...
                      <div className="costCenter-second-select">
                        <FormItem>
                          {getFieldDecorator('costCenter4Fields3', {
                            ...
                            >
                              {fields4ThrOptions &&
                                fields4ThrOptions.map((item, index) => {
                                  return (
                                    <Option value={item.viewName} key={index}>
                                      {item.viewName}
                                    </Option>
              ...
              <div className="supplier-module-switch">
                <FormItem label={this.$t('form.setting.supplier.custom.field2') /*自定义字段2*/}>
                  {getFieldDecorator('costCenterCustom.costCenter5.enabled', {
                    ...
                    <Switch
                      onChange={(e, key) => this.switchChange(e, 'costCenter5')}
                      disabled={disabled}
                    />
                  )}
                </FormItem>
                    ...
                    <FormItem>
                      {getFieldDecorator('costCenter5Fields1', {
                        ...
                        <Select
                          ...
                          onSelect={(value, option, key) =>
                            this.handleCenter1Fields1Change(value, option, 'costCenter5')
                          }
                        >
                          <Option value={this.$t('accounting.source.stateValue') /*固定值*/}>
                            {this.$t('accounting.source.stateValue') /*固定值*/}
                        ...
                        <FormItem>
                          {getFieldDecorator('costCenter5Fields2', {
                            ...
                            <Select
                              ...
                              disabled={disabled || cost5FirSelDisabled}
                              onSelect={(value, option, key) =>
                                this.handleCenter1Fields2Change(value, option, 'costCenter5')
                              }
                            >
                              {options.map((item, index) => {
                                return (
                      ...
                      <div className="costCenter-second-select">
                        <FormItem>
                          {getFieldDecorator('costCenter5Fields3', {
                            ...
                            >
                              {fields5ThrOptions &&
                                fields5ThrOptions.map((item, index) => {
                                  return (
                                    <Option value={item.viewName} key={index}>
                                      {item.viewName}
                                    </Option>
            ...
            <div className="supplierFlight-ticket">
              <span className="supplier-module-title">
                {this.$t('travel.policy.ticket') /*机票*/}
              </span>
              <div className="supplier-module">
              ...
              <div className="supplier-module">
                <FormItem
                  label={
                    this.$t(
                      'form.setting.supplier.domestic.ticket.depart'
                    ) /*国内机票出发日期-浮动天数*/
                  }
                >
                  <Divider dashed />
              ...
              <div className="supplier-module">
                <FormItem
                  label={
                    this.$t(
                      'form.setting.supplier.international.ticket'
                    ) /*国际机票出发日期-浮动天数*/
                  }
                >
                  <Divider dashed />
              ...
              <div className="supplier-module">
                <FormItem
                  label={
                    this.$t(
                      'form.setting.supplier.domestic.ticket.tracking'
                    ) /*国内机票返程日期-浮动天数*/
                  }
                >
                  <Divider dashed />
              ...
              <div className="supplier-module">
                <FormItem
                  label={this.$t('form.setting.supplier.pre-booking.days') /*提前预订天数*/}
                >
                  <Divider dashed />
              ...
              <div className="supplier-module">
                <FormItem
                  label={this.$t('form.setting.supplier.international.name') /*国际出行人姓名*/}
                >
                  <Divider dashed />
            ...
            <div className="supplier-hotel">
              ...
              <div className="supplier-module">
                <FormItem
                  label={this.$t('form.setting.supplier.check-in.date') /*入住日期-浮动天数*/}
                >
                  <Divider dashed />
              ...
              <div className="supplier-module">
                <FormItem
                  label={this.$t('form.setting.supplier.check-out.date') /*离店日期-浮动天数*/}
                >
                  <Divider dashed />
            ...
            <div className="supplier-train">
              ...
              <div className="supplier-module">
                <FormItem
                  label={this.$t('form.setting.supplier.set.out.date') /*出发日期-浮动天数*/}
                >
                  <Divider dashed />
          ...
          <div className="form-setting-buttons">
            {isEditing ? (
              <div>
                <Button type="primary" className="buttons-save" onClick={this.onSaveClick}>
                ...
                <Button
                  ...
                  onClick={() => {
                    this.props.dispatch(
                      routerRedux.push({
                        pathname: `/setting/form-list/form-list/0`,
                      })
                    );
                  }}
                >
              ...
              <div>
                <Button type="primary" className="buttons-edit" onClick={this.editClicked}>
