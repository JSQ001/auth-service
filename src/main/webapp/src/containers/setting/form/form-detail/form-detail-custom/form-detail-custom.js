...
class FormDetailCustom extends React.Component {
  ...
  componentWillMount() {
    ...
    nowWidget.map((widget, index) => {
      ...
      if (
        widget.messageKey === 'out_participant_num' ||
        widget.messageKey === 'out_participant_name'
      ) {
        outIndex = index;
      }
  ...
  getWidgetList = () => {
    ...
    if (formType) {
      ...
      Promise.all([
        formService.getWidgetList(formType, 1001),
        formService.getWidgetList(formType, 1002),
      ]).then(res => {
        res[0].data.map(item => {
        ...
        res[0].data.map(item => {
          ...
          if (this.props.isOldCompany) {
            ...
            if (item.messageKey != 'select_corporation_entity') normalWidget.push(item);
          }
        });
        this.setState(
          { normalWidget: normalWidget, combineWidget: res[1].data, loading: false },
          () => {
            this.processWidget();
          }
        );
  ...
  handleSave = () => {
    ...
    participantsImportScopeData &&
      serviceArray.unshift(formService.saveFormProperty([participantsImportScopeData]));
  ...
  validateData = form => {
    ...
    form.customFormFields.map(customFormField => {
      ...
      if (
        customFormField.messageKey === 'select_cost_center' &&
        customFormField.dataSource === ''
      ) {
        ...
        <Col span={8}>
          <div className="widget-area">
            <div className="widget-category">
              {this.$t('form.setting.normal.widget') /*普通控件*/}
            </div>
            <div className="widget-list">
              {normalWidget.map(widget => (
                <DragWidgetItem widget={widget} key={widget.messageKey} />
              ))}
            </div>
            <div className="widget-category">
              {this.$t('form.setting.combo.widget') /*组合控件*/}
            </div>
            <div className="widget-list">
  ...
  render() {
    ...
    const selectedWidget =
      nowSelectedIndex === -1 ? formDescriptionWidget : nowWidget[nowSelectedIndex];
    return (
...
export default connect(
  ...
  null,
  { withRef: true }
)(DragDropContext(HTML5Backend)(FormDetailCustom));
