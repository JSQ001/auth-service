...
class ApproveExpenseReportDetail extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.auditCapability &&
      expenseReportService.checkInvoice(this.props.info.expenseReportOid).then(res => {
        this.setState({
  ...
  handleApprovePass = (value, additionalOids) => {
    ...
    ApproveExpenseReportService.handleExpenseReportApprovePass(params)
      .then(res => {
        if (res.data.failNum === 0) {
          ...
          this.setState({ passLoading: false });
          message.error(
            `${this.$t('common.operate.filed')}，${res.data.failReason[info.expenseReportOid]}`
          );
        }
      })
      .catch(e => {
  ...
  handleApproveReject = (value, additionalItems, direct) => {
    ...
    if (selectedExpense.length > 0 && !direct) {
      ...
      info.expenseReportInvoices.map(invoice => {
        allReject =
          allReject &&
          (selectedExpense.indexOf(invoice.invoiceOid) > -1 || invoice.status === 1002);
      });
      if (allReject) {
        Modal.confirm({
      ...
      this.handleService(
        ApproveExpenseReportService.handleExpenseReportApproveReject,
        params,
        true,
        true
      );
          ...
          if (isHandleService) {
            message.error(
              `${this.$t('common.operate.filed')}，${
                res.data.failReason[this.props.info.expenseReportOid]
              }`
            );
          } else {
  ...
  render() {
    ...
    return info.status === 1002 && ~approvalChains.indexOf(info.approvalChain.approverOid) ? (
      <Affix style={{ paddingLeft: 20 }} offsetBottom={0} className="bottom-bar bottom-bar-approve">
        <ApproveBar
          backUrl={'/approval-management/approve-expense-report'}
