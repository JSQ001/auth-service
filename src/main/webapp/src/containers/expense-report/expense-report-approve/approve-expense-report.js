...
class ExpenseReportApprove extends React.Component {
      ...
      approvePendingCheckboxList: [
        {
          ...
          items: [
            {
              label: this.$t('myReimburse.documentName' /*单据名称*/),
              key: 'formOid',
              options: [],
              checkAllOption: true,
            },
      ...
      columns: [
        {
          ...
          render: (value, record, index) => {
            return `${index + 1 + this.state.pageSize * (this.state.pagination.current - 1)}${
              record.proxyInfo ? `(${this.$t('common.proxy')})` : ''
            }`;
          },
          width: '8%',
        },
        ...
        {
          title: this.$t('myReimburse.documentName' /*单据名称*/),
          dataIndex: 'formName',
          render: value =>
            value ? (
              <Popover placement="topLeft" content={value}>
  ...
  getForms = () => {
    ['approvePending', 'approved'].map(tab => {
      let checkboxList = this.state[
        tab === 'approvePending' ? 'approvePendingCheckboxList' : 'approvedCheckboxList'
      ];
  ...
  getList = () => {
    ...
    approveExpenseReportService
      .getApproveExpenseReportList(
        tab === 'approved',
        page,
        pageSize,
  ...
  dealCache(result) {
    ...
    if (result) {
      ...
      this.setState(
        { tab, defaultSearchForm, searchForm, page, [checkboxListFormKey]: checkboxListForm },
        () => {
          this.search(result);
  ...
  handleRowClick = record => {
    ...
    if (this.state.tab === 'approvePending') {
      this.props.dispatch(
        routerRedux.push({
          pathname: `/approval-management/approve-expense-report/approve-expense-report-detail/${
            record.entityOid
          }/${record.approverOid}`,
        })
      );
  ...
  handleApprove = (value, type) => {
    ...
    approveExpenseReportService[
      type === 'pass' ? 'handleExpenseReportApprovePass' : 'handleExpenseReportApproveReject'
    ](params)
      .then(res => {
        message.success(
          this.$t(
  ...
  refreshRowSelection() {
    ...
    selectedDataOids.map(selected => {
      data.map((item, index) => {
        if (selected.entityOid === item.entityOid) nowSelectedRowKeys.push(index);
      });
    });
  ...
  render() {
    ...
    return (
      <div
        className={`approve-request approve-expense-report ${
          selectedDataNum && tab === 'approvePending' ? 'bottom-100' : ''
        }`}
      >
        <Tabs activeKey={tab} onChange={this.onTabChange}>
          <TabPane
            tab={this.$t('request.detail.await.approval.by' /*待审批*/)}
            key="approvePending"
          />
        ...
        <div className="table-header">
          <div className="table-header-title">
            ...
            |
            {tab === 'approvePending' &&
              this.$t('common.total.selected', { total: selectedDataNum })}
        ...
        <Affix
          ...
          className={`bottom-bar-approve ${
            selectedDataNum && tab === 'approvePending' ? 'show' : 'hide'
          }`}
        >
          <ApproveBar
