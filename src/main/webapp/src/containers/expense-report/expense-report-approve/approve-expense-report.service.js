...
export default {
  ...
  getDocumentType(finished) {
    return httpFetch.get(
      `${config.baseUrl}/api/approvals/custom/form?entityType=1002&finished=${finished}`
    );
  },
  ...
  getApproveExpenseReportList(finished, page, size, searchParams) {
    let url = `${
      config.baseUrl
    }/api/approvals/batchfilters?entityType=1002&finished=${finished}&page=${page}&size=${size}`;
