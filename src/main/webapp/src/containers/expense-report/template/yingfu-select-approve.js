...
class YingfuSelectApprove extends React.Component {
  ...
  getList() {
    ...
    expenseReportService
      .getYingfuInvoiceInfo(this.props.expenseReport)
      .then(res => {
        ...
        yingfuInvoiceInfo.map(item => {
          if (
            item.costCenterView &&
            item.costCenterView.approverGroups &&
            item.costCenterView.approverGroups.length > 0
          ) {
  ...
  handleEditApprove = (record, index) => {
    ...
    yingfuApproverGroups &&
      yingfuApproverGroups.map(item => {
        item.index = index;
      });
    this.setState({
      visible: false,
      selectVisible: true,
      yingfuApproverGroups: yingfuApproverGroups,
    });
  };
  //选择审批人
  ...
  handleSubmitApprove = () => {
    ...
    for (let i = 0; i < yingfuInvoiceInfo.length; i++) {
      if (
        !yingfuInvoiceInfo[i].costCenterView.selectApproverGroups ||
        yingfuInvoiceInfo[i].costCenterView.selectApproverGroups.length === 0
      ) {
        message.error(
          messages('expense-report.submit.yingfu.invoiceDataTip') /*'有费用未选择审批人'*/
        );
        return;
      }
      //选择审批用户的Oid处理
  ...
  render() {
    ...
    let modalTitle = (
      <div>
        <span>
          {messages('expense-report.submit.yingfu.selectCostApprove') /*'选择成本中心审批人'*/}
        </span>
        <br />
        <span style={{ color: 'red', fontSize: 14 }}>
          ({messages(
            'expense-report.submit.yingfu.submitTip'
          ) /*'请选择成本中心对应的审批人并仔细核对，否则报销单将被驳回'*/})
        </span>
      </div>
    );
    let approveColunmTitle = (
      <div style={{ color: 'red' }}>
        <span>{messages('expense-report.submit.yingfu.approver') /*审批人*/}</span>
        <br />
        <span>(请点击选择)</span>
      </div>
    );
    return (
      <div>
        ...
        >
          <Spin spinning={loading}>
            <Table
              ...
              columns={[
                {
                  title: messages('expense') /*"费用"*/,
                  dataIndex: '',
                  width: '50%',
                  render: (value, record) => {
                    let content = `${record.costCenterView.costCenterName},${
                      record.costCenterView.currencyCode
                    } ${this.filterMoney(record.costCenterView.amount, 2, true)}`;
                    return <Popover content={content}>{content}</Popover>;
                  },
                },
                {
                  title: approveColunmTitle,
                  dataIndex: '',
                  width: '50%',
                  render: (value, record) => {
                    let contents = [];
        ...
        <Modal
          title={
            modalTitle /*emessages('expense-report.submit.yingfu.selectCostApprove')*/ /*'选择成本中心审批人'*/
          }
          okText={messages('common.ok') /*'确定'*/}
          visible={selectVisible}
          onOk={this.handleCancelSelectApprove}
          onCancel={this.handleCancelSelectApprove}
          width={800}
        >
          <Table
            dataSource={yingfuApproverGroups}
            bordered
            pagination={false}
            rowSelection={{
              type: 'radio',
            ...
            columns={[
              {
                title: messages('expense-report.submit.yingfu.approver') /*审批人*/,
                dataIndex: '',
                width: '100%',
                render: (value, record) => {
