...
const defaultCustomMessageKeys = [
  'select_department',
  'select_approver',
  'remark',
  'select_company',
  'select_corporation_entity',
  'title',
];
...
class NewExpenseReport extends React.Component {
  ...
  componentWillMount() {
    ...
    this.isCounterSignEnable(
      this.props.params ? this.props.params.formDetail.formOid : this.props.match.params.formId
    );
  ...
  dealRelactiveFormRender(item, change, field, customFormFields, type = 1) {
    ...
    if (
      ...
      !change &&
      ~interactive.indexOf(field.messageKey)
    ) {
  ...
  renderFormItem() {
    ...
    return customFormFields.map(field => {
      ...
      if (expenseReport) {
        ...
        if (change) {
          option.initialValue = customField.getDefaultValue(
            field,
            ~defaultCustomMessageKeys.indexOf(field.messageKey) ? {} : fieldDefaultValue
          );
        }
      }
      if (field.required) {
      ...
      return (
        ...
        field.messageKey != 'ying_fu_select_approver' && (
          <FormItem
            {...formItemLayout}
            label={`${field.fieldName}${fieldNameHelp}`}
            key={field.fieldOid}
          >
  ...
  isCounterSignEnable = formOid => {
    expenseReportService
      .isCounterSignEnable(this.props.company.companyOid, formOid, 'enableAddSignForSubmitter')
      .then(res => {
        this.setState({
          ...
          customFormFields.map(field => {
            applicationFieldDefaultValues.map(item => {
              if (
                ...
                ~defaultCustomMessageKeys.indexOf(field.messageKey)
              ) {
  ...
  render() {
    ...
    let signPerson = [];
    expenseReport &&
      expenseReport.countersignApproverNames &&
      expenseReport.countersignApproverNames.map(item => {
    ...
    return (
      <div className="new-expense-report" ref="newExpenseReport" style={{ position: 'relative' }}>
        ...
        <Form onSubmit={this.handleSave}>
          {!expenseReport &&
            (formDetail.formType === 3002 || formDetail.formType === 3003) && (
              <FormItem
                {...formItemLayout}
                label={this.$t('expense-report.association.request') /*关联申请单*/}
              >
                {getFieldDecorator('application', {
                  rules: [
                    {
                      required: true,
                      message: this.$t('common.name.is.required', {
                        name: this.$t('expense-report.association.request') /*关联申请单*/,
                      }),
                    },
                  ],
                })(
                  <Chooser
                    type="my_request"
                    listExtraParams={{
                      formOid: this.props.match ? this.props.match.params.formId : null,
          ...
          {signEnable && (
            <FormItem
              {...formItemLayout}
              label={this.$t('customField.special.signer') /*"加签人"*/}
              key="addSign"
            >
              {getFieldDecorator('addSign', {
                ...
                <Chooser
                  ...
                  showArrow={
                    ...
                    formDetail.customFormPropertyMap.countersignType === '2'
                  }
                  newline
                />
              )}
