...
class ExpenseReportDetail extends React.Component {
  ...
  componentDidMount() {
    ...
    if (
      window.location.href.indexOf('finance-view') > -1 ||
    ...
    return this.state.readOnly ? (
      <span>
        ...
        >
          ...
          {record.status === 1002 &&
            `(${this.$t('constants.documentStatus.has.been.rejected')})` /*(已驳回)*/}
        </a>
        ...
        ((record.digitalInvoice.cardsignType === 'ALICARDSIGN' && record.digitalInvoice.pdfUrl) ||
          ...
          >
            <Row gutter={12} type="flex" justify="start">
              <Button
                style={{ marginLeft: 20 }}
                onClick={() => this.handlePrintInvoice(record.digitalInvoice)}
              >
      ...
      <span>
        ...
        {this.deletingRecord(record) ? (
          ...
          <Popconfirm
            onConfirm={() => this.handleDeleteExpense(record)}
  ...
  getInfo = baseOnly => {
    ...
    expenseReportService.getExpenseReportDetail(expenseReportOid).then(expenseReportRes => {
      ...
      if (
        window.location.href.indexOf('approve-expense-report-detail') > -1 &&
        expenseReportRes.data.approvalChains &&
        expenseReportRes.data.approvalChains.length > 0
      ) {
      ...
      if (
        pageFrom === 'my' &&
        expenseReportRes.data.children &&
        expenseReportRes.data.children.length > 0
      ) {
        children = true;
        this.setState({ children });
        expenseReportRes.data.children.map(item => {
          //拆单逻辑，拼接子单的通知回复
          item.approvalHistoryDTOs &&
            item.approvalHistoryDTOs.map(history => {
          ...
          item.expenseReportInvoices &&
            item.expenseReportInvoices.map((i, index) => {
              i.invoiceView.statusViewShow = (
                constants.invoiceChildrenStatus.filter(
                  t =>
                    (t.id === 1001 && t.id === item.statusView && t.type === item.rejectType) ||
                    (t.id != 1001 && t.id === item.statusView)
                )[0] || { name: messages('constants.invoiceChildrenStatus.default') /*'未知状态'*/ }
              ).name;
              i.invoiceView.statusView = item.statusView;
              i.invoiceView.splitType = item.splitName;
              i.invoiceView.splitName = (
                constants.expenseReportChildrenType.filter(t => t.id === item.splitName)[0] || {
                  name: messages('constants.expenseReportChildrenType.default') /*'未知类型'*/,
                }
              ).name;
              if (index === 0) {
      ...
      invoiceDatas &&
        invoiceDatas.map(invoice => {
          ...
          if (invoice.status !== 1002) expenseReportInvoiceOids.push(invoice.invoiceOid);
        });
    ...
    !baseOnly &&
      Promise.all([
        ...
        switch (res[0].data.formType) {
          case 3001:
            formType = 'daily';
            if (res[0].data.formCode === 'vendor_payment') formType = 'pay';
            break;
          case 3002:
            formType = 'travel';
            break;
        ...
        this.setState({
          ...
          businessCardEnabled:
            res[5].data.rows && res[2].data.success && res[2].data.rows.length > 0,
        });
      });
  };
              ...
              if (res.data.failReason[info.expenseReportOid]) {
                message.error(
                  `${this.$t('common.operate.filed') /*操作失败*/}，${
                    res.data.failReason[info.expenseReportOid]
                  }`
                );
              } else {
  ...
  repaymentValid = () => {
    ...
    if (
      ...
      (!isMultiCurrency ||
        (isMultiCurrency && !this.props.profile['er.refund.foreign.disabled'])) &&
  ...
  selectLoanRequest = () => {
    ...
    if (info.custFormValues && info.custFormValues.length > 0) {
      let expenseReportCompanyInfo = info.custFormValues.filter(
        item => item.messageKey === 'select_company'
      )[0];
    ...
    expenseReportService
      .getLoanRequestList(
        ...
        info.currencyCode,
        venMasterId,
        expenseReportCompanyOid
      )
      .then(loanListRes => {
        expenseReportService
          .getDefaultLoanRequest(info.applicationOid, applicant.userOid)
          .then(defaultLoanRes => {
            ...
            loanListRes.data.map(item => {
              ...
              if (
                item.writeoffArtificialDTO.stayWriteoffAmount > 0 &&
                item.originCurrencyCode === info.currencyCode
              ) {
                //强制默认借款单，列表内只有一个
                if (
                  this.props.profile['ALL.ER.DefaultLoan.Forcible'] &&
                  defaultLoanApplicationOid
                ) {
                  if (item.applicationOid === defaultLoanApplicationOid) loanRequestList.push(item);
                } else {
              ...
              if (allOk) {
                ...
                if (rejectNum > 0) {
                  ...
                  Modal.error({
                    ...
                    content: this.$t('expense-report.expense.reject.and.warning', {
                      rejectNum,
                      warningNum,
                    }), //您有${rejectNum}条费用禁止提交，${warningNum}条费用预警
                  });
                } else {
                  Modal.confirm({
                    title: this.$t('common.info'), //提示
  ...
  submitError = e => {
    ...
    if (data.errorCode === '2001') message.error(this.$t('expense-report.empty.approve.chain'));
    //审批链为空
  ...
  handleDelete = () => {
    confirm({
      ...
      onOk: () => {
        ...
        expenseReportService
          .deleteExpenseReport(this.props.match.params.expenseReportOid)
          .then(res => {
            if (res.status === 200) {
              message.success(this.$t('common.operate.success') /*操作成功*/);
              this.goBack();
  ...
  goBack = () => {
    ...
    if (
      this.props.match.params.backType &&
      this.props.match.params.backType.indexOf('history') !== -1
    ) {
      window.history.go(-1);
    } else {
      let url = '';
      switch (this.props.match.params.pageFrom) {
        case 'my':
          ...
          break;
        case 'f_view':
          url = '/financial-management/finance-view';
      }
      this.props.dispatch(
        routerRedux.push({
          pathname: url,
  ...
  handleSubmitWithLoan = () => {
    ...
    if (selectedLoanRequest) {
      if (
        this.props.profile['All.ER.GreaterThanLoan'] &&
        reimbursementAmount > info.personalPaymentBaseAmount
      ) {
        message.error(this.$t('expense-report.select.loan.more.expenseAmount.help')); //报销单金额不能小于借款单金额
        return;
      }
  ...
  renderClass = record => {
    ...
    return (record.invoiceLabels && record.invoiceLabels.length > 0) || record.checkResultStatus
      ? isWarn
        ? 'rejected-expense'
        : ''
    ...
    Modal.info({
      ...
      content: (
        <div>
          ...
          <p>
            {this.$t(
              'expense-report.import.ex.expense.content2'
            ) /*导入完成后,您会收到一封邮件通知,然后刷新报销单页面进行查看*/}
          </p>
        </div>
      ),
      onOk() {},
  ...
  dealApprovelNode = history => {
    if (
      ...
      (history.countersignType == 2 || history.countersignType == 1)
    ) {
  ...
  dealApprovelUser = () => {
    ...
    /*代理审批逻辑*/
    info.approvalChains &&
      info.approvalChains.map((chain, indexs) => {
        chain.proxyApprovers &&
          chain.proxyApprovers.map((c, index) => {
            proxyUserInfo += `${c.fullName} ${c.employeeID} ${
              index != chain.proxyApprovers.length - 1 ? '& ' : ''
            }`;
          });
        if (chain.proxyApprovers && chain.proxyApprovers.length) {
          proxyUserInfo += `(${this.$t(
    ...
    if (!proxyUserInfo)
      proxyUserInfo = `${info.approvalChain.approverName} ${info.approvalChain.approverEmployeeID}`;
  ...
  dealInvoiceColumnsRender = () => {
    ...
    if (info && info.children && info.children.length > 0) {
      removeArryItem(
        invoiceColumns,
        invoiceColumns.filter(item => item.dataIndex === 'splitType')[0]
      );
      removeArryItem(
        invoiceColumns,
        invoiceColumns.filter(item => item.dataIndex === 'statusView')[0]
      );
        ...
        <div className="top-info">
              ...
              {(approve || audit || view) &&
                ...
                repaymentInfo.debtAmount > 0 && (
                  <span style={{ color: '#0092da' }}>
                    ...
                    }&nbsp;{this.filterMoney(repaymentInfo.debtAmount)}&nbsp;
                    {/* <Icon type="right"/> */}&nbsp;&nbsp;
                  </span>
                )}
              {this.$t('common.user.id') /*工号*/}：{applicant.employeeID}
              <span className="ant-divider" />
              {this.$t('common.department') /*部门*/}：{this.checkFunctionProfiles(
                'department.full.path.disabled',
                [true]
              )
                ? applicant.departmentName
                : applicant.departmentPath}
              <span className="ant-divider" />
              {this.$t('common.user.company') /*员工公司*/}：{applicant.companyName}
              <span className="ant-divider" />
              {this.$t('common.currency') /*币种*/}：{info.baseCurrency}
              <span className="ant-divider" />
              {this.$t('expense-report.repayment.amount') /*还款金额*/}：{info.status >= 1004 ||
              !info.loanApplicationOid
                ? `${info.currencyCode} ${this.filterMoney(info.reimbursementAmount, 2, true)}`
                : this.$t('expense-report.calculating.after.audit') /*审核通过后计算*/}
              {info.createdName != info.applicantName && <span className="ant-divider" />}
              {info.createdName != info.applicantName &&
                `${this.$t('expense-report.create.user')}：${info.createdName} ${moment(
                  info.createdDate
                ).format('YYYY-MM-DD')}`}
            </span>
          </Row>
          <Row className="row-container">
            <span className="detail-info detail-info-first">
              {info.formName}：{info.parentBusinessCode ? `${info.parentBusinessCode}-` : ''}
              {info.businessCode}
            </span>
            <span className="detail-info">
              {this.$t('common.submit.date') /*提交日期*/}：{info.lastSubmittedDate
                ? moment(info.lastSubmittedDate).format('YYYY-MM-DD')
                : moment(info.createdDate).format('YYYY-MM-DD')}
              {info.submittedBy &&
                info.submittedBy !== applicant.userOid &&
                `，${this.$t('expense-report.submitted.by', {
                  name: info.submittedName,
                })}` /*由 name 代提*/}
            </span>
            <span className="detail-info">
              {this.$t('common.column.status') /*状态*/}：{this.$t(expenseReportStatus.text)}
            </span>
          </Row>
          <RelativeExpenseReportDetail info={info} />
          <Row className="row-container">
            <span className="amount">
              {this.$t('expense-report.amount') /*单据金额*/}：{info.currencyCode}{' '}
              {this.filterMoney(info.totalAmount)}&nbsp;&nbsp;{' '}
              {this.$t('expense-report.pay.amount') /*付款金额*/}：{info.baseCurrency}{' '}
              {this.filterMoney(info.baseCurrencyRealPayAmount || info.realPaymentBaseAmount)}
            </span>
            {/*业务暂时去除*/}
            {false &&
              info.totalAmount > info.personalPaymentAmount &&
              expenseReportStatus.operate !== 'edit' && (
                <span className="personal-payment-amount">
                  {this.$t('expense-report.personal.amount') /*个人支付*/}: {info.currencyCode}{' '}
                  {this.filterMoney(info.personalPaymentAmount)}
                </span>
              )}
            {false &&
              info.totalAmount > info.personalPaymentAmount &&
              expenseReportStatus.operate !== 'edit' && (
                <span className="company-payment-amount">
                  {this.$t('expense-report.company.amount') /*公司支付*/}: {info.currencyCode}{' '}
                  {this.filterMoney(info.totalAmount - info.personalPaymentAmount)}
                </span>
              )}
          </Row>
        </div>
    ...
    let detailContent = (
      <div className="tab-container">
        <h3 className="sub-header-title">
          ...
          {!loading &&
            !readOnly && (
              <a className="edit" onClick={() => this.setState({ showNewExpenseReport: true })}>
                {this.$t('common.edit') /*编辑*/}
              </a>
            )}
        </h3>
    ...
    let expenseReportInvoicesContent = (
      <div className="detail-tabs">
        {!loading &&
          !readOnly && (
            <div className="expense-import-area">
              {!profile['account.book.upsert.disabled'] && (
                <Button type="primary" onClick={() => this.setState({ showImportExpense: true })}>
                  {this.$t('expense-report.account.import') /*账本导入*/}
                </Button>
              )}
              {
                <Button
                ...
                >
                  {this.$t('expense.new') /*新建费用*/}
                </Button>
              }
              {profile['expenseBatchImport.isOpen'] && (
                <Button onClick={() => this.openExternalExpenseImport(true)}>
                  {this.$t('expense-report.ex.expense.import') /*外部费用导入*/}
                </Button>
              )}
              {businessCardEnabled && (
                <Button
                ...
                >
                  {this.$t('expense.import.business.card') /*导入商务卡费用*/}
                </Button>
              )}
              {invoiceEnabled && (
                <Button
                ...
                >
                  {this.$t('expense.check.invoice') /*发票查验*/}
                </Button>
              )}
            </div>
          )}
        ...
        {travelSubsidy && (
          <div className="expense-report-travel-subsidy">
            <RadioGroup value={travelSubsidyType} onChange={this.handleChangeTravelSubsidyType}>
              <Radio value="normal">
                {this.$t('expense-report.expense.type.normal') /*普通类型*/}
              </Radio>
              <Radio value="travel">
                {this.$t('expense-report.expense.type.travel.subsidy') /*差补类型*/}
              </Radio>
            </RadioGroup>
            <br />
            ...
            {travelSubsidyType === 'travel' &&
              !readOnly && (
                <Button className="delete-btn" style={{ marginTop: 5 }}>
                  {this.$t('common.delete.all') /*删除全部*/}
                </Button>
              )}
          </div>
        )}
        ...
        <div className="expense-table-title">
          {hasSelectedInvoice
            ? this.$t('common.has.selected') /*已选*/
            : this.$t('common.number') /*数量*/}：&nbsp;
          <b>
            {showSelectedInvoiceNumber}&nbsp;<span className="ant-divider" />
          </b>&nbsp;
          {this.$t('common.total.amount') /*总金额*/}：&nbsp;
          <b>
            {info.baseCurrency} {this.filterMoney(showExpenseReportInvoiceSum)}
          </b>
        </div>
        <Table
          columns={this.dealInvoiceColumnsRender()}
          loading={loading}
          bordered
          rowKey="invoiceOid"
          ...
          rowSelection={
            approve &&
            this.checkFunctionProfiles('app.approval.reject.batch.disabled', [undefined, false])
              ? expenseRowSelection
              : undefined
          }
      ...
      >
        <div className="tabs-info">
          <Tabs onChange={this.handleTabsChange} type="card">
            <TabPane tab={this.$t('expense-report.info') /*报销单信息*/} key="requestInfo">
              {requestInfo}
            </TabPane>
            {approvalHistory && (
        ...
        {audit &&
          (buttonRoleSwitch ? (
            <AuditApplicationDetail
              ...
              status={info.status}
              entityType={1002}
              expenseOid={this.props.match.params.expenseReportOid}
            ...
            >
              ...
              <Button style={{ marginLeft: 15 }} className="back-btn" onClick={this.goBack}>
        ...
        {pay && (
          <Affix offsetBottom={0} className="bottom-bar bottom-bar-approve">
            {info.status === 1008 &&
              this.checkFunctionProfiles('web.financial.approval.sure.ready.pay.disabled', [
                undefined,
                false,
              ]) &&
          ...
          !loanRefund && (
            <Affix offsetBottom={0} className="bottom-bar bottom-bar-approve">
              ...
              {expenseReportStatus.operate === 'processing' &&
                this.checkFunctionProfiles('er.opt.withdraw.disabled', [undefined, false]) &&
                !(
              ...
              <Button className="back-btn" style={{ marginLeft: '10px' }} onClick={this.goBack}>
                {this.$t('common.back') /*返回*/}
              </Button>
            </Affix>
          )}
        <ListSelector
          ...
          extraParams={{
            ...
            applicantOid: applicant.userOid,
            expenseReportOid: info.expenseReportOid,
            currencyCode: this.checkFunctionProfiles('web.invoice.keep.consistent.with.expense', [
              true,
            ])
              ? info.currencyCode
              : null,
        ...
        {!loading && (
          <SlideFrame
            ...
            title={
              readOnly
                ? this.$t('expense.view') /*查看费用*/
                : nowEditExpense
                  ? this.$t('expense.edit') /*编辑费用*/
                  : this.$t('expense.new') /*新建费用*/
            }
        ...
        <Modal
          title={this.$t('common.repayment') /*还款*/}
          okText={this.$t('common.submit') /*提交*/}
          visible={showLoanModal}
          wrapClassName="loan-list-modal"
        ...
        >
          {!profile['web.report.expense.compulsory.repayment'] && (
            <Alert
              message={
                <a onClick={this.handleDirectSubmit}>
                  {this.$t('expense-report.direct.submit') /*直接提交*/}
                </a>
              }
              description={
                this.$t(
                  'expense-report.direct.submit.info'
                ) /*如果您无需还款，请点击此处直接提交报销单*/
              }
              type="info"
              showIcon
            />
          ...
          <div className="reimbursement-amount">
            {this.$t('common.repayment') /*还款*/}&nbsp;{info.currencyCode}
            {this.filterMoney(info.personalPaymentAmount - leftAmount)}&nbsp;&nbsp;
            {this.$t('common.remain') /*剩余*/}&nbsp;{info.currencyCode}
            {this.filterMoney(leftAmount)}
          </div>
          <div className="select-loan-title">
            {this.$t('expense-report.select.loan') /*选择借款单*/}
          </div>
          <Table
            size="small"
            columns={[
        ...
        <Modal
          title={this.$t('expense-report.check.result') /*检查结果*/}
        ...
        >
          {checkResult.map((item, index) => (
            <div className="check-result-item" key={index}>
              <div>{item.title}</div>
              <div
                className={`check-result-message ${
                  item.type === 0 ? 'weak-control' : 'strong-control'
                }`}
              >
                {item.message}
              </div>
          ...
          <div className="check-result-button-area">
            ...
            {!budgetError && (
              <Button type="primary" onClick={this.confirmSubmit} loading={directSubmitting}>
                {this.$t('expense-report.continue.submit') /*继续提交*/}
              </Button>
            )}
          </div>
        </Modal>
        ...
        {info.expenseReportOid &&
          this.state.showYingfuSelectApproveModal && (
            <YingfuSelectApprove
              visible={this.state.showYingfuSelectApproveModal}
              expenseReport={info}
