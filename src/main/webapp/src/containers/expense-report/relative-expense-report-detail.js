...
class RelativeExpenseReportDetail extends React.Component {
  ...
  render() {
    ...
    info.travelStartDate &&
      (info.travelStartDate = moment(info.travelStartDate).format('YYYY-MM-DD'));
    ...
    let application = info.applicationBusinessCode ? (
      <Row className="row-container">
        ...
        <a onClick={this.handleGoApplication}>
          {this.$t('request.detail.request') /*申请单*/}：{info.applicationBusinessCode}
        </a>
        {info.travelStartDate &&
          info.travelEndDate && (
            <span style={{ marginLeft: '10px', color: '#666' }}>
              {this.$t('request.detail.roundTrip')}：{info.travelStartDate} ~ {info.travelEndDate}
            </span>
          )}
      </Row>
    ) : null;
    let loanApplication = info.loanApplicationBusinessCode ? (
      <span style={{ marginRight: 20 }}>
        <Icon type="link" className="link-icon" />
        <a onClick={this.handleGoLoanApplication}>
          {this.$t('request.detail.borrowingRequestNotes') /*借款申请单*/}：{
            info.loanApplicationBusinessCode
          }
        </a>
      </span>
    ) : null;
