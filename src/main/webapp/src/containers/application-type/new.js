...
class NewApplicationType extends React.Component {
        ...
        .then(res => {
          ...
          record.allFlag ||
            (record.applicationType = {
              ...
              chooserValue: res.data.expenseTypeInfos.map(o => ({
                id: o.expenseTypeId,
                name: o.expenseTypeName,
              })),
            });

          record.applyEmployee != '101' &&
            (record.userInfos = {
              type: type[record.applyEmployee],
              values: res.data.userInfos.map(o => ({
                key: o.userTypeId,
                value: o.userTypeId,
                label: o.pathOrName,
              })),
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      let expenseTypeInfos = values.applicationType.chooserValue.map(o => ({
        expenseTypeId: o.id,
      }));
      ...
      <div>
        {loading ? (
          ...
          <Form onSubmit={this.handleSave}>
            ...
            <FormItem {...formItemLayout} label="费用申请单类型代码">
              {getFieldDecorator('typeCode', {
                rules: [{ required: true, message: '请输入' }],
                initialValue: record.typeCode || '',
              })(<Input disabled={!!record.id} />)}
            </FormItem>
            <FormItem {...formItemLayout} label="费用申请单类型名称">
            ...
            <div className="common-item-title">关联合同设置</div>
            <FormItem {...formItemLayout} colon={false} label={<span />}>
            ...
            <div className="common-item-title">合同必输设置</div>
            <FormItem {...formItemLayout} colon={false} label={<span />}>
