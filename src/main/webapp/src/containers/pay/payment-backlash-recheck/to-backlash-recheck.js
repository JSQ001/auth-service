    ...
    backlashService
      ...
      .then(res => {
        if (res.status === 200) {
          ...
          if (backlashDetail) {
            if (backlashDetail.paymentStatus === 'N') {
              //如果是编辑中，则可以发起反冲
              this.setState({
                queryFlag: false,
                data: data,
  ...
  handleApprovePass = remark => {
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        backlashService
          .updateBacklashStatusByDetailId(id, params, 'S', remark)
          .then(res => {
            if (res.status === 200) {
  ...
  handleApproveReject = () => {
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        backlashService
          .updateBacklashStatusByDetailId(id, params, 'F', remark)
          .then(res => {
            if (res.status === 200) {
              this.onCancel(true);
  ...
  onSubmit = () => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        ...
        backlashService.submitBacklash(backDetailId).then(res => {
          if (res.status === 200) {
            //提交成功后进入反冲查询页面，暂时不跳
  ...
  render() {
    ...
    return (
      <div className="new-payment-requisition-line">
        ...
        <Modal
          title={this.$t({ id: 'pay.backlash.pleaseInputRejectReason' })}
        ...
        >
          <Form>
            <div style={{ marginBottom: 15 }}>
              {this.$t({ id: 'pay.backlash.pleaseInputRejectReason' })}
            </div>
        ...
        <Modal
          title={this.$t({ id: 'pay.backlash.pleaseInputReason' })}
        ...
        >
          <Form>
            <div style={{ marginBottom: 15 }}>
              {this.$t({ id: 'pay.backlash.pleaseInputReason' })}
            </div>
