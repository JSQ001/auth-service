...
export default {
  ...
  getBacklashDTOBybacklashDetailId(param) {
    return httpFetch.get(
      `${config.payUrl}/api/cash/backlash/get/by/backlash/detail/id?id=` + param
    );
  },
  ...
  updateBacklashStatusByDetailId(detailId, remark, status, backlashRemark) {
    return httpFetch.post(
      `${
        config.payUrl
      }/api/cash/backlash/update/status?detailId=${detailId}&remark=${remark}&status=${status}&backlashRemark=${backlashRemark}`
    );
  },
