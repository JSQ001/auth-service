...
class BacklashRechecked extends React.Component {
      ...
      searchForm: [
        {
          ...
          colSpan: 6,
          id: 'billCode',
          label: this.$t({ id: 'pay.backlash.backlashCode' }),
        },
        {
          type: 'input',
          colSpan: 6,
          id: 'refBillCode',
          label: this.$t({ id: 'pay.backlash.sourceBacklashCode' }),
        },
        {
          type: 'items',
          colSpan: 6,
          ...
          items: [
            {
              ...
              label: this.$t({ id: 'pay.backlash.backFlashDateFrom' }),
            },
            {
              type: 'date',
              colSpan: 6,
              id: 'backFlashDateTo',
              label: this.$t({ id: 'pay.backlash.backFlashDateTo' }),
            },
          ],
        },
          ...
          items: [
            {
              type: 'input',
              id: 'amountFrom',
              label: this.$t({ id: 'pay.backlash.backlashAmountFrom' }),
            },
            {
              type: 'input',
              id: 'amountTo',
              label: this.$t({ id: 'pay.backlash.backlashAmountTo' }),
            },
          ],
        },
      ],
  ...
  //去反冲页面
  goBacklash = record => {
    backlashService.getReadyByDetailId(record.id).then(res => {
      if (res.status === 200) {
  ...
  canBacklashHandle = values => {
    values.backFlashDateFrom &&
      (values.backFlashDateFrom = values.backFlashDateFrom.format('YYYY-MM-DD'));
    values.backFlashDateTo &&
      (values.backFlashDateTo = values.backFlashDateTo.format('YYYY-MM-DD'));
  ...
  getCanBacklashList = (resolve, reject) => {
    ...
    let params = {
      ...canBacklashSearchParams,
      page: canBacklashPage,
      size: canBacklashPageSize,
      status: 'S',
    };
  ...
  rowClick(record) {
    ...
    backlashService.getBacklashDTOBybacklashDetailId(record.id).then(res => {
      if (res.status === 200) {
        this.setState({ openWindowFlag: true, goBacklash: res.data });
      }
    });
  ...
  render() {
    ...
    return (
      <div className="pay-my-backlash">
        ...
        />
        <SlideFrame
          title={this.$t({ id: 'pay.backlash.backlashDetails' })}
          show={openWindowFlag}
