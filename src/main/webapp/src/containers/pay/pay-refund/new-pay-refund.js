  ...
  onDelete = () => {
    ...
    Modal.confirm({
      ...
      onOk() {
        e.setState({ btnLoading: true });
        payRefundService
          .deleteById(e.state.saveData.id)
          .then(res => {
            if (res.status === 200) {
              e.setState({ btnLoading: false });
              e.onCancel();
          ...
          <Row>
            ...
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.billCode' } /*付款流水号*/)}>
                {getFieldDecorator('refBillCode', {})(<Input disabled={true} />)}
              </FormItem>
            </Col>
          </Row>
          <Row>
                ...
                {getFieldDecorator('draweeAccountNumber', {
                  ...
                  >
                    {draweeAccountNumber.map(option => {
                      return (
                        <Option key={option.bankAccountNumber}>{option.bankAccountNumber}</Option>
                      );
                    })}
                  </Select>
                )}
            ...
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.documentNumber' } /*单据编号*/)}>
                <span className="ant-form-text">
                  <a
                    onClick={() => {
                      this.viewDocumentDetail(data.documentId, data.documentCategory);
                    }}
                  >
                    {data.documentNumber}
                  </a>
          ...
          <Row>
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.submitDate' } /*提交日期*/)}>
                <span className="ant-form-text">
                  {moment(data.requisitionDate).format('YYYY-MM-DD')}
                </span>
              </FormItem>
            </Col>
            <Col span={10} offset={1}>
          ...
          <Row>
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.amount' } /*金额*/)}>
                <span className="ant-form-text">
                  {data.currency}&nbsp;&nbsp;&nbsp;&nbsp;{this.filterMoney(data.amount)}
                </span>
              </FormItem>
            </Col>
            <Col span={10} offset={1}>
