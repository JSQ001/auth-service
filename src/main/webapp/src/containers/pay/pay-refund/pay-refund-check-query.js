...
class PayRefund extends React.Component {
        ...
        },
        {
          ...
          items: [
            ...
            },
            {
              ...
              listType: 'select_returnee',
              single: true,
              disabled: true,
              labelKey: 'name',
              valueKey: 'id',
              listExtraParams: { empFlag: '1001', pageFlag: true },
            },
          ],
        },
        {
  ...
  getUnRefundList = () => {
    ...
    payRefundService
      .getUncheckData(params, unRefundPage, unRefundPageSize)
      .then(res => {
        if (res.status === 200) {
          this.setState({
  ...
  getMyRefundList = () => {
    ...
    payRefundService
      .getCheckedData(params, myRefundPage, myRefundPageSize)
      .then(res => {
        if (res.status === 200) {
          this.setState({
  ...
  render() {
    ...
    return (
      <div className="approve-contract">
        ...
        {this.renderContent()}
        <SlideFrame
          title={this.$t({ id: 'pay.refund.refundDetail' } /*退款单详情*/)}
          show={openWindowFlag}
          onClose={this.cancelWindow}
          afterClose={this.restFormFunc}
        >
          <PayRefundCheckDetail
            params={{ record: deRecord, flag: openWindowFlag, approveStatus: 'submitted' }}
            onClose={this.cancelWindow}
          />
        </SlideFrame>
        ...
        >
          <PayRefundCheckDetail
            params={{ record: myRefundRecord, flag: frameFlag, approveStatus: 'approved' }}
            onClose={this.cancelWindow1}
          />
        </SlideFrame>
      </div>
