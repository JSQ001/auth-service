...
class PayRefundDetail extends React.Component {
  ...
  getAccount = name => {
    ...
    let url = `${config.mdataUrl}/api/bank/infos/get/bank/info/by/name?name=${name}&empFlag=1003`;
    httpFetch
      .get(url)
      .then(res => {
        if (res.status === 200 && res.data[0] && res.data[0].bankInfos.length > 0) {
          this.setState({ payeeAccountNumber: res.data[0].bankInfos || [] });
        } else {
          message.error(
            this.$t(
              { id: 'pay.refund.getPayeeAccountNumberNullError' } /*获取退款方银行账户信息失败*/
            )
          );
        }
      })
  ...
  componentDidMount() {
    ...
    payRefundService
      .queryMyRefundById(record.id)
      .then(res => {
        if (res.status === 200) {
          if (
            res.data.newCashTransactionDetail.paymentStatus === 'N' ||
            res.data.newCashTransactionDetail.paymentStatus === 'F'
          ) {
            this.getAccount(res.data.newCashTransactionDetail.partnerName); //获取退款方
            this.getPayAccount(
              res.data.newCashTransactionDetail.paymentCompanyId,
              res.data.newCashTransactionDetail.currency
            ); //获取收款方银行账户
          }
  ...
  saveFunction = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        let backlashAttachmentOid =
          uploadOids && uploadOids instanceof Array && uploadOids.join(',');
  ...
  onDelete = () => {
    ...
    Modal.confirm({
      ...
      onOk() {
        e.setState({ btnLoading: true });
        payRefundService
          .deleteById(e.state.newCashTransactionDetail.id)
          .then(res => {
            if (res.status === 200) {
              e.setState({ btnLoading: false });
              e.onCancel();
  ...
  render() {
    ...
    const {
      payeeAccountNumber,
      currencyList,
      newCashTransactionDetail,
      oldCashTransactionDetail,
      draweeAccountNumber,
      showDetail,
      detailFlag,
      fileList,
    ...
    let submit = (
      <div>
        ...
        <br />
        {this.$t({ id: 'pay.refund.currentState' } /*当前状态:*/)}
        <span style={{ fontWeight: 'bold', fontSize: '15px' }}>
          {this.$t({ id: 'pay.refund.approving' } /*复核中*/)}
        </span>
      </div>
    );
    let approved = (
      <div>
        ...
        {this.$t({ id: 'pay.refund.currentState' } /*当前状态:*/)}
        <span style={{ fontWeight: 'bold', fontSize: '15px' }}>
          {this.$t({ id: 'pay.refund.approved' } /*复核通过*/)}
        </span>
      </div>
    );
    let status =
      newCashTransactionDetail.paymentStatus === 'P'
        ? 'submitted'
        : newCashTransactionDetail.paymentStatus === 'S'
          ? 'approved'
          : 'new';
    let msg = (
      <div>
        {this.$t({ id: 'pay.refund.refundAmount' } /*退款金额:*/)}{' '}
        {newCashTransactionDetail.currency}
        <span style={{ fontWeight: 'bold', fontSize: '15px' }}>
          {' '}
          {this.filterMoney(newCashTransactionDetail.amount)}{' '}
        </span>
      </div>
    );
    let title =
      status === 'new' ? (
        ...
        <Alert
          message={this.$t({ id: 'pay.refund.approving' } /*复核中*/)}
          description={submit}
          type="warning"
    ...
    let newButton = (
      <div className="slide-footer">
        <Button onClick={this.onSubmit} loading={btnLoading} type="primary">
          {this.$t({ id: 'pay.refund.submitButtonText' } /*通知财务退款*/)}
        </Button>
      ...
      <div className="new-payment-requisition-line">
        ...
        <Form onSubmit={this.saveFunction}>
          <div className="common-item-title">
            {this.$t({ id: 'pay.refund.refundInfo' } /*退款信息*/)}
          </div>
          <Row>
            ...
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.billCode' } /*付款流水号*/)}>
                {status === 'new'
                  ? getFieldDecorator('billcode', {})(<Input disabled={true} />)
          ...
          <Row>
                ...
                {status === 'new'
                  ? getFieldDecorator('payeeAccountNumber', {
                      ...
                      <Select
                        ...
                        notFoundContent={this.$t(
                          { id: 'pay.refund.notFoundContent' } /*无匹配结果*/
                        )}
                      >
            ...
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.partnerCategoryName' } /*退款方*/)}>
                {status === 'new'
                  ? getFieldDecorator('partnerName', {})(<Input disabled={true} />)
          ...
          <Row>
              ...
              <FormItem label={this.$t({ id: 'pay.refund.draweeAccountNumber' } /*收款方账号*/)}>
                {status === 'new'
                  ? getFieldDecorator('draweeAccountNumber', {
                      ...
                      <Select
                        placeholder={this.$t({ id: 'common.please.select' }) /* 请选择 */}
                        onChange={this.payeeAccountNumberChange}
                        notFoundContent={this.$t(
                          { id: 'pay.refund.notFoundContent' } /*无匹配结果*/
                        )}
                      >
            ...
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.draweeAccountName' } /*收款方户名*/)}>
                {status === 'new'
                  ? getFieldDecorator('draweeAccountName', {})(<Input disabled={true} />)
          ...
          <div className="common-item-title">
            {this.$t({ id: 'pay.refund.documentInfo' } /*业务单据信息*/)}
          </div>
          <Row>
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.documentNumber' } /*单据编号*/)}>
                <span className="ant-form-text">
                  <a
                    onClick={() => {
                      this.viewDocumentDetail(
                        oldCashTransactionDetail.documentId,
                        oldCashTransactionDetail.documentCategory
                      );
                    }}
                  >
                    {oldCashTransactionDetail.documentNumber}
                  </a>
                </span>
              </FormItem>
            </Col>
          ...
          <Row>
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.submitDate' } /*提交日期*/)}>
                <span className="ant-form-text">
                  {moment(oldCashTransactionDetail.requisitionDate).format('YYYY-MM-DD')}
                </span>
              </FormItem>
            </Col>
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.employeeName' } /*申请人*/)}>
                <span className="ant-form-text">
                  {oldCashTransactionDetail.employeeCode} - {oldCashTransactionDetail.employeeName}
                </span>
              </FormItem>
            </Col>
          </Row>
          <div className="common-item-title">
            {this.$t({ id: 'pay.refund.oldPaymentInfo' } /*原支付信息*/)}
          </div>
          <Row>
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.billCode' } /*付款流水号*/)}>
                <span className="ant-form-text">
                  <a
                    onClick={() => {
                      this.viewPayDetail(oldCashTransactionDetail.id);
                    }}
                  >
                    {oldCashTransactionDetail.billcode}
                  </a>
                </span>
              </FormItem>
            </Col>
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.payDate' } /*付款日期*/)}>
                <span className="ant-form-text">
                  {moment(oldCashTransactionDetail.payDate).format('YYYY-MM-DD')}
                </span>
              </FormItem>
            </Col>
          </Row>
          <Row>
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.amount' } /*金额*/)}>
                <span className="ant-form-text">
                  {oldCashTransactionDetail.currency}&nbsp;&nbsp;&nbsp;&nbsp;{this.filterMoney(
                    oldCashTransactionDetail.amount
                  )}
                </span>
              </FormItem>
            </Col>
          ...
          <Row>
            ...
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.refund.payAccountNumber' } /*付款方账号*/)}>
                <span className="ant-form-text">
                  {oldCashTransactionDetail.draweeAccountNumber}
                </span>
              </FormItem>
            </Col>
          </Row>
        ...
        >
          <div>
            {detailFlag === 'PAYDETAIL'
              ...
              : detailFlag === 'ACP_REQUISITION'
                ? this.wrapClose(PaymentRequisitionDetail)
                : detailFlag === 'PUBLIC_REPORT'
                  ? this.wrapClose(PublicReport)
                  : this.wrapClose(PrepaymentDetail)}
          </div>
        </Modal>
      </div>
    );
