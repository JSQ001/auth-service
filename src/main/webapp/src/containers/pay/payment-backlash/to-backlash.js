...
class ToBacklash extends React.Component {
  ...
  componentDidMount() {
    ...
    if (record.backDetail) {
      backlashService
        .getBacklashDTOBybacklashDetailId(record.backDetail.id)
        .then(res => {
          if (res.status === 200) {
            let data = {};
            let documentInfo = {};
            ...
            if (backlashDetail && !flashFlag) {
              ...
              if (backlashDetail.paymentStatus === 'N' || backlashDetail.paymentStatus === 'F') {
                //如果是编辑中或者驳回，则可以发起反冲
              ...
              } else if (backlashDetail.paymentStatus === 'P') {
                //其他状态则隐藏下部4个按钮，且描述不是输入框
                this.setState({
                  queryFlag: false,
                  data: data,
                  documentInfo: documentInfo,
  ...
  doBacklash = e => {
    ...
    if (this.state.showDetail) {
      ...
      backlashService.updateByDetailId(updateParam).then(res => {
        if (res.status === 200) {
          loading: false;
      ...
      //发起反冲
      backlashService.getBacklashByDetailId(detailId).then(res => {
        if (res.status === 200) {
          let data = {};
          let documentInfo = {};
          let sourceDetail = {};
  ...
  onSubmit = () => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        ...
        // let uploadOids = this.state.uploadOids || [];
        // if (remarks !== this.state.data.backDetail.remark || (backDetail.backlashAttachmentOID||[]).toString()!==uploadOids.toString()) {
        //   message.warning('请先保存已修改信息!');
        //   return;
        // }

        backlashService
          .submitBacklash(backDetailId)
          .then(res => {
            if (res.status === 200) {
              //提交成功后进入反冲查询页面，暂时不跳
  ...
  onDelete = () => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        ...
        loading: true;
        let backDetailId = backDetail.id;
        backlashService.deleteBacklashDetailById(backDetailId).then(response => {
  ...
  render() {
    ...
    let newButton = showButtonFlag ? (
      <div className="slide-footer">
        <Button onClick={this.onSubmit} type="primary">
          {this.$t({ id: 'pay.backlash.submit' })}
        </Button>
        <Button type="primary" htmlType="submit" loading={btnLoading}>
          {this.$t({ id: 'pay.backlash.save' })}
        </Button>
        <Button
          onClick={() => {
            this.onCancel(false);
        ...
        <Button
          ...
          style={{ color: '#fff', background: '#f04134', borderColor: '#f04134' }}
        >
          {this.$t({ id: 'pay.backlash.delete' })}
        </Button>
      </div>
    ) : (
      <div className="slide-footer">
    ...
    let approveButton = (
      ...
      >
        <Button type="primary" htmlType="submit" loading={false} style={{ margin: '0 20px' }}>
          {this.$t({ id: 'pay.backlash.gotoBacklash' })}
        </Button>
        <Button
          onClick={() => {
            this.onCancel(false);
    ...
    return (
      <div className="new-payment-requisition-line">
        ...
        {alert_seccuss && (
          <Alert
            message={
              this.$t({ id: 'pay.backlash.backlashAmount' }) +
              ' : ' +
              backlashDetail.currency +
              backlashDetail.amount
            }
          ...
          <Row>
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.backlash.submitDate' })}>
                <span className="ant-form-text">
                  {String(documentInfo.documentDate).substr(0, 10)}
                </span>
              </FormItem>
            </Col>
            <Col span={10} offset={1}>
          ...
          <Row>
            <Col span={10} offset={1}>
              <FormItem label={this.$t({ id: 'pay.backlash.amount' })}>
                <span className="ant-form-text">
                  {sourceDetail.currency} {sourceDetail.amount}
                </span>
              </FormItem>
            </Col>
            <Col span={10} offset={1}>
          ...
          {showDetail && (
            <div>
              <div className="common-item-title">
                {this.$t({ id: 'pay.backlash.backlashinfo' })}
              </div>
              <Row>
                <Col span={10} offset={1}>
                ...
                <Col span={10} offset={1}>
                  <FormItem label={this.$t({ id: 'pay.backlash.payDate' })}>
                    <span className="ant-form-text">
                      {String(backlashDetail.payDate).substr(0, 10)}
                    </span>
                  </FormItem>
                </Col>
              </Row>
              <Row>
                <Col span={10} offset={1}>
                  <FormItem label={this.$t({ id: 'pay.backlash.amountOnly' })}>
                    <span className="ant-form-text">
                      {backlashDetail.currency} {backlashDetail.amount}
                    </span>
                  </FormItem>
                </Col>
                <Col span={10} offset={1}>
                    ...
                    <FormItem label={this.$t({ id: 'pay.backlash.backlashRemark' })}>
                      {getFieldDecorator('remarks', {
                        ...
                        <TextArea
                          autosize={{ minRows: 2 }}
                          style={{ minWidth: '100%' }}
                          placeholder={this.$t({ id: 'pay.backlash.pleaseInput' })}
                          id="remarks"
                        />
                      )}
                    </FormItem>
