  ...
  constructor(props) {
    ...
    this.state = {
      ...
      billsColumns: [
        ...
        {
          title: messages('pay.total.amount'),
          dataIndex: 'documentTotalAmount',
      ...
      financeColumns: [
        ...
        {
          title: messages('chooser.data.dimension'),
          dataIndex: 'costCenter',
          render: value => value || '-',
        },
        {
          title: messages('accounting.subject'),
          dataIndex: 'subject',
          render: value => value || '-',
        },
      ],
      offHistoryColumns: [
        ...
        {
          title: messages('pay.checked.amount'),
      ...
      logColumns: [
        ...
        {
          title: messages('pay.operation.time'),
      ...
      offHistoryDate: [],
      logData: [],
      payWorkbench: '/pay/pay-workbench/pay-workbench', //付款工作台
      id: null,
    };
  }
        ...
        <Spin spinning={loading}>
          {payStatusValue && (
            <Alert
              message={
                <Badge
                  text={payStatusValue ? payStatus[payStatusValue].label : ''}
            ...
            <div>
              <Card bordered={false}>
                ...
                <Card.Grid style={gridRightStyle}>
                  {detailData[0].draweeCompanyName} {detailData[0].draweeAccountName}{' '}
                  {detailData[0].draweeAccountNumber}
                </Card.Grid>
                <Card.Grid style={gridLeftStyle}>{messages('pay.account.info')}：</Card.Grid>
                <Card.Grid style={gridRightStyle}>
                  {messages('common.currency') + ': ' + detailData[0].currency} &nbsp;&nbsp;{messages(
                    'common.currency.rate'
                  ) +
                    ': ' +
                    detailData[0].exchangeRate}
                </Card.Grid>
              ...
              <Card bordered={false}>
                ...
                <Card.Grid style={gridRightStyle}>
                  {detailData[0].partnerCategoryName || '-'}
                </Card.Grid>
                <Card.Grid style={gridLeftStyle}>{messages('pay.collection.info')}：</Card.Grid>
                <Card.Grid style={gridRightStyle}>
                  {detailData[0].payeeAccountName} {detailData[0].payeeAccountNumber}
                </Card.Grid>
              </Card>
            </div>
          )}
          ...
          {/*借款单 或 预付款单*/}
          {billsData[0] &&
            (billsData[0].documentTypeCode === 'PAYMENT_REQUISITION' ||
              billsData[0].documentTypeCode === 'PREPAYMENT_REQUISITION') && (
              <div>
          ...
          <Table
            ...
            expandedRowRender={record => (
              <p>{record.bankMessage || messages('wait.for.billing.none')}</p>
            )}
            size="middle"
          />
