...
class PayUnpaid extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      searchForm: [
        {
          ...
          selectorItem: {
            ...
            url: `${config.payUrl}/api/companyBankAuth/get/own/info/lov/${this.props.user.userOid}`,
            searchForm: [
              {
                type: 'input',
                id: 'companyCode',
            ...
            columns: [
              {
                ...
                dataIndex: 'bankAccountCompanyCode',
              },
              {
                title: messages('chooser.data.companyName' /*公司名称*/),
                dataIndex: 'bankAccountCompanyName',
              },
            ],
          ...
          defaultValue: [
            {
              bankAccountCompanyName: this.props.company.name,
              bankAccountCompanyId: this.props.company.id,
            },
          ],
          label: messages('paymentCompanySetting.paymentCompanyName') /*"付款公司名称"*/,
          labelKey: 'bankAccountCompanyName',
          valueKey: 'bankAccountCompanyId',
          single: false,
        },
        {
          type: 'input',
          colSpan: 6,
          id: 'documentNumber',
          label: messages('pay.workbench.receiptNumber') /*单据编号*/,
        },
        {
          type: 'value_list',
          colSpan: 6,
          id: 'documentTypeName',
          label: messages('pay.workbench.receiptType') /*单据类型*/,
          options: [],
          valueListCode: 2106,
        },
        {
          ...
          options: [],
          id: 'employeeId',
          label: messages('pay.workbench.applicant') /*申请人*/,
          labelKey: 'userName',
          valueKey: 'userId',
          single: true /*listExtraParams: { setOfBooksId: this.props.company.setOfBooksId }*/,
        }, //申请人
        {
          type: 'items',
          colSpan: 6,
          ...
          items: [
            {
              ...
              label: messages('pay.workbench.dateFrom') /*申请日期从*/,
            },
            {
              type: 'date',
              id: 'requisitionDateTo',
              label: messages('pay.workbench.dateTo') /*申请日期至*/,
            },
          ],
        },
        {
          ...
          items: [
            {
              type: 'input',
              id: 'amountFrom',
              label: messages('pay.workbench.mountFrom') /*总金额从*/,
            },
            {
              type: 'input',
              id: 'amountTo',
              label: messages('pay.workbench.mountTo') /*总金额至*/,
            },
          ],
        },
        {
            ...
            },
            {
              ...
              items: [
                ...
                },
                {
                  type: 'list',
                  id: 'partnerId',
                  label: messages('pay.workbench.payee') /*收款方*/,
                  options: [],
                  listType: 'user',
                  single: true,
                  event: 'PARTNER',
                  ...
                  labelKey: 'id',
                  disabled: true,
                  getUrl: `${config.mdataUrl}/api/vendor/account/by/companyId/name/code`,
                },
              ],
            },
          ],
      ...
      columns: [
        {
          title: `${messages('pay.workbench.receiptNumber') /*单据编号*/} | ${
            messages('pay.workbench.receiptType') /*单据类型*/
          }`,
          align: 'center',
        ...
        {
          title: `${messages('pay.workbench.employee.id') /*工号*/} | ${
            messages('pay.workbench.applicant') /*申请人*/
          }`,
          align: 'center',
        ...
        {
          title: messages('pay.workbench.requisition.date') /*申请日期*/,
          dataIndex: 'requisitionDate',
          align: 'center',
          render: value => moment(value).format('YYYY-MM-DD'),
        },
        {
          title: messages('pay.workbench.currency') /*币种*/,
          align: 'center',
          dataIndex: 'currency',
        },
        {
          title: messages('pay.workbench.amount') /*总金额*/,
          align: 'center',
          dataIndex: 'amount',
          render: this.filterMoney,
        },
        {
          title: messages('pay.workbench.payable.amount') /*可支付金额*/,
          align: 'center',
            ...
            return (
              ...
              value === record.amount ? (
                ...
                <div style={{ textAlign: 'right' }}>
                  <Tooltip title={messages('pay.payable.tips')}>
        ...
        {
          //本次支付金额
          title: messages('pay.this.pay.amount'),
          align: 'center',
        ...
        {
          //收款账号
          title: messages('pay.rec.account'),
          align: 'center',
        ...
        },
        {
          ...
          render: state => <Badge status="default" text={state} />,
        },
      ],
      buttonDisabled: true,
  ...
  componentWillMount() {
    ...
    searchForm[0].defaultValue = paymentService.getDefaultCompany(
  ...
  getList = () => {
    ...
    Promise.all([online, offline, file])
      .then(() => {
        this.getOnlineCash();
        this.getOfflineCash();
        this.getFileCash();
  ...
  //搜索
  search = result => {
    result.requisitionDateFrom = result.requisitionDateFrom
      ? moment(result.requisitionDateFrom).format('YYYY-MM-DD')
      : null;
    result.requisitionDateTo = result.requisitionDateTo
      ? moment(result.requisitionDateTo).format('YYYY-MM-DD')
      : null;
    if (!!this.state.partnerId) {
      result.partnerId = this.state.partnerId;
    } else {
      result.partnerId &&
        result.partnerId.length > 0 &&
        (result.partnerId = result.partnerId[0].id);
    }
    this.setState(
      {
        searchParams: result,
    ...
    if (!errFlag) {
      let noticeAlert = (
        <span>
          ...
          <span className="ant-divider" />
          {messages('pay.this.amount')}：{currency}{' '}
          <span style={{ fontWeight: 'bold', fontSize: '15px' }}> {this.filterMoney(amount)} </span>
        </span>
      );
      this.setState({
  ...
  //获取汇率
  getExchangeRate = () => {
    let currencyDate = moment(new Date()).format('YYYY-MM-DD');
    let url = `${config.mdataUrl}/api/currency/rate/company/standard/currency/get?currency=${
      this.state.currency
  ...
  getPayWay = () => {
    ...
    let url = `${
      config.payUrl
    }/api/comapnyBankPayment/get/company/bank/payment/by/bank/account/number?number=${payCompanyBankNumber}&paymentMethod=${paymentType}`;
    httpFetch
      .get(url)
      .then(res => {
  ...
  getPayAccount = () => {
    ...
    let url = `${
      config.payUrl
    }/api/CompanyBank/getCompanyBank/by/companyId/and/paymentMethodCode?companyId=${paymentCompanyId}&paymentMethodCode=${paymentMethodCode}&currency=${currency}`;
    httpFetch
      .get(url)
      .then(res => {
        res.status === 200 &&
          this.setState({ payAccountOptions: res.data, payAccountFetching: false });
      })
      .catch(() => {
  ...
  checkPaymentDetail = record => {
    this.props.dispatch(
      routerRedux.push({
        pathname: this.state.paymentDetail
          .replace(':tab', 'Unpaid')
          .replace(':subTab', this.state.radioValue)
  ...
  getOnlineList = (resolve, reject) => {
    ...
    paymentService
      .getUnpaidList(onlinePage, onlinePageSize, 'ONLINE_PAYMENT', searchParams)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            onlineData: res.data,
  ...
  getOfflineList = (resolve, reject) => {
    ...
    paymentService
      .getUnpaidList(offlinePage, offlinePageSize, 'OFFLINE_PAYMENT', searchParams)
      .then(res => {
        if (res.status === 200) {
          this.setState({
  ...
  getFileList = (resolve, reject) => {
    ...
    paymentService
      .getUnpaidList(filePage, filePageSize, 'EBANK_PAYMENT', searchParams)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            fileData: res.data,
        ...
        {onlineCash.map((item, index) => {
          return (
            <div key={index} style={{ display: 'inline-block' }}>
              {messages('common.amount')}：{item.currency}{' '}
              <span className="num-style">{this.filterMoney(item.totalAmount)}</span>
              <span className="ant-divider" />
              {messages('pay.transaction')}：<span className="num-style">
                {item.documentNumber}
                {messages('pay.bi')}
              </span>
  ...
  selectHandleChange = value => {
    if (value.key) {
      ...
      this.props.form.setFieldsValue({
        ...
        _payCompanyBankNumber: value.key,
      });
    }
  };
  ...
  render() {
    ...
    return (
      <div className="pay-unpaid">
        ...
        />
        <Radio.Group value={radioValue} style={{ margin: '20px 0' }} onChange={this.onRadioChange}>
          <Radio.Button value="online">
            {messages('constants.paymentMethodCategory.online')}
          </Radio.Button>
          <Radio.Button value="offline">
            {messages('constants.paymentMethodCategory.offline')}
          </Radio.Button>
          <Radio.Button value="file">
            {messages('constants.paymentMethodCategory.landingFile')}
          </Radio.Button>
        </Radio.Group>
        <Button
          type="primary"
        ...
        {errorAlert ? (
          <Alert message={errorAlert} type="error" showIcon style={{ marginBottom: '10px' }} />
        ) : (
          ''
        )}
        ...
        {radioValue === 'online' ? (
          <Modal
            title={messages('pay.online.confirm')}
          ...
          >
            <Form>
                ...
                {getFieldDecorator('payCompanyBankNumber', {
                  ...
                  <Select
                    ...
                    notFoundContent={
                      payAccountFetching ? (
                        ...
                        messages('pay.refund.notFoundContent')
                      )
                    }
                  ...
                  >
                    {payAccountOptions.map(option => {
                      return (
                        <Option key={option.bankAccountNumber}>
                          {option.bankAccountName +
                            '-' +
                            option.bankAccountNumber
                              .padStart(4, '*')
                              .substr(option.bankAccountNumber.padStart(4, '*').length - 4)}
                        </Option>
                      );
                    })}
                  </Select>
              ...
              <FormItem {...formItemLayout} label={messages('pay.account.number')}>
                {getFieldDecorator('_payCompanyBankNumber')(<Input disabled />)}
              </FormItem>
              ...
              <FormItem
                {...formItemLayout}
                label={messages('payment.batch.company.payWay') /*付款方式*/}
              >
          ...
          <Modal
            title={messages('pay.under.confirm')}
            visible={modalVisible}
            okText={messages('pay.pay.confirm')}
            confirmLoading={modalLoading}
            onOk={this.handleLineModalOk}
          ...
          >
            <Form>
              <Alert
                message={messages('pay.alert.tips')}
                type="info"
                showIcon
              ...
              {this.state.chequeNumberFlag ? (
                <FormItem
                  {...formItemLayout}
                  label={messages('payment.batch.company.paymentChequeNo')}
                >
                  {getFieldDecorator('chequeNumber', {})(<Input />)}
                </FormItem>
              ) : (
                ''
              )}
          ...
          <Modal
            title={messages('pay.workbench.Landing.payment')}
            visible={modalVisible}
