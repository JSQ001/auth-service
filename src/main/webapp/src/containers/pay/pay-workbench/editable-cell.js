...
class EditableCell extends React.Component {
  ...
  check = e => {
    ...
    setTimeout(() => {
      !this.props.onChangeError &&
        this.setState({ editable: false, modifyValue: this.state.value });
    });
  };
  ...
  getAccountOptions = open => {
    if (open) {
      ...
      if (this.props.record.partnerCategory === 'EMPLOYEE') {
        url = `${config.mdataUrl}/api/DepartmentGroup/getContactBankByUserId?userId=${
      ...
      paymentService.showPayHistory(this.props.record.id).then(res => {
        if (res.status === 200) {
          ...
          if (res.data.length) {
                ...
                {res.data.map(item => {
                  return (
                    <Timeline.Item key={item.id} color="green">
                      <span style={{ fontSize: 13, color: 'rgba(0,0,0,0.55)', marginRight: 5 }}>
                        {moment(item.payDate).format('YYYY-MM-DD HH:mm:ss')}
                      </span>
                      {item.employeeCode} {item.employeeName}{' '}
                      {item.operationType === 'payment'
                        ? messages('pay.pay')
                        : item.operationType === 'return'
                          ? messages('acp.payment.return')
                          : messages('acp.payment.reserved')}{' '}
                      {item.currency}
                      {(item.amount || 0)
                        .toFixed(2)
                        .toString()
            ...
            historyContent = <div>{messages('pay.history.none')}</div>;
          }
  ...
  render() {
    ...
    let history = <Spin spinning={historyLoading}>{historyContent}</Spin>;
    return (
        ...
        {editable ? (
          <div className="editable-cell-input-wrapper">
            {type === 'number' ? (
              ...
              <Select
                defaultValue={value}
                notFoundContent={
                  fetching ? <Spin size="small" /> : messages('pay.refund.notFoundContent')
                }
                onChange={value => this.setState({ value })}
                style={{ width: payType === 'payType' ? 80 : 200 }}
                onDropdownVisibleChange={
                  payType === 'payType' ? this.getPayType : this.getAccountOptions
                }
              >
                {array.map(option => {
          ...
          <Popover placement="bottom" content={history} onVisibleChange={this.payHistory}>
            <a className="editable-cell-text-wrapper" style={{ textAlign: 'right' }}>
              {modifyValue &&
                modifyValue < this.props.value && (
                  <Tooltip title={messages('pay.amount.validate')}>
