...
class PayPaying extends React.Component {
    ...
    this.state = {
      ...
      columns: [
        ...
        },
        {
          ...
          render: state => <Badge status="processing" text={state} />,
        },
        {
          ...
          render: (id, record) => {
            const menu = (
              <Menu>
                <Menu.Item>
                  <a onClick={() => this.handleSuccess(record)}>
                    {messages('pay.confirm.success')}
                  </a>
                </Menu.Item>
                <Menu.Item>
  ...
  getOnlineList = (resolve, reject) => {
    ...
    paymentService
      .getPayingList(onlinePage, onlinePageSize, 'ONLINE_PAYMENT', searchParams)
      .then(res => {
        if (res.status === 200) {
          let onlineWarningRows = [];
          res.data.map(item => {
            if ((new Date().getTime() - new Date(item.payDate).getTime()) / 1000 / 60 / 60 > 12) {
              //付款时间超过12小时
  ...
  getFileList = (resolve, reject) => {
    ...
    paymentService
      .getPayingList(filePage, filePageSize, 'EBANK_PAYMENT', searchParams)
      .then(res => {
        if (res.status === 200) {
          let fileWarningRows = [];
          res.data.map(item => {
  ...
  render() {
    ...
    return (
      <div className="pay-paying">
        ...
        <Alert
          message={messages('pay.paying.tips')}
          type="info"
          showIcon
        ...
        {warningRows.length > 0 && (
          <Alert
            message={messages('pay.warning.tips')}
        ...
        >
          <div style={{ height: 110 }}>
            ...
            <div style={{ fontSize: 12, color: 'red', marginLeft: 29 }}>
              {messages('pay.account.tips.third')}
            </div>
