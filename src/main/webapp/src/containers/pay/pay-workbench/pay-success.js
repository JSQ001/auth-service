...
class PaySuccess extends React.Component {
    ...
    this.state = {
      ...
      columns: [
        ...
        },
        {
          ...
          render: state => <Badge status="success" text={state} />,
        },
      ],
  ...
  confirmRefund = () => {
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        paymentService
          .confirmRefund(moment(values.refundDate).format('YYYY-MM-DD'), this.state.refundRow)
          .then(res => {
            if (res.status === 200) {
          ...
          .catch(e => {
            ...
            message.error(
              messages('my.refund.failed') +
                e.response.data.message +
                ',' +
                messages('common.try.again')
            );
          });
      }
    });
  ...
  checkPaymentDetail = record => {
    let path = this.state.paymentDetail
      .replace(':tab', 'Success')
  ...
  getOnlineList = (resolve, reject) => {
    ...
    paymentService
      .getSuccessList(onlinePage, onlinePageSize, 'ONLINE_PAYMENT', searchParams)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            onlineData: res.data,
  ...
  getOfflineList = (resolve, reject) => {
    ...
    paymentService
      .getSuccessList(offlinePage, offlinePageSize, 'OFFLINE_PAYMENT', searchParams)
      .then(res => {
        if (res.status === 200) {
          this.setState({
  ...
  getFileList = (resolve, reject) => {
    ...
    paymentService
      .getSuccessList(filePage, filePageSize, 'EBANK_PAYMENT', searchParams)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            fileData: res.data,
  ...
  renderOnlineContent = () => {
    ...
    onlineColumns.push({
      ...
      dataIndex: 'id',
      render: (id, record) => (
        <a onClick={() => this.handleRefund(record)}>{messages('constants.bookerType.refund')}</a>
      ),
    });
    const tableTitle = (
      <div>
    ...
    const tableTitle = (
      <div>
        ...
        {offlineCash.map((item, index) => {
          return (
            <div key={index} style={{ display: 'inline-block' }}>
              {messages('pay.transaction')}：{item.currency}{' '}
              <span className="num-style">{this.filterMoney(item.totalAmount)}</span>
              <span className="ant-divider" />
              {messages('pay.transaction')}：<span className="num-style">
                {item.documentNumber}
  ...
  render() {
    ...
    return (
      <div className="pay-success">
        ...
        <Alert
          message={messages('pay.success.alert.tips')}
          type="info"
          showIcon
        ...
        >
          <div style={{ height: 110 }}>
            ...
            <div style={{ fontSize: 12, color: 'red', marginLeft: 29 }}>
              {messages('pay.success.refund.tips')}
            </div>
