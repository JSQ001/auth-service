...
class PayFail extends React.Component {
    ...
    this.state = {
      ...
      searchForm: [
        ...
        }, //付款机构选择
        {
          ...
          colSpan: 6,
          id: 'documentNumber',
          label: messages('pay.workbench.receiptNumber'),
        }, //单据编号
        {
          type: 'value_list',
          colSpan: 6,
          id: 'documentCategory',
          label: messages('pay.workbench.receiptType'),
          options: [],
          valueListCode: 2106,
        }, //单据类型
        {
      ...
      columns: [
        {
          title: messages('pay.refund.billCode'),
          align: 'center',
          dataIndex: 'billcode',
          render: (value, record) => (
            <a
              onClick={() => {
                this.checkPaymentDetail(record);
              }}
            >
              {value}
            </a>
        ...
        {
          title: messages('pay.number.type'),
          align: 'center',
        ...
        { title: messages('common.currency'), align: 'center', dataIndex: 'currency' },
        {
        ...
        {
          title: messages('payment.batch.company.payWay'),
          align: 'center',
          dataIndex: 'paymentTypeName',
        },
        {
          title: messages('pay.type.partner'),
          align: 'center',
        ...
        {
          title: messages('pay.refund.draweeAccountNumber'),
          align: 'center',
        ...
        },
        {
          ...
          render: (state, record) => (
            <Badge
              status="error"
              text={record.refundStatus === 'Y' ? messages('pay.refund') : state}
            />
          ),
        },
      ],
  ...
  cancelPay = () => {
    paymentService
      .cancelPay(this.state.selectedRows)
      .then(res => {
        if (res.status === 200) {
  ...
  getOnlineList = (resolve, reject) => {
    ...
    paymentService
      .getFailList(onlinePage, onlinePageSize, 'ONLINE_PAYMENT', searchParams)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            onlineData: res.data,
  ...
  getFileList = (resolve, reject) => {
    ...
    paymentService
      .getFailList(filePage, filePageSize, 'EBANK_PAYMENT', searchParams)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            fileData: res.data,
    ...
    return (
      <div className="pay-fail">
        ...
        <Radio.Group value={radioValue} style={{ margin: '20px 0' }} onChange={this.onRadioChange}>
          ...
          <Radio.Button value="offline" disabled>
            {messages('pay.underline')}
          </Radio.Button>
        ...
        {radioValue === 'online' ? (
          <Modal
            title={messages('pay.repay.confirm')}
                ...
                {getFieldDecorator('payCompanyBankNumber', {
                  ...
                  >
                    {payAccountOptions.map(option => {
                      return (
                        <Option key={option.bankAccountNumber}>{option.bankAccountName}</Option>
                      );
                    })}
                  </Select>
                )}
          ...
          <Modal
            title={messages('pay.landing.repay')}
            visible={modalVisible}
            confirmLoading={modalLoading}
