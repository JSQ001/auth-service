...
export default {
  ...
  getPayWay(params) {
    return httpFetch.get(
      `${config.payUrl}/api/cash/payment/method/selectByPaymentType?paymentType=${params}`
    );
  },
  ...
  getUnpaidAmount(value, searchParams) {
    let url = `${
      config.payUrl
    }/api/cash/transactionData/select/totalAmountAndDocumentNum?paymentMethodCategory=${value}`;
  ...
  getUnpaidList(page, pageSize, value, searchParams) {
    let url = `${
      config.payUrl
    }/api/cash/transactionData/query?page=${page}&size=${pageSize}&paymentMethodCategory=${value}`;
    for (let searchName in searchParams) {
      url += searchParams[searchName] ? `&${searchName}=${searchParams[searchName]}` : '';
    }
    return httpFetch.get(url);
  ...
  confirmPayByEBank(params) {
    return httpFetch.post(
      `${config.payUrl}/api/cash/transaction/details/insertBatch/down`,
      params,
      {},
  ...
  confirmSuccess(params, date) {
    return httpFetch.post(
      `${config.payUrl}/api/cash/transaction/details/paying/paySuccess/${date}`,
      params
    );
  },
  ...
  getAmount(type, status, searchParams) {
    ...
    let url = `${
      config.payUrl
    }/api/cash/transaction/details/select/totalAmountAndDocumentNum?paymentMethodCategory=${type}&isRefundOrFail=${isRefundOrFail}`;
  ...
  getPayingList(page, pageSize, value, searchParams) {
    let url = `${
      config.payUrl
    }/api/cash/transaction/details/paying/query?page=${page}&size=${pageSize}&paymentMethodCategory=${value}`;
    for (let searchName in searchParams) {
      url += searchParams[searchName] ? `&${searchName}=${searchParams[searchName]}` : '';
    }
    return httpFetch.get(url);
  ...
  cancelPay(params) {
    return httpFetch.post(
      `${config.payUrl}/api/cash/transaction/details/payFailOrRefund/cancel`,
      params
    );
  },
  ...
  rePayEBank(params) {
    return httpFetch.post(
      `${config.payUrl}/api/cash/transaction/details/payFailOrRefund/down`,
      params,
      {},
      { responseType: 'arraybuffer' }
    );
  ...
  getFailList(page, pageSize, value, searchParams) {
    let url = `${
      config.payUrl
    }/api/cash/transaction/details/payFailOrRefund/query?page=${page}&size=${pageSize}&paymentMethodCategory=${value}`;
    for (let searchName in searchParams) {
      url += searchParams[searchName] ? `&${searchName}=${searchParams[searchName]}` : '';
    }
    return httpFetch.get(url);
  ...
  confirmRefund(date, params) {
    return httpFetch.post(
      `${config.payUrl}/api/cash/transaction/details/refund?refundDate=${date}`,
      params
    );
  },
  ...
  getSuccessList(page, pageSize, value, searchParams) {
    let url = `${
      config.payUrl
    }/api/cash/transaction/details/getAlreadyPaid?page=${page}&size=${pageSize}&paymentMethodCategory=${value}`;
    for (let searchName in searchParams) {
      url += searchParams[searchName] ? `&${searchName}=${searchParams[searchName]}` : '';
    }
    return httpFetch.get(url);
  ...
  showPayHistory(id) {
    return httpFetch.get(
      `${config.payUrl}/api/cash/transaction/details/getHistoryByDateId?id=${id}`
    );
  },
  ...
  getReceiveAccount(Oid) {
    return httpFetch.get(
      `${config.mdataUrl}/api/DepartmentGroup/getContactBankByUserOid?userOid=${Oid}`
    );
  },

  //根据userOid获取付款账户
  getPayAccount(Oid) {
    return httpFetch.get(`${config.payUrl}/api/companyBankAuth/selectAuthBank?empId=${Oid}`);
  },

  getDefaultCompany(userOid, defaultId) {
    let defaultValue = [];
    httpFetch.get(`${config.payUrl}/api/companyBankAuth/get/own/info/lov/${userOid}`).then(res => {
      res.data.map(
        item =>
          item.bankAccountCompanyId.toString() === defaultId.toString() &&
          defaultValue.push({
