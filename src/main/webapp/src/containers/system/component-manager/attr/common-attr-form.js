...
import { Input, Form, Select, Tooltip, Icon, Switch, Spin, TreeSelect, message } from 'antd';
...
class CommonAttrForm extends Component {
  ...
  titleSearch = value => {
    ...
    let languageList = dataSource
      .filter(item => item.key == value || item.value == value)
      .slice(0, 20);

    languageList = [
      ...languageList,
      ...dataSource
        .filter(item => item.key.indexOf(value) >= 0 || item.value.indexOf(value) >= 0)
  ...
  renderFormElement = (item, value) => {
    ...
    switch (item.type) {
      ...
      case 'json':
        return (
          <TextArea
            onBlur={e => this.checkJson(e, item)}
            onChange={e => this.setData(item, e.target.value)}
            value={data[item.key] == undefined ? value : data[item.key]}
            autosize={{ minRows: 3 }}
          />
