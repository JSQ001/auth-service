...
class PublicReimburseReport extends React.Component {
      ...
      searchForm: [
        ...
        },
        {
          ...
          options: [
            {
              value: 1001,
              label: this.$t({ id: 'public.reimberse.report.status.generate' } /*编辑中*/),
            },
            {
              value: 1002,
              label: this.$t({ id: 'public.reimberse.report.status.submitted' } /*审批中*/),
            },
            {
              value: 1003,
              label: this.$t({ id: 'public.reimberse.report.status.reject' } /*撤回*/),
            },
      ...
      columns: [
        ...
        /**查看信息 */
        {
          ...
          render: (text, record, index) => {
            return (
              <span>
                <a href="#" onClick={e => this.applyReport(e, record, index)}>
                  申请单
                </a>
                <span className="ant-divider" />
                <a href="#" onClick={e => this.paymentDetail(e, record, index)}>
                  支付明细
                </a>
                <span className="ant-divider" />
                <a href="#" onClick={e => this.reverseReport(e, record, index)}>
                  费用反冲单
                </a>
              </span>
            );
  ...
  //获取搜索数据
  handelSearch = values => {
    values.applyDateFrom = values.applyDateFrom
      ? moment(values.applyDateFrom).format('YYYY-MM-DD')
      : undefined;
    values.applyDateTo = values.applyDateTo
      ? moment(values.applyDateTo).format('YYYY-MM-DD')
      : undefined;
    values.checkDateFrom = values.checkDateFrom
      ? moment(values.checkDateFrom).format('YYYY-MM-DD')
      : undefined;
    values.checkDateTo = values.checkDateTo
      ? moment(values.checkDateTo).format('YYYY-MM-DD')
      : undefined;
  ...
  getList = () => {
    ...
    PublicReimburseReportService.getReimburseReport(param, page, pageSize, setOfBooksId)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            loading: false,
            ...
            pagination: {
              ...
              showTotal: () => {
                return `共有 ${
                  Number(res.headers['x-total-count']) ? Number(res.headers['x-total-count']) : 0
                } 条数据`;
              },
            },
    ...
    const paymentSelectorItem = {
      ...
      searchForm: [
        ...
        },
        {
          ...
          label: '报账单金额',
          disabled: true,
          defaultValue: Number(record.totalAmount || 0)
            .toFixed(2)
            .toString(),
          colSpan: 6,
        },
      ...
      columns: [
        ...
        },
        {
          ...
          render: recode => recode && this.state.operationTypeList[recode].label,
        },
        {
          title: '付款方账户',
          ...
          render: (value, recode) => {
            return (
              <div>
                <div>户名:{recode.draweeAccountName}</div>
                <div>账号:{recode.draweeAccountNumber}</div>
              </div>
            );
          },
        },
        ...
        { title: '付款金额', dataIndex: 'amount', render: this.filterMoney },
        {
          ...
          render: (value, recode) => {
            return (
              <div>
                <div>户名:{recode.payeeAccountName}</div>
                <div>账号:{recode.payeeAccountNumber}</div>
              </div>
            );
          },
        },
        ...
        { title: '备注', dataIndex: 'description' },
        {
          ...
          render: (value, recode) => {
            return (
              value && (
                <Badge
                  status={this.state.reverseStatus[value].state}
                  text={this.state.reverseStatus[value].label}
                />
              )
            );
          },
  ...
  export = result => {
    ...
    PublicReimburseReportService.exportExcel(result, setOfBooksId, exportParams)
      .then(res => {
        if (res.status === 200) {
    ...
    return (
      <div>
        ...
        <div className="table-header">
          <Row>
            <Col span={18}>
              <Button type="primary" onClick={this.exportReport} loading={this.state.buttonLoading}>
                导出对公报账单
              </Button>
            </Col>
            <Col span={6}>
              <Search placeholder="请输入单据编号" onSearch={this.onDocumentSearch} enterButton />
            </Col>
          </Row>
        </div>
        <Table
