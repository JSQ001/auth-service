...
class AccountingView extends React.Component {
    ...
    this.state = {
      ...
      searchForm: [
        ...
        },
        {
          ...
          isRequired: false,
          selectorItem: {
            title: this.$t('accounting.select.company'), // 选择公司
            url: `${config.mdataUrl}/api/company/dto/by/tenant`,
            searchForm: [
              { type: 'input', id: 'companyCode', label: this.$t('accounting.company.code') }, // 公司代码
              { type: 'input', id: 'name', label: this.$t('account.company.name') }, // 公司名称
            ],
            columns: [
              { title: this.$t('accounting.company.code'), dataIndex: 'companyCode' }, // 公司代码
              { title: this.$t('account.company.name'), dataIndex: 'name' }, // 公司名称
            ],
            key: 'id',
          },
          id: 'companyId',
          listExtraParams: { tenantId: this.props.company.tenantId, setOfBooksId: '' },
          label: this.$t({ id: 'accounting.view.company' }),
          labelKey: 'name',
          valueKey: 'id',
          single: false,
        },
        ...
        {
          type: 'items',
          id: 'accountingDateRange',
          items: [
            {
              type: 'date',
              id: 'accountingDateFrom',
              label: this.$t({ id: 'accounting.view.accountingDateFrom' }),
            },
            {
              type: 'date',
              id: 'accountingDateTo',
              label: this.$t({ id: 'accounting.view.accountingDateTo' }),
            },
          ],
        },
        {
          ...
          label: this.$t({ id: 'accounting.view.transactionNumber' }),
        },
        {
          type: 'items',
          ...
          items: [
            ...
            },
            {
              ...
              label: this.$t({ id: 'accounting.view.transactionDateTo' }),
            },
          ],
        },
        {
          ...
          this.setState(
            ...
            () => {
              ...
              accountingViewService.getAccountingCostCenter(setOfBooksId).then(res => {
                ...
                this.setState(
                  { columns: columnsCostCenter, x: xCostCenter + res.data.length * 130 },
                  () => {
                    this.getList();
                  }
                );
  ...
  handleDownLoad = () => {
    ...
    accountingViewService
      .exportAccountingMessage(searchParams)
      .then(response => {
      ...
      .catch(e => {
        ...
        if (e.response) {
          message.error(
            `${this.$t({ id: 'accounting.view.export.failed' })}，${e.response.message}`
          );
        }
      });
  };
  ...
  handleSearch = values => {
    values.accountingDateFrom = values.accountingDateFrom
      ? moment(values.accountingDateFrom).format('YYYY-MM-DD')
      : null;
    values.accountingDateTo = values.accountingDateTo
      ? moment(values.accountingDateTo).format('YYYY-MM-DD')
      : null;
    values.transactionDateFrom = values.transactionDateFrom
      ? moment(values.transactionDateFrom).format('YYYY-MM-DD')
      : null;
    values.transactionDateTo = values.transactionDateTo
      ? moment(values.transactionDateTo).format('YYYY-MM-DD')
      : null;
    values.accountCodeFrom &&
      values.accountCodeFrom.length > 0 &&
      (values.accountCodeFrom = values.accountCodeFrom[0].accountCode);
    values.accountCodeTo &&
      values.accountCodeTo.length > 0 &&
      (values.accountCodeTo = values.accountCodeTo[0].accountCode);
    this.setState(
      {
        searchParams: values,
        loading: true,
  ...
  render() {
    ...
    return (
      <div className="accounting-view">
        <h3 className="header-title">{this.$t({ id: 'menu.accounting-view' })}</h3>{' '}
        {/* 会计分录查询 */}
        <SearchArea
          searchForm={searchForm}
        ...
        <div className="table-header">
          <div className="table-header-buttons">
            <Button type="primary" onClick={this.handleDownLoad} loading={exportLoading}>
              导出搜索数据
            </Button>
          </div>
