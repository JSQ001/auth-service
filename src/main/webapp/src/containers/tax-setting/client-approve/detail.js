...
import ExpenseApplicationCommon from './client-application-detail';
...
class Detail extends React.Component {
  ...
  onCancel = () => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/approval-management/client-approve/client-approve`,
      })
    );
  };
  ...
  render() {
    ...
    return (
      ...
      >
        {getLoading ? <Spin /> : <ExpenseApplicationCommon clientData={headerData} />}
        {this.props.match.params.status === 'unapproved' ? (
          <div className="bottom-bar bottom-bar-approve">
            <ApproveBar
              passLoading={loading}
              style={{ paddingLeft: 20 }}
              backUrl={'/approval-management/client-approve/client-approve'}
