...
class AccountingScenariosSystem extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          /*核算要素代码*/
          title: this.$t({ id: 'accounting.elements.code' }),
          key: 'accountElementCode',
          dataIndex: 'accountElementCode',
        },
        {
          /*核算要素名称*/
          title: this.$t({ id: 'accounting.elements.name' }),
          key: 'accountElementName',
          dataIndex: 'accountElementName',
        },
        {
          /*核算要素性质*/
          title: this.$t({ id: 'accounting.elements.nature' }),
          key: 'elementNature',
        ...
        {
          /*匹配组字段*/
          title: this.$t({ id: 'accounting.matching.group.field' }),
          key: 'mappingGroupName',
  ...
  handleBack = () => {
    this.props.dispatch(
      routerRedux.replace({
        pathname:
          '/financial-accounting-setting/accounting-scenarios-system/accounting-scenarios-system',
      })
    );
  };
    ...
    return (
      <div className="accounting-elements" style={{ paddingBottom: 20 }}>
        <div className="accounting-elements-header">
          <h2>
            {this.$t(
              { id: 'accounting.scenarios' },
              { name: scenarios.glSceneCode + '-' + scenarios.glSceneName }
            )}
          </h2>
        </div>
