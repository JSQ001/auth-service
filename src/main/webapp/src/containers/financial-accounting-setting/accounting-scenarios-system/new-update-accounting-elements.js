...
class NewUpdateScenariosSystem extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      accountElementCode:
        typeof params.accountElementCode === 'undefined'
          ? []
          : [{ code: params.accountElementCode }],
    });
    if (params.id) {
    ...
    this.setState(
      {
        ...
        accountElementCode: this.props.params.id
          ? [{ code: this.props.params.accountElementCode }]
          : [],
      },
      () =>
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (!values.mappingGroupCode) values.mappingGroupCode = '';
        let method = null;
        ...
        <Form onSubmit={this.handleSubmit} className="accounting-elements-form">
          <Row gutter={20}>
            <Col span={20}>
              <FormItem
                {...formItemLayout}
                label={this.$t({ id: 'accounting.scenarios.elements' }) /*核算要素*/}
              >
          ...
          <Row gutter={20}>
            <Col span={20}>
              <FormItem
                {...formItemLayout}
                label={this.$t({ id: 'accounting.elements.name' }) /*核算要素名称*/}
              >
          ...
          <Row gutter={20}>
            <Col span={20}>
              <FormItem
                {...formItemLayout}
                label={this.$t({ id: 'accounting.elements.nature' }) /*核算要素性质*/}
              >
          ...
          <Row gutter={20}>
            <Col span={20}>
              <FormItem
                {...formItemLayout}
                label={this.$t({ id: 'accounting.matching.group.field' }) /*匹配组字段*/}
              >
                {getFieldDecorator('mappingGroupCode', {
                  initialValue: elements.mappingGroupCode,
                })(
                  <Select
                    disabled={elements.id ? true : false}
                    onFocus={this.handleGroup}
                    allowClear
                    placeholder={this.$t({ id: 'common.please.select' })}
                  >
...
export default connect(
  ...
  null,
  { withRef: true }
)(WrappedNewUpdateScenariosSystem);
