...
class NewUpdateMatchingGroup extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          /*核算要素*/
          title: this.$t({ id: 'accounting.scenarios.elements' }),
          key: 'accountElementCode',
          dataIndex: 'accountElementCode',
        },
        {
          /*要素性质*/
          title: this.$t({ id: 'accounting.matching.elements.nature' }),
          key: 'elementNature',
          dataIndex: 'elementNature',
        },
        {
          /*匹配字段*/
          title: this.$t({ id: 'accounting.match.field' }),
          key: 'mappingGroupCode',
  ...
  handleLinkElement = (e, record, index) => {
    this.context.router.push(
      menuRoute
        .getMenuItemByAttr('accounting-scenarios-system', 'key')
        .children.accountingElements.url.replace(':id', record.id)
    );
  };
  ...
  getList() {
    ...
    accountingService.getElementsGroupNotNull(params).then(response => {
      ...
      pagination.total =
        typeof this.state.matchGroup.id === 'undefined'
          ? Number(response.headers['x-total-count'])
          : data.length;
      this.setState({
        loading: false,
        data:
          typeof this.state.matchGroup.id !== 'undefined' || data.length > 0 ? data : response.data,
        pagination,
      });
    });
  }
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        values.sceneMappingId = flag
          ? this.state.matchGroup.scenarios.id
          : this.props.params.sceneMappingId;
        values.transactionSceneId = flag
          ? this.state.matchGroup.scenarios.transactionSceneId
          : this.props.params.scenarios.transactionSceneId;
        values.transactionSceneCode = flag
          ? this.state.matchGroup.scenarios.transactionSceneCode
          : this.props.params.scenarios.transactionSceneCode;
        if (typeof this.state.matchGroup.id !== 'undefined') {
  ...
  render() {
    ...
    const rowSelection =
      typeof matchGroup.id !== 'undefined'
        ? null
        : {
            selectedRowKeys,
    ...
    return (
      <div className="new-update-matching-group">
        <div className="matching-group-form">
          ...
          <span className="matching-group-form-tips">
            {this.$t({ id: 'matching.group.basicInfo' })}
          </span>
          <Form onChange={() => this.setState({ btnLoading: false })}>
            <FormItem
              {...formItemLayout}
              label={this.$t({ id: 'matching.group.code' }) /*匹配组代码*/}
            >
              {getFieldDecorator('code', {
                ...
                <Input
                  disabled={typeof matchGroup.id === 'undefined' ? false : true}
            ...
            <FormItem
              {...formItemLayout}
              label={this.$t({ id: 'matching.group.name' }) /*匹配组名称*/}
            >
          ...
          <span className="matching-group-form-tips">
            {this.$t({ id: 'accounting.scenarios.elements' })}
          </span>
          <span className="matching-group-form-label">
            {this.$t({ id: 'accounting.scenarios.elements.select' })}
          </span>
        </div>
        {typeof this.state.matchGroup.id !== 'undefined' ? null : (
          <div className="accounting-matching-table-head">
            ...
            {this.$t({ id: 'accounting.total' }, { total: pagination.total }) +
              this.$t(
                { id: 'accounting.selected' },
                { count: this.state.selectedEntityOids.length }
              )}
            <a
        ...
        <div className="slide-footer">
          <Button type="primary" onClick={this.handleSave} loading={btnLoading}>
