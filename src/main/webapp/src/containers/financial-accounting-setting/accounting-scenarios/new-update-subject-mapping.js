...
class NewUpdateMatchingGroup extends React.Component {
      ...
      if (this.state.firstRender) {
        ...
        if (typeof params.lines !== 'undefined') {
          ...
          let defaultValue = {
            sectionCode: [
              {
                id: sectionMatch.accountId,
                accountCode: sectionMatch.accountCode,
                accountName: sectionMatch.accountName,
              },
            ],
          };
          let selectorMap = {};
  ...
  handleValueList = param => {
    ...
    accountingService
      .getElementsValueType(selectorMap[param].selectorItem.listExtraParams)
      .then(response => {
        ...
        elements.map(item => {
          let option = {
            ...
            elementValue: !values[item.elementId][0].id
              ? values[item.elementId][0].code
              : values[item.elementId][0].id,
          };
        ...
        <Form onSubmit={this.handleSave}>
              ...
              <FormItem {...formItemLayout} label={this.$t({ id: 'accounting.subject' }) /*科目*/}>
                {getFieldDecorator('sectionCode', {
                  ...
                  <Chooser
                    ...
                    single={true}
                    listExtraParams={{
                      setOfBooksId: matchGroup.setOfBooksId,
                      headId: matchGroup.headId,
                    }}
          ...
          <Row gutter={22}>
            <Col span={22}>
              <FormItem
                {...formItemLayout}
                label={this.$t({ id: 'accounting.subject.name' }) /*科目名称*/}
              >
          ...
          {noLines ? null : (
            <div>
              <div className="subject-matching-title">
                {this.$t({ id: 'account.select.element' })}
              </div>
            </div>
          )}
          {noLines
                      ...
                      {getFieldDecorator(item.elementId, {
                        ...
                        item.isSystem ? (
                          <Select onFocus={() => this.handleValueList(item.elementId)}>
                            {selectorMap[item.elementId].array.map(children => (
                              <Option key={children.id}>{children.name}</Option>
                            ))}
                          </Select>
                        ) : (
                          <Chooser
