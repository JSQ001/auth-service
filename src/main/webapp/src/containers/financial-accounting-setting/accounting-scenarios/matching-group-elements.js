...
class MatchingGroupElements extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          /*优先级*/
          title: this.$t({ id: 'accounting.priority' }),
          key: 'priority',
          dataIndex: 'priority',
        },
        {
          /*匹配组代码s*/
          title: this.$t({ id: 'matching.group.code' }),
          key: 'code',
          dataIndex: 'code',
        },
        {
          /*匹配组名称*/
          title: this.$t({ id: 'matching.group.name' }),
          key: 'description',
        ...
        },
        {
          ...
          render: (decode, record, index) => {
            ...
            return (
              <span>
                {eleCodes === '' ? '-' : <Popover content={eleCodes}>{eleCodes} </Popover>}{' '}
              </span>
            );
          },
        },
        ...
        },
        {
          ...
          render: (text, record, index) => (
            <span>
              ...
              <a onClick={e => this.handleLinkSubject(e, record, index)}>
                {this.$t({ id: 'accounting.subjects.matching' })}
              </a>
            </span>
          ),
        },
  ...
  handleLinkSubject = (e, record, index) => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: '/financial-accounting-setting/accounting-scenarios/matching-group-elements/matching-group-elements/subject-matching-setting/:id/:groupId'
          .replace(':id', this.props.match.params.id)
          .replace(':groupId', record.id),
      })
    );
  };
  ...
  handleBack = () => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: '/financial-accounting-setting/accounting-scenarios/accounting-scenarios/accounting-scenarios/:setOfBooksId'.replace(
          ':setOfBooksId',
    ...
    return (
      <div className="accounting-scenarios">
        <div className="accounting-scenarios-head-tips">
          {this.$t({ id: 'section.setOfBook' })}: &nbsp;<label>
            {this.state.scenarios.setOfBooksName}
          </label>
          &nbsp;&nbsp;&nbsp;&nbsp;{this.$t({ id: 'accounting.scenarios' }, { name: '' }) +
            `: ${this.state.scenarios.transactionSceneName}`}
        </div>
