...
class subjectsMatchingSetting extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          /*科目代码*/
          title: this.$t({ id: 'accounting.subject.code' }),
          key: 'accountCode',
        ...
        {
          /*科目名称*/
          title: this.$t({ id: 'accounting.subject.name' }),
          key: 'accountName',
  ...
  componentDidMount() {
    accountingService
      .getMatchGroupById({ headId: this.props.match.params.groupId })
      .then(response => {
        let searchForm = [];
                ...
                searchForm: [
                  {
                    type: 'input',
                    id: 'valueCode',
                    label: this.$t({ id: 'account.code' }, { name: item.elementNature }),
                  },
                  {
                    type: 'input',
                    id: 'valueDesc',
                    label: this.$t({ id: 'account.name' }, { name: item.elementNature }),
                  },
                ],
                columns: [
                  {
                    title: this.$t({ id: 'account.code' }, { name: item.elementNature }),
                    dataIndex: 'code',
                  },
                  {
                    title: this.$t({ id: 'account.name' }, { name: item.elementNature }),
                    dataIndex: 'name',
                  },
                ],
                key: 'id',
  ...
  handleSearch = params => {
    ...
    for (let paramsName in params) {
      if (!!params[paramsName]) {
        ...
        option.elementValue =
          params[paramsName].length > 0 ? params[paramsName][0] : params[paramsName];
  ...
  handleDelete = () => {
    accountingService
      .batchDeleteSection(this.state.selectedRowKeys)
      .then(response => {
        message.success(`${this.$t({ id: 'common.operate.success' })}`);
        this.getList();
      })
  ...
  handleBack = () => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: '/financial-accounting-setting/accounting-scenarios/matching-group-elements/matching-group-elements/:setOfBooksId/:id'
          .replace(':id', this.props.match.params.id)
          .replace(':setOfBooksId', this.state.matchGroup.setOfBooksId),
      })
    );
  };
      ...
      <div className="subject-matching-setting">
        ...
        <div className="table-header">
          <div className="table-header-title">
            ...
            {this.$t({ id: 'common.total.selected' }, { total: selectedEntityOids.length })}
          </div>{' '}
          {/*共搜索到*条数据*/}
          <div className="table-header-buttons">
            <Button
              type="primary"
              disabled={!searchFlag && data.length > 0}
            ...
            <Popconfirm
              ...
              title={this.$t({ id: 'budget.are.you.sure.to.delete.rule' }, { controlRule: '' })}
            >
              {/* 你确定要删除organizationName吗 */}
        ...
        >
          <NewUpdateSubjectMapping onClose={this.handleAfterClose} params={lov.params} />
        </SlideFrame>
      </div>
    );
  }
