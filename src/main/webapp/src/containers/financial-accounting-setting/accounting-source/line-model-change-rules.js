...
class LineModelChangeRules extends React.Component {
  ...
  componentWillMount() {
    ...
    if (this.props.status != 'NEW') {
      ...
      const compareWay = this.props.params.changeData.compareElementCode
        ? 'compareElement'
        : 'compareData';
  ...
  componentWillReceiveProps(nextProps) {
    if (
      ...
      if (this.props.status != 'NEW') {
        const infoData = this.props.params.changeData;
        const compareWay = nextProps.params.changeData.compareElementCode
          ? 'compareElement'
          : 'compareData';
        let compareElementCode = [];
        if (compareWay === 'compareElement') {
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (status === 'NEW') {
          accountingService
            .addSourceLineModelChangeRules(data)
            .then(res => {
          ...
          accountingService
            .upSourceLineModelChangRules(valuesData)
            .then(res => {
              message.success(this.$t({ id: 'common.operate.success' }));
              this.setState({ loading: false });
              this.props.upDataEvent(res.data);
  ...
  render() {
    ...
    const compareWayData = isEdit
      ? infoData.compareData
        ? 'compareData'
        : 'compareElement'
      : compareWay;
    ...
    if (show) {
      domRender = (
        <div className="line-model-change-rules">
          <Card
            type="inner"
            title={`${this.$t({ id: 'accounting.source.sequence' })} : ` + infoData.priority}
            extra={<a onClick={this.editInfo}>{this.$t({ id: 'common.edit' })}</a>}
          >
            <Row>
              <Col span={24}>
                <span style={{ marginLeft: '8px' }}>{`${this.$t({
                  id: 'accounting.source.compare',
                })} :`}</span>
                <span style={{ marginLeft: '8px' }}>
                  <b>{infoData.changeRuleName}</b>
                </span>
                {/*比较符号*/}
                <span style={{ marginLeft: '8px' }}>
                  {infoData.compareElementCode
                    ? this.$t({ id: 'accounting.source.accountElementCode' })
                    : this.$t({ id: 'accounting.source.stateValue' })}
                </span>
                {/*核算要素/固定值*/}
                <span style={{ marginLeft: '8px' }}>
                  <b>
                    {infoData.compareElementCode
                      ? infoData.compareElementName
                      : infoData.compareData}
                  </b>
                </span>
                {/*核算要素名称/固定值*/}
                {changeDataFlag ? (
                  <span>
                    {' '}
                    <span style={{ marginLeft: '8px' }}>{`${this.$t({
                      id: 'accounting.source.changeData',
                    })} :`}</span>
                    {/*转化值*/}
                    <span style={{ marginLeft: '8px' }}>
                      <b>{infoData.changeData}</b>
                    </span>
                    {/*值*/}
                  </span>
                ) : (
        ...
        <div className="line-model-change-rules">
          <div>
            <Form onSubmit={this.handleSave}>
              <Card
                type="inner"
                title={this.props.status === 'NEW' ? '新建转换规则' : '编辑转换规则'}
              >
                <Row gutter={36}>
                  <Col span={12}>
                    <FormItem
                      {...formItemLayout}
                      label={this.$t({ id: 'accounting.source.sequence' })}
                    >
                  ...
                  <Col span={12}>
                    <FormItem
                      {...formItemLayout}
                      label={this.$t({ id: 'accounting.source.dataRules' })}
                    >
                      {getFieldDecorator('changeRule', {
                        ...
                        >
                          {changeDataOptions.map(item => {
                            return (
                              <Select.Option value={item.value}>{item.messageKey}</Select.Option>
                            );
                          })}
                        </Select>
                      )}
                  ...
                  <Col span={12}>
                    <FormItem
                      {...formItemLayout}
                      label={this.$t({ id: 'accounting.source.compareWay' })}
                    >
                      {getFieldDecorator('compareWay', {
                        ...
                        <Select onSelect={this.handleCompareWayChang}>
                          <Option value={'compareData'}>
                            {this.$t({ id: 'accounting.source.compareData' })}
                          </Option>
                          <Option value={'compareElement'}>
                            {this.$t({ id: 'accounting.source.compareElementCode' })}
                          </Option>
                        </Select>
                      )}
                    </FormItem>
                  ...
                  {compareWay === 'compareData' ? (
                    <Col span={12}>
                      <FormItem
                        {...formItemLayout}
                        label={this.$t({ id: 'accounting.source.compareData' })}
                      >
                  ...
                  {compareWay === 'compareElement' ? (
                    <Col span={12}>
                      <FormItem
                        {...formItemLayout}
                        label={this.$t({ id: 'accounting.source.compareElementCode' })}
                      >
                        {/*对比核算要素*/}
                  ...
                  {changeDataFlag ? (
                    <Col span={12}>
                      <FormItem
                        {...formItemLayout}
                        label={this.$t({ id: 'accounting.source.changeData' })}
                      >
                ...
                <Row>
                  ...
                  <Button onClick={this.handleCancel} style={{ marginLeft: '16px' }}>
    ...
    return <div className="line-model-chang-rules">{domRender}</div>;
  }
}
