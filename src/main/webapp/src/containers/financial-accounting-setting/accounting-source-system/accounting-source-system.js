...
class AccountingSourceSystem extends React.Component {
      ...
      columns: [
        ...
        // },
        {
          ...
          render: (text, record, index) => (
            <span>
              <a onClick={e => this.handleLinkDataStructure(e, record, index)}>
                {this.$t({ id: 'accounting.source.data.setting' })}
              </a>{' '}
              {/*数据结构设置*/}
              <span className="ant-divider" />
              <a onClick={e => this.handleLinkTemplate(e, record, index)}>
                {this.$t({ id: 'accounting.source.template' })}
              </a>{' '}
              {/*凭证模板设置*/}
            </span>
          ),
    ...
    this.props.dispatch(
      routerRedux.push({
        pathname: '/financial-accounting-setting/accounting-source-system/voucher-template/:id/:sourceTransactionType'
          ...
          .replace(':sourceTransactionType', record.sourceTransactionCode),
      })
    );
  };
  ...
  handleAdd = value => {
    ...
    this.setState({ loading: true });
    accountingService
      .addSourceTransaction(dataValue)
      .then(() => {
  ...
  render() {
    ...
    return (
      <div className="accounting-source">
        ...
        />
        <SlideFrame
          title={this.$t({ id: 'data.structure' })}
          show={dataVisible}
