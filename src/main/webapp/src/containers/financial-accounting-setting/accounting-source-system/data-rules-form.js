...
class DataRulesFormSystem extends React.Component {
      ...
      if (params.isNew) {
        ...
        this.setState(
          ...
          () => {
            ...
            if (
              record.dataRule === 'INTERFACE_DATA' ||
              record.dataRule === 'VALUE_OF_RULE' ||
              record.dataRule === 'VALUE_OF_API'
            ) {
              this.setState({
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        if (
          values.dataRule === 'INTERFACE_DATA' ||
          values.dataRule === 'VALUE_OF_MAPPING_GROUP' ||
          values.dataRule === 'VALUE_OF_RULE'
        ) {
        ...
        if (isNew) {
          //新建
          accountingService
            .addSystemSourceLineModelDataRules(data)
            .then(res => {
          ...
          accountingService
            .upSystemSourceLineModelDataRules(editData)
            .then(res => {
              message.success(this.$t({ id: 'common.operate.success' }));
              this.setState({ loading: false });
              this.props.form.resetFields();
  ...
  handleSelectDataRules = value => {
    ...
    if (
      dataRule === 'INTERFACE_DATA' ||
      dataRule === 'VALUE_OF_RULE' ||
      dataRule === 'VALUE_OF_API'
    ) {
        ...
        <Form onSubmit={this.handleSubmit} className="voucher-template-form">
          ...
          <FormItem {...formItemLayout} label={this.$t({ id: 'accounting.source.dataRule' })}>
            ...
            {getFieldDecorator('dataRule', {
              ...
              >
                {dataRuleOption.map(item => <Option key={item.value}>{item.name}</Option>)}
              </Select>
            )}
          </FormItem>
          {/*固定值*/
          dataRule === 'FIXED_VALUE' ? (
            <div>
              <FormItem {...formItemLayout} label={this.$t({ id: 'accounting.source.stateValue' })}>
                {/*固定值*/}
          ...
          {/*取自来源表*/
          dataRule === 'INTERFACE_DATA' ||
          dataRule === 'VALUE_OF_MAPPING_GROUP' ||
          dataRule === 'VALUE_OF_RULE' ? (
            <div>
              <FormItem
                {...formItemLayout}
                label={this.$t({ id: 'accounting.source.sourceDataCode' })}
              >
                {/*数据来源*/}
          ...
          sourceDataCode ? (
            <FormItem
              {...formItemLayout}
              label={this.$t({ id: 'accounting.source.sourceDatafile' })}
            >
              {getFieldDecorator('tableField', {
                ...
                <Chooser
                  placeholder={this.$t({ id: 'common.please.select' })}
                  type="data-source-fields_dataRules-new"
                  single={true}
          ...
          dataRule === 'VALUE_OF_API' ? (
            <div>
              <FormItem {...formItemLayout} label={this.$t({ id: 'accounting.source.fromAPI' })}>
                {/*取自API*/}
