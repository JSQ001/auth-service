...
class LineModeRulesSystem extends React.Component {
  ...
  getLineMode() {
    accountingService
      .getSourceTransactionModelbyID(this.props.match.params.lineModelId)
      .then(response => {
        this.setState({
    ...
    this.props.dispatch(
      routerRedux.replace({
        pathname: '/financial-accounting-setting/accounting-source-system/voucher-template/:id/:sourceTransactionType'
          ...
          .replace(':sourceTransactionType', this.state.sourceTransactionCode),
      })
    );
  };
  ...
  render() {
    ...
    return (
      <div className="voucher-template" style={{ paddingBottom: 20 }}>
        ...
        >
          <NewUpDataLineModeRules onClose={this.handleAfterClose} params={lov.params} />
        </SlideFrame>
      </div>
    );
  }
