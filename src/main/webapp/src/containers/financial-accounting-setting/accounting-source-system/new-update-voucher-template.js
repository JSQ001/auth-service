...
class NewUpdateSection extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.params &&
      nextProps.params.time &&
      nextProps.params.time != this.props.params.time
    ) {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        if (isNew) {
          ...
          accountingService
            .addSourceTransactionModel(data)
            .then(res => {
  ...
  renderMessage() {
    return (
      <ul>
        <li className="header-tips-li">
          <span className="header-tips-content">{this.$t({ id: 'voucher.template.tips2' })}</span>
        </li>
      </ul>
    );
  }
    ...
    return (
      <div className="new-update-voucher-template">
        ...
        <Form onSubmit={this.handleSubmit} className="voucher-template-form">
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'voucher.template.code' }) /*凭证模板行代码*/}
          >
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'voucher.template.name' }) /*凭证模板行名称*/}
          >
          ...
          <FormItem {...formItemLayout} label={'核算场景'} disabled={!this.props.params.isNew}>
            {getFieldDecorator('glSceneId', {
              ...
              >
                {scenariosOption.map(item => (
                  <Option key={item.id}>{item.glSceneName + ' - ' + item.glSceneCode}</Option>
                ))}
              </Select>
            )}
          </FormItem>
