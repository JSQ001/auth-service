...
class NewRegisterApply extends React.Component {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (params.companyId != undefined) {
          params.companyId = params.companyId.id;
        }
        if (params.departmentId != undefined) {
          params.departmentId = params.departmentId.id;
        }
        // 所属机构
  ...
  render() {
    ...
    const { cangeRecordShow, record, data, isDisabled, sourceSystemMethodOptions } = this.state;
      ...
      >
        <Form>
          ...
          <Row gutter={24}>
            ...
            <Col span={6}>
              <FormItem {...formItemLayout} label={'责任中心'}>
            ...
            <Col span={6}>
              <FormItem {...formItemLayout} label={'差额征税'}>
            ...
            <Col span={6}>
              <FormItem {...formItemLayout} label="总价税合计">
                {getFieldDecorator('applicantName', {
                  initialValue: this.props.user.userName,
                  rules: [{ required: false, message: '不可编辑' }],
                })(<Input disabled />)}
