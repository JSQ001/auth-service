...
class PrePaymentCommon extends React.Component {
      ...
      .then(res => {
        ...
        if (!headerInfo.businessCode) {
          headerInfo = {
            ...
            attachmentOids: headerData.attachmentsOids,
          };
        } else {
        ...
        headerInfo.currencyCode = headerInfo.currencyCode
  ...
  //编辑
  edit = () => {
    let paymentReqTypeId = this.state.headerData.paymentReqTypeId;
    this.props.dispatch(
      routerRedux.push({
        pathname: `/pre-payment/my-pre-payment/edit-pre-payment/${
          this.props.id
        }/${paymentReqTypeId}/${0}`,
      })
    );
      ...
      .then(() => {
        ...
        this.setState({
          pagination: {
            ...
            current:
              parseInt((pagination.total - 2) / pageSize) < page
                ? parseInt((pagination.total - 2) / pageSize) + 1
                : page + 1,
          },
          page:
            parseInt((pagination.total - 2) / pageSize) < page
              ? parseInt((pagination.total - 2) / pageSize)
              : page,
        });
        this.getList();
        this.getInfo();
    ...
    return (
      <div>
        <Row>
          ...
          <Col span={5}>
            {' '}
            本币金额：{this.props.company.baseCurrency}&nbsp;{' '}
            {this.filterMoney(record.functionAmount, 2, true)}
          </Col>
        </Row>
        ...
        {record.contractName ? (
          <div>
            ...
            <Row>
              ...
              <Col span={6}>
                <span>合同编号：</span>
                <a onClick={() => this.contractClick(record)}>
    ...
    return (
      <div className="pre-payment-common">
        ...
        <Modal
          ...
          footer={null}
        >
          <ContractDetail id={this.state.contractId} isApprovePage={true} />
        </Modal>
      </div>
    );
  }
