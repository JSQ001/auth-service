...
export default {
  ...
  getAmountByHeadId(id) {
    ...
    }/api/cash/prepayment/requisitionHead/getAmountByHeadId?headId=${id}`;
    return httpFetch.get(url);
  },
  /**
   * 获取走工作流的审批历史
  ...
  getApproveHistoryWorkflow(entityOid) {
    let url = `${
      config.baseUrl
    }/api/prepayment/reports/history?entityType=801003&entityOid=${entityOid}`;
    return httpFetch.get(url);
  },
  /**
   * 获取不走工作流的审批历史
  ...
  submit(id, userId) {
    let url = `${
      config.prePaymentUrl
    }/api/cash/prepayment/requisitionHead/updateStatus?id=${id}&status=1002&userId=${userId}`;
  ...
  back(id, userId) {
    let url = `${
      config.prePaymentUrl
    }/api/cash/prepayment/requisitionHead/updateStatus?id=${id}&status=1003&userId=${userId}`;
    return httpFetch.post(url, {});
  },
  /**
   * 撤回（走工作流）
  ...
  deleteHeadAndLineByHeadId(id) {
    let url = `${
      config.prePaymentUrl
    }/api/cash/prepayment/requisitionHead/deleteHeadAndLineByHeadId?headId=${id}`;
  ...
  getPartnerd(value, type = 1003) {
    return httpFetch.get(
      `${
        config.prePaymentUrl
      }/api/cash/prepayment/requisitionHead/getReceivablesByName?&page=0&size=9999&name=${value}&empFlag=${type}`
    );
  },
  /**
   * 获取新建预付款单所需单据类型
  ...
  getPrePaymentType(params) {
    return httpFetch.get(
      `${config.prePaymentUrl}/api/cash/pay/requisition/types/queryByEmployeeId`,
      params
    );
  },
  /**
  ...
  listUserByTypeId(id) {
    return httpFetch.get(
      `${config.prePaymentUrl}/api/cash/pay/requisition/types/users?payReqTypeId=${id}`
    );
  },
  /**
