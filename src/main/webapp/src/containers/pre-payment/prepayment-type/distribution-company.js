...
class CompanyDistribution extends React.Component {
  ...
  handleStatusChange = (e, record) => {
    ...
    PrePaymentTypeService.updateAssignCompany(params)
      .then(res => {
        if (res.status === 200) {
          this.getList();
  ...
  handleListOk = values => {
    ...
    PrePaymentTypeService.batchDistributeCompany(paramsValue)
      .then(res => {
        if (res.status === 200) {
          message.success('操作成功');
  ...
  handleBack = () => {
    this.props.dispatch(
      routerRedux.push({
        pathname: '/document-type-manage/prepayment-type/prepayment-type',
      })
    );
