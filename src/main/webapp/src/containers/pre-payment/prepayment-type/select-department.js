...
class SelectEmployeeGroup extends React.Component {
  ...
  onLoadData = treeNode => {
    return new Promise(resolve => {
      ...
      httpFetch
        .get(
          `${config.mdataUrl}/api/DepartmentGroup/get/dept/by/id?status=102&id=${model.id}&name=`
        )
        .then(res => {
          let temp = [];
  ...
  getList = () => {
    ...
    httpFetch
      .get(`${config.mdataUrl}/api/departments/root?flag=1002`)
      .then(res => {
  ...
  addToUse = () => {
    ...
    this.state.toSelectList.map(item => {
      ...
      if (
        useData.findIndex(o => o.value == model.departmentId) >= 0 ||
        useData.findIndex(o => o.value == model.id) >= 0
      ) {
        return;
      }
  ...
  onChange = e => {
    ...
    httpFetch
      .get(
        `${config.mdataUrl}/api/DepartmentGroup/selectDept/enabled?deptCode=&name=${e.target.value}`
      )
      .then(res => {
        if (res.data && res.data.length) {
          let temp = [];
