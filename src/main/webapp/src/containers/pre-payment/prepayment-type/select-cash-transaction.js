...
class SelectContract extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      searchForm: [
        {
          type: 'input',
          id: 'classCode',
          label: this.$t({ id: 'pre.payment.cash.transaction.class.code' } /*现金事务分类代码*/),
        },
        {
          type: 'input',
          id: 'description',
          label: this.$t({ id: 'pre.payment.cash.transaction.class.name' } /*现金事务分类名称*/),
        },
        {
          type: 'select',
          id: 'range',
      ...
      columns: [
        {
          title: this.$t({ id: 'pre.payment.cash.transaction.class.code' } /*现金事务分类代码*/),
          dataIndex: 'classCode',
        },
        {
          title: this.$t({ id: 'pre.payment.cash.transaction.class.name' } /*现金事务分类名称*/),
  ...
  getList() {
    ...
    return PrePaymentTypeService.getCashTransaction(model)
      .then(response => {
        this.setState(
          {
      ...
      .catch(e => {
        message.error(
          this.$t(
            { id: 'pre.payment.getCashTransaction.error' } /*获取数据失败，请稍后重试或联系管理员*/
          )
        );
  ...
  refreshSelected() {
    ...
    this.props.selectedData.map(selected => {
      data.map(item => {
        if (item['id'] == selected) nowSelectedRowKeys.push(item['id']);
      });
    });
  ...
  render() {
    ...
    return (
      <Modal
        title={this.$t({ id: 'pre.payment.choose.cash.transaction.class' }) /*选择现金事务分类*/}
        visible={visible}
        onCancel={onCancel}
            ...
            {this.state.searchParams.range === 'all' || !this.state.searchParams.range ? (
              <div style={{ display: 'inline' }}>
                ...
                {this.$t(
                  ...
                  { total: selectedData.length ? selectedData.length : '0' }
                )}
              </div>
            ) : null}
          </div>
