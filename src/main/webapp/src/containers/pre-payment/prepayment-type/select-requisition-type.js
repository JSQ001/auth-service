...
class SelectRequisitionType extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      searchForm: [
        {
          type: 'input',
          id: 'typeCode',
          label: this.$t({ id: 'pre.payment.requisition.type.code' } /*申请单类型名称*/),
        },
        {
          type: 'input',
          id: 'typeName',
          label: this.$t({ id: 'pre.payment.requisition.type.name' } /*申请单类型名称*/),
        },
        {
          type: 'select',
          id: 'range',
      ...
      columns: [
        {
          title: this.$t({ id: 'pre.payment.requisition.type.code' } /*申请单类型名称*/),
          dataIndex: 'typeCode',
        },
        {
          title: this.$t({ id: 'pre.payment.requisition.type.name' } /*申请单类型名称*/),
          dataIndex: 'typeName',
        },
      ],
      data: [],
  ...
  getList = () => {
    ...
    let params = {
      setOfBooksId: this.props.params.setOfBookId,
      range: searchParams.range ? searchParams.range : 'all',
      payRequisitionTypeId: this.props.params.payRequisitionTypeId,
      code: searchParams.typeCode,
      name: searchParams.typeName,
      idList: [],
      page: this.state.page,
      size: this.state.pageSize,
    };
    prePaymentTypeService.getRequisitionType(params, this.props.params.setOfBookId).then(res => {
      this.setState(
        {
          loading: false,
          data: res.data,
  ...
  render() {
    ...
    return (
      <Modal
        title={this.$t({ id: 'pre.payment.choose.requisition.type' } /*选择申请类型*/)}
        visible={visible}
        onOk={this.handleOk}
        width={800}
