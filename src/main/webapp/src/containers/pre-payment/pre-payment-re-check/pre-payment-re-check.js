...
*/
  ...
  componentWillMount() {
    this.setState({
      tabValue: this.props.match.params.tab === 'approved' ? 'approved' : 'unapproved',
    });
  ...
  handleRowClick = record => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/pre-payment/pre-payment-recheck/payment-details/${record.id}/${
          this.state.tabValue
        }`,
      })
    );
  ...
  onApprovedSearch = value => {
    this.setState(
      {
        approveSearchParams: {
          ...this.state.approveSearchParams,
          checkBy: this.props.user.id,
          status: 1004,
          requisitionNumber: value,
        },
            ...
            {tabValue === 'unapproved' && (
              <div>
                ...
                <CustomTable
                  ...
                  }/api/cash/prepayment/requisitionHead/query?ifWorkflow=false`}
