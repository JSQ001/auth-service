...
class PrePaymentDetail extends React.Component {
  ...
  handleApproveReject = remark => {
    const { id } = this.state.headerData;
  ...
  onCancel = () => {
    this.props.dispatch(
      routerRedux.push({
        pathname: `/pre-payment/pre-payment-recheck/pre-payment-recheck/${
  ...
  render() {
    const { loading, headerData, passLoading } = this.state;
        ...
        {headerData.status && headerData.status != 1004 && headerData ? (
          <div className="bottom-bar-approve">
            <ApproveBar
              ...
              backUrl={'/pre-payment/pre-payment-recheck/pre-payment-recheck'}
