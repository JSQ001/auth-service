...
class PaymentRequisitionDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
          ...
          render: (value, record) => {
            return (
              <div>
                ...
                >
                  {record.currencyCode}&nbsp;&nbsp;{this.filterMoney(record.amount)}
                </div>
            ...
            return (
              <Popover
                content={
                  <div>
                    <div>
                      {this.$t({ id: 'acp.account' } /*账户：*/)}
                      {record.accountNumber}
                    </div>
                    <div>
                      {this.$t({ id: 'acp.account.name' } /*户名*/)}
                      {value}
                    </div>
                  </div>
                }
      ...
      pageSize: 5,
      page: 0,
      myPaymentRequisition: '/approval-management/payment-requisition/index', //审批界面
  ...
  getLogs = () => {
    ...
    paymentRequisitionService
      .getLogs(this.props.match.params.entityOid)
      .then(res => {
        if (res.status == 200) {
          this.setState({
    ...
    return (
      <div>
        <Row>
          <Col span={2}>
            <span style={{ float: 'right' }}>
              {this.$t({ id: 'acp.amount.attribute' } /*金额属性*/)}
            </span>
          </Col>
          <Col span={6} offset={1}>
            汇率日期：
        ...
        <Row>
          ...
          <Col span={6}>
            {this.$t({ id: 'acp.delay.amount' } /*延迟支付金额*/)}
            {record.currencyCode} {this.filterMoney(record.freezeAmount, 2, true)}
          </Col>
          <Col span={5}>
            <span>
              {this.$t({ id: 'acp.enabled.amount' } /*可申请金额*/)}
              {record.currencyCode} {this.filterMoney(record.availableAmount, 2, true)}
            </span>
          </Col>
        </Row>
          ...
          record.payAmount !== null && (
            <div>
              ...
              <Row>
                <Col span={2}>
                  <span style={{ float: 'right' }}>
                    {this.$t({ id: 'acp.payment.log' } /*付款日志*/)}
                  </span>
                </Col>
                <Col span={6} offset={1}>
                  <span>
                    {this.$t({ id: 'acp.payment.amount' } /*已付款总金额*/)}： {record.currencyCode}&nbsp;{this.filterMoney(
                      record.payAmount,
                      2,
                      true
                    )}
                ...
                <Col span={6}>
                  <span>
                    {this.$t({ id: 'acp.return.amount' } /*退款总金额*/)}： {record.currencyCode}&nbsp;{this.filterMoney(
                      record.returnAmount,
                      2,
                      true
                    )}
      ...
      <div>
        ...
        <div style={{ marginTop: 20, marginBottom: 0, boxShadow: '0 2px 8px rgba(0, 0, 0, 0.15)' }}>
          {!(headerData.status === 1001) && (
            ...
            // />
            <ApproveHistory type="801005" oid={headerData.documentOid} />
          )}
        </div>
      </div>
    );
...
export default connect(
  ...
  null,
  { withRef: true }
)(wrappedPaymentRequisitionDetail);
