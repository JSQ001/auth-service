...
class BudgetJournalCheck extends React.Component {
    ...
    this.state = {
      ...
      searchForm: [
        ...
        },
        {
          type: 'select',
          id: 'journalTypeId',
          label: this.$t('budgetJournal.journalTypeId'),
          options: [],
          method: 'get' /*预算日记账类型*/,
          ...
          items: [
            {
              type: 'date',
              id: 'beginDate',
              label: this.$t('role.set.finance.submit.date.from'),
            } /*提交日期从*/,
            {
              type: 'date',
              id: 'endDate',
              label: this.$t('role.set.finance.submit.date.to'),
            } /*提交日期至*/,
          ],
        },
      ],
        ...
        } /*金额*/,
        {
          ...
          render: (value, record) => (
            <Badge
              ...
              text={this.$statusList[record.budgetJournalApprovalView.status].label}
            />
          ),
        } /*状态*/,
      ],
  ...
  handleRowClick = (value, flag) => {
    this.props.dispatch(
      routerRedux.push({
        pathname: `/approval-management/budget-journal-check/budget-journal-check-detail/${
          value.budgetJournalApprovalView.id
        }/${value.budgetJournalApprovalView.journalCode}/${this.state.tabValue}`,
      })
    );
  };
    ...
    return (
      <div>
        ...
        <CustomTable
          ...
          url={`${config.budgetUrl}/api/budget/journals/approvals/filters`}
