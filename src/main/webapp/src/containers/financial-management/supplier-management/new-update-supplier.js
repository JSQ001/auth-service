...
class NewUpdateSupplier extends React.Component {
  ...
  componentDidMount() {
    ...
    if (typeof params.id !== 'undefined') {
      ...
      this.getOptions(this.state.basicInfo[2]);
    } else {
      this.setState({
        vendorInfo: params,
        isEdit: true,
        ...
        method
          .then(response => {
            ...
            if (!this.state.isEdit) {
              ...
              message.success(`${this.$t('common.operate.success', { name: '' })}`);
            }
          })
          .catch(e => {
            if (!this.state.isEdit) {
              message.error(
                `${this.$t('common.save.filed', { name: '' })}, ${e.response.data.message}`
              );
            } else {
              message.error(
                `${this.$t('common.operate.filed', { name: '' })}, ${e.response.data.message}`
              );
            }
  ...
  submitSupplier = () => {
    vendorService
      .operationSupplier(this.state.vendorInfo, 'PENGDING')
      .then(res => {
        if (res.status === 200) {
