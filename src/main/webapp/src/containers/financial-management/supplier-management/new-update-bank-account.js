...
class NewUpdateBankAccount extends React.Component {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (typeof this.state.bankInfo.id !== 'undefined') {
          //编辑
          values.id = this.state.bankInfo.id;
          !values.bankName && (values.bankName = this.state.bankInfo.bankName);
          method = vendorService.updateBankCardInfo(values);
        } else {
          method = vendorService.addBankCardInfo(values);
        }
        method
          .then(response => {
            message.success(
              this.$t(
                typeof this.state.bankInfo.id === 'undefined'
  ...
  handleBankChange = value => {
    if (value.length && value[0].id) {
      ...
      this.props.form.setFieldsValue({
        ...
        bankAddress: value[0].province + (value[0].city ? value[0].city : ''),
      });
    }
  };
  submitSupplier = () => {
    vendorService
      .operationBank(this.state.bankInfo, 'PENGDING')
      .then(res => {
        if (res.status === 200) {
  ...
  validateAccount = (item, value, callback) => {
    ...
    let str = /^([1-9]{1})([0-9-]{14}|[0-9-]{15}|[0-9-]{16}|[0-9-]{17}|[0-9-]{18})$/;
    this.setState({
      ...
      <div className="new-update-bank-account">
        <Form onSubmit={this.handleSubmit}>
          ...
          <Row gutter={22}>
            <Col span={22}>
              <FormItem
                ...
                help={
                  validateAccount
                    ? this.props.form.getFieldValue('bankAccount') == null ||
                      this.props.form.getFieldValue('bankAccount') == ''
                      ? this.$t('common.please.enter')
                      : this.$t('supplier.bank.account.error')
                    : ''
                }
          ...
          <div className="slide-footer">
            ...
            <Button type="primary" onClick={this.submitSupplier} loading={loading}>
              提交
            </Button>
