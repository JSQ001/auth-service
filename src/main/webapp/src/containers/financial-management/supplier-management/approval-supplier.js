...
import ApprovalSupplier from 'containers/financial-management/supplier-management/approval-supplier-view';
...
class SupplierManagement extends React.Component {
  constructor(props) {
    super(props);
    const vendorStatus = {
      EDITOR: { label: '编辑中', state: 'default' },
      APPROVED: { label: '审批通过', state: 'success' },
      PENGDING: { label: '审批中', state: 'processing' },
      PROHIBIT: { label: '禁用', state: 'error' },
      REFUSE: { label: '审批驳回', state: 'warning' },
    };
    this.state = {
      loading: false,
      data: [],
      ...
      searchForm: [
        {
          ...
          labelKey: 'name',
          options: [],
          getUrl: `${config.mdataUrl}/api/ven/type/query`,
          method: 'get',
      ...
      innerColumns: [
        ...
        {
          title: this.$t('common.column.status'),
          key: 'vendorBankStatus',
          dataIndex: 'vendorBankStatus',
          width: '7%',
          render: (value, record) => {
            if (value)
              return <Badge status={vendorStatus[value].state} text={vendorStatus[value].label} />;
          },
        },
        {
          /*最近更新时间*/
      ...
      columns: [
        ...
        },
        {
          /*最后提交人*/
          title: '最后提交人',
          key: 'venOperatorNumber',
          dataIndex: 'venOperatorNumber',
          render: (value, record, index) => {
            let content = record.venOperatorNumber + '-' + record.venOperatorName;
        ...
        },
        {
          ...
          render: (text, record, index) => (
            <span>
              {<a onClick={e => this.editItem(e, record, index)}>审批</a>}
  ...
  handleLinkAccount = (e, record, index) => {
    ...
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/financial-management/approval-supplier/approval-supplier-bank-account/${
          record.id
        }/${record.source}`,
      })
    );
  ...
  getList() {
    ...
    vendorService.getVenInfoByOptionsApproval(params).then(response => {
  ...
  render() {
    ...
    return (
      <div className="supplier-management">
        ...
        >
          <ApprovalSupplier onClose={this.handleOnClose} params={slideFrame.params} />
        </SlideFrame>
        <ListSelector
          type="vendor_company"
