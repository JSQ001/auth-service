...
class NewReverse extends React.Component {
  ...
  getReverseDetail() {
    ...
    reverseService
      .getExpenseDetail(this.props.match.params.id)
      .then(resp => {
        if (resp.status === 200) {
          this.setState({
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      if (this.props.match.params.isNew) {
        let params = {
          ...
          departmentId:
            values.deparmentSelectedData[0].oid || values.deparmentSelectedData[0].departmentOid,
        ...
        reverseService
          ...
          .then(resp => {
            if (resp.status === 200) {
              ...
              this.props.dispatch(
                routerRedux.replace({
                  pathname: `/financial-management/expense-reverse/expense-reverse-detail/${
                    resp.data.reverseHeader.id
                  }`,
                })
              );
  ...
  onCancel = () => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/financial-management/expense-reverse/expense-reverse`,
      })
    );
      ...
      <div className="new-contract" style={{ marginBottom: '40px' }}>
        <Spin spinning={pageLoading}>
          <Form onSubmit={this.handleSave}>
            ...
            <FormItem {...formItemLayout} label={this.$t('accounting.view.company')}>
              {' '}
              {/*公司*/}
            ...
            <FormItem {...formItemLayout} label={this.$t('accounting.view.department')}>
              {' '}
              {/*部门*/}
            ...
            <FormItem {...formItemLayout} label={this.$t('budget.occupancy.currency')}>
              {' '}
              {/*币种*/}
              {getFieldDecorator('currency', {
                ...
                initialValue: reverseHeader
                  ? reverseHeader.currencyCode
                  : this.props.match.params.currency, //+"-人民币"
              })(
                // <Select disabled/>
                <Select
                  disabled
                  ...
                  {currencyOptions.map(option => {
                    return (
                      <Option key={option.currency} value={option.currency}>
                        {option.currency}-{option.currency === 'CNY' ? '人民币' : '美元'}
                      </Option>
                    );
                  })}
                </Select>
