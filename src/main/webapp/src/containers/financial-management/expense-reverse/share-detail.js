...
class NewShare extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          ...
          render: (desc, record) => {
            return (
              <Popover content={props.expDetail ? record.company.name : desc}>
                <span>{props.expDetail ? record.company.name : desc ? desc : ''}</span>
              </Popover>
            );
          },
        },
        {
          ...
          render: (desc, record) => {
            return (
              <Popover content={props.expDetail ? record.department.name : desc}>
                <span>{props.expDetail ? record.department.name : desc ? desc : ''}</span>
              </Popover>
            );
          },
        },
          ...
          render: (value, record, index) => {
            return record.status == 'edit' || record.status == 'new' ? (
              <div style={{ textAlign: 'right' }}>
                <InputNumber
                  precision={2}
                  value={this.props.mode ? value : -value}
                  onChange={val => this.costChange(index, val)}
                />
              </div>
            ) : (
              <span style={{ textAlign: 'right' }}>
                {this.toDecimal2(this.props.mode ? value : -value)}
              </span>
            );
          },
        },
      ...
      costCenterData: {},
      applicationCol: {
        title: this.$t('request.edit.related.application') /*关联申请单*/,
        width: 240,
