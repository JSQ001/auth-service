...
class UnpaidReverseInfo extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
            ...
            return (
              <Popover
                content={
                  <div>
                    <div>
                      {this.$t('acp.account.name')}：{record.dataDTO
                        ? record.dataDTO.accountName
                        : '-'}
                    </div>
                    <div>
                      {this.$t('acp.account')}：{record.dataDTO
              ...
              >
                <div>
                  ...
                  <div
                    style={{ overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}
                  >
                    this.$t('acp.account')}：{record.dataDTO ? record.dataDTO.accountNumber : '-'}
                  </div>
                </div>
              </Popover>
            );
        ...
        {
          //付款属性
          title: this.$t('exp.pay.attribute'),
          dataIndex: 'payAttr',
          width: 180,
          render: (value, record) => {
            return (
              <Popover
                content={
                  <div>
                    <div>
                      {this.$t('pay.way.type')}：{record.dataDTO
                        ? record.dataDTO.paymentMethodCategoryName
                        : '-'}
                    </div>
                    <div>
                      {this.$t('acp.transaction.name')}：{record.dataDTO
      ...
      searchForm: [
        {
          ...
          items: [
            ...
            },
            {
              type: 'list',
              id: 'partnerId',
              label: this.$t({ id: 'pay.workbench.payee' } /*收款方*/),
              options: [],
              listType: 'bgtUser',
              single: true,
              event: 'PARTNER',
              ...
              }/api/cash/prepayment/requisitionHead/getReceivablesByName?&page=0&size=9999&name=&empFlag=1002&pageFlag=true`,
            },
          ],
        },
        {
      ...
      modalColumns: [
        ...
        {
          title: this.$t('acp.transaction.name'),
        ...
        {
          title: this.$t('xp.pay.line.remark'),
        ...
        {
          title: this.$t('exp.reserve.money'),
          dataIndex: 'ableReservedAmount',
          width: 110,
          render: this.filterMoney,
        },
        {
          title: this.$t('expense.reverse.amount'),
          dataIndex: 'adjustAmount',
          width: 100,
        ...
        {
          title: this.$t('exp.reserve.remark'),
          dataIndex: 'description',
          render: (value, record, index) => {
            return (
              <Input
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.flag != nextProps.flag) {
      ...
      this.setState(
        { columns, flag: nextProps.flag, id: nextProps.reverseId, page: 0, changeList: [] },
        () => {
          this.getList();
        }
      );
        ...
        <Row>
          ...
          <Col span={20} offset={1}>
            <span style={{ marginRight: 30 }}>
              {record.frozenFlag ? this.$t('exp.delay.pay') : this.$t('exp.unDelay.pay')}
            </span>
    ...
    return (
      <div className="tab-container reimburse-container">
        ...
        <Modal
          ...
          title={this.$t('exp.add.reserve.pay.info')}
