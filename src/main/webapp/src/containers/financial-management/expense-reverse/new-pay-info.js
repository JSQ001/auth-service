...
class NewPayInfo extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
            ...
            return (
              <Popover
                content={
                  <div>
                    <div>
                      {this.$t('acp.account.name')}：{record.accountName}
                    </div>
                    <div>
                      {this.$t('acp.account')}：{record.accountNumber}
                    </div>
                  </div>
                }
              >
        ...
        {
          title: this.$t('pay.workbench.amount'),
          dataIndex: 'amount',
          width: 90,
          render: (value, record) => (
            <span>
        ...
        {
          title: this.$t('exp.can.adjust.amount'),
        ...
        {
          title: this.$t('exp.adjust.amount'),
          dataIndex: 'adjustAmount',
          render: (value, record, index) => {
            return (
              <InputNumber
                defaultValue={record.adjustAmount ? record.adjustAmount : record.ableReservedAmount}
  ...
  componentDidMount() {
    ...
    if (
      this.props.params.lineFlag &&
      this.props.params.reverseId &&
      this.props.params.listType === 'unpaidLine'
    ) {
      this.setState(
        {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      nextProps.params.lineFlag &&
      nextProps.params.reverseId &&
      nextProps.params.listType === 'unpaidLine'
    ) {
      this.setState(
        {
  ...
  getUpaidLineList() {
    ...
    reverseService
      .getUpaidLineList(reverseId, page, pageSize)
      .then(resp => {
        if (resp.status === 200) {
          let data = resp.data;
          data.map(item => {
  ...
  expandedRowRender = record => {
    ...
    contractHeaderLineDTO = expensePaymentScheduleDTO.contractHeaderLineDTO
      ? expensePaymentScheduleDTO.contractHeaderLineDTO
      : {};
    return (
      <div>
        <Row>
            ...
            <Alert
              ...
              message={
                <div>
                  ...
                  <span style={{ color: '#1890ff' }}>{selectedRowKeys.length}</span>&nbsp;{this.$t(
                    'exp.number.expense'
                  )},{this.$t('request.detail.total.amount')}&nbsp;&nbsp;<span
                    style={{ color: '#1890ff' }}
                  >
                    {amount}
                  </span>
