...
*/
    ...
    columns.push({
      ...
      width: 100,
      key: 'amount',
      render: desc => this.filterMoney(this.props.mode === 'negative' ? -desc : desc),
    });
    ...
    this.setState({
      columns,
      flag: true,
      x: this.state.headerData.defaultApportionInfo.costCenterItems.length
        ? width + this.state.headerData.defaultApportionInfo.costCenterItems.length * 150
        : false,
    });
  };
      ...
      res.data &&
        ...
        res.data.map(item => {
          ...
          item.costCenterItems &&
            item.costCenterItems.length &&
            item.costCenterItems.map(o => {
