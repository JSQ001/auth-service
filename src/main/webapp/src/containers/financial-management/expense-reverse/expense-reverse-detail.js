...
class ExpenseReverseDetail extends React.Component {
  ...
  onSubmit = () => {
    ...
    reverseService
      .submitReverseDocument(this.state.headerData.reverseHeader.id)
      .then(resp => {
        if (resp.status === 200) {
      ...
      .catch(e => {
        this.setState({ submitLoading: false });
        message.error(
          e.response.data ? e.response.data.message : this.$t({ id: 'exp.summit.failed' })
        );
      });
  };
  ...
  render() {
    ...
    if (headerData.reverseHeader) {
      isEdit =
        headerData.reverseHeader.status === 1001 ||
        headerData.reverseHeader.status === 1003 ||
        headerData.reverseHeader.status === 1005;
    }
