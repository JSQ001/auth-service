...
class EditReverseInfo extends React.Component {
  ...
  componentDidMount() {
    ...
    if (
      ...
      !this.state.isNew &&
      params.id &&
      this.props.params.record !== this.state.record
    ) {
      let shareParams = {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      !this.state.isNew &&
      params.id &&
      nextProps.params.record !== this.state.record
    ) {
      let shareParams = {
        relatedApplication: params.headerData.relatedApplication,
        defaultApportion: params.defaultApportion,
  ...
  getInfo() {
    ...
    reverseService
      ...
      .catch(e => {
        ...
        if (e.response) message.error(e.response.data ? e.response.data : this.$t('common.error'));
      });
  }
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        reverseService
          .updataReverseLine(params, bodyParams)
          .then(resp => {
            if (resp.status === 200) {
          ...
          .catch(e => {
            this.setState({ saveLoading: false });
            message.error(
              e.response.data ? e.response.data.message : this.$t('common.save.filed' /*保存失败(*/)
            );
          });
      }
    });
  ...
  handleDelete = e => {
    e.preventDefault();
    reverseService
      .deleteReverseLine(this.state.id)
      .then(resp => {
        if (resp.status === 200) {
      ...
      .catch(e => {
        message.error(
          e.response.data ? e.response.data.message : this.$t('adjust.delete.fail' /*删除失败(*/)
        );
      });
  };
      ...
      <div className="edit-reverse-info">
        <Form onSubmit={this.handleSave}>
            ...
            <Col span={12}>
              <FormItem {...formItemLayout} label={this.$t('travel.policy.feeType' /*费用类型(*/)}>
                {getFieldDecorator('expenseTypeId', {
                  initialValue: record.expenseTypeId
                    ? [{ id: record.expenseType.id, name: record.expenseType.name }]
                    : [],
                })(
                  <Chooser
            ...
            <Col span={12}>
              <FormItem {...formItemLayout} label={this.$t('common.happened.date' /*发生日期(*/)}>
                {getFieldDecorator('createDate', {
                  initialValue: record ? moment(record.expenseDate) : moment(),
          ...
          <Row gutter={24}>
            <Col span={12}>
              <FormItem {...formItemLayout} label={this.$t('exp.invoice.operation')}>
                {getFieldDecorator('invoiceOperationType', {
                  rules: [{ required: true, message: this.$t('common.please.select') }],
                  initialValue: record ? record.invoiceOperationType : '',
                })(
                  <Select
                    style={{ width: '100%' }}
                    disabled={record.invoiceType !== '01'}
                ...
                <Col span={11} offset={2}>
                  <FormItem label={this.$t('expense.invoice.date')}>
                    {getFieldDecorator('invoiceDate', {
                      ...
                      initialValue: record.expenseReverseInvoice
                        ? moment(record.expenseReverseInvoice.createdDate)
                        : moment(record.digitalInvoice.createdDate),
                ...
                <Col span={11}>
                  <FormItem label={this.$t('expense.invoice.code')}>
                    {getFieldDecorator('invoiceCode', {
                      ...
                      initialValue: record.expenseReverseInvoice
                        ? record.expenseReverseInvoice.invoiceCode
                        : record.digitalInvoice.billingCode,
                ...
                <Col span={11} offset={2}>
                  <FormItem label={this.$t('expense.invoice.number')}>
                    {getFieldDecorator('invoiceNumber', {
                      ...
                      initialValue: record.expenseReverseInvoice
                        ? record.expenseReverseInvoice.invoiceNumber
                        : record.digitalInvoice.billingNo,
                    })(<Input placeholder={this.$t('common.please.enter')} />)}
                  </FormItem>
                </Col>
                <Col span={11}>
                      ...
                      <Col span={10}>
                        <FormItem>
                          {getFieldDecorator('expenseReverseInvoice.taxRate', {
                            initialValue: record.expenseReverseInvoice
                              ? record.expenseReverseInvoice.taxRate.toFixed(2)
                      ...
                      <Col span={12} offset={1}>
                        <FormItem>
                          {getFieldDecorator('expenseReverseInvoice.taxAmount', {
                            initialValue: record.expenseReverseInvoice
                              ? record.expenseReverseInvoice.taxAmount.toFixed(2)
                ...
                <Col span={11}>
                  <FormItem label={this.$t('expense.invoice.amount.without.tax')}>
                    {getFieldDecorator('expenseReverseInvoice.exculedTaxAmount', {
                      initialValue: record.expenseReverseInvoice
                        ? record.expenseReverseInvoice.exculedTaxAmount.toFixed(2)
                        : record.exculedTaxAmount.toFixed(2),
                    })(<Input disabled />)}
                  </FormItem>
                </Col>
                <Col span={11} offset={2}>
                  <FormItem label={this.$t('exp.Instalment')}>
                    {getFieldDecorator('expenseReverseInvoice.installmentFlag', {
                      initialValue: record.expenseReverseInvoice
                        ? record.expenseReverseInvoice.installmentFlag
                        : record.digitalInvoice.installmentFlag,
                    })(<Checkbox disabled />)}
                  </FormItem>
                </Col>
              </Row>
            ...
            <Col span={12}>
              <FormItem {...formItemLayout} label={this.$t('exp.reserve.tax')}>
                {getFieldDecorator('taxAmount', {
                  rules: [{ required: true, message: this.$t('common.please.enter') }],
                  initialValue: typeof record.taxAmount !== 'undefined' ? record.taxAmount : '-',
          ...
          <FormItem
            ...
            labelCol={{ span: 3 }}
            wrapperCol={{ span: 20 }}
            label={this.$t('exp.reserve.remark')}
          >
          ...
          >
            ...
            <Button style={{ marginLeft: 40 }} onClick={this.onCancel}>
              取消
            </Button>
            {/*
