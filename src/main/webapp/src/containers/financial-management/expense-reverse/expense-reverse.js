...
class ExpenseReverse extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      canSearchForm: [
          ...
          items: [
            {
              type: 'date',
              id: 'applyDateFrom',
              label: this.$t('epx.adjust.apply.dateFrom'),
              event: 'DATE_FROM',
            },
            {
              type: 'date',
              id: 'applyDateTo',
              label: this.$t('exp.adjust.apply.dateTo'),
              event: 'DATE_TO',
            },
          ],
          colSpan: 6,
        ...
        {
          type: 'list',
          listType: 'select_department_contract',
          options: [],
          id: 'departmentId',
          ...
          items: [
            {
              type: 'date',
              id: 'applyDateFrom',
              label: this.$t('expense.reverse.date.from'),
              event: 'DATE_FROM',
            },
            {
              type: 'date',
              id: 'applyDateTo',
              label: this.$t('expense.reverse.date.to'),
              event: 'DATE_TO',
            },
          ],
          colSpan: 6,
          ...
          items: [
            {
              ...
              label: this.$t('expense.reverse.amount.from'),
            },
      ...
      canReverseColumns: [
        {
          title: this.$t('ye.wu.da.lei'),
          key: 'businessClassName',
          dataIndex: 'businessClassName',
          align: 'center',
          render: desc => (
            <span>
        ...
        {
          title: this.$t('common.total.amount'),
          key: 'amount',
          dataIndex: 'amount',
          width: 80,
          align: 'center',
          render: (desc, currencyCode) => (
            <span>
        ...
        },
        {
          ...
          render: (text, record) => (
            <span>
              <a href="#" onClick={e => this.goReverse(e, record)}>
                {this.$t('acp.payment.reserved')}
              </a>
            </span>
          ),
        },
      ...
      myReverseColumns: [
        {
          title: this.$t('exp.reverse.number'),
          key: 'reportReverseNumber',
          dataIndex: 'reportReverseNumber',
          align: 'center',
          render: (value, record) => (
            <a onClick={e => this.goReverse(e, record)}>
              <Popover content={value}>{value ? value : '-'}</Popover>
            </a>
          ),
        }, //原单据编号
        {
          title: this.$t('expense.reverse.check.source.number'),
          key: 'sourceReportHeaderCode',
        ...
        {
          title: this.$t('exp.old.business.type'),
          key: 'businessClassName',
          dataIndex: 'businessClassName',
          align: 'center',
          render: desc => (
        ...
        {
          title: this.$t('detail.reverse.date'),
          key: 'reverseDate',
        ...
        {
          title: this.$t('exp.reserve.status'),
          key: 'status',
          dataIndex: 'status',
          align: 'center',
          render: (value, record) => {
  ...
  componentDidMount() {
    reverseService.myReverseList().then(response =>
      this.setState({
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      typeof nextState.canReverseTotal !== 'undefined' ||
      typeof nextState.myReverseTotal !== 'undefined' ||
  ...
  searchCanReverse = result => {
    ...
    result.applyDateFrom = result.applyDateFrom
      ? result.applyDateFrom.format('YYYY-MM-DD')
      : undefined;
  ...
  goReverse = (e, record) => {
    ...
    if (this.state.currentTab === 'CANREVERSE') {
      this.props.dispatch(
        routerRedux.replace({
          pathname: `/financial-management/expense-reverse/new-reverse/${record.documentId}/${
            record.businessClass
          }/${this.state.isNew}/${record.currencyCode}`,
  ...
  searchMyReverse = result => {
    ...
    result.reverseDateFrom = result.applyDateFrom
      ? result.applyDateFrom.format('YYYY-MM-DD')
      : undefined;
  ...
  onChangeTabs = key => {
    this.setState({ currentTab: key }, () => {
      if (key === 'CANREVERSE') this.customTable1.search(this.state.canSearchParams);
      else this.customTable2.search(this.state.searchParams);
    });
  };
    ...
    return (
      <div className="expense-reverse">
        <Tabs defaultActiveKey={currentTab} onChange={this.onChangeTabs}>
          <TabPane
            tab={`${this.$t('exp.can.reserve')}(${this.state.canReverseTotal})`}
            key={this.state.tabs[0].key}
          />
          <TabPane
            tab={`${this.$t('exp.finish.reserve')}(${this.state.myReverseTotal})`}
            key={this.state.tabs[1].key}
          />
        </Tabs>
        {currentTab === 'CANREVERSE' && (
          <div>
