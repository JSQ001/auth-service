...
class ExpenseInfo extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      params.headerData.documentHeader.documentId
    ) {
      let shareParams = { defaultApportion: params.defaultApportion };
      this.setState(
        {
          oldInfo: nextProps.params.record,
          nowPage: 'form',
          defaultApportion: params.defaultApportion,
          headerData: params.headerData,
          isShowInvoice: params.isShowInvoice,
  ...
  deleteShare = index => {
    ...
    this.setState(
      { shareData, applicationData, isRefreshShareTabel: !this.state.isRefreshShareTabel },
      () => {
        this.setDefaultAmount();
      }
    );
            ...
            <Col span={12}>
              <FormItem {...formItemLayout} label={this.$t('common.expense.type')}>
                {getFieldDecorator('expenseTypeId', {
                  initialValue: oldInfo.id
                    ? [{ id: oldInfo.expenseType.id, name: oldInfo.expenseType.name }]
                    : [],
                })(
                  <Chooser
