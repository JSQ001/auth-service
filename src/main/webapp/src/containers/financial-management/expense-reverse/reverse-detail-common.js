...
class ReverseDetailCommon extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      contractStatus: {
        ...
        1002: {
          label: this.$t({ id: 'my.contract.state.submitted' } /*审核中*/),
          state: 'processing',
        },
      ...
      modalColumns: [
        {
          title: this.$t({ id: 'common.sequence' }),
          align: 'center',
          dataIndex: 'index',
          key: 'index',
          width: 60,
        ...
        {
          //费用类型
          title: this.$t({ id: 'common.expense.type' }),
          dataIndex: 'expenseTypeName',
          key: 'expenseTypeName',
          width: 100,
        }, //发生日期
        {
          title: this.$t({ id: 'common.happened.date' }),
          dataIndex: 'createdDate',
          key: 'createdDate',
          width: 110,
          render: desc => <span>{moment(desc).format('YYYY-MM-DD')}</span>,
        },
        {
          //金额
          title: this.$t({ id: 'common.amount' }),
          dataIndex: 'amount',
          key: 'amount',
          width: 90,
        ...
        {
          //本位币金额
          title: this.$t({ id: 'common.base.currency.amount' }),
            ...
            return (
              <div>
                {record.vatInvoice && (
                  ...
                  >
                    {this.$t({ id: 'exp.invoice.info' } /*发票信息*/)}
                  </a>
                )}
            ...
            searchForm: [
              {
                type: 'input',
                id: 'expenseTypeName',
                label: this.$t({ id: 'itemMap.expenseTypeName' }),
              },
            ],
            columns: [
              {
         ...
         },*/
        {
          ...
          items: [
            //发生日期从
            {
              type: 'date',
              id: 'applyDateFrom',
              label: this.$t({ id: 'exp.happen.date.from' }),
              event: 'DATE_FROM',
            },
            {
              type: 'date',
              id: 'applyDateTo',
              label: this.$t({ id: 'exp.happen.date.to' }),
              event: 'DATE_TO',
            },
          ],
        },
          ...
          items: [
            {
              type: 'input',
              id: 'amountFrom',
              label: this.$t({ id: 'approve.request.moneyFrom' }),
            },
  ...
  componentDidMount() {
    if (this.state.flag && this.props.headerData.documentHeader) {
      this.setState({ flag: false }, () => {
        reimburseService
          .getDefaultApportion(this.props.headerData.documentHeader.documentId)
          .then(res => {
    ...
    if (this.state.historyFlag && this.props.id) {
      this.setState({ historyFlag: false }, () => {
        ...
        reverseService
          .getApproveHistory(this.props.id)
          .then(res => {
        ...
        infoList: [
          {
            ...
            value: this.props.headerData.reverseHeader.employeeName,
          },
          {
            label: this.$t({ id: 'exp.company' }),
            value: this.props.headerData.reverseHeader.companyName,
          },
          {
            label: this.$t({ id: 'common.department' }),
            value: this.props.headerData.reverseHeader.departmentName,
          },
          {
            label: this.$t({ id: 'ye.wu.da.lei' }),
            value: this.props.headerData.reverseHeader.businessClassName,
          },
          {
            label: this.$t({ id: 'detail.source.report.number' }),
            value: this.props.headerData.reverseHeader.sourceReportHeaderCode,
          },
          {
            label: this.$t({ id: 'exp.reserve.money' }),
            value: this.props.headerData.documentHeader.reverseAmount,
          },
        ],
        attachments: [],
      };
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.flag && nextProps.headerData.documentHeader) {
      this.setState({ flag: false }, () => {
        reimburseService
          .getDefaultApportion(nextProps.headerData.documentHeader.documentId)
          .then(res => {
            this.setState({ defaultApportion: res.data });
          })
          .catch(err => {
    ...
    if (this.state.historyFlag && nextProps.id) {
      this.setState({ historyFlag: false }, () => {
        this.setState({ historyLoading: true });
        reverseService
          .getApproveHistory(nextProps.id)
          .then(res => {
            this.setState({ approveHistory: res.data, historyLoading: false });
          })
          .catch(err => {
    ...
    if (nextProps.headerData.reverseHeader) {
      let documentParams = {
        ...
        infoList: [
          {
            label: this.$t({ id: 'common.applicant' }),
            value: nextProps.headerData.reverseHeader.employeeName,
          },
          {
            label: this.$t({ id: 'exp.company' }),
            value: nextProps.headerData.reverseHeader.companyName,
          },
          {
            label: this.$t({ id: 'common.department' }),
            value: nextProps.headerData.reverseHeader.departmentName,
          },
          {
            label: this.$t({ id: 'ye.wu.da.lei' }),
            value: nextProps.headerData.reverseHeader.businessClassName,
          },
          {
            label: this.$t({ id: 'detail.source.report.number' }),
            value: nextProps.headerData.reverseHeader.sourceReportHeaderCode,
          },
          {
            label: this.$t({ id: 'exp.reserve.money' }),
            value: nextProps.headerData.documentHeader.reverseAmount,
          },
        ],
        attachments: [],
      };
  ...
  addReverseInfo = () => {
    //'添加费用'
    this.setState({
      reverseInfoVisible: true,
      slideFrameTitle: this.$t({ id: 'exp.add.expense' }),
      listType: 'reverseLine',
    });
  };

  //编辑费用冲销行
  editReverseInfo = record => {
    //编辑费用反冲信息
    this.setState({
      editReverseVisible: true,
      slideFrameTitle: this.$t({ id: 'exp.edit.reserve.info' }),
      reverseRecord: record,
    });
  };

  //查看原费用
  checkReverseInfo = record => {
    //查看费用
    this.setState({
      detailVisible: true,
      slideFrameTitle: this.$t({ id: 'expense.view' }),
      costRecord: record,
    });
  };
  ...
  addUnpaidLine = () => {
    ...
    this.setState({
      payInfoVisible: true,
      slideFrameTitle: this.$t({ id: 'exp.amount.change' }),
      listType: 'unpaidLine',
    });
  };

  //编辑待付款冲销行
  editUnpaidInfo = record => {
    //编辑待支付反冲信息
    this.setState({
      editUnpaidVisible: true,
      slideFrameTitle: this.$t({ id: 'exp.edit.unReserve.info' }),
      unpaidRecord: record,
    });
  };
  ...
  handleEdit = () => {
    ...
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/financial-management/expense-reverse/new-reverse/${
          headerData.documentHeader.documentId
        }/${headerData.reverseHeader.businessClass}/${this.state.isNew}/${
          headerData.reverseHeader.currencyCode
        }`,
  ...
  handleRecall = () => {
    reverseService
      .recallReverseDocument(this.props.id, this.props.userId)
      .then(resp => {
        if (resp.status === 200) {
      ...
      .catch(e => {
        message.error(
          e.response.data ? e.response.data.message : this.$t({ id: 'exp.withDraw.failed' })
        ); //撤回失败
      });
  };
  ...
  handleSave = () => {
    ...
    reverseService
      .saveExpenseLine(this.props.id, params)
      .then(resp => {
        if (resp.status === 200) {
  ...
  render() {
    ...
    return (
      <div className="contract-detail-common reimburse" style={{ marginBottom: 20 }}>
            ...
            {isEdit && (
              <div style={{ float: 'right', paddingBottom: 5 }}>
                <Button type="primary" onClick={this.handleEdit}>
                  {this.$t({ id: 'my.contract.edit' } /*编 辑*/)}
                </Button>
              </div>
            )}
            {status === 1002 && (
              <div style={{ float: 'right', marginBottom: 8 }}>
                <Button type="primary" onClick={this.handleRecall}>
                  {this.$t({ id: 'my.contract.withdrawal' } /*撤 回*/)}
                </Button>
              </div>
            )}
          </DocumentBasicInfo>
        ...
        <div>
          <div className="tab-container" style={{ marginBottom: 20 }}>
            <h3 style={{ padding: '0 0 10px', margin: 0 }} className="sub-header-title">
              {this.$t({ id: 'exp.expense.info' })}
            </h3>
            <Row gutter={24}>
              <Col span={18} style={{ lineHeight: '60px', height: '32px' }}>
                {isEdit && (
                  <Button onClick={this.addExpense} type="primary">
                    {this.$t({ id: 'expense.new' })}
                  </Button>
                )}
              </Col>
              <Col span={6} style={{ textAlign: 'right', paddingTop: 27, marginLeft: -12 }}>
                <span>
                  {this.$t({ id: 'exp.amount.total' })}：<span style={{ color: 'green' }}>
                    {headerData.reverseHeader.currencyCode}{' '}
                    {this.filterMoney(headerData.reverseHeader.amount)}
                  </span>
                </span>
              </Col>
            </Row>
        ...
        <Modal
          visible={newExpenseVisible}
          title={this.$t({ id: 'exp.add.expense' })}
          ...
          footer={[
            ...
            <Button
              key="back"
              onClick={() =>
                this.setState({ newExpenseVisible: false, searchParams: {}, modalData: [] })
              }
            >
        ...
        >
          ...
          <div className="table-header">
            <div className="table-header-title">
              ...
              {this.$t(
                ...
                { total: selectedRowKeys.length === 0 ? '0' : selectedRowKeys.length }
              )}
              {/* 已选 total 条 */}
            </div>
          </div>
          ...
          <AttachmentInformation
            sourceReportLineId={this.state.modalRecord.id}
            visible={this.state.showAttInfo}
            close={() => {
              this.setState({ showAttInfo: false });
            }}
          />
        </Modal>
        <SlideFrame
