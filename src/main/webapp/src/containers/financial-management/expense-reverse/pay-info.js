...
class UnpaidReverseInfo extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        ...
        {
          //反冲付款金额
          title: this.$t({ id: 'exp.reserve.pay.amount' }),
        ...
        {
          title: this.$t({ id: 'exp.receivables' }),
          align: 'center',
        ...
        {
          //收款账户
          title: this.$t({ id: 'acp.accountName' }),
            ...
            return (
              <Popover
                content={
                  <div>
                    <div>
                      {this.$t({ id: 'acp.account' } /*账户：*/)}：{record.dataDTO
                        ? record.dataDTO.accountName
                        : '-'}
                    </div>
                    <div>
                      {this.$t({ id: 'acp.account.name' } /*户名*/)}：{record.dataDTO
                        ? record.dataDTO.accountNumber
                        : '-'}
                    </div>
                  </div>
                }
        ...
        {
          //付款属性
          title: this.$t({ id: 'exp.pay.attribute' }),
          dataIndex: 'payAttr',
          width: 200,
          render: (value, record) => {
            return (
              <Popover
                content={
                  <div>
                    <div>
                      {this.$t({ id: 'adjust.authority.set' } /*付款方式类型*/)}：{record.dataDTO
                        ? record.dataDTO.paymentMethodCategoryName
                        : '-'}
                    </div>
                    <div>
                      {this.$t({ id: 'acp.cshTransactionClassName' } /*付款用途*/)}：{record.dataDTO
                        ? record.dataDTO.cshTransactionClassName
                        : '-'}
                    </div>
                  </div>
                }
          ...
          render: (value, record) => {
            return (
              <div>
                ...
                <Popconfirm
                  placement="top"
                  title={this.$t({ id: 'configuration.detail.tip.delete' })}
                  onConfirm={() => {
          ...
          items: [
            {
              type: 'value_list',
              id: 'partnerCategory',
              label: this.$t({ id: 'pay.workbench.type' } /*类型*/),
              valueListCode: 2107,
              options: [],
              event: 'code',
            },
            {
              type: 'list',
              id: 'partnerId',
              label: this.$t({ id: 'my.receivable' } /*收款方*/),
              options: [],
              listType: 'bgtUser',
              single: true,
              event: 'PARTNER',
              ...
              }/api/cash/prepayment/requisitionHead/getReceivablesByName?&page=0&size=9999&name=&empFlag=1002`,
            },
          ],
        },
        {
      ...
      modalColumns: [
        ...
        {
          title: this.$t({ id: 'acp.transaction.name' } /*付款用途：*/),
        ...
        {
          //原付款行备注
          title: this.$t({ id: 'exp.pay.line.remark' }),
          dataIndex: 'expensePaymentScheduleDTO.description',
          align: 'center',
          render: desc => (
            <span>
              <Popover content={desc ? desc : '-'}>{desc ? desc : '-'}</Popover>
        ...
        {
          title: this.$t({ id: 'exp.reserve.money' }),
          dataIndex: 'ableReservedAmount',
          width: 110,
          align: 'center',
          render: this.filterMoney,
        }, //反冲金额
        {
          title: this.$t({ id: 'expense.reverse.amount' }),
          dataIndex: 'adjustAmount',
          width: 100,
          align: 'center',
          render: (value, record, index) => {
            return (
              <InputNumber
                defaultValue={record.adjustAmount ? record.adjustAmount : ''}
        ...
        {
          //反冲备注
          title: this.$t({ id: 'exp.reserve.remark' }),
          dataIndex: 'description',
          align: 'center',
          render: (value, record, index) => {
            return (
              <Input //defaultValue={record.description ? record.description : ''}
                onChange={e => this.changeRemark(e, index, record)}
              />
            );
          },
        },
  ...
  componentDidMount() {
    if (this.state.flag != this.props.flag) {
      ...
      if (this.props.headerData.reverseHeader.status === 1002) columns.splice(-1, 1);
      this.setState(
        { columns, flag: this.props.flag, id: this.props.id, page: 0, changeList: [] },
        () => {
          this.getList();
        }
      );
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.flag != nextProps.flag) {
      const { columns } = this.state;
      if (nextProps.headerData.reverseHeader.status === 1002) columns.splice(-1, 1);
      this.setState(
        { columns, flag: nextProps.flag, id: nextProps.id, page: 0, changeList: [] },
        () => {
          this.getList();
        }
      );
  ...
  getList = () => {
    ...
    reverseService
      .getReversePayLine(id, page, pageSize)
      .then(resp => {
        if (resp.status === 200) {
          this.setState({
            data: resp.data,
  ...
  getUpaidLineList() {
    ...
    reverseService
      ...
      .catch(e => {
        ...
        message.error(
          e.response.data ? e.response.data.message : this.$t({ id: 'exp.get.unPay.failed' })
        );
      });
  }
  ...
  handleSave = () => {
    ...
    reverseService
      ...
      .catch(e => {
        ...
        message.error(
          e.response.data ? e.response.data.message : this.$t({ id: 'wait.for.save.addFail' })
        );
      });
  };
          ...
          <div>
            <Row>
              ...
              <Col span={20} offset={1}>
                ...
                >
                  {contractHeaderLineDTO.contractNumber
                    ? contractHeaderLineDTO.contractNumber
                    : '_'}
                </a>
                ...
                <span>
                  {schedulePaymentDate ? moment(schedulePaymentDate).format('YYYY-MM-DD') : '-'}
                </span>
              </Col>
            </Row>
            <Divider />
        ...
        <Row>
          ...
          <Col span={20} offset={1}>
            <span style={{ marginRight: 30 }}>
              {this.$t({ id: record.frozenFlag ? 'exp.delay.pay' : 'exp.unDelay.pay' })}
            </span>
  ...
  render() {
    ...
    return (
      <div className="tab-container reimburse-container">
        ...
        <Modal
          visible={newPayVisible}
          title={this.$t({ id: 'exp.add.reserve.pay.info' })}
        ...
        >
          ...
          <div className="table-header">
            <div className="table-header-title">
              {this.$t({ id: 'common.total' }, { total: linePagination.total })}
              {/* 共 total 条数据 */}
              &nbsp;<span>/</span>&nbsp;
              {this.$t(
