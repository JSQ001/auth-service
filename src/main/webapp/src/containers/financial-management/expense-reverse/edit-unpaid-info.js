...
class EditUnpaidInfo extends React.Component {
  ...
  componentDidMount() {
    ...
    if (params && params.lineFlag && params.record !== this.state.record) {
      this.setState({
        ...
        contractInfo: params.record.dataDTO.contractHeaderId
          ? params.record.dataDTO.expensePaymentScheduleDTO.contractHeaderLineDTO
          : {},
      });
    }
  }
  ...
  handleDelete = e => {
    e.preventDefault();
    reverseService
      .deletePayLine(this.state.record.id)
      .then(resp => {
        if (resp.status === 200) {
        ...
        <Form onSubmit={this.handleSave} style={{ marginTop: 20 }}>
          ...
          <FormItem {...formItemLayout} label={this.$t('exp.pay.delay')}>
            ...
            <span style={{ marginLeft: 20 }}>
              {record && record.dataDTO.frozenFlag
                ? this.$t('exp.pay.delay')
                : this.$t('exp.pay.unDelay')}
            </span>
          </FormItem>
          <div>
            ...
            <div style={{ marginBottom: '16px', marginLeft: '60px' }}>
              <Row gutter={8}>
                ...
                <Col span={15}>
                  <Input disabled value={!contractInfo.lineId ? '' : contractInfo.contractNumber} />
                  <div style={{ marginTop: '8px' }}>
                    {!contractInfo.lineId
                      ? null
                      : `${acp.contract.lineNumber}${contractInfo.lineNumber} | ${
                          acp.contract.date
                        }${moment(contractInfo.dueDate).format('YYYY-MM-DD')}`}
                  </div>
                </Col>
                <Col span={4} style={{ textAlign: 'left' }} className="ant-form-item-label">
                  {contractInfo.lineNumber && (
                    <a onClick={() => this.detail(contractInfo.headerId)}>
                      {this.$t('expense.invoice.view.detail')}
                    </a>
                  )}
                </Col>
              </Row>
          ...
          <div className="slide-footer">
            ...
            <Button onClick={this.onCancel} style={{ marginLeft: 30 }}>
              {this.$t('common.cancel')}
            </Button>
            <Button
              style={{ background: 'red', color: '#fff', marginLeft: 30 }}
