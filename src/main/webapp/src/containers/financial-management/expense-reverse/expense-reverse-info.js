...
class ExpenseReverseInfo extends React.Component {
  constructor(props) {
    ...
    this.state = {
      columns: [
        ...
        {
          //反冲本币金额
          title: this.$t({ id: 'exp.reserve.baseCurrency.amount' }),
        ...
        {
          //发票操作类型
          title: this.$t({ id: 'exp.invoice.operation.type' }),
          dataIndex: 'invoiceOperationType',
          width: 140,
          align: 'center',
          render: (value, record) => {
            return (
              <div>
                ...
                {value === 'BACK_LASE' && (
                  ...
                  >
                    {this.$t({ id: 'exp.red.invoice.reserve' })}
                  </a>
                )}
              </div>
            );
          ...
          render: (value, record) => {
            return (
              <div>
                ...
                <a onClick={() => this.checkOldExpense(record)}>
                  {this.$t({ id: 'exp.old.expense.detail' })}
                </a>
              </div>
            );
          },
  ...
  componentDidMount() {
    if (this.state.flag != this.props.flag) {
      ...
      if (
        this.props.headerData.reverseHeader.status === 1001 ||
        this.props.headerData.reverseHeader.status === 1003 ||
        this.props.headerData.reverseHeader.status === 1005
      ) {
        columns.push({
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.flag != nextProps.flag) {
      let columns = this.state.columns;
      if (
        nextProps.headerData.reverseHeader.status === 1001 ||
        nextProps.headerData.reverseHeader.status === 1003 ||
        nextProps.headerData.reverseHeader.status === 1005
      ) {
        columns.push({
          title: this.$t({ id: 'common.operation' }),
          dataIndex: 'id',
  ...
  handleDelete = record => {
    reverseService
      ...
      .catch(e => {
        message.error(
          e.response.data ? e.response.data.message : this.$t({ id: 'common.delete.failed' })
        );
      });
  };
  ...
  getInfo = () => {
    reimburseService
      .getReimburseDetailById(this.state.headerData.documentHeader.documentId)
      .then(res => {
        let expenseTypeList = [];
  ...
  */
  ...
  getReverseLineList() {
    ...
    reverseService
      .getExpenseLine(linePage, linePageSize, params)
      .then(resp => {
        if (resp.status === 200) {
          this.setState({
            loading: false,
    ...
    return (
      <div className="tab-container reimburse-container">
        <h3 style={{ padding: '0 0 10px', margin: 0 }} className="sub-header-title">
          {this.$t({ id: 'exp.expense.reserve.info' })}
        </h3>
          ...
          {!this.props.disabled && (
            <div className="table-header-buttons" style={{ float: 'left' }}>
              <Button type="primary" onClick={this.addItem}>
                {this.$t({ id: 'exp.add.expense' })}
              </Button>
            </div>
          )}
          <div style={{ float: 'right' }}>
            <span>
              {this.$t({ id: 'exp.expense.baseCurrency.amount' })}：<span
                style={{ color: 'green' }}
              >
                {amountData.totalFunctionAmount}
              </span>
            </span>
          </div>
          <div style={{ float: 'right', marginRight: '20px' }}>
            <span>
              {this.$t({ id: 'exp.reserve.amount.total' })}:<span style={{ color: 'green' }}>
        ...
        >
          ...
          <AttachmentInformation
            sourceReportLineId={this.state.sourceReportLineId}
        ...
        <ApprotionInfo
          mode="negative"
          close={() => {
            this.setState({ showApprotionOut: false });
          }}
          headerData={this.state.headerData}
          visible={this.state.showApprotionOut}
          id={this.state.invoiceId}
          z-index={1001}
        />
      </div>
