...
class CheckCostApplication extends React.Component {
      ...
      searchForm: [
        {
          ...
          renderOption: option =>
            `${option.employeeID}-${option.fullName}${
              option.status != 1001 ? `(${this.$t('check.cost.application.search.leaved')})` : ''
            }`,
        }, //申请人姓名/工号
        {
          type: 'input',
          id: 'businessCode',
          label: this.$t('check.cost.application.search.businessCode' /*单号*/),
        },
        {
          type: 'items',
          id: 'dateRange',
      ...
      checkboxListForm: [
        {
          ...
          items: [
            {
              label: this.$t('documentType.expense.request' /*费用申请单*/),
              key: 'cost',
              options: [],
            },
          ],
  ...
  getList = firstRequest => {
    ...
    costApplicationService
      .getFinanceViewList(page, pageSize, sort, searchParams)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            loading: false,
