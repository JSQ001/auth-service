...
class CshWriteOffBacklashCheck extends Component {
      ...
      columns3: [
        ...
        },
        {
          ...
          render: functionalAmountDr => {
            return <span>{this.filterMoney(functionalAmountDr, 2)}</span>;
          },
        },
        {
          title: '本币贷方',
          ...
          render: functionalAmountCr => {
            return <span>{this.filterMoney(functionalAmountCr, 2)}</span>;
          },
        },
      ],
      data3: [],
  ...
  getSegments = () => {
    ...
    cshWriteOffBacklashCheckService
        ...
        if (res.status === 200) {
          ...
          res.data.map(item => {
            columns3.push({
              ...
              dataIndex: item.segmentClassField,
              align: 'center',
            });
            column3Width += 100;
          });
  ...
  onPassClick = (e, operationType) => {
    ...
    let id = nowRecord.id;
    cshWriteOffBacklashCheckService
      .reverseUpdateStatus(id, operationType, opinion)
      .then(res => {
        if (res.status === 200) {
          ...
          <div>
            ...
            <Table
              rowClassName={(record, index) =>
                rowColorIndex === index ? 'row-backgroundcolor' : ''
              }
            ...
            <Card>
              <Row gutter={24} type="flex" justify="start">
                <Col span={3}>
                  ...
                  <Button
                    disabled={!whetherAccout || !nowRecord.id}
                    type="primary"
                    onClick={this.onCreateCredenceClick}
                  >
                    创建凭证
                  </Button>
                </Col>
                <Col span={18}>
                  ...
                  <DatePicker
                    ...
                    onChange={this.onAccountDateChange}
                    value={accountDate}
                  />
                </Col>
            ...
            <Affix className="bottom-bar" offsetBottom="0">
              <Row gutter={12} type="flex" justify="start">
                ...
                <Col span={1.5}>
                  <Button type="primary" onClick={e => this.onPassClick(e, '1')}>
                    通过
                  </Button>
                </Col>
                <Col span={1.5}>
                  <Button
                    onClick={e => this.onPassClick(e, '-1')}
              ...
              <Row gutter={24} type="flex" justify="start">
                <Col span={21}>
                  ...
                  <DatePicker
                    ...
                    style={{ marginLeft: '10px' }}
                    value={
                      nowRecord.writeOffDate ? moment(nowRecord.writeOffDate, 'YYYY-MM-DD') : ''
                    }
                  />
                </Col>
                {/* <Col span={2} style={{ color: '#1E90FF' }}>{nowRecord.documentNumber}</Col> */}
