...
class FinanceView extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      status: [
        ...
        {
          label: this.$t('finance.view.search.ticketReject' /*开票驳回*/),
          value: '10011004',
          state: 'error',
        },
        {
      ...
      searchForm: [
        {
          type: 'combobox',
          id: 'userOid',
          label: this.$t('finance.view.search.application'),
          placeholder: this.$t('common.please.enter') + this.$t('finance.view.search.application'),
          options: [],
          searchUrl: `${config.mdataUrl}/api/search/users/all`,
          method: 'get',
          searchKey: 'keyword',
          labelKey: 'fullName',
          valueKey: 'userOid',
        ...
        }, //申请人姓名/工号
        {
          type: 'combobox',
          id: 'businessCode',
          label: this.$t('finance.view.search.businessCode'),
          placeholder: this.$t('common.please.enter') + this.$t('finance.view.search.businessCode'),
          options: [],
          searchUrl: `${config.baseUrl}/api/expense/report/loanApplication/search`,
          method: 'get',
          searchKey: 'keyword',
          getParams: { type: '10021008' },
          labelKey: '_self',
          valueKey: '_self',
        }, //父单/子单/借款单号
        {
          ...
          items: [
            {
              type: 'date',
              id: 'beginDate',
              label: this.$t('finance.view.search.dateFrom'),
              defaultValue: moment().subtract(1, 'month'),
            }, //提交日期从
            {
              type: 'date',
              id: 'endDate',
              label: this.$t('finance.view.search.dateTo'),
              defaultValue: moment(),
            }, //提交日期至
          ],
        },
          ...
          items: [
            {
              ...
              label: this.$t('finance.view.search.approvalDateFrom'),
            }, //审批日期从
            {
              type: 'date',
              id: 'approvalEndDate',
              label: this.$t('finance.view.search.approvalDateTo'),
            }, //审批日期至
          ],
        },
        {
          type: 'items',
          id: 'auditedApprovalDateRange',
          items: [
            {
              type: 'date',
              id: 'auditedApprovalStartDate',
              label: this.$t('finance.view.search.auditedDateFrom'),
            }, //审核日期从
            {
              type: 'date',
              id: 'auditedApprovalEndDate',
              label: this.$t('finance.view.search.auditedDateTo'),
            }, //审批日期至
          ],
        },
        {
      ...
      checkboxListForm: [
        {
          ...
          items: [
            {
              label: this.$t('finance.view.search.reimbursement' /*报销单*/),
              key: 'expense',
              options: [],
            },
            {
              label: this.$t('finance.view.search.borrowingDocument' /*借款单*/),
              key: 'loan',
              options: [],
            },
          ],
      ...
      columns: [
        ...
        { title: this.$t('finance.view.search.applicant' /*申请人*/), dataIndex: 'applicant' },
        {
          ...
          render: (date, record) =>
            date
              ...
              : moment(record.createDate).format('YYYY-MM-DD'),
          sorter: true,
        },
        {
          title: this.$t('finance.view.search.documentType' /*单据类型*/),
          dataIndex: 'formName',
        ...
        {
          title: this.$t('finance.view.search.totalAmount' /*总金额*/),
        ...
        // 凭证编号
        {
          title: this.$t('finance.audit.journalNo'),
        ...
        },
        {
          ...
          dataIndex: 'status',
          width: this.props.language.local === 'zh_cn' ? '8%' : '13%',
          render: (value, record) => {
            let applicationType = '';
        ...
        //状态
        {
          ...
          render: (value, record) => (
            <a onClick={event => this.print(record, event)}>
              {this.$t('finance.view.search.print')}
            </a>
          ),
        },
      ],
  ...
  getList = firstRequest => {
    ...
    financeViewService
      .getFinanceViewList(page, pageSize, searchParams)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            loading: false,
  ...
  search = result => {
    ...
    result.approvalStartDate &&
      (result.approvalStartDate = moment(result.approvalStartDate).format('YYYY-MM-DD 00:00:00'));
    result.approvalEndDate &&
      (result.approvalEndDate = moment(result.approvalEndDate).format('YYYY-MM-DD 23:59:59'));
    result.auditedApprovalStartDate &&
      (result.auditedApprovalStartDate = moment(result.auditedApprovalStartDate).format(
        'YYYY-MM-DD 00:00:00'
      ));
    result.auditedApprovalEndDate &&
      (result.auditedApprovalEndDate = moment(result.auditedApprovalEndDate).format(
        'YYYY-MM-DD 23:59:59'
      ));
  ...
  handleRowClick = record => {
    ...
    if (record.entityType === 1002) {
      ...
      this.props.dispatch(
        routerRedux.replace({
          pathname: `/financial-management/finance-view/expense-report-detail-view/${
            record.entityOid
          }/f_view`,
        })
      );
  ...
  renderAllExpandedRow = record => {
    ...
    if (record.printFree) {
      result.push(
        this.renderExpandedRow(this.$t('common.print.free'), this.$t('common.print.require'))
      );
    }
    if (result.length > 0) {
      return result;
      ...
      <div className="finance-view">
          ...
          extraFields={[
            <div>
              ...
              <CostCenterSearchForm
                title={this.$t('finance.audit.dimension')}
        ...
        <div className="table-header">
          <div className="table-header-buttons">
            <Button type="primary" onClick={this.handleExport} loading={exportLoading}>
              {this.$t('finance.view.search.exportSearchData')}
            </Button>
            {/*导出搜索数据*/}
          </div>
          <div className="table-header-title">
            {this.$t('common.total', { total: pagination.total || 0 })}
          </div>
          {/*共多少条数据*/}
        </div>
        <Table
