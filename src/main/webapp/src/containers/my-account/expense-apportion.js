...
class ExpenseApportion extends React.Component {
  ...
  initColumns = nextProps => {
    ...
    costCenterItemsApportion.length > 0 &&
      (columns.splice(0, 0, costCenterColums), (scrollX += 300));
    //非只读模式下添加操作列
    ...
    if (!value || value.length === 0) {
      ...
      if (expenseReportOid && expenseTypeId && !defaultExpenseApportion && !loading) {
        ...
        expenseReportService.getDefaultApportionment(expenseReportOid, expenseTypeId).then(res => {
          ...
          this.setState(
            { defaultExpenseApportion: res.data[0], loading: false, expenseApportion: res.data },
            () => {
              afterSetState();
              onChange(res.data);
            }
        ...
        this.setState({ expenseApportion: [defaultExpenseApportion] }, () => {
          afterSetState();
      ...
      this.setState({ expenseApportion, defaultExpenseApportion: expenseApportion[0] }, () => {
        afterSetState();
      });
    }
  };
        ...
        let html = (
          <div className={'department'}>
            {costCenterItemsApportion.map((costCenterItem, costCenterIndex) => {
              ...
              record.costCenterItems &&
                record.costCenterItems.map((item, indexCostCenterItems) => {
              ...
              if (isHaveItem) {
                let isDepartment =
                  costCenterItem.costCenterOid === '00000000-0000-0000-0000-000000000000';
                let card = (
                  <InputGroup
                    style={{
                      margin: '5px 0px',
                  ...
                  >
                    <Input
                      ...
                      style={{ width: '30%', borderRight: '0px' }}
                      disabled={true}
                    />
                    {this.departmentCostCenterItem(
                      record,
                      index,
                      costCenterItem,
                      costCenterItemIndex,
      ...
      case 'amount':
        return readOnly ? (
          ...
          <InputNumber
            ...
            formatter={value =>
              typeof value === 'number' && !isNaN(value) ? Number(value.toFixed(2)) : value
            }
          />
        );
      case 'proportion':
        ...
        return isEditable ? (
          <div>
            <a onClick={() => this.handleCopyExpenseApportion(record)}>
              {this.$t('common.copy') /*复制*/}
            </a>
            {expenseApportion.length > 1 && (
              <a onClick={() => this.handleRemoveExpenseApportion(index)} style={{ marginLeft: 5 }}>
    ...
    if (isDepartment) {
      ...
      departmentItem =
        readOnly || !isEditable ? (
          <Input
            defaultValue={
              departmentValue.costCenterItemName ? departmentValue.costCenterItemName : ''
            }
            style={{ borderLeft: '0px', width: '70%' }}
            disabled={true}
          />
        ) : (
          <Chooser
      ...
      chooserItem.key = 'id';
      chooserItem.url = `${config.mdataUrl}/api/dimension/item/page/by/cond?dimensionId=${
      ...
      costCenterItems =
        readOnly || !isEditable ? (
          <Input
            defaultValue={value.dimensionItemName ? value.dimensionItemName : ''}
            style={{ borderLeft: '0px', width: '70%' }}
            disabled={true}
          />
        ) : (
          <Chooser
            ...
            single
            showClear={true}
            onChange={costCenterItem =>
              this.handleChangeCostCenter(costCenterItem, index, costCenterIndex)
            }
  ...
  handelImportCallBack = result => {
    ...
    if (result.success) {
      ...
      let errorDom = (
        <div>
          <div>
            {this.$t('importer.import.allFail', {
              total: result.result.length,
            }) /*导入失败：{total}条*/}
            {
              <a style={{ fontSize: 14, marginLeft: 10 }} onClick={() => this.switchUpload(true)}>
  ...
  render() {
    ...
    return (
      <div className="expense-apportion">
        <FormItem
          {...formItemLayout}
          label={this.$t('expense.apportion') /*费用分摊*/}
        ...
        <Importer
          ...
          templateUrl={`${
            config.baseUrl
          }/api/apportion/template?expenseReportOid=${expenseReportOid}`}
          uploadUrl={`${
            config.baseUrl
          }/api/apportion/import?expenseReportOid=${expenseReportOid}&expenseTypeID=${expenseTypeId}`}
          accept={'.csv'}
