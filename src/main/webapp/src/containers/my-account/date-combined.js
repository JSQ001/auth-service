  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.value) {
      try {
        ...
        let maxDuration = moment(valueWillSet.endDate)
          .utc()
          .diff(moment(valueWillSet.startDate).utc(), 'days');
        this.setState({
          value: valueWillSet,
          dateRange: [moment(valueWillSet.startDate).utc(), moment(valueWillSet.endDate).utc()],
    ...
    return (
      <div>
        <RangePicker
          ...
          disabled={disabled}
          placeholder={[
            this.$t('expense.date.combined.check.in'),
            this.$t('expense.date.combined.check.out'),
          ]}
          format="YYYY-MM-DD"
          style={{ width: '100%' }}
        />
              ...
              {options.map(item => {
                return (
                  <Option key={item} value={item}>
                    {this.$t('expense.date.combined.duration', { number: item })}
                  </Option>
                );
              })}
            </Select>
