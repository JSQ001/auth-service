...
class Invoice extends React.Component {
    ...
    this.state = {
      ...
      columns: [
        ...
        },
        {
          ...
          dataIndex: 'taxPrice',
          render: taxPrice =>
            taxPrice && !isNaN(taxPrice) ? (Number(taxPrice) / 100).toFixed(2) : '-',
        },
      ],
      travelColumns: [
        {
  ...
  changeProps = (invoiceData, isInvoiceFree, invoiceFreePrint) => {
    ...
    invoiceData.invoiceLabels &&
      invoiceData.invoiceLabels.map(label => {
        if (label.type === 'INVOICE_FREE') {
          isInvoiceFree = true;
          !isFinancial && (invoiceFreePrint = false);
        }
      });
    invoiceData.invoiceGoods &&
      invoiceData.invoiceGoods.map((goods, index) => {
        goods.index = index;
      });
  ...
  render() {
    ...
    let electronicFlag =
      invoice.cardsignType === 'APPCARDSIGN' || invoice.cardsignType === 'JSCARDSIGN';
    ...
    let from = null;
    electronicFlag &&
      (from = `${this.$t('expense.invoice.type.wechat') /*微信*/}    ${
        this.$t('expense.invoice.check.code') /*校验码*/
      }：${invoice.checkCode}`);
    ...
    return brief && !canExpense ? (
      <div className="invoice-brief-mode">
        <Row>
          <Col span={6} className="invoice-brief-amount-area">
            {!electronicFlag && (
              <a onClick={() => this.setState({ brief: false })}>
                {this.$t('expense.invoice.view.detail') /*查看详情*/}
              </a>
            )}
            <Row
              className="invoice-brief-amount-detail"
              style={{ marginTop: electronicFlag && 30 }}
            >
              <Col span={14}>{this.$t('expense.invoice.amount.without.tax') /*金额合计*/}</Col>
              <Col span={10}>
                {typeof invoice.feeWithoutTax === 'number'
                  ? (invoice.feeWithoutTax / 100).toFixed(2)
                  : '-'}
              </Col>
              ...
              <Col span={10}>
                {typeof invoice.tax === 'number' ? (invoice.tax / 100).toFixed(2) : '-'}
              </Col>
          ...
          <Col span={18} className="invoice-brief-content-area">
            <div className="invoice-type">
              ...
              {handFlag &&
                !disabledEdit && (
                  <Button icon="edit" type="primary" onClick={handleEdit}>
                    {this.$t('common.update') /*修改*/}
                  </Button>
                )}
              {invoiceFreePrint &&
                (electronicFlag || (alipayFlag && invoice.pdfUrl)) && (
            ...
            <Row className="invoice-brief-content" gutter={10}>
              {(!handFlag || profile['InvoiceControl.InvD.All.display']) && (
                <Col span={12}>
                  ...
                  {invoice.billingTime
                    ? new Date(invoice.billingTime * 1000).format('yyyy-MM-dd')
                    : ''}
                </Col>
              )}
              <Col span={12}>
      ...
      <div className="invoice">
        {invoice.recepitLabel &&
          invoice.recepitLabel.length > 0 && (
        ...
        <div className="invoice-detail-area">
          <Row>
            {(!handFlag || profile['InvoiceControl.InvD.All.display']) &&
              this.renderDetail(
                this.$t('expense.invoice.date') /*开票日期*/,
                (invoice.billingTime ? new Date(invoice.billingTime * 1000) : new Date()).format(
                  'yyyy-MM-dd'
                )
              )}
          ...
          <Row className="invoice-row">
            ...
            <Col span={20} className="invoice-detail">
              ...
              ({this.$t(
                'expense.invoice.amount.without.tax'
              ) /*金额合计*/}：{typeof invoice.feeWithoutTax === 'number'
                ? (invoice.feeWithoutTax / 100).toFixed(2)
                : '-'}{' '}
              {this.$t('expense.invoice.tax') /*税额合计*/}：{typeof invoice.tax === 'number'
          ...
          <Row>
            ...
            {this.renderDetail(
              this.$t('expense.invoice.tax.payer.identity.number') /*纳税人识别号*/,
              invoice.draweeNo,
              24
            )}
            {this.renderDetail(
              this.$t('expense.invoice.address.phone') /*地址/电话*/,
              invoice.draweeAddressPhone,
              24
            )}
            {this.renderDetail(
              this.$t('expense.invoice.opening.bank.account') /*开户行/账户*/,
              invoice.draweeBankAccount,
              24
            )}
          </Row>
          ...
          <Row>
            ...
            {this.renderDetail(
              this.$t('expense.invoice.address.phone') /*地址/电话*/,
              invoice.payeeAddressPhone,
              24
            )}
            {this.renderDetail(
              this.$t('expense.invoice.opening.bank.account') /*开户行/账户*/,
              invoice.payeeBankAccount,
              24
            )}
          </Row>
        </div>
        <div className="invoice-operate">
          {!canExpense && (
            <a onClick={() => this.setState({ brief: true })}>
              {this.$t('expense.invoice.retract.detail') /*收起详情*/}
            </a>
          )}
          {invoiceFreePrint &&
            electronicFlag && <a onClick={this.handlePrint}>{this.$t('common.print') /*打印*/}</a>}
        </div>
      </div>
    );
  }
