...
export default {
  ...
  getExpenseList(page, pageSize) {
    return httpFetch.get(
      `${config.baseUrl}/api/invoices/init/all/by?page=${page}&size=${pageSize}`
    );
  },
  // 打印支付宝发票
  printAlipayInvoice(invoice) {
    ...
    return httpFetch.get(
      `${config.baseUrl}/api/invoice/alipay/pdf`,
      params,
      {},
    ...
    return httpFetch.post(`${config.baseUrl}/api/getCardSign`, params).then(res => {
      if (res.data && res.data.accessToken) {
        ...
        if (!invoice.cardsignType || invoice.cardsignType === 'JSCARDSIGN') {
          url =
            'https://qyapi.weixin.qq.com/cgi-bin/card/invoice/reimburse/getinvoiceinfo?access_token=';
        } else {
  ...
  invoiceTemplateDown(templateId) {
    return httpFetch.get(
      `${config.baseUrl}/api/expense/template/` + templateId,
      {},
      {},
      { responseType: 'arraybuffer' }
    );
  ...
  invoiceTemplateUpload(formData, expenseReportOid, expenseTypeOid) {
    return httpFetch.post(`${config.baseUrl}/api/expense/import`, formData, {
  ...
  updateBusinessCardRemark(cardId, remark) {
    return httpFetch.put(
      `${config.mdataUrl}/api/bankcard/transaction/add/remark/${cardId}?remark=${remark}`
    );
  },
  ...
  getReceiptDisplay(receiptCheckEnabled, invoiceType, tenantId, setOfBookId, companyID) {
    let params = [
      { factorCode: 'TENANT', factorValue: tenantId },
      {
    ...
    return httpFetch.post(
      `${config.baseUrl}/config/api/config/hit/RECEIPT_DISPLAY_CONTROL?tenantId=${tenantId}`,
      params
    );
  },
};
