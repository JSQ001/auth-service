...
class NewExpense extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      mileageAllowanceExpenseColumns: [
        {
          title: this.$t('expense.mileage.depart.info') /*出发信息*/,
          dataIndex: 'start',
          render: (value, record) =>
            this.renderMileageAllowanceExpenseOrder(value, record.departTime),
        },
        {
          title: this.$t('expense.mileage.arrive.info') /*到达信息*/,
          dataIndex: 'end',
          render: (value, record) =>
            this.renderMileageAllowanceExpenseOrder(value, record.arriveTime),
        },
        {
          title: this.$t('expense.mileage') /*里程*/ + '(KM)',
          dataIndex: 'mileage',
          render: (value, record) => (
            <div>
              {this.$t('common.actual') /*实际*/}：{value}
              <br />
  ...
  renderMileageAllowanceExpenseOrder = (value, date) => (
    <div>
      {new Date(date).format('yyyy-MM-dd hh:mm:ss')}
      <br />
      {value.place}
    </div>
  );
  ...
  componentDidMount() {
    ...
    //更换费用录入类型时，重置界面到type
    if (
      this.props.params.expenseSource !== this.state.typeSource &&
      !this.props.params.nowExpense
    ) {
      this.setState({
    ...
    if (this.props.params.nowExpense && this.props.params.nowExpense.expenseTypeId) {
      ...
      baseService.getExpenseTypeById(this.props.params.nowExpense.expenseTypeId).then(res => {
        ...
        let readOnly =
          ...
          (res.data.readonly && res.data.messageKey !== 'private.car.for.public');
        this.setState(
          {
          ...
          () => {
            ...
            if (
              res.data.pasteInvoiceNeeded &&
              expenseDetail.vatInvoice &&
              expenseDetail.digitalInvoice &&
              expenseDetail.digitalInvoice.cardsignType === 'HAND'
            ) {
            ...
            expenseDetail.data.map((field, index) => {
              if (
                field.showOnList &&
                !(expenseDetail.paymentType === 1001 && field.messageKey == 'company.payment.type')
              ) {
                value[field.fieldOid] = this.getFieldValue(
                  field.fieldType,
                  field.value,
                  field.showValue,
                  field
      ...
      let currencyCode = this.props.params.expenseReport
        ? this.props.params.expenseReport.currencyCode
    ...
    return new Promise((resolve, reject) => {
      ...
      if (businessCardConsumptions.length > 0) {
        ...
        if (profile['All.BusinessCardAmount.FeeAmount'] !== 1) {
          if (
            this.checkFunctionProfiles('All.BusinessCardAmount.FeeAmount', [true, 2, 'true']) &&
            values.amount !== businessCard.oriCurAmt
          ) {
            errorMessages.push(
              this.$t('expense.business.card.amount.equal') /*费用金额必须与商务卡消费金额一致*/
            );
          }
          if (
            ...
            errorMessages.push(
              this.$t('expense.business.card.amount.less') /*费用金额必须小于等于商务卡消费金额*/
            );
          }
        }
      }
        ...
        targetFields &&
          targetFields.map(field => {
            if (key === field.fieldOid && field.editable) {
              if (field.fieldType === 'PARTICIPANTS') value = JSON.stringify(value);
              ...
              if (
                ...
                field.list.map(item => {
                  if (item.value === value[0].value) field.valueKey = item.messageKey;
                });
              }
              //如果有值列表，则删除值列表数组
      ...
      if (
        ...
        errorMessages.push(
          this.$t('expense.amount.must.less.than.origin') /*金额不能大于原始金额*/
        );
      }
      ...
      if (!target.vatInvoice) {
        ...
        if (target.invoiceDate) {
          ...
          target.invoiceDate = moment(new Date(digitalInvoice.billingTime * 1000)).format(
            'YYYY-MM-DD'
          );
        }
      ...
      if (target.vatInvoice || (digitalInvoice && digitalInvoice.cardsignType !== 'HAND')) {
        ...
        let priceTaxAmount = !target.priceTaxAmount
          ? digitalInvoice.fee / 100
          : target.priceTaxAmount;

        // 手工录入的发票检测
        if (
          !values.nonVATinclusiveAmount &&
          digitalInvoice &&
          +digitalInvoice.fee < +digitalInvoice.feeWithoutTax &&
          digitalInvoice.cardsignType === 'HAND'
        ) {
          // 税额合计不能大于价税合计
        ...
        if (invoiceControl === 0 || !invoiceControl) {
          target.amount > priceTaxAmount &&
            errorMessages.push(
              this.$t(
                'expense.amount.must.less.than.or.equal.to.invoice.price.and.tax.amount'
              ) /*费用金额必须小于等于发票的价税合计*/
            );
          ...
          target.amount !== priceTaxAmount &&
            errorMessages.push(
              this.$t(
                'expense.amount.must.equal.to.invoice.price.and.tax.amount'
              ) /*费用金额必须等于发票的价税合计*/
            );
        }
      }
      ...
      if (expenseType.apportionEnabled && expenseReport) {
        ...
        expenseApportion.map(apportion => {
          ...
          if (tempApportion.amount === 0 || tempApportion.proportion === 0) {
            amountZeroTip &&
              errorMessages.push(
                this.$t('expense.apportion.amount.can.not.be.zero') /*费用分摊项金额与比例不能为0*/
              );
            amountZeroTip = false;
          }
          if (!tempApportion.costCenterItems || tempApportion.costCenterItems.length === 0) {
            itemNullTip &&
              errorMessages.push(
                this.$t('expense.apportion.item.can.not.be.null') /*费用分摊项不能为空*/
              );
            itemNullTip = false;
          } else {
            ...
            tempApportion.costCenterItems.map(item => {
              if (item.required && !item.costCenterItemOid) {
                /*费用分摊项不能为空*/
                errorMessages.push(
                  `${item.fieldName}:${this.$t('expense.apportion.item.can.not.be.null')}`
                );
                itemNullTip = false;
              }
          ...
          if (!tempApportion.personName) {
            personNullTip &&
              errorMessages.push(
                this.$t('expense.apportion.person.can.not.be.null') /*费用分摊相关人不能为空*/
              );
            personNullTip = false;
          }
        ...
        if (expenseApportionProportion !== 10000) {
          errorMessages.push(
            this.$t(
              'expense.apportion.amount.must.be.equal.to.expense.proportion'
            ) /*分摊比率和需等于100%*/
          );
        }
      ...
      if (
        ...
        if (allowanceControl === 3) {
          ...
          errorMessages.push(
            this.$t('expense.amount.must.equal.to.allowance') /*费用金额必须等于补贴金额*/
          );
        } else if ((!allowanceControl || allowanceControl === 1) && orderAmount < target.amount) {
          errorMessages.push(
            this.$t('expense.amount.can.not.more.than.allowance') /*费用金额不能大于补贴金额*/
          );
        }
      }
      if (this.isRequiredFile() && attachments.length === 0) {
        ...
        errorMessages.push(
          this.$t('expense.please.upload.attachments.for.auditing') /*请上传图片以便后续审核*/
        );
      }
      // 手输的发票 cardSigntype = HAND
      target.vatInvoice &&
        (!digitalInvoice || (digitalInvoice && digitalInvoice.cardsignType === 'HAND')) &&
  ...
  switchField = field => {
    switch (field.fieldType) {
      ...
      case 'DOUBLE':
        ...
        if (this.state.unitPriceMode && field.messageKey === 'unit.price') {
          return (
            <CustomAmount
              style={{ width: '100%' }}
              disabled={this.checkFunctionProfiles('unit.price.modify.enable', [false, undefined])}
        ...
        return (
          <CustomAmount
            style={{ width: '100%' }}
            disabled={!field.editable || field.messageKey === 'unit.price'}
          />
        );
      case 'TEXT':
        if (field.customEnumerationOid) {
          return field.list ? (
            <Searcher
              ...
              searcherItem={{
                ...
                url: `${config.baseUrl}/api/custom/enumerations/${
                  field.customEnumerationOid
                }/items/by/user`,
                key: 'value',
              }}
              listExtraParams={{
      ...
      case 'DATE':
        return (
          <DatePicker disabled={!field.editable} getCalendarContainer={this.getPopupContainer} />
        );
      case 'MONTH':
        ...
        .then(res => {
          if (res.data && res.data.rows && res.data.rows.length > 0 && res.data.rows[0].hitValue) {
            ...
            this.setState(
              ...
              () => {
                ((digitalInvoice && invoiceTypeNo) || !digitalInvoice) &&
                  this.checkInvoiceRender('taxRate') &&
                  type &&
                  expenseService.getRateByInvoiceType(type).then(res => {
  ...
  handleCreateInvoice = invoice => {
    ...
    let transInvoice = () => {
      this.setState(
        { nowPage: 'form', digitalInvoice: invoice, editingInvoice: editingInvoice },
        () => {
          if (editingInvoice) {
  ...
  handleSelectBusinessCardConsumptions = result => {
    this.setState(
      {
        businessCardConsumptions: result,
        nowPage: 'form',
        nowBusinessCardConsumptionIndex: 0,
        fromExpense: false,
      },
  ...
  deleteBusinessCardConsumption = () => {
    ...
    if (nowBusinessCardConsumptionIndex + 1 === businessCardConsumptions.length) {
      ...
      this.props.form.resetFields();
      this.setState(
        { nowBusinessCardConsumptionIndex: nowBusinessCardConsumptionIndex + 1, expenseType: {} },
        this.setValuesByBusinessCard
      );
    }
  };
  ...
  getFieldValue = (type, value, name, field) => {
    ...
    if (
      value &&
      ((field && field.messageKey === 'dateCombined') || type === 'START_DATE_AND_END_DATE')
    ) {
      return value.replace(/T[0-9:-]{8}Z/gim, 'T12:00:00Z');
    }
    if (type === 'DATETIME' || type === 'DATE') return value ? moment(value) : null;
  ...
  getFieldName = (type, value, messageKey) => {
    if (type === 'TEXT' && messageKey === 'dateCombined') {
      if (value && JSON.parse(value)) {
        ...
        return `${new Date(result.startDate).format('yyyy-MM-dd')} ～ ${new Date(
          result.endDate
        ).format('yyyy-MM-dd')}`;
      } else {
        return '-';
    ...
    if (type === 'DATE') return value ? new Date(value).format('yyyy-MM-dd') : '-';
    if (type === 'MONTH') return value ? new Date(value).format('yyyy-MM') : '-';
    if (type === 'DATETIME') return value ? new Date(value).format('yyyy-MM-dd hh:mm:ss') : '-';
    if (type === 'GPS') return value ? JSON.parse(value).address : '-';
    if (type === 'PARTICIPANT') return value ? JSON.parse(value).fullName || '-' : '-';
    if (type === 'PARTICIPANTS') {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      if (!err) {
        ...
        if (amountIsNotChangeBig && values.amount > nowExpense.originalAmount) {
          message.error(
            this.$t(
              'expense.audited.amount.can.not.be.more.than.origin.amount'
            ) /*核定金额不能大于原金额*/
          );
          return;
        }
        if (values.actualCurrencyRate > nowExpense.originalActualCurrencyRate) {
          message.error(
            this.$t(
              'expense.audited.rate.can.not.be.more.than.origin.rate'
            ) /*核定汇率不能大于原汇率*/
          );
          return;
        }
        if (
          !values.originalApprovedNonVat &&
          isNonVat &&
          !this.props.profile['All.FeeAmount.AllowZero']
        ) {
        ...
        if (
          nowExpense.digitalInvoice &&
          nowExpense.digitalInvoice.fee &&
          values.amount > nowExpense.digitalInvoice.fee / 100
        ) {
          message.error(
            this.$t(
              'expense.amount.must.less.than.or.equal.to.invoice.price.and.tax.amount'
  ...
  handleSaveInvoice = () => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      if (!isError) {
        ...
        target.billingCode = values.invoiceCode
          ? values.invoiceCode.toUpperCase()
          : values.invoiceCode;
        target.checkCode = values.checkCode;
        let invoiceControl = invoiceUserFp['InvoiceControl.InvoiceAmount.FeeAmount.ALL.Equal'];
        if (
          (invoiceControl === 0 || !invoiceControl) &&
          values.priceTaxAmount < nowExpense.amount
        ) {
          message.error(
            this.$t(
              'expense.amount.must.less.than.or.equal.to.invoice.price.and.tax.amount'
          ...
          (invoiceControl === 2 || invoiceControl === true) &&
          values.priceTaxAmount !== nowExpense.amount
        ) {
          message.error(
            this.$t(
...
* */
  setInvoiceData = (updateInvoiceType = true, isCoverFormItem = false) => {
    ...
    if (digitalInvoice) {
      ...
      if (digitalInvoice.billingTime)
        target.invoiceDate = moment(
          digitalInvoice.billingTime ? new Date(digitalInvoice.billingTime * 1000) : new Date()
        );
  ...
  checkInvoiceRender = (itemName, type = 1) => {
    ...
    if (type === 1 && nowExpense && nowExpense.invoiceOid && !showAttr) {
      if (
        !(recordTaxRateConfig && itemName === 'taxRate') &&
        !(recordNonVATinclusiveAmountConfig && itemName === 'nonVATinclusiveAmount') &&
        !(recordTaxAmountConfig && itemName === 'taxAmount')
      ) {
      ...
      <div className="vat-invoice-area">
        ...
        >
          <div className="invoice-info">
            {this.$t('expense.invoice.enter.info') /*发票填写说明*/}
          </div>
        </Popover>
        {companyOpenInvoice && (
          <div className="test-info">
            {/*当发票类型为 {type}时，请选择*/}
            {this.$t('expense.invoice.checked.text', {
              type: testInvoiceTypes.map(type => type.invoiceTypeName).join(' , '),
            })}
            <a
              onClick={() =>
                this.setState({ nowPage: 'type', typeSource: 'invoice', fromExpense: true })
              }
            >
              {/*发票查验*/}
        ...
        <Row gutter={20} type="flex" align="top">
          ...
          {invoiceTypeNoValue ? (
            <Row gutter={20} type="flex" align="top">
              {this.checkInvoiceRender(invoiceCode) && (
              ...
              {this.checkInvoiceRender(invoiceNumber) && (
                <Col span={12}>
              ...
              {(recordTaxRateConfig || this.checkInvoiceRender(taxRate)) && (
                <Col span={12}>
              ...
              {this.checkInvoiceRender(vatInvoiceCurrencyCode) && (
                <Col span={12}>
                  <FormItem label={this.$t('common.currency') /*币种*/}>
                    {getFieldDecorator(vatInvoiceCurrencyCode, {
                      rules: [{ required: this.checkInvoiceRender(vatInvoiceCurrencyCode, 2) }],
                    })(
                      <Select
                        dropdownMatchSelectWidth={false}
                        showSearch={true}
              ...
              {(recordTaxAmountConfig || this.checkInvoiceRender(taxAmount)) && (
                <Col span={12}>
                      ...
                      rules: [
                        {
                          ...
                          validator: (rule, value, callback) => {
                            if (value > priceTaxAmountValue)
                              ...
                              if (
                                (recordTaxAmountConfig || this.checkInvoiceRender(taxAmount, 2)) &&
                ...
                <Col span={12}>
                  <FormItem label={this.$t('expense.invoice.amount.without.tax') /*金额合计*/}>
                    {getFieldDecorator(nonVATinclusiveAmount, {
                      rules: [
                        {
                          required:
                            recordNonVATinclusiveAmountConfig ||
                            this.checkInvoiceRender(nonVATinclusiveAmount, 2),
                          validator: (rule, value, callback) => {
                            if (value > priceTaxAmountValue)
                              // 金额合计不能大于价税合计
                      ...
                      <CustomAmount
                        style={{ width: '100%' }}
                        placeholder={this.$t('expense.invoice.amount.help') /*请输入不含税金额*/}
                        //precision={2} step={0.01} min={0}
                ...
                <Col span={12}>
                  <FormItem label={this.$t('expense.invoice.check.code') /*校验码*/}>
                    {getFieldDecorator(checkCode, {
                      ...
                      <Input
                        placeholder={
                          this.$t('expense.invoice.check.code.help') /*请输入校验码后6位*/
                        }
                        maxLength="6"
                      />
                    )}
              ...
              {(audit || auditCapability) && (
                <Col span={24} style={{ margin: '10px 0 20px', textAlign: 'right' }}>
                  <FormItem>
                    ...
                    <Button onClick={() => this.setState({ editingInvoice: false })}>
  ...
  handleChangeVatInvoice = checked => {
    this.setState({ editingInvoice: checked }, () => {
      ...
      if (
        checked &&
        ((digitalInvoice && digitalInvoice.cardsignType === 'HAND') || !digitalInvoice)
      ) {
        this.setInvoiceData();
      }
    });
  ...
  handleToTalMoneyChange = e => {
    const { nowExpense, isNonVat } = this.state;
    if (isNonVat) {
      let originalApprovedNonVat = nowExpense.taxRate
        ? e / (1 + nowExpense.taxRate)
        : (e * 100) / (100 + parseFloat(nowExpense.digitalInvoice.invoiceGoods[0].taxRate));
      ...
      this.props.form.setFieldsValue({
        ...
        originalApprovedNonVat: originalApprovedNonVat,
      });
      !e &&
        this.props.form.setFieldsValue({ originalApprovedVat: 0.0, originalApprovedNonVat: 0.0 });
    }
    this.reRender();
  };
  // 标签显示信息
  ...
  changeOriginalApprovedNonVat = e => {
    ...
    if (typeof e == 'number') {
      if (!this.props.profile['All.FeeAmount.AllowZero'] && !e) {
        ...
        this.props.form.setFieldsValue({ originalApprovedVat: 0.0 });
      } else {
  ...
  renderInvoiceEditingArea = () => {
    ...
    let hasInvoiceInstead =
      !profile['invoice.instead.disabled'] && (readOnly ? thirdWithReceipt : pasteInvoiceNeeded);
    let invoiceInsteadText = profile['invoice.instead.comments']
      ? this.$t('expense.is.remark') /*是否备注*/
      : this.$t('expense.is.instead.invoice') /*是否替票*/;
    let invoiceInsteadReasonText = profile['invoice.instead.comments']
      ? this.$t('expense.remark.reason') /*备注理由*/
      : this.$t('expense.instead.invoice.reason') /*替票理由*/;
    let isHandDigitalInvoice = digitalInvoice && digitalInvoice.cardsignType === 'HAND';
    let invoiceInstead =
      hasInvoiceInstead &&
    ...
    let thirdEditInvoice =
      !this.props.readOnly && expenseType.readonly && !isDiDi && thirdWithReceipt;
    return (
      <div>
        {hasInvoiceInstead && (
          <FormItem {...formItemLayout} label={invoiceInsteadText}>
            {getFieldDecorator('invoiceInstead', {
              ...
              <Switch
                onChange={checked =>
                  !checked && this.props.form.setFieldsValue({ invoiceInsteadReason: '' })
                }
              />
            )}
          </FormItem>
        ...
        {pasteInvoiceNeeded &&
          (digitalInvoice ? isHandDigitalInvoice : true) &&
          !invoiceUserFp['account.book.VAT.special.invoice.disabled'] && (
            <FormItem {...formItemLayout} label={this.$t('expense.enter.invoice') /*录入发票*/}>
              {getFieldDecorator('vatInvoice', {
                ...
                <Switch
                  onChange={this.handleChangeVatInvoice}
                  disabled={
                    (digitalInvoice && digitalInvoice.cardsignType !== 'HAND') ||
  ...
  deleteAttachment = attachmentId => {
    ...
    if (required && attachments.length === 1) {
      message.error(
        this.$t('expense.fileUpload.delete') /*当前页面须至少上传1个附件，请上传其他附件后再删除*/
      );
      return false;
    } else {
  ...
  renderMileageForm = () => {
    ...
    let referentPriceFileText =
      currencyValue && referentPriceFileValue
        ? `${referentPriceFile.name}: ${currencyValue} ${referentPriceFileValue}`
        : null;
    // 里程 file
    let mileageFile = expenseType.fields.filter(field => field.messageKey === 'ER_KM')[0] || {};
    // 参考里程 file
    let referenceMileageFile =
      expenseType.fields.filter(field => field.messageKey === 'reference.mileage')[0] || {};
    // 参考里程文案 需兼容老数据 不存在参考里程值时不显示提示文案
    let referenceMileageFileValue = this.getFieldValue(
    ...
    let referenceMileageFileText = referenceMileageFileValue
      ? `${referenceMileageFile.name}: ${referenceMileageFileValue} KM`
      : null;
    // 单价 里程
    let renderFiles = [
      ...
      <div>
        {renderFiles.map(renderFile => (
          <FormItem {...formItemLayout} label={renderFile.name} key={`${renderFile.fieldOid}`}>
            <Col span={10}>
              <FormItem>
                {getFieldDecorator(`${renderFile.fieldOid}`, {
                  ...
                  initialValue: this.getFieldValue(
                    renderFile.fieldType,
                    renderFile.value,
                    renderFile.showValue,
                    renderFile
            ...
            {renderFile.text && (
              <Col span={12}>
                <Alert message={renderFile.text} key={renderFile.fieldOid} type={'warning'} />
              </Col>
            )}
          </FormItem>
        ))}
  ...
  getRateDeviation = () => {
    baseService
      .getRateDeviation(this.props.company.tenantId, this.props.company.setOfBooksId)
      .then(res => {
        this.setState({
  ...
  render() {
    ...
    let amount =
      readOnly && !auditAmountEditing ? nowExpense.amount : Number(getFieldValue('amount'));
    let actualCurrencyRate = !auditAmountEditing
      ? nowExpense.actualCurrencyRate
      : getFieldValue('actualCurrencyRate');
    ...
    if (showRateDescription) {
      ...
      rateDescription +=
        rateDeviation + `%，${this.$t('common.base.currency.amount') /*本位币金额*/}：`;
    ...
    let invoiceInstead =
      ...
      (readOnly && this.props.readOnly
        ...
        : getFieldValue('invoiceInstead'));
    let vatInvoice =
      readOnly && this.props.readOnly ? nowExpense.vatInvoice : getFieldValue('vatInvoice');
    let isHandDigitalInvoice = digitalInvoice && digitalInvoice.cardsignType === 'HAND';
    let hasExpenseApportion = readOnly
      ? nowExpense.apportionUsed && expenseReport
      : expenseType.apportionEnabled && expenseReport;
    const nowBusinessCardConsumption = businessCardConsumptions[nowBusinessCardConsumptionIndex];
    let prendingAuditOperateAuth =
      expenseReport &&
      (expenseReport.status === 1003 || expenseReport.status === 1002) &&
      !nowExpense.valid &&
      ((audit && ~location.search.indexOf('prending_audit')) || auditCapability) &&
    ...
    let hasEditedAmount =
      ...
      (nowExpense.actualCurrencyRate &&
        nowExpense.actualCurrencyRate !== nowExpense.originalActualCurrencyRate);
    let mileageAllowanceExpenseDTO = nowExpense.mileageAllowanceExpenseDTO;
    let thirdEditAmount =
      !this.props.readOnly && expenseType.readonly && expenseType.isAmountEditable;
    ...
    let currencyCodeDisabled =
      ...
      (expenseReport &&
        ...
        this.checkFunctionProfiles('web.invoice.keep.consistent.with.expense', [true]));
    //审批历史记录
    let approvals = (
        ...
        <div className="approvals" style={{ marginLeft: 10 }}>
          {approvalHistory &&
            approvalHistory.length > 0 && (
              <Tag
                color="blue-inverse"
                style={{ marginBottom: 20, fontSize: 14, cursor: 'default' }}
              >
                {this.$t('expense.approval.history') /*审批历史*/}
              </Tag>
            )}
          <Timeline>
            {approvalHistory &&
              approvalHistory.map(item => {
                ...
                return (
                  <Timeline.Item
                    ...
                    dot={
                      <Icon
                        type={getApprovelHistory(item.operate).icon}
                        style={{ color: getApprovelHistory(item.operate).color }}
                      />
                    }
                  >
                    <Row>
                      ...
                      <Col span={4} className="operation-type">
                        {this.$t(getApprovelHistory(item.operate).text) || '-'}
                      </Col>
                      <Col span={5} className="operation-name">
                        {item.role ? `${item.role} ${<span className="ant-divider" />} ` : ''}{' '}
                        {item.operator.fullName +
                          ' ' +
                          (item.operator.employeeID ? item.operator.employeeID : '')}
                      </Col>
                      <Col span={7} className="operation-remark" style={{ color: 'red' }}>
                        {operateDetail[0]}
                      </Col>
                    </Row>
                  </Timeline.Item>
        ...
        {thirdWithReceipt &&
          ...
          !invoiceUserFp['account.book.VAT.special.invoice.disabled'] && (
            <FormItem
              ...
              onChange={checked => this.setState({ editingInvoice: checked })}
            >
              {nowExpense.vatInvoice ? this.$t('common.yes') : this.$t('common.no')}
              {!digitalInvoice &&
                prendingAuditOperateAuth && (
                  <div style={{ fontSize: 12 }}>
                    <Icon type="edit" />{' '}
                    <a onClick={this.handleEditInvoice}>
                      {this.$t('expense.invoice.edit') /*修改发票*/}
                    </a>
                  </div>
                )}
            </FormItem>
    ...
    let originNonVat =
      nowExpense.originalApprovedNonVat != null
        ? this.filterMoney(nowExpense.originalApprovedNonVat)
        : this.filterMoney(nowExpense.originalAmount);
    let originVat = nowExpense.originalApprovedVat
      ? this.filterMoney(nowExpense.originalApprovedVat)
      : 0.0;
      ...
      <Animate showProp="show" transitionName="expense-fade">
        <DivExpense show={showExpenseDom}>
          <div className="new-expense">
                  ...
                  >
                    {nowExpense &&
                      ...
                      nowExpense.invoiceLabels.length > 0 && (
                        <div className="tip-wrap">
                          ...
                          {nowExpense.invoiceLabels.filter(item => item.level === 'INFO').length >
                            0 && (
                            <Alert
                    ...
                    <Form onSubmit={this.handleSave}>
                      <Spin spinning={loading}>
                        {/* 卡片显示录入发票信息 */}
                        {digitalInvoice &&
                          (isHandDigitalInvoice ? vatInvoice : true) &&
                          !editingInvoice && (
                            <Invoice
                              invoice={digitalInvoice}
                              disabledEdit={
                                readOnly && !thirdEditInvoice && !(audit || auditCapability)
                              }
                        ...
                        <div className="expense-type-container">
                              ...
                              <Row>
                                <Col span={6} className="card-amount-area">
                                  <span className="card-name">
                                    {this.$t('expense.business.card.consumption') /*商务卡消费*/}
                                  </span>
                                  <br />
                                  <span className="card-currency">
                                    {this.$t('expense.account.amount') /*入账金额*/}
                                  </span>
                                  <br />
                                  <span className="card-amount">
                                    {nowBusinessCardConsumption.posCurCod}&nbsp;{nowBusinessCardConsumption.posCurAmt.toFixed(
                                      2
                                    )}
                                  </span>
                                  <br />
                                  <span className="card-origin-amount">
                                    {this.$t('expense.transaction.amount') /*交易金额*/}&nbsp;{
                                      nowBusinessCardConsumption.oriCurCod
                                    }&nbsp;{nowBusinessCardConsumption.oriCurAmt.toFixed(2)}
                                  </span>
                                </Col>
                                <Col span={18} className="card-detail-area">
                                  ...
                                  <Row className="card-detail" gutter={10}>
                                    <Col span={12}>
                                      <span>
                                        {this.$t('expense.transaction.date') /*交易时间*/}：
                                      </span>&nbsp;
                                      {nowBusinessCardConsumption.trsDate}&nbsp;{this.formatTime(
                                        nowBusinessCardConsumption.trxTim
                                      )}
                                    </Col>
                                    <Col span={12}>
                                      <span>{this.$t('expense.bill.month') /*账单月*/}：</span>&nbsp;
                                      {nowBusinessCardConsumption.bilMon}
                                    </Col>
                                    <Col span={12}>
                                      <span>
                                        {this.$t('expense.transaction.card') /*交易卡号*/}：
                                      </span>&nbsp;
                                      {nowBusinessCardConsumption.bankName}尾号&nbsp;{(
                                        nowBusinessCardConsumption.crdNum + '0'
                                      ).slice(-5, -1)}
                                    </Col>
                                    <Col span={12}>
                                      <span>
                                        {this.$t('expense.transaction.type') /*交易类型*/}：
                                      </span>
                                      {this.getConsumptionType(nowBusinessCardConsumption.trsCod)}
                                    </Col>
                                    {readOnly && (
                                      <Col span={24}>
                                  ...
                                  {!readOnly &&
                                    getFieldDecorator('businessCardRemark', {
                                      initialValue: nowBusinessCardConsumption.remark || '',
                                    })(
                                      <Input
                                        placeholder={
                                          this.$t(
                                            'expense.please.enter.remark.not.required'
                                          ) /*请输入备注，非必填*/
                                        }
                                        maxLength="200"
                                        onBlur={this.handleSaveBusinessCardRemark}
                                      />
                                    )}
                                </Col>
                              </Row>
                          ...
                          {!this.props.readOnly &&
                            ...
                            businessCardConsumptions.length === 0 && (
                              ...
                              >
                                <Icon type="plus" />
                                {this.$t('expense.add.business.card') /*新增商务卡*/}
                              </div>
                            )}
                          ...
                          {readOnly ? (
                            <div className="expense-read-only">
                              <FormItem
                                {...formItemLayout}
                                label={
                                  <img className="expense-type-img" src={expenseType.iconURL} />
                                }
                                colon={false}
                              ...
                              >
                                ...
                                <div className="expense-type-amount">
                                  ...
                                  <b>
                                    {thirdEditAmount
                                      ? getFieldDecorator('invoiceCurrencyCode', {
                                          ...
                                          <Select
                                            ...
                                            placeholder={
                                              this.$t('common.please.select') /* 请选择 */
                                            }
                                          >
                                            {currencyList.map(item => {
                                              return (
                                    ...
                                    &nbsp;&nbsp;
                                    {auditAmountEditing || thirdEditAmount
                                  ...
                                  {!auditAmountEditing &&
                                    prendingAuditOperateAuth && (
                                      ...
                                      >
                                        {this.$t('expense.audited.amount') /*核定金额*/}
                                      </a>
                                    )}
                                </div>
                                {showRateDescription &&
                                  ...
                                  !auditAmountEditing && (
                                    <div className="expense-rate-description">
                                      {this.$t('expense.actual.rate') /*实际汇率*/}:{' '}
                                      {actualCurrencyRate.toFixed(4)} <br />
                                      {rateDescription}
                                    </div>
                                  )}
                                {auditAmountEditing &&
                                    ...
                                    <div>
                                      <Row style={{ marginTop: '10px' }}>
                                        <span style={{ float: 'left' }}>
                                          {this.$t('expense.origin.approve.amount')}:
                                        </span>
                                      ...
                                      <Row style={{ marginTop: '10px' }}>
                                        <span style={{ float: 'left' }}>
                                          {this.$t('expense.origin.approve.rate')}:
                                        </span>
                                        <Col span={10} style={{ marginLeft: '10px' }}>
                                ...
                                {(hasEditedAmount || auditAmountEditing) && (
                                  <Tag>
                                    {this.$t('expense.origin.amount') /*原金额*/}：{this.filterMoney(
                                      nowExpense.originalAmount
                                    )}&nbsp;&nbsp;
                                    {showRateDescription &&
                                      !isBaseCurrency &&
                                      `${
                                        this.$t('expense.origin.rate') /*原汇率*/
                                      }：${this.filterMoney(
                                ...
                                {digitalInvoice && (
                                  <Tag>
                                    {this.$t('expense.origin.approve.amount') /*原币金额*/}：{
                                      originNonVat
                                    }{' '}
                                    &nbsp;&nbsp;
                                    {this.$t('expense.origin.approve.rate') /*原税税额*/}：{
                                      originVat
                                    }
                                  </Tag>
                                )}
                                {auditAmountEditing && (
                                  <div>
                                    ...
                                    <Button
                                      onClick={() => this.setState({ auditAmountEditing: false })}
                                    >
                                      {this.$t('common.cancel')}
                                    </Button>
                                  </div>
                              ...
                              {this.checkFunctionProfiles('web.invoice.pay.by.company.disabled', [
                                false,
                                undefined,
                              ]) && (
                                <FormItem
                                  {...formItemLayout}
                                  label={this.$t('expense.company.pay') /*公司支付*/}
                                >
                                  {nowExpense.paymentType === 1002
                                    ? this.$t('common.yes')
                                    : this.$t('common.no')}
                                </FormItem>
                              )}
                                ...
                                nowExpense.data.map(
                                  field =>
                                    this.checkFunctionProfiles(
                                      ['fweb.invoice.pay.by.company.disabled'],
                                      [[false, undefined]]
                                    ) &&
                                    field.showOnList &&
                                    nowExpense.paymentType === 1002 &&
                                ...
                                nowExpense.data.map(
                                  field =>
                                    ...
                                    (!unitPriceMode ||
                                      (!nowExpense.mileageAllowanceExpenseDTO &&
                                        unitPriceMode &&
                                        !~mileageMessageKey.indexOf(field.messageKey))) && (
                                      <FormItem
                                        {...formItemLayout}
                                        label={field.name}
                                        key={`${field.fieldOid}`}
                                ...
                                >
                                  {getFieldDecorator('comment', {
                                    ...
                                    <TextArea
                                      ...
                                      style={{ width: '100%' }}
                                      maxLength="200"
                                      placeholder={`${this.$t('common.please.enter')},${this.$t(
                            ...
                            <div>
                              ...
                              <FormItem
                                {...formItemLayout}
                                label={this.$t('common.happened.date') /*发生日期*/}
                                required
                              >
                                {getFieldDecorator('createdDate', {
                                  rules: [
                                  ...
                                  initialValue: digitalInvoice
                                    ? digitalInvoice.billingTime
                                      ? moment(new Date(digitalInvoice.billingTime * 1000))
                                      : moment(new Date())
                                    : moment(new Date()),
                                })(
                                  <DatePicker
                                    format="YYYY-MM-DD"
                                    disabled={isSubsidyType}
                                    allowClear={false}
                                    getCalendarContainer={this.getPopupContainer}
                                    style={{ width: '100%' }}
                                  />
                                    ...
                                    {getFieldDecorator('amount', {
                                      ...
                                      <CustomAmount
                                        ...
                                        disabled={
                                          (expenseType.messageKey === 'private.car.for.public' &&
                                            unitPriceMode) ||
                                          (isSubsidyType && amountEditConfigSubsidyType === 1) ||
                                          expenseType.valid
                                        }
                                      />
                                    )}
                              ...
                              {expenseType.valid && (
                                <FormItem
                                  ...
                                  label={
                                    `${this.$t('common.price')}${
                                      expenseType.unit
                                        ? `/${this.$t(`expense.invoice.unit.${expenseType.unit}`)}`
                                        : ''
                                    }` /*单价*/
                                  }
                                  required
                              ...
                              {!isBaseCurrency && (
                                <FormItem
                                  {...formItemLayout}
                                  label={this.$t('common.currency.rate') /*汇率*/}
                                >
                                  <Col span={10}>
                                    <FormItem>
                                      {getFieldDecorator('actualCurrencyRate', {
                                        initialValue:
                                          this.props.params.nowExpense !== null
                                            ? this.props.params.nowExpense.actualCurrencyRate
                                            : 1.0,
                                      })(
                                        <CustomAmount
                                          style={{ width: '100%' }}
                              ...
                              {/*录入发票表单*/}
                              {vatInvoice && (editingInvoice || !digitalInvoice)
                          ...
                          {mileageAllowanceExpenseDTO && (
                            <div className="expense-mileage-allowance">
                              <FormItem
                                {...formItemLayout}
                                label={this.$t('expense.total.mileage') /*累计里程*/}
                              >
                                {mileageAllowanceExpenseDTO.mileage}KM
                                <span className="reference-mileage">
                                  {this.$t('expense.reference.mileage') /*参考里程*/}：&nbsp;{
                                    mileageAllowanceExpenseDTO.referenceMileage
                                  }KM
                                </span>
                              </FormItem>
                              <FormItem
                                {...formItemLayout}
                                label={this.$t('expense.total.allowance') /*补贴总额*/}
                              >
                                {mileageAllowanceExpenseDTO.currency}&nbsp;&nbsp;{mileageAllowanceExpenseDTO.referenceAmount.toFixed(
                                  2
                                )}
                                <Popover
                                  placement="top"
                                  ...
                                  content={
                                    <Table
                                      columns={[
                                        {
                                          title: this.$t('expense.mileage') /*里程*/,
                                          dataIndex: 'startUnit',
                                          render: (startUnit, record) =>
                                            `${startUnit}${
                                              record.endUnit === -1
                                                ? this.$t('expense.mileage.over') /*以上*/
                                                : '~' + record.endUnit
                                            }`,
                                        },
                                        {
                                          title: `${this.$t('common.price') /*单价*/}/KM ${
                                            mileageAllowanceExpenseDTO.currency
                                          }`,
                                          dataIndex: 'unitPrice',
                                        },
                                        {
                                ...
                                >
                                  <a style={{ marginLeft: 10 }}>
                                    {this.$t('expense.mileage.view.amount.detail') /*查看计价明细*/}
                                  </a>
                                </Popover>
                              </FormItem>
                              <FormItem
                                {...formItemLayout}
                                label={this.$t('expense.mileage.detail') /*里程明细*/}
                                style={{ marginBottom: 12 }}
                              />
                              <Table
                                size="small"
                                    ...
                                    Modal.info({
                                      ...
                                      content: (
                                        <div className="order-content">
                                          <Row gutter={20}>
                                            <Col span={8}>
                                              {this.$t('expense.mileage.depart.time') /*上车时间*/}
                                            </Col>
                                            <Col span={16}>
                                              {moment(record.departTime)
                                                .utc()
                                                .format('YYYY-MM-DD HH:mm:ss')}
                                            </Col>
                                            <Col span={8}>
                                              {this.$t('expense.mileage.depart.place') /*上车地点*/}
                                            </Col>
                                            <Col span={16}>{record.start.place}</Col>
                                            <Col span={8}>
                                              {this.$t('expense.mileage.arrive.time') /*下车时间*/}
                                            </Col>
                                            <Col span={16}>
                                              {moment(record.arriveTime)
                                                .utc()
                                                .format('YYYY-MM-DD HH:mm:ss')}
                                            </Col>
                                            <Col span={8}>
                                              {this.$t('expense.mileage.arrive.place') /*下车地点*/}
                                            </Col>
                                            <Col span={16}>{record.end.place}</Col>
                                            <Col span={8}>
                                              {this.$t('expense.actual.mileage') /*实际里程*/}
                                            </Col>
            ...
            {nowPage === 'form' && (
              <div className="footer-operate">
                ...
                <Button onClick={this.onCancel}>
                  {this.$t(readOnly ? 'common.back' : 'common.cancel')}
                </Button>
                {expenseReport &&
                  nowExpense.invoiceOid &&
                  readOnly && (
                    <div className="footer-page">
                      <Button
                        ...
                        onClick={() =>
                          this.setState({ showExpenseDom: !showExpenseDom }, () => {
            ...
            {(audit || view || pay || auditCapability) &&
              expenseReport.expenseReportInvoices && (
                <ImageAudit
                  visible={showImageAudit}
