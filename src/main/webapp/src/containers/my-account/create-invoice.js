      ...
      <div className="create-invoice">
        <Steps current={currentStep} progressDot>
          ...
          <Step
            title={
              this.$t(
                createType === 1 ? 'expense.create' : 'expense.invoice.create'
              ) /*生成费用/生成发票*/
            }
          />
        </Steps>
          ...
          <div style={{ marginTop: 24 }}>
            <Form className="create-invoice" onSubmit={this.handleSubmit}>
                ...
                {getFieldDecorator('invoiceTypeNo', {
                  ...
                  >
                    {invoiceTypes.data.map(item => {
                      return (
                        <Option key={item.invoiceTypeNo} value={item.invoiceTypeNo}>
                  ...
                  rules: [
                    ...
                    },
                    {
                      ...
                      validator: (rule, value, callback) => {
                        if (!value || (value.length !== 10 && value.length !== 12)) callback(true);
                        else callback();
                      },
                    },
                  ],
              ...
              {(invoiceTypeNo === '04' || invoiceTypeNo === '10' || invoiceTypeNo === '11') && (
                ...
                >
                  {getFieldDecorator('checkCode', {
                    ...
                    <Input
                      placeholder={this.$t('expense.invoice.check.code.help') /*请输入校验码后6位*/}
                      maxLength="6"
                    />
                  )}
                </FormItem>
              ...
              {(invoiceTypeNo === '01' || invoiceTypeNo === '03') && (
                <FormItem
                  {...formItemLayout}
                  label={this.$t('expense.invoice.amount') /*发票金额*/}
                >
          ...
          <div style={{ paddingTop: 20 }}>
            ...
            {canSubmit && (
              <div style={{ textAlign: 'center', marginBottom: 30 }}>
                <a onClick={() => onCreate(invoice)} style={{ fontSize: 16 }}>
                  {this.$t(createType === 1 ? 'expense.create' : 'common.save') /*生成费用*/}&nbsp;&nbsp;<Icon type="right-circle-o" />
                </a>
              </div>
            )}
            {!canSubmit && (
              <div style={{ textAlign: 'center', marginBottom: 30 }}>
                <a onClick={() => this.setState({ currentStep: 0 })} style={{ fontSize: 16 }}>
                  <Icon type="left-circle-o" />&nbsp;&nbsp;{this.$t('common.back') /*返回*/}
                </a>
              </div>
            )}
          </div>
