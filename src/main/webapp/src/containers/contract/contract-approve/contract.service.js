...
export default {
  ...
  getContractTypeByCompany(params) {
    return httpFetch.get(`${config.contractUrl}/api/contract/type/contract/type/by/user`, params);
  },
  ...
  getContractHistory(id) {
    return httpFetch.get(
      `${config.workflowUrl}/api/workflow/approval/history?entityType=801004&entityOid=${id}`
    );
  },
  ...
  getUnapprovedWorkflowContractList(page, size, searchParams) {
    let url = `${
      config.contractUrl
    }/api/contract/header/approvals/filters?finished=false&page=${page}&size=${size}`;
    for (let searchName in searchParams) {
      url += searchParams[searchName] ? `&${searchName}=${searchParams[searchName]}` : '';
    }
    return httpFetch.get(url);
  ...
  getApprovedWorkflowContractList(page, size, searchParams) {
    let url = `${
      config.contractUrl
    }/api/contract/header/approvals/filters?finished=true&page=${page}&size=${size}`;
    for (let searchName in searchParams) {
      url += searchParams[searchName] ? `&${searchName}=${searchParams[searchName]}` : '';
    }
    return httpFetch.get(url);
  ...
  */
  ...
  getContractWorkflowHistory(entityType, entityOid) {
    return httpFetch.get(
      `${
        config.workflowUrl
      }/api/workflow/approval/history?entityType=${entityType}&entityOid=${entityOid}`
    );
  },
   ...
   */
  listUserByTypeId(id) {
    return httpFetch.get(`${config.contractUrl}/api/contract/type/users?contractTypeId=${id}`);
  },
  /**
   * 获取合同关联申请单
   */
