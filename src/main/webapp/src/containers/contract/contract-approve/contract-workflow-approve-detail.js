...
class ContractWorkflowApproveDetail extends React.Component {
  ...
  //获取快捷回复内容
  getQuickTags = () => {
    httpFetch.get(`${config.workflowUrl}/api/quick/reply`).then(res => {
      if (res.status === 200) {
  ...
  goBack = () => {
    ...
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/approval-management/contract-approve/contract-approve`,
      })
    );
  };
            ...
            <Row>
              <Col span={21}>
                <ApproveBar
                  ...
                  backUrl={'/approval-management/contract-approve/contract-approve'}
                  rejectLoading={rejectLoading}
                  documentType={801004}
                  documentOid={this.props.match.params.entityOid}
                />
              </Col>
            </Row>
          </Affix>
