...
class MyContract extends React.Component {
    ...
    this.state = {
      ...
      searchForm: [
        ...
        },
        {
          ...
          valueKey: 'currency',
          colSpan: 6,
          renderOption: item => item.currency + '-' + item.currencyName,
          event: 'CURRENCY',
        },
        {
          type: 'items',
  ...
  rowClick = record => {
    ...
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/contract-manager/my-contract/contract-detail/${record.id}`,
      })
    );
  };
  ...
  handleCreate = e => {
    ...
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/contract-manager/my-contract/new-contract/${e.key}`,
      })
    );
  };
