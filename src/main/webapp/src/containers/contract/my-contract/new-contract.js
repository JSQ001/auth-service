...
class NewContract extends React.Component {
  ...
  componentWillMount() {
    ...
    //this.getCompanies();
    // httpFetch.post(`${config.mdataUrl}/api/ven/info`, {}).then(res => {  //获取供应商列表
    //   res.status === 200 && this.setState({ venderOptions: res.data.body.body.venInfoBeans })
    // });
    //获取责任部门列表 flag = 1001全部, 1002启用, 1003未启用
    httpFetch.get(`${config.mdataUrl}/api/departments/root/v2?flag=1001`).then(res => {
      res.status === 200 && this.setState({ unitIdOptions: res.data });
    });
    this.getCurrencyOptions();
    if (this.props.match.params.contractTypeId) {
      this.listUserByTypeId(this.props.match.params.contractTypeId);
    }
  }
  // componentDidMount = () => {
  //   Number(this.props.match.params.id) && this.getInfo(); //合同编辑
  ...
  getCurrencyOptions = () => {
    ...
    !this.state.currencyOptions.length &&
      this.service.getCurrencyMdataList(params).then(res => {
        let currencyOptions = res.data.records;
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        (values.employeeId = values.user.id),
          (values.companyId = values.companyId && values.companyId[0].id);
        values.currency = values.currency && values.currency.key;
        let nowCurrencyObj = this.state.currencyOptions.find(
          item => item.currencyCode === values.currency
        );
        ...
        contractService
          ...
          .then(res => {
            if (res.status === 200) {
              ...
              this.props.dispatch(
                routerRedux.replace({
                  pathname: `/contract-manager/my-contract/contract-detail/${res.data.id}`,
                })
              );
            }
          })
  ...
  handleUpdate = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        this.state.uploadOids && (params.attachmentOIDs = this.state.uploadOids.toString());
  ...
  onCancel = () => {
    ...
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/contract-manager/my-contract/my-contract`,
      })
    );
  };
  ...
  onBack = () => {
    ...
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/contract-manager/my-contract/contract-detail/${this.props.match.params.id}`,
      })
    );
  };
  ...
  changeUnitId = (value, isInit) => {
    ...
    // httpFetch.get(`${config.mdataUrl}/api/departments/users/${value[0].departmentOid}`).then(res => {
    ...
    //     httpFetch.get(`${config.mdataUrl}/api/departments/users/${option.departmentOid}`).then(res => {
              ...
              <Col span={10}>
                <FormItem label={this.$t({ id: 'contract.createdBy' })} {...formItemLayout}>
                  {getFieldDecorator('user', {
                    ...
                    <Lov
                      code="contract_user_authorize"
                      valueKey="id"
                      labelKey="fullName"
                      allowClear={false}
                      single
                      extraParams={{ contractTypeId: this.props.match.params.contractTypeId }}
                    />
                  )}
                </FormItem>
              </Col>
                ...
                >
                  ...
                  {getFieldDecorator('companyId', {
                    ...
                    initialValue: record.id
                      ? [{ id: record.companyId, name: record.companyName }]
                      : [],
                  })(
                    <Chooser
              ...
              <Col span={10}>
                <FormItem label={this.$t({ id: 'common.currency' })} {...formItemLayout}>
                  {getFieldDecorator('currency', {
                    ...
                    initialValue: record.id
                      ? { key: record.currency, label: record.currencyName }
                      : {},
                  })(
                    <Select
                      labelInValue
                    ...
                    >
                      {currencyOptions.map(option => {
                        return (
                          <Option key={option.currencyCode}>
