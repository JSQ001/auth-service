...
class CompanyDistribution extends React.Component {
  ...
  handleBack = () => {
    this.props.dispatch(
      routerRedux.push({
        pathname: `/document-type-manage/contract-type/contract-type`,
      })
    );
  };
  ...
  render() {
    ...
    companyTypeList.map((item, index) => {
      ...
      if (index === 2) {
        periodRow.push(
          <Col style={{ padding: '20px 25px 0', borderRadius: '6px 6px 0 0' }} key="1">
            {periodCol}
          </Col>
        );
      }
    ...
    return (
      <div style={{ paddingBottom: 20 }}>
        <div style={{ background: '#f5f5f5' }}>
          <div style={{ background: '#f5f5f5', height: '80', borderRadius: '4' }}>{periodRow}</div>
          <div
            className="table-header"
            style={{ marginTop: 60, background: '#ffffff', paddingTop: 10 }}
          >
