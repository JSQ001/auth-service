...
class AccountPeriodDefine extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          title: this.$t('account.period.define.code' /*会计期代码*/),
        ...
        {
          title: this.$t('account.period.define.name' /*会计期名称*/),
        ...
        {
          title: this.$t('account.period.define.rule.total-period' /*期间总数*/),
          dataIndex: 'totalPeriodNum',
          width: 200,
          align: 'left',
          tooltips: true,
        },
        ...
        },
        {
          ...
          render: (text, record) => (
            <span>
              ...
              <a onClick={() => this.ruleItem(record)}>
                {this.$t('account.period.define.rule' /*会计期规则*/)}
              </a>
            </span>
          ),
        },
  ...
  render() {
    ...
    return (
      <div>
        <SearchArea searchForm={searchForm} submitHandle={this.search} clearHandle={() => {}} />
        ...
        <SlideFrame
          ...
          onClose={() => {
            this.setState({ slideFrameVisible: false });
          }}
        >
          <SlideFrameContent
