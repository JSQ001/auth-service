...
class NewSetOfBooks extends React.Component {
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        httpFetch[method](`${config.mdataUrl}/api/setOfBooks`, params)
          .then(res => {
  ...
  getAccountSetOptions = () => {
    this.state.accountSetOptions.length === 0 &&
      httpFetch.get(`${config.mdataUrl}/api/account/set/query`).then(res => {
  ...
  getPeriodSetOptions = () => {
    this.state.periodSetOptions.length === 0 &&
      httpFetch.get(`${config.mdataUrl}/api/periodset`).then(res => {
