...
class AccountPeriodControl extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          title: this.$t('account.period.control.sobCode'),
          key: 'setOfBooksCode',
        ...
        {
          title: this.$t('legal.entity.detail.sob.name'),
          key: 'setOfBooksName',
        ...
        {
          title: this.$t('account.period.control.periodSetCode'),
          key: 'periodSetCode',
        ...
        {
          title: this.$t('set-of-books.base.currency'),
          key: 'functionalCurrencyCode',
          dataIndex: 'functionalCurrencyCode',
          width: 150,
          align: 'left',
          tooltips: true,
        }, //本位币
        {
          title: this.$t('subject.sheet'),
          key: 'accountSetCode',
  ...
  getList = () => {
    ...
    periodControlService
      .getAccountPeriodList(page, pageSize, searchParams)
      .then(res => {
        if (res.status === 200) {
          this.setState({
            data: res.data,
  ...
  handleRowClick = record => {
    ...
    this.props.dispatch(
      routerRedux.push({
        pathname: `/finance-setting/account-period-control/account-period-detail/${
          record.periodSetId
        }/${record.setOfBooksId}`,
      })
    );
    ...
    return (
      <div className="account-period-control">
        <h3 className="header-title">
          {this.$t('account.period.control.title') /* 账套级会计期间控制 */}
        </h3>
        <SearchArea searchForm={searchForm} submitHandle={this.search} clearHandle={this.clear} />
        <div className="table-header">
          <div className="table-header-title">
            {this.$t('common.total', { total: pagination.total || 0 }) /* 共 total 条数据 */}
          </div>
        </div>
        <Table
          columns={columns}
