...
class AccountPeriodDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        {
          title: this.$t('account.period.control.detail.column.year'),
          key: 'periodYear',
        ...
        }, //年
        {
          ...
          dataIndex: 'periodSeq',
          render: (value, record) => {
            return <div>{Number.parseInt(value) - Number.parseInt(record.periodYear) * 10000}</div>;
          },
        }, //序号
        {
          title: this.$t('finance.audit.startDate'),
          key: 'startDate',
          dataIndex: 'startDate',
          render: date => moment(date).format('YYYY-MM-DD'),
        }, //日期从
        {
          title: this.$t('finance.audit.endDate'),
          key: 'endDate',
          dataIndex: 'endDate',
          render: date => moment(date).format('YYYY-MM-DD'),
        }, //日期至
        {
          title: this.$t('budget.strategy.detail.period.quarter'),
          key: 'quarterNum',
          dataIndex: 'quarterNum',
        }, //季度
        {
          title: this.$t('account.period.control.detail.column.period'),
          key: 'periodName',
          dataIndex: 'periodName',
        }, //期间
        {
          title: this.$t('account.period.control.detail.column.period-status'),
          key: 'periodStatusCode',
          dataIndex: 'periodStatusCode',
          render: code => (
            <Badge
              status={code === 'O' ? 'success' : 'error'}
              text={
                code === 'N'
                  ? this.$t('account.period.control.detail.not.open' /*未打开*/)
                  : code === 'O'
                    ? this.$t('account.period.control.detail.has.open' /*已打开*/)
        ...
        }, //期间状态
        {
          ...
          render: (periodId, record) => (
            <a
              onClick={() => {
                this.operaPeriodStatus(periodId, record.periodSetId, record.periodStatusCode);
              }}
            >
              {record.periodStatusCode === 'O'
                ? this.$t('account.period.control.detail.close.period' /*关闭期间*/)
  ...
  getPeriodInfo = () => {
    periodControlService
      .getAccountPeriodInfo(
        this.props.match.params.periodSetId,
  ...
  HandleReturn = () => {
    this.props.dispatch(
      routerRedux.push({
        pathname: `/finance-setting/account-period-control/account-period-control`,
      })
    );
  };
  ...
  getClosedList = () => {
    ...
    periodControlService
      .getClosePeriod(pageClose, pageSizeClose, periodSetId, setOfBooksId)
      .then(res => {
        this.setState({
          dataClose: res.data,
          loadingClose: false,
  ...
  getOpenList = () => {
    ...
    periodControlService
      .getOpenPeriod(pageOpen, pageSizeOpen, periodSetId, setOfBooksId)
      .then(res => {
        this.setState({
          dataOpen: res.data,
          loadingOpen: false,
  ...
  render() {
    ...
    periodInfoList.map((item, index) => {
      ...
      if (index === 2) {
        periodRow.push(
          <Row
            style={{ background: '#f7f7f7', padding: '20px 25px 0', borderRadius: '6px' }}
            key="1"
          >
            {periodCol}
          </Row>
      ...
      if (index === 4) {
        periodRow.push(
          <Row
            style={{ background: '#f7f7f7', padding: '0 25px 5px', borderRadius: '6px' }}
            key="2"
          >
            {periodCol}
          </Row>
      ...
      <div className="account-period-detail">
        <Spin spinning={false}>
          <h3 className="header-title">
            {this.$t('account.period.control.detail.title') /* 账套期间信息 */}
          </h3>
          {periodRow}
          <h3 className="header-title" style={{ margin: '24px 0 10px' }}>
            {this.$t('account.period.control.detail.title.close') /* 会计期间-未打开 */}
          </h3>
          <Table
          ...
          />
          <h3 className="header-title" style={{ margin: '40px 0 10px' }}>
            {this.$t('account.period.control.detail.title.open') /* 会计期间-已打开 */}
          </h3>
          <Table
            rowKey={record => record.id}
            columns={columns}
