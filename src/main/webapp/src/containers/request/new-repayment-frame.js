...
class NewPaymentFrame extends React.Component {
  ...
  getAcceptAccount = () => {
    ...
    httpFetch
      .get(`${config.mdataUrl}/api/v2/my/company/receipted/invoices?isAll=true&page=0&size=100`)
      .then(res => {
        this.setState({
          accountFetching: false,
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        httpFetch
          .post(`${config.baseUrl}/api/repayment/submit`, values)
          .then(() => {
        ...
        <Form onSubmit={this.handleSave}>
          ...
          <FormItem {...formItemLayout} label="开户银行">
            {getFieldDecorator('payBankName', {
              ...
              <Select
                placeholder="请选择"
                notFoundContent={bankFetching ? <Spin size="small" /> : '无匹配结果'}
              >
          ...
          <Row>
            ...
            <Col span={17} offset={1}>
              <ImageUpload attachmentType="REPAYMENT_IMAGES" onChange={this.getUploadImageOId} />
            </Col>
          </Row>
          <h4 className="title">
            收款方信息 <span>请按以下收款方账号打款</span>
          </h4>
          <FormItem {...formItemLayout} label="开户名">
