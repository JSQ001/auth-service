...
class LoanRequestDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      activeKey: 'detail',
      tabs: [{ key: 'info', name: '申请单信息' }, { key: 'status', name: '审批进度' }],
      status: [
      ...
      repaymentColumns: [
        ...
        { title: '还款单号', dataIndex: 'businessCode' },
        {
          title: '还款方式',
          dataIndex: 'type',
          render: value => (value === '0' ? '现金还款' : value === '1' ? '转账还款' : '报销单还款'),
        },
        ...
        { title: '还款金额', dataIndex: 'repaymentValue', render: this.filterMoney },
        {
          ...
          render: value =>
            value === '1001' ? (
              <Badge text="还款中" status="warning" />
            ) : value === '1002' ? (
              <Badge text="已还款" status="success" />
            ) : (
              <Badge text="被驳回" status="error" />
            ),
        },
      ],
      repaymentData: [],
  ...
  getInfo = () => {
    ...
    httpFetch
      .get(`${config.baseUrl}/api/loan/application/${this.props.params.applicationOid}`)
      .then(res => {
        this.setState({
          loading: false,
          info: res.data,
  ...
  getRepayment = () => {
    ...
    httpFetch
      .get(
        `${config.baseUrl}/api/repayment/list?page=0&size=10&loanApplicationOid=${
    ...
    fields.map(field => {
      switch (field.messageKey) {
        case 'select_corporation_entity':
          field.value &&
            httpFetch
              .get(`${config.baseUrl}/api/my/company/receipted/invoice/${field.value}`)
              .then(res => {
        ...
        case 'contact_bank_account':
          field.value &&
            httpFetch
              .get(`${config.mdataUrl}/api/contact/bank/account/${field.value}`)
              .then(res => {
        ...
        case 'select_approver':
          field.value &&
            httpFetch.get(`${config.mdataUrl}/api/users/oids?userOids=${field.value}`).then(res => {
        ...
        case 'select_cost_center':
          field.value &&
            httpFetch.get(`${config.mdataUrl}/api/dimension/item/${field.value}`).then(res => {
        ...
        <div className="top-info">
          ...
          <Row className="row-container">
            <span className="amount">
              总金额：{info.currencyCode} {this.renderMoney(info.totalAmount || 0)}
            </span>
            {writeoffInfo.hasWriteoffAmount !== info.totalAmount && (
              <span>
                {writeoffInfo.hasWriteoffAmount
                  ? `已还款：${this.renderMoney(writeoffInfo.hasWriteoffAmount)}`
                  : ''}
                {writeoffInfo.hasWriteoffAmount ? <span className="ant-divider" /> : ''}
                {writeoffInfo.lockedWriteoffAmount ? (
                  <span style={{ color: '#00a854' }}>
                    还款中：{this.renderMoney(writeoffInfo.lockedWriteoffAmount)}
                  </span>
                ) : (
                  ''
                )}
                {writeoffInfo.lockedWriteoffAmount && writeoffInfo.stayWriteoffAmount ? (
                  <span className="ant-divider" />
                ) : (
                  ''
                )}
                {writeoffInfo.stayWriteoffAmount ? (
                  <span>
                    <span style={{ color: '#108ee9', marginRight: 10 }}>
                      待还款：{this.renderMoney(writeoffInfo.stayWriteoffAmount)}
                    </span>
      ...
      <div className="loan-request-detail background-transparent">
        ...
        {tapValue === 'requestInfo' && (
          <Tabs className="detail-tabs" activeKey={activeKey} onChange={this.onDetailTabsChange}>
            ...
            <TabPane tab="还款记录" key="repayment">
              {repaymentContent}
            </TabPane>
          </Tabs>
        )}
