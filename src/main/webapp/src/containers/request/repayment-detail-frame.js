  ...
  render() {
    ...
    let alertContent = (
      <div className="alert-content-warn">
        ...
        {info.status === '1002' && (
          <div>
            {moment(info.writeoffDetail[0].createdDate).format('YYYY-MM-DD HH:mm:ss')} {} 确认还款{' '}
            {this.renderMoney(info.repaymentValue)}
          </div>
        )}
        {info.status === '1003' && (
          <div>
            {moment(info.writeoffDetail[0].createdDate).format('YYYY-MM-DD HH:mm:ss')}{' '}
          ...
          <Alert
            message={
              info.status === '1001'
                ? '还款中'
                : info.status === '1002'
                  ? `还款金额：${info.curreny} ${this.renderMoney(info.repaymentValue)}`
                  : '被驳回'
            }
