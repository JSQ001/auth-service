...
class MyGLWorkOrderDetail extends Component {
            ...
            if (record.lineStatus === 'normal') {
              ...
              return (
                <Chooser
                  ...
                  value={
                    record.responsibilityCenterId
                      ? [
                          {
                            id: record.responsibilityCenterId,
                            responsibilityCenterCodeName: record.responsibilityCenterCodeName,
                          },
                        ]
                      : []
                  }
                  type="responsibility_select"
                  labelKey="responsibilityCenterCodeName"
                  valueKey="id"
                  single={true}
                  listExtraParams={{ glWorkOrderTypeId: record.workOrderTypeId }}
                  showClear={false}
                />
              );
            }
  ...
  onCopyClick = (e, record, index) => {
    e.preventDefault();
    record.responsibilityCenterCodeName = record.responsibilityCenterName;
  ...
  onUnitChange = (value, record, index) => {
    let { data } = this.state;
    data[index].responsibilityCenterId = value[0].id;
    data[index].responsibilityCenterCodeName = value[0].responsibilityCenterCodeName;
    this.setState({ data });
  };
  /**
   * 科目变化事件
    ...
    myGlWorkOrderService
      ...
      .then(res => {
        if (res.status === 200) {
          ...
          let documentType = res.data.type;
          ...
          //当单据类型中设置责任中心为必输项, 且包含默认责任中心字段, 设置为必输且自动生成默认字段
          columns.map(item => {
            if (item.dataIndex == 'responsibilityCenterName') {
              item.title =
                documentType.responsibilityCenterRequired == 'Y' ? (
                  <span>
                    <span style={{ color: 'red' }}>*</span>&nbsp;责任中心
                  </span>
                ) : (
                  '责任中心'
                );
  ...
  addDimensionColumns = dimensionData => {
    ...
    if (columns.length <= 8) {
      dimensionData.map(item => {
        ...
        //维值id-字段名称
        let dimensionKey = 'dimensionValue' + item.dimensionSequence + 'Id';
        //维值name-字段名称
        let dimensionName = 'dimensionValue' + item.dimensionSequence + 'Name';
        //拼接列
        let dimensionColumn = {
          title: item.requiredFlag ? (
            <span>
              <span style={{ color: 'red' }}>*</span>&nbsp;{dimensionTitle}
            </span>
          ) : (
            dimensionTitle
          ),
  ...
  edit = () => {
    ...
    this.props.dispatch(
      routerRedux.push({
        pathname: `/my-gl-work-order/edit-gl-work-order/${docHeadData.companyId}/${
  ...
  additionalMethod2 = record => {
    //如果当前页的条数太大，就把当前页的条数变成当前页数据的数量值
    let { pagination, pageSize, data, documentType, dimensionData } = this.state;
    let newRecord = {};
    //如果是复制过来的数据
    if (record.id) {
      newRecord = record;
      ...
      newRecord = {
        ...
        responsibilityCenterId:
          documentType && documentType.defaultResponsibilityCenter
            ? documentType.defaultResponsibilityCenter
            : null,
        responsibilityCenterCodeName:
          documentType && documentType.defaultResponsibilityCenter
            ? documentType.defaultResponsibilityCenterName
            : null,
      };
      ...
      dimensionData.map(item => {
        if (item.defaultValue != null) {
          ...
          //维值name-字段名称
          let dimensionName = 'dimensionValue' + item.dimensionSequence + 'Name';
          newRecord[dimensionKey] = item.defaultValue;
          newRecord[dimensionName] = item.defaultValueName;
        }
      });
    }
    data.push(newRecord);
  ...
  onSave = () => {
    //校验必输字段
    let { data, dimensionData, documentType } = this.state;
    ...
    data.map((item, index) => {
      ...
      if (documentType.responsibilityCenterRequired == 'Y' && item.responsibilityCenterId == null) {
        nullFlag += 1;
        return;
      }
      dimensionData.map(dimension => {
        //维值id-字段名称
        let dimensionKey = 'dimensionValue' + dimension.dimensionSequence + 'Id';
        if (dimension.requiredFlag && item[dimensionKey] == null) {
          nullFlag += 1;
          return;
        }
      });
  ...
  onSubmit = () => {
    ...
    let params = {
      ...
      versionNumber: docHeadData.versionNumber,
    };
    myGlWorkOrderService
  ...
  onImportOk = transactionID => {
    myGlWorkOrderService
      ...
      .then(res => {
        if (res.status === 200) {
          this.setState({ showImportFrame: false }, () => this.getDocInfoById());
        }
      })
      .catch(e => {
        console.log(`导入失败：${e}`);
    ...
    return (
      ...
      >
        ...
        >
          ...
          docHeadData.status === 1005 ? (
            <Row style={{ marginLeft: '30px' }}>
              <Button type="primary" loading={operationLoading} onClick={this.onSubmit}>
                提交
              </Button>
              <Button
        ...
        <ImporterNew
          ...
          templateUrl={`${
            config.accountingUrl
          }/api/general/ledger/work/order/head/export/template?headId=${
            this.props.match.params.id
          }`}
          uploadUrl={`${
            config.accountingUrl
          }/api/general/ledger/work/order/head/import/new?headId=${this.props.match.params.id}`}
          errorUrl={`${
            config.accountingUrl
          }/api/general/ledger/work/order/head/import/new/error/export`}
          errorDataQueryUrl={`${
            config.accountingUrl
          }/api/general/ledger/work/order/head/import/new/query/result`}
          deleteDataUrl={`${
            config.accountingUrl
          }/api/general/ledger/work/order/head/import/new/delete`}
