...
import { Button, Affix, message, Row, Col, Tabs, Popover } from 'antd';
...
class GLWorkOrderCheckDetail extends Component {
  ...
  onBack = () => {
    ...
    this.props.dispatch(
      routerRedux.replace({
        pathname: `/approval-management/gl-work-order-approval/gl-work-order-approval`,
      })
    );
  };
  /**
  ...
  render() {
    ...
    return (
      <div
        className="gl-work-order-detail"
        style={{
          background: 'white',
      ...
      >
        ...
        <div style={{ paddingBottom: 10, marginBottom: 60 }}>
          <ApproveHistory type="801008" oid={docHeadData.documentOid || ''} />
        </div>
        {(docStatus &&
          docStatus === '1002' && (
            <Affix offsetBottom={0} className="bottom-bar-approve">
              <ApproveBar
                ...
                backUrl={'/approval-management/gl-work-order-approval/gl-work-order-approval'}
                rejectLoading={rejectLoading}
                documentType={801008}
                documentOid={this.state.docHeadData.documentOid}
              />
            </Affix>
          )) ||
          (docStatus &&
