...
class UpdateBudgetParameterSetting extends React.Component {
  ...
  getStatusOptions() {
    httpFetch
      .get(
        `${config.budgetUrl}/api/budget/parameterSettings/parameterValues/${
          this.props.params.parameterId
        }`
      )
      .then(res => {
        //状态
    ...
    return (
      <div className="new-budget-organization">
        <Form onSubmit={this.handleSave}>
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'budget.parameterCode' }) /* 参数代码 */}
          >
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'budget.parameterName' }) /* 参数名称 */}
          >
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'budget.parameterValueName' }) /* 参数值名称 */}
          >
            {getFieldDecorator('parameterValueCode', {
              ...
              <Select placeholder={this.$t({ id: 'common.please.select' })}>
                {statusOptions.map(option => {
                  return (
                    <Option key={option.parameterValueCode}>{option.parameterValueName}</Option>
                  );
                })}
              </Select>
            )}
...
export default connect(
  ...
  null,
  { withRef: true }
)(WrappedUpdateBudgetParameterSetting);
