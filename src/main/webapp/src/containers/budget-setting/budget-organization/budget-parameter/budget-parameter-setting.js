...
class BudgetParameterSetting extends React.Component {
  ...
  componentWillMount() {
    //查出当前预算组织数据
    httpFetch
      .get(`${config.budgetUrl}/api/budget/organizations/${this.props.match.params.id}`)
      .then(response => {
        this.setState({
  ...
  getList() {
    ...
    let url = `${config.budgetUrl}/api/budget/parameterSettings/dto/query?organizationId=${
      this.props.match.params.id
    }&page=${this.state.pagination.page}&size=${this.state.pagination.pageSize}`;
  ...
  handleCreate = () => {
    if (this.state.organization.enabled) {
      this.context.router.push(
        menuRoute
          .getMenuItemByAttr('budget-organization', 'key')
          .children.newBudgetStructure.url.replace(':id', this.props.id)
      );
    } else {
      notification['error']({
  ...
  render() {
    ...
    return (
      <div className="budget-structure" style={{ paddingBottom: 20 }}>
        ...
        {/* 编辑参数设置 */}
        <SlideFrame
          title={this.$t({ id: 'budget.parameter.setting.edit' })}
          show={showSlideFrame}
