...
class NewDimension extends React.Component {
        ...
        searchForm: [
          {
            ...
            label: this.$t({ id: 'structure.dimensionValueCode' }),
          },
          {
            type: 'input',
            id: 'dimensionItemName',
            label: this.$t({ id: 'structure.dimensionValueName' }),
          },
        ],
        columns: [
          {
            title: this.$t({ id: 'structure.dimensionValueCode' }),
            dataIndex: 'dimensionItemCode',
          },
          {
            title: this.$t({ id: 'structure.dimensionValueName' }),
  ...
  componentDidMount() {
    ...
    let value = {
      ...
      defaultDimensionCode: dimension.defaultDimensionValue.length
        ? [
            {
              key: dimension.defaultDimensionValue[0].defaultDimValueId,
              id: dimension.defaultDimensionValue[0].defaultDimValueId,
              dimensionItemCode: dimension.defaultDimensionValue[0].defaultDimValueCode,
              dimensionItemName: dimension.defaultDimValueName,
            },
          ]
        : [],
    };
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        budgetService
          .structureUpdateDimension(values)
          .then(res => {
          ...
          .catch(e => {
            if (e.response) {
              message.error(
                `${this.$t({ id: 'common.operate.filed' })}, ${e.response.data.message}`
              );
            }
            this.setState({ loading: false });
          });
  ...
  handleDimensionCode = value => {
    if (typeof value !== 'undefined') {
      if (value.length > 0) {
        ...
        if (
          dimension.dimensionId === value[0].dimensionId &&
          dimension.defaultDimValueCode === this.props.form.getFieldValue('defaultDimensionCode')
        ) {
          return;
        }
        this.props.form.setFieldsValue({
          dimensionName: value[0].dimensionName || dimension.dimensionName,
          defaultDimensionCode: [],
          defaultDimValueName: '',
        });
        let extraParams = this.state.extraParams;
  ...
  handleDimensionValue = value => {
    if (typeof value !== 'undefined') {
      this.props.form.setFieldsValue({
        defaultDimValueName: value.length > 0 ? value[0].dimensionItemName : undefined,
      });
      this.setState({
