...
export default {
  ...
  structureAssignCompany(params) {
    return httpFetch.post(
      `${config.budgetUrl}/api/budget/structure/assign/companies/batch`,
      params
    );
  },
  ...
  saveResponsibilityCenter(params) {
    return httpFetch.post(
      `${config.budgetUrl}/api/budget/structures/assign/responsibility`,
      params
    );
  },
  /**
   ...
   * @param {*} id
   */
  getResponsibilityCenter(id) {
    return httpFetch.get(`${config.budgetUrl}/api/budget/structures/assign/responsibility/${id}`);
  },
  /**
   * 获取责任中心
   */
