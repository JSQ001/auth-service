    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (values.defaultFlag && !values.enabled) {
          message.error(
            this.$t(
              { id: 'budget.scenarios.default.must.be.enabled' } /*默认预算场景的状态必须为启用*/
            )
          );
          return false;
        }
        ...
        httpFetch
          .post(`${config.budgetUrl}/api/budget/scenarios`, values)
          .then(res => {
          ...
          .catch(e => {
            if (e.response) {
              message.error(
                `${this.$t({ id: 'common.create.filed' } /*新建失败*/)}, ${e.response.data.message}`
              );
            }
            this.setState({ loading: false });
          });
  ...
  render() {
    ...
    return (
      <div className="new-budget-scenarios">
        <Alert
          message={this.$t({ id: 'common.help' } /*帮助提示*/)}
          description={this.$t(
            {
              id: 'budget.scenarios.help.info',
            } /*预算组织为当前用户所在账套下的生效的预算组织，同一账套下预算场景代码不允许重复，一个预算组织下允许多个预算场景同时生效。*/
          )}
          type="info"
          showIcon
        />
        <Form onSubmit={this.handleSave}>
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'budget.scenarios.is.default' } /*是否默认*/)}
          >
