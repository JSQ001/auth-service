...
class NewBudgetControlRules extends React.Component {
  ...
  componentWillMount() {
    ...
    budgetService
      .getStrategy({
        organizationId: this.props.match.params.orgId || this.props.organization.id,
        enabled: true,
      })
      .then(response => {
  ...
  validateRuleCode = (item, value, callback) => {
    budgetService
      .getRuleByOptions({ organizationId: this.props.match.params.orgId, controlRuleCode: value })
      .then(response => {
        let flag = false;
        ...
        <div className="budget-control-rules-form">
          <Form onSubmit={this.handleSave} className="budget-control-rules-form">
            <Row gutter={60}>
                    ...
                    rules: [
                      { required: true, message: this.$t({ id: 'common.please.enter' }) },
                      {
                        validator: (item, value, callback) =>
                          this.validateRuleCode(item, value, callback),
                      },
                    ],
              ...
              <Col span={8}>
                <FormItem label={this.$t({ id: 'budget.strategy' })} colon={true}>
