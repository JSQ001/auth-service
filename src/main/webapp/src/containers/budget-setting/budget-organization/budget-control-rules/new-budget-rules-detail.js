...
class NewBudgetRulesDetail extends React.Component {
  ...
  handleSelectParam = value => {
    const { lov, paramValueMap } = this.state;
    ...
    if (typeof ruleParameterType === 'undefined') {
      ...
      if (lov.type === 'BGT_RULE_PARAMETER_DIM') {
        temp = {
          ...
          listExtraParams: { dimensionId: value.split('+')[1] },
          codeKey: 'dimensionItemCode',
        };
      } else {
        temp = paramValueMap[value] || { disabled: true };
      }
  ...
  ruleParameterTypeChange = value => {
    const { lov, ruleParamsArray } = this.state;
    lov.type = value;
    let ruleParameterCode;
    switch (value) {
      case 'BGT_RULE_PARAMETER_BUDGET':
        ruleParameterCode = 2015;
        break;
      case 'BGT_RULE_PARAMETER_ORG':
        ruleParameterCode = 2016;
        break;
      case 'BGT_RULE_PARAMETER_DIM':
        ruleParameterCode = 2017;
        break;
    }
        ...
        <Form onSubmit={this.handleSubmit}>
          <Row gutter={30}>
            <Col span={20}>
              <FormItem
                {...formItemLayout}
                label={this.$t({ id: 'budget.ruleParameterType' }) /*规则参数类型*/}
              ...
              >
                {getFieldDecorator('ruleParameterType', {
                  ...
                  <Select
                    ...
                    onChange={this.ruleParameterTypeChange}
                  >
          ...
          <Row gutter={30}>
            <Col span={20}>
              <FormItem
                {...formItemLayout}
                label={this.$t({ id: 'budget.ruleParameter' }) /*规则参数*/}
              ...
              >
                {getFieldDecorator('ruleParameter', {
                  ...
                  <Select
                    onChange={this.handleSelectParam}
                    className="input-disabled-color"
                    placeholder={this.$t({ id: 'common.please.select' })}
                  >
                    ...
                    },
                    {
                      validator: (item, value, callback) => {
                        (helpMap.filtrateMethod =
                          value === 'INCLUDE'
                            ? this.$t({
                                id: 'budget.filtrateMethodHelp.contain',
                              }) /*值范围为闭区间，包含左右边界值*/
                            : value === 'EXCLUDE'
                              ? this.$t({ id: 'budget.filtrateMethodHelp.exclude' })
                              : this.$t({
                                  id: 'common.please.select',
                                })) /*值范围为开区间，不包含左右边界值*/,
                          (validateStatusMap.filtrateMethod =
                            typeof value === 'undefined' ? 'error' : '');
                        this.setState({
                          helpMap,
                          loading: false,
                          validateStatusMap,
          ...
          <Row gutter={30}>
            <Col span={20}>
              <FormItem
                {...formItemLayout}
                label={this.$t({ id: 'budget.parameterLowerLimit' }) /*下限值*/}
          ...
          <Row gutter={30}>
            <Col span={20}>
              <FormItem
                {...formItemLayout}
                label={this.$t({ id: 'budget.parameterUpperLimit' }) /*上限值*/}
          ...
          <Row gutter={30}>
            <Col span={20}>
              <FormItem
                {...formItemLayout}
                label={this.$t({ id: 'budget.invalidDate' }) /*失效日期*/}
              >
