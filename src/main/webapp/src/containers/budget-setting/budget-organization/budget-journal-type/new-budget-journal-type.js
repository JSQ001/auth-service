  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        httpFetch
          .post(`${config.budgetUrl}/api/budget/journal/types`, values)
          .then(res => {
          ...
          .catch(e => {
            message.error(
              `${this.$t({ id: 'common.save.filed' })},同一预算组织下的预算日记账类型代码不能重复!`
            );
            this.setState({ loading: false });
          });
      }
                  ...
                  {getFieldDecorator('form', {
                    ...
                    >
                      {this.state.linkForm.map(option => {
                        return (
                          <Option
                            key={option.formOid + '?' + option.formType + '?' + option.formName}
                          >
                            {option.formName}
                          </Option>
                        );
