...
class NewBudgetVersions extends React.Component {
  ...
  getStatusOptions() {
    budgetVersionsService.getSystemValueList(2001).then(res => {
      //状态
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (!this.state.statusError) {
          ...
          typeof this.state.version.id === 'undefined'
            ? this.saveData(toleValues)
            : this.updateVersion(toleValues);
        }
      }
    });
  };
    ...
    budgetVersionsService
      ...
      .then(response => {
        if (response.status === 200) {
          message.success(
            this.$t(
              { id: 'common.create.success' },
              { name: this.$t({ id: 'budgetVersion.version' }) }
            )
          );
      ...
      <div className="new-budget-versions">
        ...
        <div className="new-budget-versions-from">
          <Form onSubmit={this.handleSave}>
            ...
            <FormItem {...formItemLayout} label={this.$t({ id: 'budgetVersion.versionCode' })}>
            ...
            <FormItem {...formItemLayout} label={this.$t({ id: 'budgetVersion.versionStatus' })}>
            ...
            <FormItem
              {...formItemLayout}
              label={this.$t({ id: 'budgetVersion.versionDescription' })}
            >
              {getFieldDecorator('description', {
                initialValue: version.description,
              })(<Input />)}
            </FormItem>
            <FormItem {...formItemLayout} label={this.$t({ id: 'budgetVersion.versionDate' })}>
              {getFieldDecorator('versionDate', {
                initialValue:
                  typeof version.id === 'undefined'
                    ? null
                    : moment(version.versionDate, 'YYYY-MM-DD'),
