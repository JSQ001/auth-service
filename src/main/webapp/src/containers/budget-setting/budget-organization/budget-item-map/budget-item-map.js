...
class BudgetItemMap extends React.Component {
      ...
      searchForm: [
        {
          ...
          options: [],
          id: 'sourceType',
          label: this.$t({ id: 'itemMap.sourceType' }),
        } /*来源类别*/,
        {
          type: 'select',
          id: 'itemId',
          ...
          method: 'get',
          getParams: { organizationId: this.props.id, enabled: true },
        },
      ],
      columns: [
        {
          /*来源类别*/
          title: this.$t({ id: 'itemMap.sourceType' }),
          align: 'center',
          key: 'sourceType',
          dataIndex: 'sourceType',
          render: (text, record, index) => this.renderColumns(text, record, index, 'sourceType'),
        },
        {
          /*明细类型*/
          title: this.$t({ id: 'itemMap.detailType' }),
          align: 'center',
          key: 'sourceItemName',
          dataIndex: 'sourceItemName',
          render: (text, record, index) => this.renderColumns(text, record, index, 'detail'),
        },
        {
          /*预算项目*/
          title: this.$t({ id: 'itemMap.item' }),
          align: 'center',
          key: 'budgetItemName',
          dataIndex: 'budgetItemName',
          render: (text, record, index) => this.renderColumns(text, record, index, 'item'),
        }, //操作
        {
  ...
  saveItem = (e, record, index) => {
    ...
    if (
      record.sourceType !== '' &&
      typeof record.budgetItemId !== 'undefined' &&
      typeof record.sourceItemId !== 'undefined'
    ) {
      record.budgetOrganizationId = this.props.id;
      budgetService
        .insertOrUpdateItemMap([record])
        .then(response => {
      ...
      if (typeof record.id === 'undefined') {
        ...
        this.setState({
          ...
          pagination: {
            ...this.state.pagination,
            pageSize:
              this.state.pagination.pageSize - 1 >= 10 ? this.state.pagination.pageSize - 1 : 10,
        ...
        this.setState(
          {
            ...
            pagination: {
              ...this.state.pagination,
              page:
                (this.state.pagination.total - 1) % 10 === 0
                  ? (this.state.pagination.total - 1) / 10 - 1
                  : parseInt((this.state.pagination.total - 1) / 10),
  ...
  componentWillMount() {
    ...
    itemSelectorItem.searchForm[1].getUrl = itemSelectorItem.searchForm[1].getUrl.replace(
      ':organizationId',
      this.props.id
    );
    itemSelectorItem.searchForm[2].getUrl = itemSelectorItem.searchForm[2].getUrl.replace(
      ':organizationId',
      this.props.id
    );
    let paramValueMap = {
    ...
    if (record.edit) {
      switch (dataIndex) {
        ...
        case 'detail': {
          ...
          paramValueMap.columns[1].title = this.$t({
            id: flag ? 'expense.type.name' : 'application.type.management.name',
          });
          paramValueMap.searchForm[0].label = this.$t({
            id: flag ? 'expense.type.name' : 'application.type.management.name',
          });
          return (
            <Chooser
      ...
      switch (dataIndex) {
        case 'sourceType':
          return decode === 'EXPENSE_TYPE'
            ? `${this.$t({ id: 'itemMap.expenseType' })}`
            : `${this.$t({ id: 'itemMap.applyType' })}`;
          break;
        case 'detail':
          return record.sourceItemName ? record.sourceItemName : '-';
          break;
        case 'item':
          return record.budgetItemName;
          break;
      }
    }
  };
  ...
  handleImportOk = transactionID => {
    httpFetch
      .post(`${config.budgetUrl}/api/budget/itemsMapping/import/new/confirm/${transactionID}`)
      .then(res => {
        if (res.status === 200) {
          this.getList();
        }
    ...
    params.map(item => {
      if (item.edit) {
        ...
        if (
          ...
          (item.sourceType !== '' && item.item.length > 0 && item.detail.length > 0)
        ) {
        ...
        <div className="table-header">
          ...
          <div className="table-header-buttons">
            ...
            <ImporterNew
              ...
              uploadUrl={`${config.budgetUrl}/api/budget/itemsMapping/import/new?orgId=${
                this.props.id
              }`}
              errorUrl={`${config.budgetUrl}/api/budget/itemsMapping/import/new/error/export`}
              errorDataQueryUrl={`${
                config.budgetUrl
              }/api/budget/itemsMapping/import/new/query/result`}
            ...
            />
            <Button type="primary" loading={btnLoading} disabled={isSave} onClick={this.handleSave}>
