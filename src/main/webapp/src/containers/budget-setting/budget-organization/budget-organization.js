...
class BudgetOrganization extends React.Component {
        ...
        }, //状态
        {
          ...
          render: (text, record) => (
            <span>
              ...
              <a onClick={e => this.parameterSetting(e, record)}>
                {this.$t({ id: 'budget.parameter.setting' })}
              </a>
            </span>
          ),
        }, //操作
      ...
      searchForm: [
        {
          type: 'select',
          id: 'setOfBooksId',
          label: this.$t({ id: 'budget.organization.set.of.books' }),
          options: [],
        ...
        }, //账套
        {
          type: 'input',
          id: 'organizationCode',
          label: this.$t({ id: 'budget.organization.code' }),
        }, //预算组织代码
        {
          type: 'input',
          id: 'organizationName',
          label: this.$t({ id: 'budget.organization.name' }),
        }, //预算组织名称
      ],
      searchParams: {
        setOfBooksId: '',
  ...
  parameterSetting = (e, record) => {
    ...
    this.props.dispatch(
      routerRedux.push({
        pathname: '/budget-setting/budget-organization/budget-parameter/budget-parameter-setting/:id'.replace(
          ':id',
          record.id
        ),
      })
  ...
  getList() {
    ...
    return budgetOrganizationService
  ...
  render() {
    ...
    return (
      <div className="budget-organization">
        <h3 className="header-title">{this.$t({ id: 'menu.budget-organization' })}</h3>{' '}
        {/* 预算组织定义 */}
        <SearchArea
        ...
        {/* 编辑预算组织 */}
        <SlideFrame
          title={this.$t({ id: 'budget.organization.edit' })}
          show={showSlideFrame}
          onClose={() => this.setState({ showSlideFrame: false })}
        >
          <UpdateBudgetOrganization onClose={this.handleCloseSlide} params={nowOrganization} />
        </SlideFrame>
      </div>
    );
  }
