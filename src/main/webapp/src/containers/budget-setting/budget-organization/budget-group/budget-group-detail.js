...
class BudgetGroupDetail extends React.Component {
  ...
  componentWillMount() {
    ...
    selectorItem.url = `${config.budgetUrl}/api/budget/groupDetail/${
      this.props.match.params.id
    }/query/filter`;
    budgetGroupService
      .filterItemByGroupIdAndOrganizationId(this.props.match.params.id, this.props.organization.id)
      .then(response => {
        let result = [];
  ...
  updateHandleInfo = params => {
    ...
    budgetGroupService
      .updateOrganizationGroup(Object.assign({}, this.state.groupData, params))
      .then(response => {
      ...
      .catch(e => {
        if (e.response) {
          message.error(
            `${this.$t('wait.for.save.modifyFail') /*修改失败*/}, ${e.response.data.message}`
          );
        }
        this.setState({
          editing: false,
  ...
  refreshSelected() {
    ...
    selectedData.map(selected => {
      data.map(item => {
        if (item.id === selected.id) nowSelectedRowKeys.push(item.id);
      });
    });
  ...
  getList = () => {
    ...
    return budgetGroupService
      .getItemByGroupId(this.props.match.params.id, page, pageSize)
      .then(response => {
        response.data.map(item => {
          item.key = item.id;
        });
  ...
  deleteItem = (text, record) => {
    this.setState({ loading: true }, () => {
      budgetGroupService
        .deleteItemFromGroup(this.props.match.params.id, record.id)
        .then(response => {
  ...
  handleBatchDelete = () => {
    ...
    this.setState({ loading: true }, () => {
      budgetGroupService
        .batchDeleteItemFromGroup(this.props.match.params.id, paramList)
        .then(response => {
          message.success(this.$t('common.delete.success', { name: '' }) /*删除成功*/);
          this.setState(
            {
  ...
  handleAdd = result => {
    ...
    if (result.result.length > 0) {
      ...
      budgetGroupService
        .batchAddItemToGroup(this.props.match.params.id, result.result)
        .then(response => {
      ...
      <div>
        ...
        <div className="table-header">
          ...
          <div className="table-header-buttons">
            ...
            <Button onClick={this.handleBatchDelete} disabled={selectedData.length === 0}>
              {this.$t('common.delete') /*删除*/}
            </Button>
          </div>
        </div>
