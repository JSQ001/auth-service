      ...
      this.setState(
        {
          ...
          periodStrategyValue: this.handlePeriodStrategy(
            params.newParams.periodStrategy && params.newParams.periodStrategy.value
          ),
        },
        () => {
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        httpFetch
          .post(`${config.budgetUrl}/api/budget/control/strategy/mp/conds`, values)
          .then(res => {
  ...
  handleUpdate = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        this.setState({ loading: true });
        httpFetch
          .put(`${config.budgetUrl}/api/budget/control/strategy/mp/conds`, values)
          .then(res => {
            if (res.status === 200) {
              this.setState({ loading: false });
              this.props.onClose(true);
    ...
    const content = (
      <div style={{ color: '#999' }}>
        <div style={{ marginBottom: '10px' }}>
          {this.$t(
            {
              id: 'budget.strategy.detail.period.title',
            } /*控制策略控制期段即以何种方式对预算进行控制*/
          )}
        </div>
        <div>
          <span style={{ color: '#666' }}>
            【{this.$t({ id: 'budget.strategy.detail.period.period' } /*期间*/)}】：
          </span>
          {this.$t({ id: 'budget.strategy.detail.period.content.period' } /*按当月录入预算额控制*/)}
          <br />
          <span style={{ color: '#666' }}>
            【{this.$t({ id: 'budget.strategy.detail.period.quarter' } /*季度*/)}】：
          </span>
          {this.$t({ id: 'budget.strategy.detail.period.content.quarter' } /*按当季预算额控制*/)}
          <br />
          <span style={{ color: '#666' }}>
            【{this.$t({ id: 'budget.strategy.detail.period.year' } /*年度*/)}】：
          </span>
          {this.$t({ id: 'budget.strategy.detail.period.content.year' } /*按全年预算额控制*/)}
          <br />
          <span style={{ color: '#666' }}>
            【{this.$t({ id: 'budget.strategy.detail.period.qtd' }) /*季度至今*/}】：
          </span>
          {this.$t({ id: 'budget.strategy.detail.period.content.qtd' }) /*按季度初至今预算额控制*/}
          <br />
          <span style={{ color: '#666' }}>
            【{this.$t({ id: 'budget.strategy.detail.period.ytd' } /*年度至今*/)}】：
          </span>
          {this.$t({ id: 'budget.strategy.detail.period.content.ytd' } /*按年初至今预算额控制*/)}
          <br />
          <span style={{ color: '#666' }}>
            【{this.$t({ id: 'budget.strategy.detail.period.rqb' } /*季度滚动*/)}】：
          </span>
          {this.$t(
            {
              id: 'budget.strategy.detail.period.content.rqb',
            } /*按当月至后两个月共3个月合计预算额控制*/
          )}
          <br />
          <span style={{ color: '#666' }}>
            【{this.$t({ id: 'budget.strategy.detail.period.ytq' } /*累计季度*/)}】：
          </span>
          {this.$t(
            { id: 'budget.strategy.detail.period.content.ytq' } /*按年初至当季度预算额控制*/
          )}
        </div>
      </div>
    );
    return (
      <div className="new-strategy-control-detail">
        <Form onSubmit={updateParams.id ? this.handleUpdate : this.handleSave}>
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'budget.strategy.detail.type' } /*类型*/)}
            style={{ margin: '24px 0' }}
          >
            {getFieldDecorator('organizationName', {
              ...
              <RadioGroup>
                <RadioButton value="10">
                  {this.$t({ id: 'budget.strategy.detail.formula' } /*公式*/)}
                </RadioButton>
              </RadioGroup>
            )}
          </FormItem>
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'budget.strategy.detail.control.object' } /*控制对象*/)}
          >
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'budget.strategy.detail.compare' } /*比较*/)}
          >
          ...
          <Row>
            <Col span={6} className="ant-form-item-label label-style">
              {this.$t({ id: 'budget.strategy.detail.manner' } /*方式*/)}：{' '}
            </Col>
              ...
              <FormItem className="ant-col-offset-1">
                {getFieldDecorator('value', {
                  ...
                  <InputNumber
                    ...
                    parser={value =>
                      mannerValue !== 'PERCENTAGE' ? value : value.replace('%', '')
                    }
                    onChange={value => {
                      this.setState({ valueValue: value });
                    }}
                  />
                )}
              </FormItem>
          ...
          <Row>
            <Col span={6} className="ant-form-item-label label-style">
              {this.$t({ id: 'budget.strategy.detail.control.period' } /*控制期段*/)}：{' '}
            </Col>
            <Col span={12} className="ant-col-offset-1">
              <FormItem>
                {getFieldDecorator('periodStrategy', {
                  ...
                  <Select
                    onChange={this.handlePeriodStrategy}
                    placeholder={this.$t({ id: 'common.please.select' }) /*请选择*/}
                  >
            ...
            <Col span={2} className="ant-col-offset-1">
              <Popover
                ...
                content={content}
                title={this.$t(
                  { id: 'budget.strategy.detail.budget.control.period' } /*预算控制期段*/
                )}
              >
                <Icon
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'budget.strategy.detail.condition' } /*触发条件*/)}
          >
...
export default connect(
  ...
  null,
  { withRef: true }
)(WrappedNewStrategyControlDetail);
