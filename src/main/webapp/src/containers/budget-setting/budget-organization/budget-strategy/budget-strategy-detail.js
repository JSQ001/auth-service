...
class BudgetStrategyDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      infoList: [
        {
          type: 'input',
          id: 'controlStrategyCode',
          label: this.$t({ id: 'budget.strategy.code' } /*预算控制策略代码*/),
          isRequired: true,
          disabled: true,
        },
        {
          type: 'input',
          id: 'controlStrategyName',
          label: this.$t({ id: 'budget.strategy.name' } /*预算控制策略名称*/),
          isRequired: true,
        },
      ...
      columns: [
        ...
        },
        {
          ...
          render: (message, record) =>
            record.controlMethod.value === 'NO_MESSAGE' ? (
              <span>-</span>
            ) : (
              <Popover
                placement="topLeft"
                content={record.messageCode ? record.messageCode.label : '-'}
              >
                {record.messageCode ? record.messageCode.label : '-'}
              </Popover>
            ),
        },
        {
  ...
  getBasicInfo() {
    httpFetch
      .get(`${config.budgetUrl}/api/budget/control/strategies/${this.props.match.params.id}`)
      .then(res => {
        if (res.status === 200) {
  ...
  getList() {
    let url = `${config.budgetUrl}/api/budget/control/strategy/details/query?size=${
      this.state.pageSize
    }&page=${this.state.page}&controlStrategyId=${this.props.match.params.id}&organizationId=${
  ...
  handleUpdate = params => {
    ...
    this.setState({ baseInfoLoading: true }, () => {
      httpFetch
        .put(`${config.budgetUrl}/api/budget/control/strategies`, params)
        .then(res => {
          if (res.status === 200) {
          ...
          <Row>
            ...
            <Col span={6}>
              <Search
                ...
                placeholder={this.$t(
                  { id: 'budget.strategy.input.name.code' } /*请输入策略明细名称/代码*/
                )}
