...
class NewBudgetStrategyDetail extends React.Component {
  ...
  componentWillMount() {
    ...
    this.getSystemValueList(2022).then(res => {
      //预算控制消息
  ...
  handleSave = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        httpFetch
          .post(`${config.budgetUrl}/api/budget/control/strategy/details`, values)
          .then(res => {
            if (res.status === 200) {
              this.setState({ loading: false });
              message.success(
                this.$t({ id: 'common.create.success' }, { name: '' }) /* 新建成功 */
              );
              this.handleCancel();
            }
          })
          .catch(e => {
            if (e.response) {
              message.error(
                `${this.$t({ id: 'common.create.filed' }) /* 新建失败 */}, ${
  ...
  handleMethodChange = value => {
    ...
    if (value === 'BLOCK') {
      controlMethodNotice = this.$t(
        { id: 'budget.strategy.notice.forbid.submit' } /*如果满足触发条件，当单据提交时，禁止提交*/
      );
    } else if (value === 'ALLOWED') {
      controlMethodNotice = this.$t(
        { id: 'budget.strategy.notice.show.notice' } /*如果满足触发条件，当单据提交时，进行提示*/
      );
    } else {
      ...
      <div className="new-budget-strategy-detail">
        <Form onSubmit={this.handleSave}>
              ...
              <FormItem label={this.$t({ id: 'budget.strategy.message' } /*消息*/)}>
                {getFieldDecorator('messageCode', {
                  ...
                  <Select
                    ...
                    disabled={controlMethodValue === 'NO_MESSAGE'}
                  >
                    {messageCodeOptions &&
          ...
          <div>
            ...
            <Button className="btn-cancel" style={{ marginLeft: 15 }} onClick={this.handleCancel}>
...
export default connect(
  ...
  null,
  { withRef: true }
)(WrappedNewBudgetStrategyDetail);
