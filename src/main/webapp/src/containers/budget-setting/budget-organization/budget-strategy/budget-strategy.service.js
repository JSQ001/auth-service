...
export default {
  ...
  getStrategyList(page, size, organizationId, searchParams) {
    let url = `${
      config.budgetUrl
    }/api/budget/control/strategies/query?page=${page}&size=${size}&organizationId=${organizationId}`;
  ...
  getStrategyDetailList(page, size, strategyId, organizationId, keyWords) {
    let url = `${
      config.budgetUrl
    }/api/budget/control/strategy/details/query?page=${page}&size=${size}&controlStrategyId=${strategyId}&organizationId=${organizationId}`;
  ...
  getCondition(page, size, id) {
    return httpFetch.get(
      `${
        config.budgetUrl
      }/api/budget/control/strategy/mp/conds/query?page=${page}&size=${size}&controlStrategyDetailId=${id}`
    );
  },
};
