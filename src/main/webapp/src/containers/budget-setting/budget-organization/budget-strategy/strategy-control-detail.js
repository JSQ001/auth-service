...
class StrategyControlDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      infoList: [
        {
          type: 'input',
          label: this.$t({ id: 'common.sequence' } /*序号*/),
          id: 'detailSequence',
          isRequired: true,
          disabled: true,
        },
        {
          type: 'input',
          label: this.$t({ id: 'budget.strategy.rule.code' } /*规则代码*/),
          id: 'detailCode',
          isRequired: true,
          disabled: true,
        },
        {
          type: 'value_list',
          label: this.$t({ id: 'budget.strategy.control.strategy' } /*控制策略*/),
          id: 'controlMethod',
          isRequired: true,
          options: [],
          valueListCode: 2005,
        ...
        {
          type: 'input',
          label: this.$t({ id: 'budget.strategy.rule.name' } /*控制规则名称*/),
          id: 'detailName',
          isRequired: true,
        },
        {
          type: 'value_list',
          label: this.$t({ id: 'budget.strategy.message' } /*消息*/),
          id: 'messageCode',
          options: [],
          valueListCode: 2022,
          isRequired: true,
        ...
        {
          type: 'input',
          label: this.$t({ id: 'budget.strategy.event' } /*事件*/),
          id: 'expWfEvent',
        },
      ],
      infoData: {},
      ...
      columns: [
        ...
        {
          title: this.$t({ id: 'budget.strategy.detail.control.period' } /*控制期段*/),
          align: 'center',
  ...
  getBasicInfo() {
    httpFetch
      .get(
        `${config.budgetUrl}/api/budget/control/strategy/details/${this.state.strategyControlId}`
      )
      .then(res => {
        if (res.status === 200) {
  ...
  getList() {
    let url = `${config.budgetUrl}/api/budget/control/strategy/mp/conds/query?page=${
      this.state.page
    }&size=${this.state.pageSize}&controlStrategyDetailId=${this.state.strategyControlId}`;
  ...
  handleUpdate = params => {
    ...
    this.setState({ baseInfoLoading: true }, () => {
      httpFetch
        .put(`${config.budgetUrl}/api/budget/control/strategy/details`, params)
        .then(response => {
    ...
    infoList.map(item => {
      if (item.id === 'messageCode') {
        if (e === 'NO_MESSAGE') {
          item.disabled = true;
          item.isRequired = false;
          this.infoRef._reactInternalInstance._renderedComponent._instance.setValues({
            messageCode: '',
          });
        } else {
          item.disabled = false;
      ...
      <div className="strategy-control-detail" style={{ paddingBottom: 20 }}>
        ...
        <SlideFrame
          title={
            ...
            ' ' +
            this.$t({ id: 'budget.strategy.detail.condition' } /*触发条件*/)
          }
          show={showSlideFrame}
          onClose={() => this.showUpdateSlide(false)}
        >
          <NewStrategyControlDetail onClose={this.handleCloseSlide} params={{ newParams, isNew }} />
        </SlideFrame>
