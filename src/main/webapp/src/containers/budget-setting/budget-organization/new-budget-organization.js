      ...
      if (!err) {
        ...
        budgetOrganizationService
          ...
          .then(res => {
            ...
            message.success(
              this.$t({ id: 'common.create.success' }, { name: values.organizationName })
            ); //新建成功
            this.props.dispatch({
          ...
          .catch(e => {
            if (e.response) {
              message.error(
                `${this.$t('common.create.filed') /*新建失败*/}, ${e.response.data.message}`
              );
            } else {
              console.log(e);
            }
    ...
    return (
      <div className="new-budget-organization">
        <Alert
          ...
          description={
            this.$t({
              id: 'budget.organization.new.info',
            }) /*同一账套下只能有一个生效的预算组织代码，且同一租户下预算组织代码不允许重复。保存后不可修改。*/
          }
          type="info"
          showIcon
        />
        <Form onSubmit={this.handleSave}>
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'budget.organization.set.of.books' }) /* 账套 */}
          >
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'budget.organization.code' }) /* 预算组织代码 */}
          >
          ...
          <FormItem
            {...formItemLayout}
            label={this.$t({ id: 'budget.organization.name' }) /* 预算组织名称 */}
          >
