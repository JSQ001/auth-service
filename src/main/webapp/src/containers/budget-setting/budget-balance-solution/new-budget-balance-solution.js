...
class NewBudgetBalanceSolution extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      //参数值列表
      queryScopeList: [
        { code: '1001', messageKey: this.$t('common.all') },
        { code: '1002', messageKey: this.$t('budget.balance.select') },
      ],
      //弹窗返回值
            ...
            return (
              <div>
                <Select
                  defaultValue={text}
                  value={text}
                  onChange={value => this.onParameterTypeChange(value, record)}
                >
            ...
            return (
              <div>
                <Select
                  defaultValue={text}
                  value={text}
                  onChange={value => this.onParameterCodeChange(value, record)}
                >
            ...
            return (
              <div>
                <Select
                  ...
                  disabled={record.parameterCode ? false : true}
                  style={{ width: '50%' }}
                >
                ...
                &nbsp;<span>&nbsp;</span>&nbsp;
                <Input
                  value={this.$t('budget.balance.select.data', {
                    total: record.solutionParameterList.length,
                  })}
                  onClick={e => this.onListSelectorOk(e, record)}
                  disabled={record.parameterCode && record.queryScope === '1002' ? false : true}
                  style={{ width: '45%' }}
                />
              </div>
            );
  ...
  onListSelectorOk = (e, record) => {
    ...
    switch (record.parameterCode) {
      ...
      case 'BUDGET_SCENARIO':
        listSelectorType = 'budget_scenarios';
        listSelectorExtraParams = { organizationId: this.state.organizationId };
        labelKey = 'scenarioName';
        break;
      case 'BUDGET_VERSION':
        listSelectorType = 'budget_versions';
        listSelectorExtraParams = { organizationId: this.state.organizationId };
        labelKey = 'versionName';
        break;
      case 'BUDGET_STRUCTURE':
        listSelectorType = 'budget_structure';
        listSelectorExtraParams = { organizationId: this.state.organizationId };
      ...
      case 'BUDGET_ITEM':
        ...
        break;
      case 'YEAR': {
        listSelectorType = 'year';
        labelKey = 'periodYear';
        listSelectorExtraParams = { setOfBooksId: this.props.match.params.setOfBooksId };
        break;
      }
      case 'QUARTER': {
        listSelectorType = 'quarter';
        listSelectorExtraParams = { type: 2021 };
        labelKey = 'name';
        break;
      }
      case 'MONTH': {
        listSelectorType = 'period';
        labelKey = 'periodName';
        listSelectorExtraParams = { setOfBooksId: this.props.match.params.setOfBooksId };
        break;
      }
      ...
      case 'RESPONSIBILITYCENTER':
        {
          (listSelectorType = 'responsibility_default'),
            (labelKey = 'responsibilityName'),
            // valueKey = 'id',
            // codeKey = 'responsibilityCode',
            (listSelectorExtraParams = { setOfBooksId: this.props.company.setOfBooksId });
        }
        break;
      case 'RESPONSIBILITYCENTER_GROUP':
        {
          (listSelectorType = 'responsibility_group'),
            (labelKey = 'groupName'),
            // valueKey = 'id',
            // codeKey = 'groupCode',
            (listSelectorExtraParams = { setOfBooksId: this.props.company.setOfBooksId });
  ...
  paramCodeChangeQueryScope = (value, record) => {
    ...
    if (value === 'COMPANY') {
      ...
      record.queryScopeList.push({
        code: '1003',
        messageKey: this.$t('budget.balance.current.company'),
      });
    }
    if (value === 'UNIT') {
      record.queryScopeList.length = 2;
      record.queryScopeList.push({
        code: '1003',
        messageKey: this.$t('budget.balance.current.department'),
      });
    }
    if (value === 'EMPLOYEE') {
      record.queryScopeList.length = 2;
      record.queryScopeList.push({
        code: '1003',
        messageKey: this.$t('budget.balance.current.user'),
      });
    }
  ...
  componentWillMount = () => {
    ...
    if (Number(this.props.match.params.id)) {
      budgetBalanceSolutionService
        .getBudgetBalanceSolutionById(this.props.match.params.id)
        .then(res => {
          //设置values里面的值
          ...
          res.data.solutionLineList.map(record => {
            ...
            if (record.solutionParameterList) {
              record.solutionParameterList.map(item => {
                ...
                if (record.parameterCode === 'CURRENCY') {
                  nowsolutionParameterList.push({
                    currencyCode: item.parameterValueCode,
                    key: item.parameterValueCode,
                  });
                }
                //参数的值为公司时
                ...
                if (record.parameterCode === 'UNIT') {
                  nowsolutionParameterList.push({
                    departmentId: item.parameterValueId,
                    key: item.parameterValueId,
                  });
                }
                //参数的值为部门组时
                ...
                //参数的值为预算版本时
                if (record.parameterCode == 'BUDGET_VERSION') {
                  nowsolutionParameterList.push({
                    id: item.parameterValueId,
                    key: item.parameterValueId,
                  });
                ...
                //参数的值为预算表时
                if (record.parameterCode == 'BUDGET_STRUCTURE') {
                  nowsolutionParameterList.push({
                    id: item.parameterValueId,
                    key: item.parameterValueId,
                  });
                ...
                //默认
                if (nowsolutionParameterList.length == 0) {
                  nowsolutionParameterList.push({
                    id: item.parameterValueId,
                    key: item.parameterValueId,
                  });
            ...
            //queryScopeList
            let nowQueryScopeList = [
              { code: '1001', messageKey: this.$t('common.all') },
              { code: '1002', messageKey: this.$t('budget.balance.select') },
            ];
            if (record.parameterCode === 'COMPANY') {
              nowQueryScopeList.length = 2;
              nowQueryScopeList.push({
                code: '1003',
                messageKey: this.$t('budget.balance.current.company'),
              });
            }
  ...
  getBudgetRefParameters = () => {
    budgetBalanceSolutionService.getBudgetRefParameters().then(res => {
      ...
      res.data.map(item => {
        if (item.code !== 'BUDGET_SCENARIO' && item.code !== 'BUDGET_VERSION') {
  ...
  getDimensionRefParameters = () => {
    budgetBalanceSolutionService
      .getDimensionRefParameters(this.props.match.params.setOfBooksId)
      .then(res => {
        this.setState({
  ...
  onFormSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        let params = {};
        let nowReleaseIds = [];
        params.setOfBooksId = this.props.match.params.setOfBooksId
        ...
        data.map(record => {
          if (
            record.queryScope === '1002' &&
            record.parameterCode &&
            record.solutionParameterList.length === 0
          ) {
            sign = false;
          ...
          record.solutionParameterList.map(item => {
            ...
            if (record.parameterCode === 'CURRENCY') {
              saveSolutionParameterList.push({ parameterValueCode: item.currencyCode });
            } else {
              saveSolutionParameterList.push({ parameterValueId: item.id });
            }
          });
  ...
  onNewBgtBalSolutionCancel = () => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: '/budget-setting/budget-balance-solution/budget-balance-solution/:setOfBooksId'.replace(
          ':setOfBooksId',
          this.props.match.params.setOfBooksId
        ),
      })
      ...
      <div>
        <Form onSubmit={this.onFormSubmit}>
          <List bordered="true" size="small" split>
            <div className="common-item-title" style={{ borderBottom: '1px solid #D0D0D0' }}>
              ...
              <Col span={6}>
                <FormItem {...formItemLayout} label={this.$t('budget.balance.condition.code')}>
                  {getFieldDecorator('conditionCode', {
                    ...
                    <Input
                      disabled={conditionCodeDisabled}
          ...
          <List bordered size="middle" style={{ marginBottom: '50px' }}>
            <div className="common-item-title" style={{ borderBottom: '1px solid #D0D0D0' }}>
              {this.$t('budget.balance.permission')}
            </div>
            <Row>
              <Col span={23} push={1}>
            ...
            <Row>
              <Col span={6} push={1} style={{ marginTop: '10px', marginBottom: '30px' }}>
                <Input
                  ...
                  disabled={visibleUserScope === 1001 ? true : false}
                  value={
                    visibleUserScope === 1001
                      ? this.$t('common.all.user')
                      : this.$t('budget.balance.select.data', { total: releaseIdsList.length })
                  }
                />
              </Col>
            </Row>
...
export default connect(
  ...
  null,
  { withRef: true }
)(WrappedNewBudgetBalanceSolution);
