...
class BudgetBalanceSolution extends Component {
      ...
      searchForm: [
        {
          type: 'select',
          id: 'setOfBooksId',
          label: this.$t('budget.balance.set.of.books'),
          options: [],
          isRequired: 'true',
          labelKey: 'name',
          valueKey: 'id',
          ...
          render: text => {
            return (
              <Badge
                ...
                text={text ? this.$t('common.status.enable') : this.$t('common.status.disable')}
              />
            );
          },
        },
  ...
  onDeleteClick = (e, record) => {
    ...
    budgetBalanceSolutionService
      .deleteBudgetBalanceSolution(record.id)
      .then(res => {
        if (res.status === 200) {
  ...
  getList = () => {
    ...
    budgetBalanceSolutionService
      .getBudgetBalanceSolution(this.state.setOfBooksId, params)
      .then(res => {
        this.setState({
          loading: false,
          data: res.data,
  ...
  handleCreate = () => {
    this.props.dispatch(
      routerRedux.replace({
        pathname: '/budget-setting/budget-balance-solution/new-budget-balance-solution/:setOfBooksId/:id'.replace(
          ':setOfBooksId',
