...
export default {
  ...
  getBudgetBalanceSolution(setOfBooksId, params) {
    return httpFetch.get(
      `${config.budgetUrl}/api/budget/solution/header/list?setOfBooksId=${setOfBooksId}`,
      params
    );
  },
  /**
  ...
  getDimensionRefParameters(setOfBooksId) {
    return httpFetch.get(
      `${config.mdataUrl}/api/dimension/page/by/cond?setOfBooksId=${setOfBooksId}`
    );
  },
  /**
   * 获取当前账套对应的预算组织数据
  ...
  getOrganization(setOfBooksId) {
    return httpFetch.get(
      `${
        config.budgetUrl
      }/api/budget/organizations/query/all?setOfBooksId=${setOfBooksId}&enabled=true`
    );
  },
  /**
   * 预算余额查询方案创建，修改
