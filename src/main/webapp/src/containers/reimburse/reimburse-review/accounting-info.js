  ...
  createAccounting = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        reimburseService
          .createAccounting(this.state.headerData.id, accountDate)
          .then(res => {
            this.getUnApprovedList();
            this.getApprovedList();
          })
      ...
      <div className="tab-container reimburse-container">
              ...
              <Col style={{ textAlign: 'left', width: '110px', marginTop: 3 }} span={1}>
                <span>
                  {this.state.headerData.auditFlag ? (
                    <Button type="primary" htmlType="submit" disabled loading={this.props.loading}>
                      凭证日期
                    </Button>
                  ) : (
                    <Button type="primary" htmlType="submit" loading={this.state.createLoading}>
                      创建凭证
                    </Button>
                  )}
                </span>
        ...
        <Row>
          <Tabs
            ...
            style={{ paddingTop: 0, marginTop: -24 }}
          >
            <TabPane
              tab="费用凭证"
              key="expenseAccounting"
            ...
            >
              <div className="table-header">
                <div className="table-header-title">
                  {this.$t(
                    { id: 'common.total' },
                    {
                      total: unapprovedPagination.total ? unapprovedPagination.total : '0',
                    } /*共搜索到 {total} 条数据*/
                  )}
                </div>
              </div>
              <Table
                ...
                pagination={unapprovedPagination}
                loading={loading}
              ...
              <div className="table-header">
                <div className="table-header-title">
                  {this.$t(
                    { id: 'common.total' },
                    {
                      total: approvedPagination.total ? approvedPagination.total : '0',
                    } /*共搜索到 {total} 条数据*/
                  )}
                </div>
              </div>
