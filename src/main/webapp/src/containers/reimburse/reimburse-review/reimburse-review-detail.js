...
class ReimburseReviewDetail extends React.Component {
  ...
  handleApprovePass = remark => {
    ...
    reimburseService
      .auditAccounting(this.state.headerData.id, 'APPROVE', remark)
      .then(res => {
        if (res.status === 200) {
          if ('SUCCESS' === res.data) {
  ...
  handleApproveReject = remark => {
    ...
    reimburseService
      .auditAccounting(this.state.headerData.id, 'REJECT', remark)
      .then(res => {
        if (res.status === 200) {
          if ('SUCCESS' === res.data) {
  ...
  render() {
    ...
    return (
      <div
        ...
        style={{ margin: '10px -6px', borderTop: '1px solid #ececec' }}
      >
        <ReimburseReviewDetailCommon
          getInfo={this.getInfo}
          headerData={headerData}
          id={this.props.match.params.id}
          getContractStatus={this.getStatus}
        />
        {!headerData.auditFlag && headerData ? (
          <Affix offsetBottom={2} className="bottom-bar bottom-bar-approve">
            <ApproveBar
              backUrl={'/financial-management/reimburse-review/reimburse-review'}
          ...
          <Affix offsetBottom={2} className="bottom-bar">
