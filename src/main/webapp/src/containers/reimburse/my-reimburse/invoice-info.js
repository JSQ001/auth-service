...
class InvoiceInfo extends Component {
    ...
    reimburseService.getRate(value).then(res => {
      ...
      this.setState({ rateList: res.data, isCalculation }, () => {
        ...
        if (taxRate || this.props.form.getFieldValue('taxRate')) {
          ...
          this.props.form.setFieldsValue({
            taxAmount: rateTotal,
            nonVATinclusiveAmount: costTotal,
            taxRate: taxRate,
          });
          this.props.onAmountChange &&
            this.props.onAmountChange(amount, rate, this.state.isCalculation);
        } else {
          this.props.form.setFieldsValue({
            taxRate: taxRate,
            taxAmount: '',
            nonVATinclusiveAmount: '(输入价税合计&税率后自动算出)',
          });
  ...
  formatCost = value => {
    ...
    this.props.onAmountChange &&
      this.props.onAmountChange(value, this.state.defaultRate, this.state.isCalculation);
  };
  ...
  amountChange = value => {
    ...
    if (this.state.defaultRate !== '') {
      ...
      let costTotal = this.toDecimal2(val / (1 + rate));
      this.props.onAmountChange &&
        this.props.onAmountChange(costTotal, this.state.defaultRate, this.state.isCalculation);
    }
  };
        ...
        <Col span={24}>
          <div className="invoice-info-box">
            <div className="invoice-info-title">
              <Popover
                trigger="click"
                content={<img style={{ width: '600px' }} src={invoiceImg} />}
              >
                <a>发票填写说明</a>
              </Popover>
            </div>
                ...
                <div className="ant-row ant-form-item">
                  <Row>
                    <div className="ant-col-6 ant-form-item-label">
                      <label htmlFor="invoiceCode" title="发票代码">
                        价税合计
                      </label>
                    </div>
                    <Col span={7}>
                        ...
                        {getFieldDecorator('vatInvoiceCurrencyCode', {
                          ...
                          <Select disabled optionLabelProp="value" onChange={this.currencyChange}>
                            {currencyList &&
                              currencyList.map(o => {
                                return (
                                  <Select.Option value={o.currency} key={o.currency}>
                                    {o.currencyName}
                                  </Select.Option>
                                );
                              })}
                      ...
                      <FormItem>
                        {getFieldDecorator('taxRate', {
                          ...
                          <Select placeholder="税率" onChange={this.rateChange}>
                            {rateList.map(o => {
                              return (
                                <Select.Option value={o.taxRateValue} key={o.taxRateValue}>
                                  {o.taxRateKey}
                                </Select.Option>
                              );
                            })}
                      ...
                      <FormItem>
                        {getFieldDecorator('taxAmount', {
                          ...
                          <Input
                            placeholder="税额合计"
                            disabled
                            onChange={this.invoiceNumberChange}
                          />
                        )}
                      </FormItem>
                    </Col>
