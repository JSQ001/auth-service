...
class Verification extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        ...
        {
          title: '预付款单号',
          dataIndex: 'prepaymentRequisitionNumber',
          render: value => {
            return <Popover content={value}>{value}</Popover>;
          },
        },
        {
          title: '预付款单类型',
          dataIndex: 'prepaymentRequisitionTypeDesc',
          width: 120,
          render: value => {
            return <Popover content={value}>{value}</Popover>;
          },
        ...
        {
          title: '总金额',
          dataIndex: 'prepaymentRequisitionAmount',
          width: 80,
  ...
  getList = () => {
    ...
    reimburseService.getWriteOffList(params).then(res => {
      res.data.map(item => {
        let record = this.state.changeList.find(
          o => o.cshTransactionDetailId == item.cshTransactionDetailId
        );
        if (record) {
  ...
  render() {
    ...
    return (
      ...
      >
        <Alert
          message="核销金额不能超过计划付款行金额"
          type={messageType}
          showIcon
          style={{ marginBottom: 10 }}
        />
        <div>
          计划付款行金额：{model.currency} {this.formatMoney(model.amount)}
        </div>
        <div style={{ marginBottom: 10 }}>
          <span style={{ marginRight: 24 }}>
            已核销金额：{model.currency} {this.formatMoney(model.writeOffAmount)}
          </span>
          <span>
            可核销金额：{model.currency} {this.formatMoney(model.amount - model.writeOffAmount)}
          </span>
        </div>
        <Table
          loading={loading}
