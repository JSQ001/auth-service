...
class ShareForm extends Component {
      ...
      this.setState(
        ...
        () => {
          if (!this.state.isNew) {
            this.setState({
              application: {
                id: newProps.model.applicationId,
                businessCode: newProps.model.applicationCode,
              },
            });
          }
        }
  ...
  showSelectApplication = () => {
    ...
    this.setState({
      ...
      selectedData: this.state.application.id ? [this.state.application.id] : [],
    });
  };
  ...
  handleListOk = values => {
    if (values && values.result && values.result.length) {
      this.setState(
        { application: values.result[0], showSelectApplication: false, applincationParams: {} },
        () => {
          let application = this.state.application;
          this.props.form.setFieldsValue({
            company: [{ id: application.companyId, name: application.companyName }],
            department: [
              { departmentId: application.departmentId, name: application.departmentName },
            ],
      ...
      this.setState(
        { application: {}, showSelectApplication: false, applincationParams: {} },
        () => {
          let application = this.state.application;
          this.props.form.setFieldsValue({
            company: [{ id: application.companyId, name: application.companyName }],
      ...
      <div>
        ...
        >
          {visible && (
            <Form>
              {this.props.flag && (
                <FormItem {...formItemLayout} label="关联申请单">
                  {getFieldDecorator('applicationId', {
                    initialValue: isNew
                      ? { key: '', label: '' }
                      : { key: model.applicationId, label: model.applicationCode },
              ...
              <FormItem {...formItemLayout} label="公司">
                {getFieldDecorator('company', {
                  initialValue: isNew
                    ? this.props.flag
                      ? []
                      : [{ id: defaultApportion.companyId, name: defaultApportion.companyName }]
                    : [{ id: model.company.id, name: model.company.name }],
                  rules: [{ message: '请选择', required: true }],
                })(
                  <Chooser
              ...
              <FormItem {...formItemLayout} label="分摊金额">
                {getFieldDecorator('cost', {
                  initialValue: isNew ? '' : model.cost,
                  rules: [{ validator: this.checkPrice }],
                })(
                  <InputNumber
                    disabled={model.defaultApportion}
              ...
              {typeList &&
                typeList.map(item => {
                  return (
                    <FormItem key={item.dimensionId} {...formItemLayout} label={item.fieldName}>
                      {getFieldDecorator(item.dimensionId, {
                        initialValue: isNew
                          ? { key: item.id, label: item.dimensionItemName }
                          : {
                              key: model[item.dimensionId].key,
                              label: model[item.dimensionId].label,
                            },
                      })(
                        <Select labelInValue onFocus={() => this.handleFocus(item.dimensionId)}>
                          {data[item.dimensionId] &&
                            data[item.dimensionId].map(o => {
                              return (
                                <Select.Option key={parseInt(o.id)} value={parseInt(o.id)}>
                                  {o.dimensionItemName}
                                </Select.Option>
                              );
                            })}
