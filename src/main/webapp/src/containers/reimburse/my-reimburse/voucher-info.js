...
class VoucherInfo extends React.Component {
  ...
  getAccountInfo = () => {
    ...
    reimburseService
      .getVoucherInfo(tenantId, 'EXP_REPORT', transactionNumber)
      .then(res => {
        if (res.status === 200) {
          this.setState({
  ...
  getCshWriteOffInfo = () => {
    const { tenantId, transactionNumber } = this.state;
    reimburseService
      .getVoucherInfo(tenantId, 'CSH_WRITE_OFF', transactionNumber)
      .then(res => {
        if (res.status === 200) {
          this.setState({
  ...
  getPaymentInfo = () => {
    const { tenantId, transactionNumber } = this.state;
    reimburseService
      .getVoucherInfo(tenantId, 'CSH_PAYMENT', transactionNumber)
      .then(res => {
        if (res.status === 200) {
          this.setState({
  ...
  getExpReverseInfo = () => {
    const { tenantId, transactionNumber } = this.state;
    reimburseService
      .getVoucherInfo(tenantId, 'EXP_REVERSE', transactionNumber)
      .then(res => {
        if (res.status === 200) {
          this.setState({
  ...
  render() {
    ...
    let voucherList = (
      <div>
        ...
        <div style={{ marginTop: accountData.length === 0 ? 20 : -30 }} />
        ...
        <div style={{ marginTop: cshWriteOfftData.length === 0 ? 20 : -30 }} />
        ...
        <div style={{ marginTop: paymentData.length === 0 ? 20 : -30 }} />
