...
React.Component.prototype.filterMoney = (money, fixed = 2, isString = false, isNumber = false) => {
  ...
  if (money.indexOf('.') > -1) {
    ...
    numberString =
      integer.replace(/(\d)(?=(\d{3})+(?!\d))/g, isNumber ? '$1' : '$1,') + '.' + decimals;
  } else {
...
React.Component.prototype.service = {
  ...
  getCurrencyList: userOid => {
    return httpFetch.get(
      `${config.mdataUrl}/api/currency/rate/company/standard/currency/getAll${
  ...
  //主数据，获取帐套下币种
  getCurrencyMdataList: params => {
    return httpFetch.get(`${config.mdataUrl}/api/currency/rate/list`, params);
  },

  getCurrencyListByCompany: companyOid => {
    return httpFetch.get(
      `${config.mdataUrl}/api/currency/rate/company/standard/currency/getAll/companyOid${
        companyOid ? `?companyOid=${companyOid}` : ''
      }`
    );
  },
};
